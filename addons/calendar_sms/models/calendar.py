            ...
            if event.recurrency:
                ...
                for event_start in event._get_recurrent_date_by_event():
                    ...
                    last_found = self.do_check_alarm_for_one_date(
                        event_start, event, max_delta, 0, "sms", after=last_sms_cron, missing=True
                    )
                    for alert in last_found:
                        event.browse(alert["event_id"])._do_sms_reminder()
                        found = True
                    if (
                        found and not last_found
                ...
                for alert in self.do_check_alarm_for_one_date(
                    event_start, event, max_delta, 0, "sms", after=last_sms_cron, missing=True
                ):
                    event.browse(alert["event_id"])._do_sms_reminder()

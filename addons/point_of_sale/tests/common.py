...
class TestPointOfSaleCommon(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.pos_order_session0 = self.env["pos.session"].create({"user_id": 1, "config_id": self.pos_config.id})

        # create a VAT tax of 10%, included in the public price
        Tax = self.env["account.tax"]
        account_tax_10_incl = Tax.create(
            {"name": "VAT 10 perc Incl", "amount_type": "percent", "amount": 10.0, "price_include": 1}
        )
        ...
        account_tax_05_incl = Tax.create(
            {"name": "VAT 5 perc Incl", "amount_type": "percent", "amount": 5.0, "price_include": 0}
        )
        ...
        self.product4.write({"taxes_id": [(6, 0, [account_tax_05_incl.id, account_tax_05_incl_chicago.id])]})

...
class TestAngloSaxonCommon(common.TransactionCase):
    ...
    def setUp(self):
        ...
        account_expense = self.env["account.account"].create(
            {"name": "Expense", "code": "EXP00", "user_type_id": account_type_oth.id, "reconcile": True}
        )
        account_output = self.env["account.account"].create(
            {"name": "Output", "code": "OUT00", "user_type_id": account_type_oth.id, "reconcile": True}
        )
...
class TestAngloSaxonFlow(TestAngloSaxonCommon):
    ...
    def test_create_account_move_line(self):
        ...
        self.assertEqual(
            aml_output.credit, self.product.standard_price, "Cost of Good Sold entry missing or mismatching"
        )
        self.assertEqual(
            aml_expense.debit, self.product.standard_price, "Cost of Good Sold entry missing or mismatching"
        )

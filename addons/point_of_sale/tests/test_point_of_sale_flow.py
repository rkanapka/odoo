...
class TestPointOfSaleFlow(TestPointOfSaleCommon):
    ...
    def test_order_to_payment(self):
        ...
        self.assertLess(
            ...
            0.01,
            "The order has a wrong amount, tax included.",
        )

        self.assertLess(
            abs(self.pos_order_pos0.amount_tax - (900 - (450 * 2 / 1.1) + 300 * 0.05 * 3)),
            0.01,
            "The order has a wrong tax amount.",
        )
        ...
        self.assertLess(
            abs(defs["amount"] - ((450 * 2 + 300 * 3 * 1.05) * 0.95 - 100.0)),
            0.01,
    ...
    def test_order_refund(self):
        ...
        refund_payment = self.PosMakePayment.with_context(**payment_context).create({"amount": refund.amount_total})
    ...
    def test_order_to_picking(self):
        ...
        self.assertEqual(
            self.pos_order_pos3.picking_id.move_lines.mapped("state"), ["done"], "Move Lines should be in done state."
        )
    ...
    def test_order_to_invoice(self):
        ...
        self.assertEqual(float_compare(invoice.amount_total, 1752.75, precision_digits=2), 0, "Invoice not correct")
        ...
        self.assertTrue(
            self.AccountBankStatement.with_context(context_journal)._default_journal(), "Journal has not been selected"
        )
        ...
        account_statement_line = self.AccountBankStatementLine.create(
            {"amount": 1000, "partner_id": self.partner4.id, "statement_id": account_statement.id, "name": "EXT001"}
        )
    ...
    def test_create_from_ui(self):
        ...
        rescue_session = self.PosSession.search(
            [("config_id", "=", self.pos_config.id), ("state", "=", "opened"), ("rescue", "=", True)]
        )
        self.assertEqual(
            len(rescue_session), 1, "One (and only one) rescue session should be created for orphan orders"
        )
        self.assertIn(
    ...
    def test_order_to_payment_currency(self):
        ...
        self.pos_order_pos0 = self.PosOrder.create(
            {
                "company_id": self.company_id,
                "pricelist_id": self.partner1.property_product_pricelist.copy(

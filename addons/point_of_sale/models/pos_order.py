...
class PosOrder(models.Model):
        ...
        for order in self.filtered(lambda o: not o.account_move or o.state == "paid"):
            ...
            if move is None:
                ...
                journal_id = (
                    ...
                    .sudo()
                    .get_param("pos.closing.journal_id_%s" % current_company.id, default=order.sale_journal.id)
                )
            ...
            for line in order.lines:
                ...
                if cur != cur_company:
                    ...
                    data["amount_currency"] = (
                        -abs(line.price_subtotal) if data.get("credit") else abs(line.price_subtotal)
                    )
                ...
                for tax in taxes.compute_all(price, cur, line.qty)["taxes"]:
                    if cur != cur_company:
                        ...
                        amount_tax = cur.with_context(date=date_order).compute(
                            tax["amount"], cur_company, round=round_tax
                        )
                    else:
            ...
            if receivable_amounts["amount_currency"]:
                data["currency_id"] = cur.id
                data["amount_currency"] = (
                    -abs(receivable_amounts["amount_currency"])
                    if data.get("credit")
                    else abs(receivable_amounts["amount_currency"])
                )
    ...
    def _reconcile_payments(self):
        cash_basis_percentage_before_rec = {
            move: move.line_ids._get_matched_percentage() for move in self.mapped("account_move")
        }
        for order in self:
            aml = (
                order.statement_ids.mapped("journal_entry_ids")
                | order.account_move.line_ids
                | order.invoice_id.move_id.line_ids
            )
            aml = aml.filtered(
                lambda r: not r.reconciled
                and r.account_id.internal_type == "receivable"
                and r.partner_id == order.partner_id.commercial_partner_id
            )
        ...
        for move in self.mapped("account_move"):
            partial_reconcile = self.env["account.partial.reconcile"].search(
                ["|", ("credit_move_id.move_id", "=", move.id), ("debit_move_id.move_id", "=", move.id)], limit=1
            )
    ...
    def _filtered_for_reconciliation(self):
        ...
        if (
            self.env["ir.config_parameter"].sudo().get_param("point_of_sale.order_reconcile_mode", "all")
            == "partner_only"
        ):
    ...
    company_id = fields.Many2one(
        "res.company", string="Company", required=True, readonly=True, default=lambda self: self.env.user.company_id
    )
    ...
    amount_paid = fields.Float(
        compute="_compute_amount_all", string="Paid", states={"draft": [("readonly", False)]}, readonly=True, digits=0
    )
    ...
    picking_type_id = fields.Many2one(
        "stock.picking.type", related="session_id.config_id.picking_type_id", string="Operation Type"
    )
    ...
    sale_journal = fields.Many2one(
        "account.journal", related="session_id.config_id.journal_id", string="Sales Journal", store=True, readonly=True
    )
                ...
                moves |= Move.create(
                    {
                        ...
                        "location_dest_id": (
                            destination_id
                            if line.qty >= 0
                            else return_pick_type != picking_type
                            and return_pick_type.default_location_dest_id.id
                            or location_id
                        ),
                    }
                ...
                if pos_pack_lots and lots_necessary:
                    for pos_pack_lot in pos_pack_lots:
                        stock_production_lot = StockProductionLot.search(
                            [("name", "=", pos_pack_lot.lot_name), ("product_id", "=", move.product_id.id)]
                        )
    ...
    def _prepare_bank_statement_line_payment_values(self, data):
        ...
        account_def = (
            self.env["ir.property"].with_context(company_cxt).get("property_account_receivable_id", "res.partner")
        )
        args["account_id"] = (
            (self.partner_id.property_account_receivable_id.id) or (account_def and account_def.id) or False
        )
...
class PosOrderLine(models.Model):
    ...
    product_id = fields.Many2one(
        "product.product", string="Product", domain=[("sale_ok", "=", True)], required=True, change_default=True
    )
    ...
    tax_ids_after_fiscal_position = fields.Many2many(
        "account.tax", compute="_get_tax_ids_after_fiscal_position", string="Taxes"
    )
    ...
    def _compute_amount_line_all(self):
        for line in self:
            ...
            tax_ids_after_fiscal_position = (
                fpos.map_tax(line.tax_ids, line.product_id, line.order_id.partner_id) if fpos else line.tax_ids
            )
            price = line.price_unit * (1 - (line.discount or 0.0) / 100.0)
    ...
    def _onchange_product_id(self):
        if self.product_id:
            ...
            tax_ids_after_fiscal_position = (
                fpos.map_tax(self.tax_ids, self.product_id, self.order_id.partner_id) if fpos else self.tax_ids
            )

    ...
    def default_get(self, fields):
        ...
        if config_id:
            ...
            if self.env.context.get("balance", False) == "start":
                ...
                vals["cashbox_lines_ids"] = [
                    [0, 0, {"coin_value": line.coin_value, "number": 0, "subtotal": 0.0}] for line in lines
                ]
        return vals
...
class PosConfig(models.Model):
    ...
    def _default_pricelist(self):
        return self.env["product.pricelist"].search(
            [
                ("company_id", "in", (False, self.env.user.company_id.id)),
    ...
    name = fields.Char(
        string="Point of Sale Name", index=True, required=True, help="An internal identification of the point of sale."
    )
    ...
    iface_customer_facing_display = fields.Boolean(
        string="Customer Facing Display", help="Show checkout to customers with a remotely-connected screen."
    )
    ...
    iface_scan_via_proxy = fields.Boolean(
        string="Scan via Proxy", help="Enable barcode scanning with a remotely connected barcode scanner."
    )
    ...
    iface_display_categ_images = fields.Boolean(
        string="Display Category Pictures", help="The product categories will be displayed with pictures."
    )
    ...
    receipt_header = fields.Text(
        string="Receipt Header", help="A short text that will be inserted as a header in the printed receipt."
    )
    receipt_footer = fields.Text(
        string="Receipt Footer", help="A short text that will be inserted as a footer in the printed receipt."
    )
    ...
    tip_product_id = fields.Many2one(
        "product.product", string="Tip Product", help="This product is used as reference on customer receipts."
    )
    ...
    customer_facing_display_html = fields.Html(
        string="Customer facing display content", translate=True, default=_compute_default_customer_html
    )
        ...
        for pos_config in self:
            if pos_config.journal_id:
                pos_config.currency_id = (
                    pos_config.journal_id.currency_id.id or pos_config.journal_id.company_id.currency_id.id
                )
            else:
    ...
    def _compute_current_session(self):
        for pos_config in self:
            session = pos_config.session_ids.filtered(
                lambda r: r.user_id.id == self.env.uid and not r.state == "closed" and not r.rescue
            )
    ...
    def _check_company_payment(self):
        if self.env["account.journal"].search_count(
            [("id", "in", self.journal_ids.ids), ("company_id", "!=", self.company_id.id)]
        ):
    ...
    def _check_currencies(self):
        ...
        if any(
            self.journal_ids.mapped(lambda journal: journal.currency_id and journal.currency_id != self.currency_id)
        ):
            raise ValidationError(
                _(
    ...
    def _onchange_picking_type_id(self):
        if (
            self.picking_type_id.default_location_src_id.usage == "internal"
            and self.picking_type_id.default_location_dest_id.usage == "customer"
        ):
    ...
    def create(self, values):
        ...
        pos_config = super().create(values)
    ...
    def write(self, vals):
        ...
        config_display = self.filtered(
            lambda c: c.is_posbox
            and c.iface_customer_facing_display
            and not (c.customer_facing_display_html or "").strip()
        )
        if config_display:
    ...
    def _check_modules_to_install(self):
        ...
        for pos_config in self:
            for field_name in [f for f in pos_config.fields_get_keys() if f.startswith("module_")]:
                ...
                if getattr(pos_config, field_name) and module_to_install.state not in (
                    "installed",
                    "to install",
                    "to upgrade",
                ):
    ...
    def open_session_cb(self):
        ...
        if not self.current_session_id:
            self.current_session_id = self.env["pos.session"].create({"user_id": self.env.uid, "config_id": self.id})

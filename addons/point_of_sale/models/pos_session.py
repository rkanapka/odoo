...
class PosSession(models.Model):
    ...
    def _confirm_orders(self):
        for session in self:
            ...
            journal_id = (
                ...
                .sudo()
                .get_param("pos.closing.journal_id_%s" % company_id, default=session.config_id.journal_id.id)
            )
            if not journal_id:
            ...
            move = (
                ...
                ._create_account_move(session.start_at, session.name, int(journal_id), company_id)
            )
    ...
    journal_ids = fields.Many2many(
        "account.journal", related="config_id.journal_ids", readonly=True, string="Available Payment Methods"
    )
    ...
    def _check_pos_config(self):
        if (
            self.search_count(
                [("state", "!=", "closed"), ("config_id", "=", self.config_id.id), ("rescue", "=", False)]
            )
            > 1
        ):

...
from odoo.tools import html2plaintext, html_escape as escape


class WebsiteEventTrackController(http.Controller):

    @http.route(
        ["""/event/<model("event.event"):event>/track/<model("event.track", "[('event_id','=',event[0])]"):track>"""],
        type="http",
        auth="public",
        website=True,
    )
                ...
                for loc in list(locations):
                    if locations[loc] and (locations[loc][-1][2] > start_date):
                        ...
                        locations[loc].append(
                            [False, locations[loc] and locations[loc][-1][2] or dates[0][0], start_date, 1]
                        )
    ...
    @http.route(
        ["""/event/<model("event.event"):event>/agenda"""], type="http", auth="public", website=True, sitemap=False
    )
    def event_agenda(self, event, tag=None, **post):
        event = event.with_context(tz=event.date_tz or "UTC")
        local_tz = pytz.timezone(event.date_tz or "UTC")
        days_tracks = collections.defaultdict(list)
        ...
        return request.render(
            "website_event_track.agenda", {"event": event, "days": days, "tracks_by_days": tracks_by_days, "tag": tag}
        )

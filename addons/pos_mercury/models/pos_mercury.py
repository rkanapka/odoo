...
class BarcodeRule(models.Model):
    ...
    type = fields.Selection(selection_add=[("credit", "Credit Card")])
...
class AccountBankStatementLine(models.Model):
    ...
    mercury_prefixed_card_number = fields.Char(
        string="Card Number", compute="_compute_prefixed_card_number", help="The card number used for the payment."
    )
...
class PosOrder(models.Model):
    ...
    def _payment_fields(self, ui_paymentline):
        fields = super()._payment_fields(ui_paymentline)
    ...
    def add_payment(self, data):
        statement_id = super().add_payment(data)
        ...
        statement_lines = statement_lines.filtered(
            lambda line: float_compare(
                line.amount, data["amount"], precision_rounding=line.journal_currency_id.rounding
            )
            == 0
        )
...
class AutoVacuum(models.AbstractModel):
    ...
    def power_on(self, *args, **kwargs):
        ...
        return super().power_on(*args, **kwargs)

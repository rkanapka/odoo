...
class Task(models.Model):
        ...
        for task in self.sorted(key="id", reverse=True):
            ...
            for child_task in task.child_ids:
                if child_task.stage_id and child_task.stage_id.fold:
                    ...
                    children_hours += max(
                        child_task.planned_hours, child_task.effective_hours + child_task.children_hours
                    )

            task.children_hours = children_hours
            task.effective_hours = sum(
                task.sudo().timesheet_ids.mapped("unit_amount")
            )  # use 'sudo' here to allow project user (without timesheet user right) to create task
    ...
    effective_hours = fields.Float(
        compute="_hours_get", store=True, string="Hours Spent", help="Computed using the sum of the task work done."
    )
    total_hours = fields.Float(
        compute="_hours_get", store=True, string="Total", help="Computed as: Time Spent + Remaining Time."
    )
    total_hours_spent = fields.Float(
        compute="_hours_get", store=True, string="Total Hours", help="Computed as: Time Spent + Sub-tasks Hours."
    )
    ...
    children_hours = fields.Float(
        compute="_hours_get",
        store=True,
        string="Sub-tasks Hours",
    ...
    _constraints = [
        (
            models.BaseModel._check_recursion,
            "Circular references are not permitted between tasks and sub-tasks",
            ["parent_id"],
        )
    ]

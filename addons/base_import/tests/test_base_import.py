...
class TestMatchHeadersSingle(TransactionCase):
    ...
    def test_match_by_string(self):
        match = self.env["base_import.import"]._match_header(
...
class TestMatchHeadersMultiple(TransactionCase):
    ...
    def test_noheaders(self):
        self.assertEqual(self.env["base_import.import"]._match_headers([], [], {}), ([], {}))
    ...
    def test_nomatch(self):
        self.assertEqual(
            ...
            (["foo", "bar", "baz", "qux"], dict.fromkeys(range(4))),
        )
...
class TestPreview(TransactionCase):
    ...
    def test_csv_success(self):
        ...
        self.assertItemsEqual(
            list(result), ["matches", "headers", "fields", "preview", "headers_type", "options", "debug"]
        )
...
class test_convert_import_data(TransactionCase):
    ...
    def test_all(self):
        ...
        data, fields = import_wizard._convert_import_data(
            ["name", "somevalue", "othervalue"], {"quoting": '"', "separator": ",", "headers": True}
        )
    ...
    def test_parse_relational_fields(self):
        ...
        data, import_fields = import_wizard._convert_import_data(
            ["name", "parent_id/.id", "parent_id/date", "parent_id/credit_limit"], options
        )
    ...
    def test_filtered(self):
        ...
        data, fields = import_wizard._convert_import_data(
            ["name", False, "othervalue"], {"quoting": '"', "separator": ",", "headers": True}
        )
    ...
    def test_empty_rows(self):
        ...
        data, fields = import_wizard._convert_import_data(
            ["name", "somevalue"], {"quoting": '"', "separator": ",", "headers": True}
        )
    ...
    def test_nofield(self):
        ...
        self.assertRaises(
            ValueError, import_wizard._convert_import_data, [], {"quoting": '"', "separator": ",", "headers": True}
        )
...
class test_failures(TransactionCase):
    def test_big_attachments(self):
        ...
        writer.writerows([["name", "db_datas"], ["foo", base64.b64encode(im.tobytes()).decode("ascii")]])

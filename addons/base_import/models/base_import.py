...
class Import(models.TransientModel):
        ...
        if self.file_name:
            ...
            if ext in EXTENSIONS:
                try:
                    ...
                    _logger.warn(
                        "Failed to read file '%s' (transient id %s) using file extension", self.file_name, self.id
                    )
        ...
        for row in pycompat.imap(sheet.row, range(sheet.nrows)):
            ...
            for cell in row:
                if cell.ctype is xlrd.XL_CELL_NUMBER:
                    ...
                    values.append(pycompat.text_type(cell.value) if is_float else pycompat.text_type(int(cell.value)))
    ...
    def _read_ods(self, options):
        ...
        return (row for row in doc.getFirstSheet() if any(x for x in row if x.strip()))
    ...
    def _read_csv(self, options):
        ...
        csv_iterator = pycompat.csv_reader(
            io.BytesIO(csv_data), quotechar=str(options["quoting"]), delimiter=str(options["separator"])
        )
    ...
    def _try_match_column(self, preview_values, options):
        ...
        try:
            ...
            res = {int(v) for v in preview_values if v}
    ...
    def _convert_import_data(self, fields, options):
        ...
        data = [
            list(row)
            for row in pycompat.imap(mapper, rows_to_import)
    ...
    def _parse_import_data_recursive(self, model, prefix, data, import_fields, options):
        ...
        for name, field in all_fields.items():
            ...
            if field["type"] in ("date", "datetime") and name in import_fields:
                ...
                dt = datetime.datetime
                server_format = (
                    DEFAULT_SERVER_DATE_FORMAT if field["type"] == "date" else DEFAULT_SERVER_DATETIME_FORMAT
                )
                        ...
                        if line[index]:
                            try:
                                line[index] = dt.strftime(
                                    dt.strptime(pycompat.to_native(line[index]), user_format), server_format
                                )

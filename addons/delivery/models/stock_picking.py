...
class StockQuantPackage(models.Model):
    ...
    shipping_weight = fields.Float(
        string="Shipping Weight", help="Can be changed during the 'put in pack' to adjust the weight of the shipping."
    )
        ...
        return {
            ...
            "context": {
                ...
                "current_package_carrier_type": (
                    self.picking_id.carrier_id.delivery_type
                    if self.picking_id.carrier_id.delivery_type not in ["base_on_rule", "fixed"]
                    else "none"
                ),
            },
        }
...
class StockPicking(models.Model):
    ...
    @api.one
    @api.depends(
        "move_line_ids", "move_line_ids.result_package_id", "move_line_ids.product_uom_id", "move_line_ids.qty_done"
    )
    ...
    weight = fields.Float(
        compute="_cal_weight", digits=dp.get_precision("Stock Weight"), store=True, compute_sudo=True
    )

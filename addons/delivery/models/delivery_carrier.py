...
class DeliveryCarrier(models.Model):
    ...
    amount = fields.Float(
        string="Amount", help="Amount of the order to benefit from a free shipping, expressed in the company currency"
    )
    ...
    def onchange_countries(self):
        self.state_ids = [
            (6, 0, self.state_ids.filtered(lambda state: state.id in self.country_ids.mapped("state_ids").ids).ids)
        ]

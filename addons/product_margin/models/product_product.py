...
class ProductProduct(models.Model):
    ...
    purchase_avg_price = fields.Float(
        compute="_compute_product_margin_fields_values", string="Avg. Unit Price", help="Avg. Price in Vendor Bills "
    )
    ...
    sales_gap = fields.Float(
        compute="_compute_product_margin_fields_values", string="Sales Gap", help="Expected Sale - Turn Over"
    )
    purchase_gap = fields.Float(
        compute="_compute_product_margin_fields_values", string="Purchase Gap", help="Normal Cost - Total Cost"
    )
    ...
    total_margin = fields.Float(
        compute="_compute_product_margin_fields_values", string="Total Margin", help="Turnover - Standard price"
    )
    expected_margin = fields.Float(
        compute="_compute_product_margin_fields_values", string="Expected Margin", help="Expected Sale - Normal Cost"
    )
    ...
    def _compute_product_margin_fields_values(self, field_names=None):
        ...
        for val in self:
            ...
            res[val.id]["total_margin_rate"] = (
                res[val.id]["turnover"] and res[val.id]["total_margin"] * 100 / res[val.id]["turnover"] or 0.0
            )
            res[val.id]["expected_margin_rate"] = (
                res[val.id]["sale_expected"]
                and res[val.id]["expected_margin"] * 100 / res[val.id]["sale_expected"]
                or 0.0
            )

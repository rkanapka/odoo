...
class TestLandedCosts(TestStockLandedCostsCommon):
    ...
    def test_00_landed_costs_on_incoming_shipment(self):
        ...
        if chart_of_accounts != generic_coa:
            raise unittest.SkipTest(
                "Skip this test as it works only with %s (%s loaded)" % (generic_coa.name, chart_of_accounts.name)
            )
        ...
        stock_landed_cost = self._create_landed_costs(
            {"equal_price_unit": 10, "quantity_price_unit": 150, "weight_price_unit": 250, "volume_price_unit": 20},
            income_ship,
        )
    ...
    def test_01_negative_landed_costs_on_incoming_shipment(self):
        ...
        stock_negative_landed_cost = self._create_landed_costs(
            {"equal_price_unit": -5, "quantity_price_unit": -50, "weight_price_unit": -50, "volume_price_unit": -5},
            income_ship,
        )
        ...
        self.assertTrue(
            stock_negative_landed_cost.account_move_id, "Landed costs should be available account move lines"
        )
        account_entry = self.env["account.move.line"].read_group(
    ...
    def _validate_additional_landed_cost_lines(self, stock_landed_cost, valid_vals):
        for valuation in stock_landed_cost.valuation_adjustment_lines:
            ...
            if split_method == "equal":
                ...
                self.assertEqual(
                    add_cost, valid_vals["by_weight_oven"], self._error_message(valid_vals["by_weight_oven"], add_cost)
                )
                ...
                self.assertEqual(
                    add_cost, valid_vals["by_volume_oven"], self._error_message(valid_vals["by_volume_oven"], add_cost)
                )

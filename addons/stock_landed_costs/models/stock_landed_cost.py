...
class LandedCost(models.Model):
    ...
    picking_ids = fields.Many2many(
        "stock.picking", string="Pickings", copy=False, states={"done": [("readonly", True)]}
    )
    cost_lines = fields.One2many(
        "stock.landed.cost.lines", "cost_id", "Cost Lines", copy=True, states={"done": [("readonly", True)]}
    )
    valuation_adjustment_lines = fields.One2many(
        "stock.valuation.adjustment.lines", "cost_id", "Valuation Adjustments", states={"done": [("readonly", True)]}
    )
    ...
    account_journal_id = fields.Many2one(
        "account.journal", "Account Journal", required=True, states={"done": [("readonly", True)]}
    )
    company_id = fields.Many2one("res.company", string="Company", related="account_journal_id.company_id")
    ...
    def _check_sum(self):
        ...
        for landed_cost in self:
            ...
            val_to_cost_lines = defaultdict(float)
            for val_line in landed_cost.valuation_adjustment_lines:
                val_to_cost_lines[val_line.cost_line_id] += val_line.additional_landed_cost
            if any(
                tools.float_compare(cost_line.price_unit, val_amount, precision_digits=prec_digits) != 0
...
class AdjustmentLines(models.Model):
    ...
    def _create_accounting_entries(self, move, qty_out):
        ...
        credit_account_id = (
            ...
            or cost_product.property_account_expense_id.id
            or cost_product.categ_id.property_account_expense_categ_id.id
        )

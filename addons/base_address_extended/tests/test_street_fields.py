...
class TestStreetFields(TransactionCase):
    ...
    def setUp(self):
        ...
        self.env.ref("base.ch").write(
            {"street_format": "header %(street_name)s, %(street_number)s - %(street_number2)s trailer"}
        )
    ...
    def create_and_assert(self, partner_name, country_id, street, street_name, street_number, street_number2):
        ...
        self.assertEqual(
            ...
            street_name,
            "wrong street name for %s: %s" % (partner_name, partner.street_name),
        )
        self.assertEqual(
            partner.street_number or "",
            street_number,
            "wrong house number for %s: %s" % (partner_name, partner.street_number),
        )
        self.assertEqual(
            partner.street_number2 or "",
            street_number2,
            "wrong door number for %s: %s" % (partner_name, partner.street_number2),
        )
    ...
    def write_and_assert(self, partner, vals, street, street_name, street_number, street_number2):
        ...
        self.assertEqual(
            partner.street_number2 or "", street_number2, "wrong door number: %s" % partner.street_number2
        )
    ...
    def test_00_res_partner_name_create(self):
        self.create_and_assert(
            "Test00", self.env.ref("base.us").id, "40/2b Chaussee de Namur", "Chaussee de Namur", "40", "2b"
        )
        self.create_and_assert(
            "Test01", self.env.ref("base.us").id, "40 Chaussee de Namur", "Chaussee de Namur", "40", ""
        )
        self.create_and_assert("Test02", self.env.ref("base.us").id, "Chaussee de Namur", "de Namur", "Chaussee", "")
        self.create_and_assert(
            "Test03",
            self.env.ref("base.mx").id,
            "Av. Miguel Hidalgo y Costilla 601",
            "Av.",
            "Miguel Hidalgo y Costilla 601",
            "",
        )
    ...
    def test_01_header_trailer(self):
        self.create_and_assert(
            ...
            "header Chaussee de Namur, 40 - 2b trailer",
            "Chaussee de Namur",
            "40",
            "2b",
        )
        self.create_and_assert(
            "Test11", self.env.ref("base.ch").id, "header Chaussee de Namur, 40 trailer", "Chaussee de Namur", "40", ""
        )
        self.create_and_assert(
            "Test12", self.env.ref("base.ch").id, "header Chaussee de Namur trailer", "Chaussee de Namur", "", ""
        )

    def test_02_res_partner_write(self):
        p1 = self.create_and_assert(
            "Test20", self.env.ref("base.be").id, "Chaussee de Namur, 40/2b", "Chaussee de Namur", "40", "2b"
        )
        self.write_and_assert(
            p1, {"street": "Chaussee de Namur, 43"}, "Chaussee de Namur, 43", "Chaussee de Namur", "43", ""
        )
        self.write_and_assert(p1, {"street": "Chaussee de Namur"}, "Chaussee de Namur", "Chaussee de Namur", "", "")
        self.write_and_assert(
            p1, {"street_name": "Chee de Namur", "street_number": "40"}, "Chee de Namur, 40", "Chee de Namur", "40", ""
        )
        ...
        self.write_and_assert(
            p1, {"country_id": self.env.ref("base.us").id}, "40/4 Chee de Namur", "Chee de Namur", "40", "4"
        )

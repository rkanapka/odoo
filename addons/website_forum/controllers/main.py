...
from odoo import SUPERUSER_ID, _, http, modules, tools
...
class WebsiteForum(http.Controller):
    ...
    def validate_email(self, token, id, email, forum_id=None, **kwargs):
        if forum_id:
            ...
            .sudo()
            .browse(int(id))
            .process_forum_validation_token(token, email, forum_id=forum_id)[0]
        )
        if done:
    ...
    @http.route(
        ['/forum/<model("forum.forum"):forum>/tag', '/forum/<model("forum.forum"):forum>/tag/<string:tag_char>'],
        type="http",
        auth="public",
        website=True,
        sitemap=False,
    ...
    @http.route(
        [
            """/forum/<model("forum.forum"):forum>/question/<model("forum.post", "[('forum_id','=',forum[0]),('parent_id','=',False),('can_view', '=', True)]"):question>"""
        ],
        type="http",
        auth="public",
        website=True,
    ...
    def post_create(self, forum, post_parent=None, post_type=None, **post):
        ...
        if post.get("content", "") == "<p><br></p>":
            return request.render(
                "website.http_error",
                {"status_code": _("Bad Request"), "status_message": _("Question should not be empty.")},
            )
    ...
    def post_edit(self, forum, post, **kwargs):
        ...
        template = (
            "website_forum.new_link" if post.post_type == "link" and not post.parent_id else "website_forum.edit_post"
        )
    ...
    @http.route(
        ['/forum/<model("forum.forum"):forum>/users', '/forum/<model("forum.forum"):forum>/users/page/<int:page>'],
        type="http",
        auth="public",
        website=True,
    )
    ...
    @http.route(
        ['/forum/<model("forum.forum"):forum>/partner/<int:partner_id>'], type="http", auth="public", website=True
    )
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/user/<model("res.users"):user>/edit',
        type="http",
        auth="user",
        website=True,
    )
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/user/<model("res.users"):user>/save',
        type="http",
        auth="user",
        methods=["POST"],
        website=True,

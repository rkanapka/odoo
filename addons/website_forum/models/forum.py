...
class Forum(models.Model):
    ...
    default_post_type = fields.Selection(
        [("question", "Question"), ("discussion", "Discussion"), ("link", "Link")],
        string="Default Post",
        required=True,
        default="question",
    )
    allow_question = fields.Boolean(
        "Questions",
        help="Users can answer only once per question. Contributors can edit answers and mark the right ones.",
        default=True,
    )
    allow_discussion = fields.Boolean("Discussions", default=True)
    allow_link = fields.Boolean(
        "Links", help="When clicking on the post, it redirects to an external link", default=True
    )
    ...
    karma_editor = fields.Integer(
        string="Editor Features: image and links", default=30, oldname="karma_editor_link_files"
    )
    ...
    def _check_default_post_type(self):
        if (
            (self.default_post_type == "question" and not self.allow_question)
            ...
            raise ValidationError(
                _("You cannot choose %s as default post since the forum does not allow it.") % self.default_post_type
            )
...
class Post(models.Model):
    ...
    post_type = fields.Selection(
        [("question", "Question"), ("link", "Article"), ("discussion", "Discussion")],
        string="Type",
        default="question",
        required=True,
    )
    ...
    can_display_biography = fields.Boolean(
        "Is the author's biography visible from his post", compute="_get_post_karma_rights"
    )
    ...
    def _check_post_type(self):
        if (
            (self.post_type == "question" and not self.forum_id.allow_question)
    ...
    def check_mail_message_access(self, res_ids, operation, model_name=None):
        ...
        return super().check_mail_message_access(res_ids, operation, model_name=model_name)
    ...
    def write(self, vals):
        ...
        if "tag_ids" in vals:
            tag_ids = {tag.get("id") for tag in self.resolve_2many_commands("tag_ids", vals["tag_ids"])}
            if any(set(post.tag_ids.ids) != tag_ids for post in self) and any(
                self.env.user.karma < post.forum_id.karma_edit_retag for post in self
            ):
    ...
    def convert_comment_to_answer(self, message_id, default=None):
        ...
        karma_convert = (
            comment.author_id.id == self.env.user.partner_id.id
    ...
    def unlink_comment(self, message_id):
        ...
        karma_unlink = (
            ...
            and self.forum_id.karma_comment_unlink_own
            or self.forum_id.karma_comment_unlink_all
        )
    ...
    def create(self, vals):
        ...
        if vote.post_id.parent_id:
            karma_value = self._get_karma_value(
                "0", vote.vote, vote.forum_id.karma_gen_answer_upvote, vote.forum_id.karma_gen_answer_downvote
            )
        else:
            karma_value = self._get_karma_value(
                "0", vote.vote, vote.forum_id.karma_gen_question_upvote, vote.forum_id.karma_gen_question_downvote
            )
    ...
    def write(self, values):
        if "vote" in values:
            for vote in self:
                ...
                ) and not vote.post_id.can_upvote:
                    raise KarmaError("Not enough karma to upvote.")
                elif (
                    values["vote"] == "-1" or vote.vote == "1" and values["vote"] == "0"
                ) and not vote.post_id.can_downvote:
                    raise KarmaError("Not enough karma to downvote.")
...
class Tags(models.Model):
    ...
    post_ids = fields.Many2many(
        "forum.post", "forum_tag_rel", "forum_tag_id", "forum_id", string="Posts", domain=[("state", "=", "active")]
    )

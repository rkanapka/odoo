...
class DiagramView(http.Controller):
    ...
    def get_diagram_info(self, id, model, node, connector, src_node, des_node, label, **kw):
        ...
        graphs = ir_view.graph_get(int(id), model, node, connector, src_node, des_node, label, (140, 180))
        nodes = graphs["nodes"]
        ...
        for tr in transitions:
            list_tr.append(tr)
            connectors.setdefault(tr, {"id": int(tr), "s_id": transitions[tr][0], "d_id": transitions[tr][1]})
        ...
        for tr in data_connectors:
            ...
            t.update(source=tr[src_node][1], destination=tr[des_node][1], options={}, signal=label)
        ...
        return dict(nodes=nodes, conn=connectors, display_name=name, parent_field=graphs["node_parent_field"])

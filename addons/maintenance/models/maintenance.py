...
class MaintenanceEquipmentCategory(models.Model):
    ...
    technician_user_id = fields.Many2one(
        "res.users", "Responsible", track_visibility="onchange", default=lambda self: self.env.uid, oldname="user_id"
    )
...
class MaintenanceEquipment(models.Model):
    ...
    technician_user_id = fields.Many2one(
        "res.users", string="Technician", track_visibility="onchange", oldname="user_id"
    )
    ...
    next_action_date = fields.Date(
        compute="_compute_next_maintenance", string="Date of the next preventive maintenance", store=True
    )
        ...
        for equipment in self.filtered(lambda x: x.period > 0):
            ...
            if next_maintenance_todo and last_maintenance_done:
                ...
                if (
                    ...
                    and fields.Date.from_string(next_maintenance_todo.request_date) > fields.Date.from_string(date_now)
                ):
                    # If the new date still in the past, we set it for today
                    if fields.Date.from_string(last_maintenance_done.close_date) + timedelta(
                        days=equipment.period
                    ) < fields.Date.from_string(date_now):
                        next_date = date_now
                    else:
    ...
    def _message_get_auto_subscribe_fields(self, updated_fields, auto_follow_fields=None):
        ...
        if auto_follow_fields is None:
            auto_follow_fields = []
        return super()._message_get_auto_subscribe_fields(updated_fields, auto_follow_fields)
...
class MaintenanceRequest(models.Model):
    ...
    archive = fields.Boolean(
        default=False, help="Set archive to true to hide the maintenance request without deleting it."
    )
    maintenance_type = fields.Selection(
        [("corrective", "Corrective"), ("preventive", "Preventive")], string="Maintenance Type", default="corrective"
    )
    ...
    def onchange_category_id(self):
        if (
            not self.technician_user_id
            or not self.equipment_id
            or (self.technician_user_id and not self.equipment_id.technician_user_id)
        ):
            self.technician_user_id = self.category_id.technician_user_id

...
class WebsiteForm(http.Controller):
    ...
    @http.route(
        "/website_form/<string:model_name>", type="http", auth="public", methods=["POST"], website=True, csrf=False
    )
    def website_form(self, model_name, **kwargs):
        ...
        model_record = (
            request.env["ir.model"].sudo().search([("model", "=", model_name), ("website_form_access", "=", True)])
        )
        if not model_record:
    ...
    def extract_data(self, model, values):
        ...
        missing_required_fields = [
            label for label, field in authorized_fields.items() if field["required"] and not label in data["record"]
        ]
        if any(error_fields):

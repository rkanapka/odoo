...
class website_form_model_fields(models.Model):
    ...
    def init(self):
        ...
        self._cr.execute(
            "UPDATE ir_model_fields" " SET website_form_blacklisted=true" " WHERE website_form_blacklisted IS NULL"
        )
        ...
        self._cr.execute("ALTER TABLE ir_model_fields " " ALTER COLUMN website_form_blacklisted SET DEFAULT true")
    ...
    def formbuilder_whitelist(self, model, fields):
        ...
        self.env.cr.execute(
            "UPDATE ir_model_fields" " SET website_form_blacklisted=false" " WHERE model=%s AND name in %s",
            (model, tuple(fields)),
        )
        return True

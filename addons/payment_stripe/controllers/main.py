    ...
    def stripe_create_charge(self, **post):
        ...
        if tx.type == "form_save" and tx.partner_id:
            payment_token_id = (
                ...
                .sudo()
                .create(
                    {"acquirer_id": tx.acquirer_id.id, "partner_id": tx.partner_id.id, "stripe_token": stripe_token}
                )
            )
            tx.payment_token_id = payment_token_id
            response = tx._create_stripe_charge(
                acquirer_ref=payment_token_id.acquirer_ref, email=stripe_token["email"]
            )
        else:

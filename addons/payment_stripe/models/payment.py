...
class PaymentTransactionStripe(models.Model):
    ...
    def _create_stripe_charge(self, acquirer_ref=None, tokenid=None, email=None):
        ...
        r = requests.post(
            api_url_charge, auth=(self.acquirer_id.stripe_secret_key, ""), params=charge_params, headers=STRIPE_HEADERS
        )
        return r.json()
    ...
    def _create_stripe_refund(self):
        ...
        r = requests.post(
            api_url_refund, auth=(self.acquirer_id.stripe_secret_key, ""), params=refund_params, headers=STRIPE_HEADERS
        )
        return r.json()
    ...
    def _stripe_form_get_tx_from_data(self, data):
        ...
        if not reference:
            ...
            _logger.error(
                "Stripe: invalid reply received from stripe API, looks like " "the transaction failed. (error: %s)",
                stripe_error or "n/a",
            )
...
class PaymentTokenStripe(models.Model):
    ...
    def stripe_create(self, values):
        ...
        if values.get("cc_number"):
            ...
            r = requests.post(
                url_token, auth=(payment_acquirer.stripe_secret_key, ""), params=payment_params, headers=STRIPE_HEADERS
            )
            token = r.json()
    ...
    def _stripe_create_customer(self, token, description=None, acquirer_id=None):
        ...
        if token["object"] != "token":
            _logger.error(
                'payment.token.stripe_create_customer: Cannot create a customer for object type "%s"',
                token.get("object"),
            )
        ...
        if token["type"] != "card":
            _logger.error(
                'payment.token.stripe_create_customer: Cannot create a customer for token type "%s"', token.get("type")
            )
            raise Exception("We are unable to process your credit card information.")
        ...
        customer_params = {"source": token["id"], "description": description or token["card"]["name"]}

        r = requests.post(
            url_customer, auth=(payment_acquirer.stripe_secret_key, ""), params=customer_params, headers=STRIPE_HEADERS
        )
        customer = r.json()

        if customer.get("error"):
            _logger.error(
                "payment.token.stripe_create_customer: Customer error:\n%s", pprint.pformat(customer["error"])
            )

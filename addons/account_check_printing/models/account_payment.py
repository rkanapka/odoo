...
class AccountRegisterPayments(models.TransientModel):
    ...
    def _onchange_journal_id(self):
        if hasattr(super(), "_onchange_journal_id"):
    ...
    def _onchange_amount(self):
        if hasattr(super(), "_onchange_amount"):
            ...
            res.update(
                {
                    "check_amount_in_words": (
                        self.currency_id.amount_to_text(res["amount"]) if self.multi else self.check_amount_in_words
                    ),
                }
            )
        return res
...
class AccountPayment(models.Model):
    ...
    def _check_communication(self, payment_method_id, communication):
        super()._check_communication(payment_method_id, communication)
    ...
    def print_checks(self):
        ...
        if not self[0].journal_id.check_manual_sequencing:
            ...
            last_printed_check = self.search(
                [("journal_id", "=", self[0].journal_id.id), ("check_number", "!=", 0)],
    ...
    def set_check_amount_in_words(self):
        for payment in self:
            payment.check_amount_in_words = (
                payment.currency_id.amount_to_text(payment.amount) if payment.currency_id else ""
            )

...
class AccountJournal(models.Model):
    ...
    check_manual_sequencing = fields.Boolean(
        "Manual Numbering", default=False, help="Check this option if your pre-printed checks are not numbered."
    )
    check_sequence_id = fields.Many2one(
        "ir.sequence", "Check Sequence", readonly=True, copy=False, help="Checks numbering sequence."
    )
    check_next_number = fields.Integer(
        "Next Check Number",

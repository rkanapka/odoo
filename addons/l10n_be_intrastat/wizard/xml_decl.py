...
class XmlDeclaration(models.TransientModel):
    ...
    arrivals = fields.Selection(
        [("be-exempt", "Exempt"), ("be-standard", "Standard"), ("be-extended", "Extended")],
        required=True,
        default="be-standard",
    )
    dispatches = fields.Selection(
        [("be-exempt", "Exempt"), ("be-standard", "Standard"), ("be-extended", "Extended")],
        required=True,
        default="be-standard",
    )
    ...
    def create_xml(self):
        ...
        if not company.company_registry:
            self._company_warning(
                _("The registry number of your company is not set, " "please make sure to configure it first.")
            )
        if len(self.year) != 4:
    ...
    def _get_intrastat_linekey(self, declcode, inv_line, dispatchmode, extendedmode):
        ...
        if inv_line.invoice_id.intrastat_country_id:
            ...
            excnt = (
                inv_line.invoice_id.partner_shipping_id.country_id.code
                or inv_line.invoice_id.partner_id.country_id.code
            )
        ...
        if not exreg:
            if company.region_id:
                ...
                self._company_warning(
                    _(
                        "The Intrastat Region of the selected company is not set, "
        ...
        if extendedmode:
            ...
            if inv_line.invoice_id.transport_mode_id:
                ...
                self._company_warning(
                    _(
                        "The default Intrastat transport mode of your company "
            ...
            if inv_line.invoice_id.incoterm_id:
                ...
                self._company_warning(
                    _("The default Incoterm of your company is not set, " "please make sure to configure it first.")
                )
        else:
            extpc = ""
            exdeltrm = ""
        ...
        return intrastatkey(
            EXTRF=declcode, EXCNT=excnt, EXTTA=extta, EXREG=exreg, EXGO=exgo, EXTPC=extpc, EXDELTRM=exdeltrm
        )
    ...
    def _is_situation_triangular(self, company, po_line=False, so_line=False):
        ...
        return stock_move and (
            (
                ...
                and stock_move.picking_partner_id.country_id.code != company.country_id.code
            )
            or (
                stock_move.partner_id.country_id.code == company.country_id.code
                and stock_move.picking_partner_id.country_id.code == company.country_id.code

...
class HrPayslip(models.Model):
    ...
    employee_id = fields.Many2one(
        "hr.employee", string="Employee", required=True, readonly=True, states={"draft": [("readonly", False)]}
    )
    ...
    line_ids = fields.One2many(
        "hr.payslip.line", "slip_id", string="Payslip Lines", readonly=True, states={"draft": [("readonly", False)]}
    )
    ...
    paid = fields.Boolean(
        string="Made Payment Order ? ", readonly=True, copy=False, states={"draft": [("readonly", False)]}
    )
    note = fields.Text(string="Internal Note", readonly=True, states={"draft": [("readonly", False)]})
    contract_id = fields.Many2one(
        "hr.contract", string="Contract", readonly=True, states={"draft": [("readonly", False)]}
    )
    details_by_salary_rule_category = fields.One2many(
        "hr.payslip.line", compute="_compute_details_by_salary_rule_category", string="Details by Salary Rule Category"
    )
    ...
    payslip_run_id = fields.Many2one(
        "hr.payslip.run", string="Payslip Batches", readonly=True, copy=False, states={"draft": [("readonly", False)]}
    )
    ...
    def get_contract(self, employee, date_from, date_to):
        ...
        clause_final = (
            [("employee_id", "=", employee.id), ("state", "=", "open"), "|", "|"] + clause_1 + clause_2 + clause_3
        )
...
class HrPayslipWorkedDays(models.Model):
    ...
    contract_id = fields.Many2one(
        "hr.contract", string="Contract", required=True, help="The contract for which applied this input"
    )
...
class HrPayslipRun(models.Model):
    ...
    slip_ids = fields.One2many(
        "hr.payslip", "payslip_run_id", string="Payslips", readonly=True, states={"draft": [("readonly", False)]}
    )
    state = fields.Selection(
        [
            ("draft", "Draft"),

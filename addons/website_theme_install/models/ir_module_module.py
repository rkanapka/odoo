...
class IrModuleModule(models.Model):
    ...
    def button_choose_theme(self):
        ...
        hidden_categories_ids = [
            hidden_category.id if hidden_category else 0,
            theme_hidden_category.id if theme_hidden_category else 0,
        ]

        self.search(
            [  # Uninstall the theme(s) which is (are) installed
                ("state", "=", "installed"),
                "|",
                ("category_id", "not in", hidden_categories_ids),
                ("name", "=", "theme_default"),
                "|",
                ("category_id", "=", theme_category_id),
                ("category_id.parent_id", "=", theme_category_id),
            ]

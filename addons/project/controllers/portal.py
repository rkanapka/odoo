    ...
    def portal_my_tasks(
        ...
        **kw
        ...
        for proj in projects:
            searchbar_filters.update({str(proj.id): {"label": proj.name, "domain": [("project_id", "=", proj.id)]}})
        ...
        tasks = request.env["project.task"].search(
            domain, order=order, limit=self._items_per_page, offset=(page - 1) * self._items_per_page
        )
        request.session["my_tasks_history"] = tasks.ids[:100]
        if groupby == "project":
            grouped_tasks = [
                request.env["project.task"].concat(*g) for k, g in groupbyelem(tasks, itemgetter("project_id"))
            ]
        else:
            grouped_tasks = [tasks]

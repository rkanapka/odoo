...
class Project(models.Model):
    ...
    def _compute_task_needaction_count(self):
        ...
        mapped_data = {
            project_data["project_id"][0]: int(project_data["project_id_count"]) for project_data in projects_data
        }
        for project in self:
    ...
    label_tasks = fields.Char(
        string="Use Tasks as", default="Tasks", help="Gives label to tasks on project's kanban view."
    )
    ...
    user_id = fields.Many2one(
        "res.users", string="Project Manager", default=lambda self: self.env.user, track_visibility="onchange"
    )
    ...
    def map_tasks(self, new_project_id):
        ...
        task_ids = (
            ...
            .search([("project_id", "=", self.id)], order="parent_id")
            .ids
        )
        old_to_new_tasks = {}
        for task in self.env["project.task"].browse(task_ids):
            ...
            project_id = (
                new_project_id
                if not parent_id
                else self.env["project.project"].browse(new_project_id).subtask_project_id.id
            )
    ...
    def message_subscribe(self, partner_ids=None, channel_ids=None, subtype_ids=None, force=True):
        ...
        res = super().message_subscribe(
            partner_ids=partner_ids, channel_ids=channel_ids, subtype_ids=subtype_ids, force=force
        )
        if not subtype_ids or any(
            subtype.parent_id.res_model == "project.task"
            for subtype in self.env["mail.message.subtype"].browse(subtype_ids)
        ):
            for partner_id in partner_ids or []:
                self.mapped("tasks").filtered(
                    lambda task: not task.stage_id.fold and partner_id not in task.message_partner_ids.ids
                ).message_subscribe(partner_ids=[partner_id], channel_ids=None, subtype_ids=None, force=False)
            for channel_id in channel_ids or []:
                self.mapped("tasks").filtered(
                    lambda task: not task.stage_id.fold and channel_id not in task.message_channel_ids.ids
                ).message_subscribe(partner_ids=None, channel_ids=[channel_id], subtype_ids=None, force=False)
        return res
    ...
    def message_unsubscribe(self, partner_ids=None, channel_ids=None):
        ...
        return super().message_unsubscribe(partner_ids=partner_ids, channel_ids=channel_ids)
...
class Task(models.Model):
    ...
    sequence = fields.Integer(
        string="Sequence", index=True, default=10, help="Gives the sequence order when displaying a list of tasks."
    )
    ...
    date_last_stage_update = fields.Datetime(
        string="Last Stage Update", default=fields.Datetime.now, index=True, copy=False, readonly=True
    )
    ...
    user_id = fields.Many2one(
        "res.users", string="Assigned to", default=lambda self: self.env.uid, index=True, track_visibility="always"
    )
    partner_id = fields.Many2one("res.partner", string="Customer", default=_get_default_partner)
    manager_id = fields.Many2one(
        "res.users", string="Project Manager", related="project_id.user_id", readonly=True, related_sudo=False
    )
    company_id = fields.Many2one(
        "res.company", string="Company", default=lambda self: self.env["res.company"]._company_default_get()
    )
    color = fields.Integer(string="Color Index")
    ...
    legend_blocked = fields.Char(
        related="stage_id.legend_blocked", string="Kanban Blocked Explanation", readonly=True, related_sudo=False
    )
    legend_done = fields.Char(
        related="stage_id.legend_done", string="Kanban Valid Explanation", readonly=True, related_sudo=False
    )
    legend_normal = fields.Char(
        related="stage_id.legend_normal", string="Kanban Ongoing Explanation", readonly=True, related_sudo=False
    )
    parent_id = fields.Many2one("project.task", string="Parent Task", index=True)
    child_ids = fields.One2many("project.task", "parent_id", string="Sub-tasks", context={"active_test": False})
    subtask_project_id = fields.Many2one(
        "project.project", related="project_id.subtask_project_id", string="Sub-task Project", readonly=True
    )
    ...
    working_hours_open = fields.Float(
        compute="_compute_elapsed", string="Working hours to assign", store=True, group_operator="avg"
    )
    working_hours_close = fields.Float(
        compute="_compute_elapsed", string="Working hours to close", store=True, group_operator="avg"
    )
    working_days_open = fields.Float(
        compute="_compute_elapsed", string="Working days to assign", store=True, group_operator="avg"
    )
    ...
    def _compute_attachment_ids(self):
        for task in self:
            attachment_ids = (
                self.env["ir.attachment"].search([("res_id", "=", task.id), ("res_model", "=", "project.task")]).ids
            )
    ...
    def _compute_elapsed(self):
        task_linked_to_calendar = self.filtered(lambda task: task.project_id.resource_calendar_id and task.create_date)
    ...
    def _check_subtask_project(self):
        for task in self:
            if task.parent_id.project_id and task.project_id != task.parent_id.project_id.subtask_project_id:
                raise UserError(
                    _(
                        "You can't define a parent task if its project is not correctly configured. The sub-task's project of the parent task's project should be this task's project"
                    )
                )
    ...
    def fields_view_get(self, view_id=None, view_type="form", toolbar=False, submenu=False):
        ...
        obj_tm and obj_tm.name or "Hours"
    ...
    def _notification_recipients(self, message, groups):
        ...
        new_group = (
            ...
            and any(user.has_group("project.group_project_user") for user in partner.user_ids),
            {
    ...
    def message_get_email_values(self, notif_mail=None):
        res = super().message_get_email_values(notif_mail=notif_mail)
        headers = {}
        if res.get("headers"):
            try:
    ...
    def _message_post_after_hook(self, message):
        if self.email_from and not self.partner_id:
            ...
            if new_partner:
                self.search(
                    [("partner_id", "=", False), ("email_from", "=", new_partner.email), ("stage_id.fold", "=", False)]

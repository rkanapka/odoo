...
class TestPortalProjectBase(TestProjectBase):
    ...
    def setUp(self):
        ...
        self.task_3 = (
            ...
            .create({"name": "Test3", "user_id": self.user_portal.id, "project_id": self.project_pigs.id})
        )
        self.task_4 = (
            self.env["project.task"]
            .with_context({"mail_create_nolog": True})
            .create({"name": "Test4", "user_id": self.user_public.id, "project_id": self.project_pigs.id})
        )
        self.task_5 = (
            self.env["project.task"]
            .with_context({"mail_create_nolog": True})
            .create({"name": "Test5", "user_id": False, "project_id": self.project_pigs.id})
        )
        self.task_6 = (
            self.env["project.task"]
            .with_context({"mail_create_nolog": True})
...
class TestPortalProject(TestPortalProjectBase):
    ...
    def test_employee_project_access_rights(self):
        ...
        self.assertEqual(
            set(tasks.ids), test_task_ids, "access rights: project user cannot see all tasks of an employees project"
        )
    ...
    def test_followers_project_access_rights(self):
        ...
        self.assertRaises(
            AccessError,
            self.env["project.task"].sudo(self.user_projectuser).with_context({"mail_create_nolog": True}).create,
            {"name": "Pigs task", "project_id": pigs.id},
        )

...
class TestProjectBase(TestMail):
    ...
    def setUpClass(cls):
        ...
        cls.task_1 = (
            ...
            .with_context({"mail_create_nolog": True})
            .create({"name": "Pigs UserTask", "user_id": cls.user_projectuser.id, "project_id": cls.project_pigs.id})
        )
        cls.task_2 = (
            cls.env["project.task"]
            .with_context({"mail_create_nolog": True})
            .create(
                {"name": "Pigs ManagerTask", "user_id": cls.user_projectmanager.id, "project_id": cls.project_pigs.id}
            )
        )

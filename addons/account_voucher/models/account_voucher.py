...
class AccountVoucher(models.Model):
    ...
    state = fields.Selection(
        [("draft", "Draft"), ("cancel", "Cancelled"), ("proforma", "Pro-forma"), ("posted", "Posted")],
        "Status",
        readonly=True,
    ...
    partner_id = fields.Many2one(
        "res.partner", "Partner", change_default=1, readonly=True, states={"draft": [("readonly", False)]}
    )
    ...
    def first_move_line_get(self, move_id, company_currency, current_currency):
        ...
        move_line = {
            ...
            "amount_currency": (
                sign * abs(self.amount) if company_currency != current_currency else 0.0  # amount < 0 for refunds
            ),
        ...
        for line in self.line_ids:
            ...
            if tax_calculation_rounding_method == "round_globally":
                ...
                tax_lines_vals += (
                    ...
                    ._apply_taxes(move_line, move_line.get("debit", 0.0) - move_line.get("credit", 0.0))
                )
    ...
    def action_move_line_create(self):
        ...
        for voucher in self:
            ...
            move_line = (
                self.env["account.move.line"]
                .with_context(ctx)
                .create(voucher.with_context(ctx).first_move_line_get(move.id, company_currency, current_currency))
            )
...
class AccountVoucherLine(models.Model):
    ...
    price_unit = fields.Float(
        string="Unit Price", required=True, digits=dp.get_precision("Product Price"), oldname="amount"
    )
    ...
    company_id = fields.Many2one(
        "res.company", related="voucher_id.company_id", string="Company", store=True, readonly=True
    )
    ...
    def product_id_change(
        self, product_id, partner_id=False, price_unit=False, company_id=None, currency_id=None, type=None
    ):

...
class WebsiteEventController(http.Controller):
    ...
    def events(self, page=1, **searches):
        ...
        dates = [
            ...
            ["today", _("Today"), [("date_end", ">", sd(today)), ("date_begin", "<", sdn(today))], 0],
            [
                "week",
                _("This Week"),
                [
            ...
            [
                ...
                [
                    ...
                    (
                        "date_begin",
                        "<",
                        (today.replace(day=1) + relativedelta(months=2)).strftime("%Y-%m-%d 00:00:00"),
                    ),
                ],
                0,
            ],
            ["old", _("Old Events"), [("date_end", "<", today.strftime("%Y-%m-%d 00:00:00"))], 0],
        ]
    ...
    @http.route(
        ['/event/<model("event.event"):event>/register'], type="http", auth="public", website=True, sitemap=False
    )
    ...
    def get_formated_date(self, event):
        ...
        month = babel.dates.get_month_names("abbreviated", locale=event.env.context.get("lang") or "en_US")[
            start_date.month
        ]
        return ("%s %s%s") % (
            month,
    ...
    def get_country_events(self, **post):
        ...
        if not events:
            events = Event.search(
                [("date_begin", ">=", "%s 00:00:00" % fields.Date.today()), ("state", "=", "confirm")],
                order="date_begin",
            )
        for event in events:
            if country_code and event.country_id.code == country_code:
                result["country"] = country
            result["events"].append({"date": self.get_formated_date(event), "event": event, "url": event.website_url})
    ...
    def registration_new(self, event, **post):
        ...
        return request.env["ir.ui.view"].render_template(
            "website_event.registration_attendee_details", {"tickets": tickets, "event": event}
        )
    ...
    def registration_confirm(self, event, **post):
        ...
        for registration in registrations:
            ...
            Attendees += Attendees.sudo().create(Attendees._prepare_attendee_values(registration))

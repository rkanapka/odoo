    ...
    def _set_website_menu(self):
        for event in self:
            if event.menu_id and not event.website_menu:
                ...
                submenu_to_delete = event.menu_id.child_id.filtered(
                    lambda menu: menu.name not in required_page_names and menu.name in standard_page_names
                )
                    ...
                    if name not in existing_page_names:
                        if not url:
                            newpath = self.env["website"].new_page(
                                name + " " + event.name, template=xml_id, ispage=False
                            )["url"]

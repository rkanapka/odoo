...
class ISRTest(AccountingTestCase):
    ...
    def create_account(self, number):
        ...
        return self.env["res.partner.bank"].create({"acc_number": number})
    ...
    def test_l10n_ch_postals(self):
        ...
        self.assertEqual(
            account_test_postal_ok.acc_type,
            "postal",
            "A valid postal number in acc_number should set its type to 'postal'",
        )
        self.assertEqual(
            account_test_postal_ok.l10n_ch_postal,
            "010391391",
            "A postal account should have a postal reference identical to its account number",
        )
        ...
        self.assertNotEqual(
            account_test_postal_wrong.acc_type, "postal", "A non-postal account cannot be of type 'postal'"
        )
        ...
        self.assertEqual(
            account_test_iban_ok.l10n_ch_postal, "000250097798", "A valid swiss IBAN should set the postal reference"
        )
    ...
    def test_isr(self):
        ...
        test_bank = self.env["res.bank"].create({"name": "Money Drop", "l10n_ch_postal_chf": "010391391"})

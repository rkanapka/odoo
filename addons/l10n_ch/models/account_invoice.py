...
class AccountInvoice(models.Model):
    ...
    l10n_ch_isr_number = fields.Char(
        compute="_compute_l10n_ch_isr_number", store=True, help="The reference number associated with this invoice"
    )
    ...
    l10n_ch_currency_name = fields.Char(
        related="currency_id.name", help="The name of this invoice's currency"
        ...
        for record in self:
            if record.number and record.partner_bank_id and record.partner_bank_id.l10n_ch_postal:
                ...
                internal_ref = invoice_ref.zfill(
                    l10n_ch_ISR_NUMBER_LENGTH - l10n_ch_ISR_NUMBER_ISSUER_LENGTH - 1
                )  # -1 for mod10r check character
    ...
    @api.depends(
        ...
        "partner_bank_id.bank_id.l10n_ch_postal_eur",
        "partner_bank_id.bank_id.l10n_ch_postal_chf",
    )
        ...
        for record in self:
            if record.l10n_ch_isr_number and record.l10n_ch_isr_postal and record.currency_id.name:
                ...
                record.l10n_ch_isr_optical_line = (
                    left + ">" + record.l10n_ch_isr_number + "+ " + record.l10n_ch_isr_postal + ">"
                )
    ...
    def _compute_l10n_ch_isr_valid(self):
        ...
        for record in self:
            record.l10n_ch_isr_valid = (
                ...
                and record.number
                and record.l10n_ch_isr_postal
                and record.partner_bank_id
                and record.partner_bank_id.l10n_ch_postal
    ...
    def action_invoice_sent(self):
        ...
        rslt = super().action_invoice_sent()

...
class Repair(models.Model):
    ...
    product_uom = fields.Many2one(
        "product.uom", "Product Unit of Measure", readonly=True, required=True, states={"draft": [("readonly", False)]}
    )
    ...
    operations = fields.One2many(
        "mrp.repair.line", "repair_id", "Parts", copy=True, readonly=True, states={"draft": [("readonly", False)]}
    )
    ...
    fees_lines = fields.One2many(
        "mrp.repair.fee", "repair_id", "Operations", copy=True, readonly=True, states={"draft": [("readonly", False)]}
    )
    internal_notes = fields.Text("Internal Notes")
    quotation_notes = fields.Text("Quotation Notes")
    company_id = fields.Many2one(
        "res.company", "Company", default=lambda self: self.env["res.company"]._company_default_get("mrp.repair")
    )
    ...
    @api.one
    @api.depends(
        "operations.price_subtotal", "invoice_method", "fees_lines.price_subtotal", "pricelist_id.currency_id"
    )
        ...
        for fee in self.fees_lines:
            if fee.tax_id:
                tax_calculate = fee.tax_id.compute_all(
                    fee.price_unit, self.pricelist_id.currency_id, fee.product_uom_qty, fee.product_id, self.partner_id
                )
                for c in tax_calculate["taxes"]:
                    val += c["amount"]
        self.amount_tax = val
    ...
    def action_repair_done(self):
        ...
        for repair in self:
            ...
            owner_id = False
            available_qty_owner = self.env["stock.quant"]._get_available_quantity(
                repair.product_id, repair.location_id, repair.lot_id, owner_id=repair.partner_id, strict=True
            )
...
class RepairLine(models.Model):
    ...
    product_uom_qty = fields.Float(
        "Quantity", default=1.0, digits=dp.get_precision("Product Unit of Measure"), required=True
    )
    ...
    def _compute_price_subtotal(self):
        taxes = self.tax_id.compute_all(
            self.price_unit,
            self.repair_id.pricelist_id.currency_id,
...
class RepairFee(models.Model):
    ...
    product_uom_qty = fields.Float(
        "Quantity", digits=dp.get_precision("Product Unit of Measure"), required=True, default=1.0
    )

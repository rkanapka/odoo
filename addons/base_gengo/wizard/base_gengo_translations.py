...
try:
    ...
    _logger.warning(
        "Gengo library not found, Gengo features disabled. If you plan to use it, please install the gengo library from http://pypi.python.org/pypi/gengo"
    )
...
class BaseGengoTranslations(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if "lang_id" in fields:
            res["lang_id"] = (
                self.env["res.lang"].search([("code", "=", self.env.context.get("lang") or "en_US")], limit=1).id
            )
        return res

    sync_type = fields.Selection(
        [("send", "Send New Terms"), ("receive", "Receive Translation"), ("both", "Both")],
        "Sync Type",
        default="both",
        required=True,
    )
    ...
    def gengo_authentication(self):
        ...
        if not user.company_id.gengo_public_key or not user.company_id.gengo_private_key:
            return (
                False,
                _(
                    "Gengo `Public Key` or `Private Key` are missing. Enter your Gengo authentication parameters under `Settings > Companies > Gengo Parameters`."
                ),
            )
        try:
            gengo = Gengo(
    ...
    def act_update(self):
        ...
        for wizard in self:
            ...
            if wizard.sync_limit > 200 or wizard.sync_limit < 1:
                raise UserError(
                    _(
                        "The number of terms to sync should be between 1 to 200 to work with Gengo translation services."
                    )
                )
    ...
    def _update_terms(self, response, term_ids):
        ...
        """
        vals = {"order_id": response.get("order_id", ""), "state": "inprogress"}
        term_ids.write(vals)
        ...
        for term in term_ids:
            if re.search(r"\w", term.src or ""):
                ...
                jobs[time.strftime("%Y%m%d%H%M%S") + "-" + str(term.id)] = {
                    ...
                    "comment": comment,
                    "callback_url": "%s/website/gengo_callback?pgk=%s&db=%s"

...
class CustomerPortal(CustomerPortal):
    ...
    def portal_order_page(self, order=None, access_token=None, **kw):
        ...
        return super().portal_order_page(order=order, access_token=access_token, **kw)

    def _portal_quote_user_can_accept(self, order_id):
        result = super()._portal_quote_user_can_accept(order_id)
...
class sale_quote(http.Controller):
    ...
    def view(self, order_id, pdf=None, token=None, message=False, **post):
        ...
        if order_sudo.validity_date:
            days = (
                fields.Date.from_string(order_sudo.validity_date) - fields.Date.from_string(fields.Date.today())
            ).days + 1
        if pdf:
            pdf = (
                request.env.ref("website_quote.report_web_quote")
                .sudo()
                .with_context(set_viewport_size=True)
        ...
        values = {
            ...
            "need_payment": order_sudo.invoice_status == "to invoice"
        ...
        if order_sudo.require_payment or values["need_payment"]:
            ...
            values["form_acquirers"] = [
                acq for acq in acquirers if acq.payment_flow == "form" and acq.view_template_id
            ]
    ...
    def decline(self, order_id, token, **post):
        ...
        if message:
            _message_post_helper(
                message=message, res_id=order_id, res_model="sale.order", **{"token": token} if token else {}
            )
    ...
    def update_line_dict(
        self, line_id, remove=False, unlink=False, order_id=None, token=None, input_quantity=False, **kwargs
    ):
    ...
    def add(self, option_id, order_id, token, **post):
        ...
        if Order.state not in ["draft", "sent"]:
            return request.render(
                "website.http_error",
                {"status_code": "Forbidden", "status_message": _("You cannot add options to a confirmed order.")},
            )

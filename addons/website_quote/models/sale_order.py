        ...
        if self.order_id.template_id:
            self.name = next(
                (
                    ...
                    if quote_line.product_id.id == self.product_id.id
                ),
                self.name,
            )
        return domain
...
class SaleOrder(models.Model):
    ...
    def copy(self, default=None):
        if self.template_id and self.template_id.number_of_days > 0:
            ...
            default["validity_date"] = fields.Date.to_string(
                datetime.now() + timedelta(self.template_id.number_of_days)
            )
    ...
    def _compute_amount_undiscounted(self):
        ...
        for line in self.order_line:
            total += (
                line.price_subtotal + line.price_unit * ((line.discount or 0.0) / 100.0) * line.product_uom_qty
    ...
    def onchange_template_id(self):
        ...
        for line in template.quote_line:
            ...
            if self.pricelist_id:
                price = self.pricelist_id.with_context(uom=line.product_uom_id.id).get_product_price(
                    line.product_id, line.product_uom_qty, False
                )
        ...
        for option in template.options:
            if self.pricelist_id:
                price = self.pricelist_id.with_context(uom=option.uom_id.id).get_product_price(
                    option.product_id, 1, False
                )
            else:
    ...
    def open_quotation(self):
        ...
        return {"type": "ir.actions.act_url", "target": "self", "url": "/quote/%s/%s" % (self.id, self.access_token)}
    ...
    def get_portal_confirmation_action(self):
        ...
        return super().get_portal_confirmation_action()
...
class SaleOrderOption(models.Model):
    ...
    def _onchange_product_id(self):
        ...
        if pricelist and product:
            ...
            self.price_unit = pricelist.with_context(uom=self.uom_id.id).get_product_price(
                product, self.quantity, partner_id
            )

...
class SaleQuoteTemplate(models.Model):
    ...
    number_of_days = fields.Integer(
        "Quotation Duration", help="Number of days for the validity date computation of the quotation"
    )
...
class SaleQuoteLine(models.Model):
    ...
    sequence = fields.Integer(
        "Sequence", help="Gives the sequence order when displaying a list of sale quote lines.", default=10
    )
    quote_id = fields.Many2one(
        "sale.quote.template", "Quotation Template Reference", required=True, ondelete="cascade", index=True
    )
    name = fields.Text("Description", required=True, translate=True)
    product_id = fields.Many2one("product.product", "Product", domain=[("sale_ok", "=", True)], required=True)
    layout_category_id = fields.Many2one("sale.layout_category", string="Section")
    website_description = fields.Html(
        "Line Description", related="product_id.product_tmpl_id.quote_description", translate=html_translate
    )
...
class SaleQuoteOption(models.Model):
    ...
    template_id = fields.Many2one(
        "sale.quote.template", "Quotation Template Reference", ondelete="cascade", index=True, required=True
    )
    name = fields.Text("Description", required=True, translate=True)
    product_id = fields.Many2one("product.product", "Product", domain=[("sale_ok", "=", True)], required=True)
    layout_category_id = fields.Many2one("sale.layout_category", string="Section")

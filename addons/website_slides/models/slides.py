...
class Channel(models.Model):
    ...
    promoted_slide_id = fields.Many2one(
        "slide.slide", string="Featured Slide", compute="_compute_promoted_slide_id", store=True
    )

    @api.depends(
        "custom_slide_id", "promote_strategy", "slide_ids.likes", "slide_ids.total_views", "slide_ids.date_published"
    )
    ...
    def _count_presentations(self):
        ...
        for res_group in res:
            result[res_group["channel_id"][0]][res_group["slide_type"]] = (
                result[res_group["channel_id"][0]].get(res_group["slide_type"], 0) + res_group["__count"]
            )
        for record in self:
            ...
            record.total = (
                record.nbr_presentations + record.nbr_documents + record.nbr_videos + record.nbr_infographics
            )
    ...
    visibility = fields.Selection(
        [("public", "Public"), ("private", "Private"), ("partial", "Show channel but restrict presentations")],
        default="public",
        required=True,
    )
    ...
    upload_group_ids = fields.Many2many(
        ...
        help="Groups allowed to upload presentations in this channel. If void, every user can upload.",
    )
    ...
    def _compute_access(self):
        ...
        self.can_upload = self.can_see and (
            not self.upload_group_ids or bool(self.upload_group_ids & self.env.user.groups_id)
        )
    ...
    def _count_presentations(self):
        ...
        for res_group in res:
            result[res_group["category_id"][0]][res_group["slide_type"]] = (
                result[res_group["category_id"][0]].get(res_group["slide_type"], 0) + res_group["__count"]
            )
        for record in self:
            record.nbr_presentations = result[record.id].get("presentation", 0)
            record.nbr_documents = result[record.id].get("document", 0)
...
class Slide(models.Model):
        ...
        for record in self:
            if record.datas and (not record.document_id or record.slide_type in ["document", "presentation"]):
                ...
                if not record.mime_type:
                    # embed youtube video
                    record.embed_code = (
                        '<iframe src="//www.youtube.com/embed/%s?theme=light" allowFullScreen="true" frameborder="0"></iframe>'
                        % (record.document_id)
                    )
                else:
    ...
    def write(self, values):
        ...
        if values.get("channel_id"):
            custom_channels = self.env["slide.channel"].search(
                [("custom_slide_id", "=", self.id), ("id", "!=", values.get("channel_id"))]
            )
    ...
    def check_field_access_rights(self, operation, fields):
        ...
        fields = super().check_field_access_rights(operation, fields)
        ...
        if self.ids:
            ...
            limited_access = all(
                channel.visibility == "partial" and not len(channel.group_ids & self.env.user.groups_id)
    ...
    def get_related_slides(self, limit=20):
        ...
        yield from self.search(domain, limit=limit)

    def get_most_viewed_slides(self, limit=20):
        yield from self.search(
            [("website_published", "=", True), ("channel_id.visibility", "!=", "private"), ("id", "!=", self.id)],
            limit=limit,
    ...
    def _post_publication(self):
        ...
        for slide in self.filtered(lambda slide: slide.website_published and slide.channel_id.publish_template_id):
            ...
            html_body = publish_template.with_context(base_url=base_url).render_template(
            ...
            slide.channel_id.message_post(
                subject=subject, body=html_body, subtype="website_slides.mt_channel_slide_published"
            )
        return True
    ...
    def _parse_youtube_document(self, document_id, only_preview_fields):
        ...
        fetch_res = self._fetch_data(
            ...
            {"id": document_id, "key": key, "part": "snippet", "fields": "items(id,snippet)"},
            "json",
        )
        ...
        if google_values["mimeType"].startswith("video/"):
            ...
            if "exportLinks" in google_values:
                values["datas"] = self._fetch_data(
                    google_values["exportLinks"]["application/pdf"], params, "pdf", extra_params=True
                )["values"]
                # Content indexing
                if google_values["exportLinks"].get("text/plain"):
                    values["index_content"] = self._fetch_data(
                        google_values["exportLinks"]["text/plain"], params, extra_params=True
                    )["values"]
                elif google_values["exportLinks"].get("text/csv"):
                    values["index_content"] = self._fetch_data(
                        google_values["exportLinks"]["text/csv"], params, extra_params=True
                    )["values"]

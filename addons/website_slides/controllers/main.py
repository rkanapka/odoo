...
class WebsiteSlides(http.Controller):
    ...
    def channel(
        self, channel, category=None, tag=None, page=1, slide_type=None, sorting="creation", search=None, **kw
    ):
        user = request.env.user
        ...
        values = {
            ...
            "display_channel_settings": not request.httprequest.cookies.get("slides_channel_%s" % (channel.id), False)
    ...
    def dialog_preview(self, **data):
        ...
        existing_slide = Slide.search(
            [("channel_id", "=", int(data["channel_id"])), ("document_id", "=", document_id)], limit=1
        )
        if existing_slide:
            preview["error"] = (
                _(
    ...
    def create_slide(self, *args, **post):
        ...
        if post.get("category_id"):
            if post["category_id"][0] == 0:
                values["category_id"] = (
                    request.env["slide.category"]
                    .create({"name": post["category_id"][1]["name"], "channel_id": values.get("channel_id")})
                    .id
                )
            else:

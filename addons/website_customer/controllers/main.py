    ...
    def customers(self, country=None, industry=None, page=0, **post):
        ...
        if industry:
            ...
            if industry.id not in (x["industry_id"][0] for x in industries if x["industry_id"]):
                if industry.exists():
                    industries.append({"industry_id_count": 0, "industry_id": (industry.id, industry.name)})

        industries.sort(key=lambda d: (d.get("industry_id") or (0, ""))[1])

        industries.insert(0, {"industry_id_count": partners_count, "industry_id": (0, _("All Sectors of Activity"))})
        ...
        if country:
            ...
            if country.id not in (x["country_id"][0] for x in countries if x["country_id"]):
                if country.exists():
                    countries.append({"country_id_count": 0, "country_id": (country.id, country.name)})
        ...
        tags = Tag.search(
            [("website_published", "=", True), ("partner_ids", "in", partners.ids)], order="classname, name ASC"
        )

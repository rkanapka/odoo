...
class EventTypeMail(models.Model):
    ...
    event_type_id = fields.Many2one("event.type", string="Event Type", ondelete="cascade", required=True)
...
class EventMailScheduler(models.Model):
    ...
    def _compute_done(self):
        if self.interval_type in ["before_event", "after_event"]:
            ...
            self.done = len(self.mail_registration_ids) == len(self.event_id.registration_ids) and all(
    ...
    def _compute_scheduled_date(self):
        if self.event_id.state not in ["confirm", "done"]:
            ...
            self.scheduled_date = datetime.strptime(date, tools.DEFAULT_SERVER_DATETIME_FORMAT) + _INTERVALS[
                self.interval_unit
            ](sign * self.interval_nbr)
    ...
    def execute(self):
        ...
        if self.interval_type == "after_sub":
            ...
            lines = [
                ...
                for registration in (
                    self.event_id.registration_ids - self.mapped("mail_registration_ids.registration_id")
                )
            ]
            if lines:
        ...
        schedulers = self.search(
            [
                ...
                (
                    "scheduled_date",
                    "<=",
                    datetime.strftime(fields.datetime.now(), tools.DEFAULT_SERVER_DATETIME_FORMAT),
                ),
            ]
        )
...
class EventMailRegistration(models.Model):
    ...
    def _compute_scheduled_date(self):
        if self.registration_id:
            ...
            date_open_datetime = (
                date_open
                and datetime.strptime(date_open, tools.DEFAULT_SERVER_DATETIME_FORMAT)
                or fields.datetime.now()
            )
            self.scheduled_date = date_open_datetime + _INTERVALS[self.scheduler_id.interval_unit](

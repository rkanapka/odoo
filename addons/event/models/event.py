...
class EventType(models.Model):
    ...
    default_registration_min = fields.Integer(
        "Minimum Registrations", default=0, help="It will select this default minimum value when you choose this event"
    )
    default_registration_max = fields.Integer(
        "Maximum Registrations", default=0, help="It will select this default maximum value when you choose this event"
    )
    ...
    default_timezone = fields.Selection("_tz_get", string="Timezone", default=lambda self: self.env.user.tz)
    # communication
    use_hashtag = fields.Boolean("Use Default Hashtag")
    default_hashtag = fields.Char("Twitter Hashtag")
    use_mail_schedule = fields.Boolean("Automatically Send Emails", default=True)
...
class EventEvent(models.Model):
    ...
    name = fields.Char(
        string="Event Name", translate=True, required=True, readonly=False, states={"done": [("readonly", True)]}
    )
    ...
    event_type_id = fields.Many2one(
        "event.type", string="Category", readonly=False, states={"done": [("readonly", True)]}, oldname="type"
    )
    ...
    seats_availability = fields.Selection(
        [("limited", "Limited"), ("unlimited", "Unlimited")], "Maximum Attendees", required=True, default="unlimited"
    )
    ...
    seats_reserved = fields.Integer(
        oldname="register_current", string="Reserved Seats", store=True, readonly=True, compute="_compute_seats"
    )
    seats_available = fields.Integer(
        oldname="register_avail", string="Available Seats", store=True, readonly=True, compute="_compute_seats"
    )
    ...
    registration_ids = fields.One2many(
        "event.registration", "event_id", string="Attendees", readonly=False, states={"done": [("readonly", True)]}
    )
    # Date fields
    ...
    date_begin = fields.Datetime(
        string="Start Date", required=True, track_visibility="onchange", states={"done": [("readonly", True)]}
    )
    date_end = fields.Datetime(
        string="End Date", required=True, track_visibility="onchange", states={"done": [("readonly", True)]}
    )
    date_begin_located = fields.Char(string="Start Date Located", compute="_compute_date_begin_tz")
    date_end_located = fields.Char(string="End Date Located", compute="_compute_date_end_tz")

    state = fields.Selection(
        [("draft", "Unconfirmed"), ("cancel", "Cancelled"), ("confirm", "Confirmed"), ("done", "Done")],
        string="Status",
        default="draft",
        readonly=True,
        required=True,
    ...
    def _compute_date_begin_tz(self):
        if self.date_begin:
            self.date_begin_located = format_tz(
                self.with_context(use_babel=True).env, self.date_begin, tz=self.date_tz
            )
        else:
    ...
    def _check_seats_limit(self):
        if any(
            event.seats_availability == "limited" and event.seats_max and event.seats_available < 0 for event in self
        ):
    ...
    def name_get(self):
        ...
        for event in self:
            ...
            dates = [
                fields.Date.to_string(fields.Datetime.context_timestamp(event, dt))
                for dt in [date_begin, date_end]
                if dt
            ]
    ...
    def button_cancel(self):
        if any("done" in event.mapped("registration_ids.state") for event in self):
            raise UserError(
                _(
                    "There are already attendees who attended this event. Please reset it to draft if you want to cancel this event."
                )
            )
...
class EventRegistration(models.Model):
    ...
    event_id = fields.Many2one(
        "event.event", string="Event", required=True, readonly=True, states={"draft": [("readonly", False)]}
    )
    partner_id = fields.Many2one("res.partner", string="Contact", states={"done": [("readonly", True)]})
    date_open = fields.Datetime(
        string="Registration Date", readonly=True, default=lambda self: fields.datetime.now()
    ...
    state = fields.Selection(
        [("draft", "Unconfirmed"), ("cancel", "Cancelled"), ("open", "Confirmed"), ("done", "Attended")],
        string="Status",
        default="draft",
        readonly=True,
        copy=False,
    ...
    def _check_seats_limit(self):
        if (
            self.event_id.seats_availability == "limited"
    ...
    def _check_auto_confirmation(self):
        ...
        if any(
            ...
            or (not registration.event_id.seats_available and registration.event_id.seats_availability == "limited")
    ...
    def confirm_registration(self):
        ...
        onsubscribe_schedulers = self.event_id.event_mail_ids.filtered(lambda s: s.interval_type == "after_sub")
    ...
    def message_get_suggested_recipients(self):
        ...
        try:
            for attendee in self:
                is_public = (
                    attendee.sudo().with_context(active_test=False).partner_id.user_ids in public_users
                    if public_users
                    else False
                )
    ...
    def get_date_range_str(self):
        ...
        if diff.days <= 0:
            ...
            return _("on ") + format_tz(
                self.with_context({"use_babel": True}).env, self.event_begin_date, tz=self.event_id.date_tz or "UTC"
            )

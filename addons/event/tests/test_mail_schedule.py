...
class TestMailSchedule(TestEventCommon):
    ...
    def test_00_event_mail_schedule(self):
        ...
        mails = self.env["mail.mail"].search(
            [
                ("subject", "ilike", "registration"),
                ("date", ">=", datetime.datetime.strftime(now, tools.DEFAULT_SERVER_DATETIME_FORMAT)),
            ],
            order="date DESC",
            limit=3,
        )
        ...
        self.assertEqual(
            ...
            datetime.datetime.strftime(
                event_date_begin + relativedelta(days=-1), tools.DEFAULT_SERVER_DATETIME_FORMAT
            ),
            "event: incorrect scheduled date",
        )

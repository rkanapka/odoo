...
class AccountJournal(models.Model):
    ...
    def _prepare_liquidity_account(self, name, company, currency_id, type):
        ...
        """
        res = super()._prepare_liquidity_account(name, company, currency_id, type)
        if company.country_id.id == self.env.ref("base.mx").id:
            mx_tags = self.env["account.account"].mx_search_tags(res.get("code", ""))
            if mx_tags:
                res.update({"tag_ids": [(6, 0, [tag.id for tag in mx_tags])]})
        return res
...
class AccountAccount(models.Model):
    ...
    def mx_search_tags(self, code):
        ...
        re_res = re.search(
            "^(?P<first>[1-8][0-9][0-9])[,.]" "(?P<second>[0-9][0-9])[,.]" "(?P<third>[0-9]{2,3})$", code
        )
        if not re_res:
            return account_tag
        ...
        return account_tag.search(
            [("name", "=like", "%s.%s%%" % (account[0], account[1])), ("color", "=", 4)], limit=1
        )

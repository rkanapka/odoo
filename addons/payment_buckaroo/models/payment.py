...
class TxBuckaroo(models.Model):
    ...
    def _buckaroo_form_get_tx_from_data(self, data):
        ...
        if not reference or not pay_id or not shasign:
            error_msg = _("Buckaroo: received data with missing reference (%s) or pay_id (%s) or shasign (%s)") % (
                reference,
                pay_id,
                shasign,
            )
        ...
        if shasign_check.upper() != shasign.upper():
            error_msg = _("Buckaroo: invalid shasign, received %s, computed %s, for data %s") % (
                shasign,
                shasign_check,
                data,
            )

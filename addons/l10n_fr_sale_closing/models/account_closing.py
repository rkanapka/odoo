...
class AccountClosing(models.Model):
    ...
    date_closing_stop = fields.Datetime(
        string="Closing Date", help="Date to which the values are computed", readonly=True, required=True
    )
    date_closing_start = fields.Datetime(
        string="Starting Date", help="Date from which the total interval is computed", readonly=True, required=True
    )
    ...
    def _compute_amounts(self, frequency, company):
        ...
        previous_closing = self.search(
            [("frequency", "=", frequency), ("company_id", "=", company.id)], limit=1, order="sequence_number desc"
        )

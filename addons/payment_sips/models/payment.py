...
class AcquirerSips(models.Model):
    ...
    sips_merchant_id = fields.Char(
        "Merchant ID", help="Used for production only", required_if_provider="sips", groups="base.group_user"
    )
    ...
    sips_version = fields.Char(
        "Interface Version", required_if_provider="sips", groups="base.group_no_one", default="HP_2.3"
    )
        ...
        sips_tx_values.update(
            {
                ...
                + "normalReturnUrl=%s|" % urls.url_join(base_url, SipsController._return_url)
                + "automaticResponseUrl=%s|" % urls.url_join(base_url, SipsController._return_url)
...
class PaymentTransactionSips(models.Model):
    ...
    def _get_next_reference(self, reference, acquirer=None):
        ...
        return super()._get_next_reference(reference, acquirer=acquirer)
...
class TxSips(models.Model):
    ...
    def _sips_form_get_invalid_parameters(self, data):
        ...
        if self.acquirer_reference and data.get("transactionReference") != self.acquirer_reference:
            invalid_parameters.append(
                ("transactionReference", data.get("transactionReference"), self.acquirer_reference)
            )
    ...
    def _sips_form_validate(self, data):
        ...
        if status in self._sips_valid_tx_status:
            msg = "Payment for tx ref: %s, got response [%s], set as done." % (self.reference, status)
            _logger.info(msg)
            data.update(state="done", state_message=msg)
            res = True
        elif status in self._sips_error_tx_status:
            msg = "Payment for tx ref: %s, got response [%s], set as " "error." % (self.reference, status)
            data.update(state="error", state_message=msg)
        elif status in self._sips_wait_tx_status:
            msg = "Received wait status for payment ref: %s, got response " "[%s], set as error." % (
                self.reference,
                status,
            )
            data.update(state="error", state_message=msg)
        elif status in self._sips_refused_tx_status:
            msg = "Received refused status for payment ref: %s, got response" " [%s], set as error." % (
                self.reference,
                status,
            )
            data.update(state="error", state_message=msg)
        elif status in self._sips_pending_tx_status:
            msg = "Payment ref: %s, got response [%s] set as pending." % (self.reference, status)
            data.update(state="pending", state_message=msg)
        elif status in self._sips_cancel_tx_status:
            msg = "Received notification for payment ref: %s, got response " "[%s], set as cancel." % (
                self.reference,
                status,
            )
            data.update(state="cancel", state_message=msg)
        else:
            msg = "Received unrecognized status for payment ref: %s, got " "response [%s], set as error." % (
                self.reference,
                status,
            )
            data.update(state="error", state_message=msg)

...
class SipsController(http.Controller):
    ...
    @http.route(["/payment/sips/ipn/"], type="http", auth="none", methods=["POST"], csrf=False)
    def sips_ipn(self, **post):
        """Sips IPN."""
        _logger.info("Beginning Sips IPN form_feedback with post data %s", post)  # debug
        if not post:
    ...
    @http.route(["/payment/sips/dpn"], type="http", auth="none", methods=["POST"], csrf=False)

...
class pos_session(models.Model):
    ...
    def _check_session_timing(self):
        ...
        if not date_today - timedelta(hours=24) <= session_start:
            raise UserError(
                _(
                    "This session has been opened another day. To comply with the French law, you should close sessions on a daily basis. Please close session %s and open a new one."
                )
                % self.name
            )
        return True
...
class pos_order(models.Model):
    ...
    def _get_new_hash(self, secure_seq_number):
        ...
        if prev_order and len(prev_order) != 1:
            raise UserError(
                _(
                    "An error occured when computing the inalterability. Impossible to get the unique previous posted point of sale order."
                )
            )
...
class PosOrderLine(models.Model):
    ...
    def write(self, vals):
        ...
        if set(vals).intersection(LINE_FIELDS):
            if any(
                l.company_id._is_accounting_unalterable() and l.order_id.state in ["done", "invoiced"] for l in self
            ):
                raise UserError(
                    _(

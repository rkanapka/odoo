...
class HrExpenseSheetRegisterPaymentWizard(models.TransientModel):
    ...
    def _default_partner_id(self):
        ...
        return (
            expense_sheet.address_id.id
            or expense_sheet.employee_id.id
            and expense_sheet.employee_id.address_home_id.id
        )

    partner_id = fields.Many2one("res.partner", string="Partner", required=True, default=_default_partner_id)
    journal_id = fields.Many2one(
        "account.journal", string="Payment Method", required=True, domain=[("type", "in", ("bank", "cash"))]
    )
    company_id = fields.Many2one(
        "res.company", related="journal_id.company_id", string="Company", readonly=True, required=True
    )

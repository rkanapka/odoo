...
class AccountMoveLine(models.Model):
    ...
    expense_id = fields.Many2one(
        "hr.expense", string="Expense", copy=False, help="Expense where the move line come from"
    )
    ...
    def reconcile(self, writeoff_acc_id=False, writeoff_journal_id=False):
        ...
        account_move_ids = [
            l.move_id.id for l in self if float_compare(l.move_id.matched_percentage, 1, precision_digits=5) == 0
        ]
        if account_move_ids:

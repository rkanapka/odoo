...
class HrExpense(models.Model):
    ...
    untaxed_amount = fields.Float(
        string="Subtotal", store=True, compute="_compute_amount", digits=dp.get_precision("Account")
    )
    total_amount = fields.Float(
        string="Total", store=True, compute="_compute_amount", digits=dp.get_precision("Account")
    )
    ...
    account_id = fields.Many2one(
        ...
        string="Account",
        states={"post": [("readonly", True)], "done": [("readonly", True)]},
        default=lambda self: self.env["ir.property"].get("property_account_expense_categ_id", "product.category"),
    ...
    state = fields.Selection(
        [("draft", "To Submit"), ("reported", "Reported"), ("done", "Posted"), ("refused", "Refused")],
    ...
    def _prepare_move_line(self, line):
        ...
        return {
            ...
            and abs(line.get("amount_currency"))
    ...
    def action_move_create(self):
        ...
        for expense in self:
            ...
            total, total_currency, move_lines = expense._compute_expense_totals(
                company_currency, move_lines, move.date
            )
    ...
    def get_empty_list_help(self, help_message):
        if help_message and help_message.find("oe_view_nocontent_create") == -1:
            ...
            if alias_record and alias_record.alias_domain and alias_record.alias_name:
                link = (
                    "<a id='o_mail_test' href='mailto:%(email)s?subject=Lunch%%20with%%20customer%%3A%%20%%2412.32'>%(email)s</a>"
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        employee = self.env["hr.employee"].search(
            ["|", ("work_email", "ilike", email_address), ("user_id.email", "ilike", email_address)], limit=1
        )
...
class HrExpenseSheet(models.Model):
    ...
    total_amount = fields.Float(
        string="Total Amount", store=True, compute="_compute_amount", digits=dp.get_precision("Account")
    )
    company_id = fields.Many2one(
        "res.company",
        string="Company",
    ...
    journal_id = fields.Many2one(
        ...
        states={"done": [("readonly", True)], "post": [("readonly", True)]},
        default=lambda self: self.env["ir.model.data"].xmlid_to_object("hr_expense.hr_expense_account_journal")
        or self.env["account.journal"].search([("type", "=", "purchase")], limit=1),
    ...
    department_id = fields.Many2one(
        "hr.department", string="Department", states={"post": [("readonly", True)], "done": [("readonly", True)]}
    )
    ...
    def _create_set_followers(self, values):
        ...
        for partner in users.mapped("partner_id"):
            values["message_follower_ids"] += MailFollowers._add_follower_command(
                self._name, [], {partner.id: None}, {}
            )[0]
    ...
    def _compute_amount(self):
        ...
        for expense in self.expense_line_ids:
            total_amount += expense.currency_id.with_context(
                date=expense.date, company_id=expense.company_id.id
    ...
    def action_sheet_move_create(self):
        ...
        expense_line_ids = self.mapped("expense_line_ids").filtered(
            lambda r: not float_is_zero(
                r.total_amount, precision_rounding=(r.currency_id or self.env.user.company_id.currency_id).rounding
            )
        )

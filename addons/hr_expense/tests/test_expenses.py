...
class TestCheckJournalEntry(TransactionCase):
    ...
    def setUp(self):
        ...
        account_expense = self.env["account.account"].create(
            {"code": "X2120", "name": "HR Expense - Test Purchase Account", "user_type_id": user_type.id}
        )
        ...
        self.bank_journal = self.env["account.journal"].create(
            {"name": "Payment Journal", "code": "PAY", "type": "bank", "company_id": company.id}
        )
    ...
    def test_partial_payment_multiexpense(self):
        ...
        expense_line2 = self.expense_line.copy({"sheet_id": self.expense.id})
        self.expense.approve_expense_sheets()
        self.expense.action_sheet_move_create()

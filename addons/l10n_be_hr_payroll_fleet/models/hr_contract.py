...
class HrContract(models.Model):
    ...
    car_id = fields.Many2one(
        ...
        default=lambda self: self.env["fleet.vehicle"].search(
            [("driver_id", "=", self.employee_id.address_home_id.id)], limit=1
        ),
    ...
    new_car_model_id = fields.Many2one(
        "fleet.vehicle.model", string="Model", domain=lambda self: self._get_possible_model_domain()
    )
    ...
    @api.depends(
        ...
        "new_car",
        "new_car_model_id",
        "car_id.total_depreciated_cost",
        "car_id.atn",
        "new_car_model_id.default_atn",
        "new_car_model_id.default_total_depreciated_cost",
        "car_id.co2_fee",
        "car_id.log_contracts",
        "car_id.log_contracts.state",  # YTI TODO: Store total_depreciated_costs on
        "car_id.log_contracts.recurring_cost_amount_depreciated",
    )  # the fleet vehicle to avoid these dependencies
    ...
    def _onchange_employee_id(self):
        ...
        self.car_id = self.env["fleet.vehicle"].search(
            [("driver_id", "=", self.employee_id.address_home_id.id)], limit=1
        )

...
class FleetVehicle(models.Model):
    ...
    total_cost = fields.Float(
        compute="_compute_total_cost", string="Total Cost", help="This include all the costs and the CO2 fee"
    )
    ...
    def _compute_total_depreciated_cost(self):
        for car in self:
            car.total_depreciated_cost = car.co2_fee + sum(
                car.log_contracts.filtered(lambda contract: contract.state == "open").mapped(
    ...
    def _compute_vehicle_name(self):
        ...
        for vehicle in self:
            ...
            vehicle.name += " \u2022 " + str(round(vehicle.total_depreciated_cost, 2)) + " \u2022 " + acquisition_date
    ...
    def _get_car_atn(self, acquisition_date, car_value, fuel_type, co2):
        ...
        if start:
            number_of_month = (
                (now.year - start.year) * 12.0 + now.month - start.month + int(bool(now.day - start.day + 1))
            )
    ...
    def _compute_atn(self):
        ...
        for model in self:
            model.default_atn = self.env["fleet.vehicle"]._get_car_atn(
                now, model.default_car_value, model.default_fuel_type, model.default_co2
            )

...
class FleetVehicle(models.Model):
    ...
    vin_sn = fields.Char(
        "Chassis Number", help="Unique number written on the vehicle motor (VIN/SN number)", copy=False
    )
    driver_id = fields.Many2one(
        "res.partner", "Driver", track_visibility="onchange", help="Driver of the vehicle", copy=False
    )
    ...
    acquisition_date = fields.Date(
        "Immatriculation Date", required=False, help="Date when the vehicle has been immatriculated"
    )
    ...
    tag_ids = fields.Many2many(
        "fleet.vehicle.tag", "fleet_vehicle_vehicle_tag_rel", "vehicle_tag_id", "tag_id", "Tags", copy=False
    )
    ...
    contract_renewal_name = fields.Text(
        compute="_compute_contract_reminder", string="Name of contract to renew soon", multi="contract_info"
    )
    ...
    def create(self, data):
        ...
        vehicle.message_post(
            body=_("%s %s has been added to the fleet!") % (vehicle.model_id.name, vehicle.license_plate)
        )
        return vehicle

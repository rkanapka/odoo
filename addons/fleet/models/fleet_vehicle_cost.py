...
class FleetVehicleLogContract(models.Model):
    ...
    start_date = fields.Date(
        "Contract Start Date", default=fields.Date.context_today, help="Date when the coverage of the contract begins"
    )
    ...
    notes = fields.Text(
        "Terms and Conditions", help="Write here all supplementary information relative to this contract", copy=False
    )
    ...
    odometer = fields.Float(
        string="Odometer at creation", help="Odometer measure of the vehicle at the moment of the contract creation"
    )
        ...
        for contract in contracts:
            ...
            if contract.generated_cost_ids:
                last_autogenerated_cost = VehicleCost.search(
                    [("contract_id", "=", contract.id), ("auto_generated", "=", True)],
                    offset=0,
                    limit=1,
                    order="date desc",
                )
    ...
    def scheduler_manage_contract_expiration(self):
        ...
        for vehicle, value in res.items():
            Vehicle.browse(vehicle).message_post(
                body=_("%s contract(s) will expire soon and should be renewed and/or closed!") % value
            )
...
class FleetVehicleLogFuel(models.Model):
    ...
    purchaser_id = fields.Many2one(
        "res.partner", "Purchaser", domain="['|',('customer','=',True),('employee','=',True)]"
    )

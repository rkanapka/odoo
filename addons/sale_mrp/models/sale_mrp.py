    ...
    def _get_anglo_saxon_price_unit(self):
        ...
        if self.product_id.invoice_policy == "delivery":
            for s_line in self.sale_line_ids:
                ...
                qty_done = sum(
                    [
                        x.uom_id._compute_quantity(x.quantity, x.product_id.uom_id)
                        for x in s_line.invoice_lines
                        if x.invoice_id.state in ("open", "paid")
                    ]
                )
                ...
                if bom.type == "phantom":
                    ...
                    for product_id in components:
                        ...
                        average_price_unit += factor * self._compute_average_price(

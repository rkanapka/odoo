...
class TestSaleMrpFlow(common.TransactionCase):
    ...
    def test_00_sale_mrp_flow(self):
        ...
        def create_product(name, uom_id, route_ids=[]):
            return self.Product.create(
                {"name": name, "type": "product", "uom_id": uom_id, "uom_po_id": uom_id, "route_ids": route_ids}
            )

        def create_bom_lines(bom_id, product_id, qty, uom_id, procure_method):  # TODO: remove procure_method
            self.MrpBomLine.create(
                {"product_id": product_id, "product_qty": qty, "bom_id": bom_id, "product_uom_id": uom_id}
            )

        def create_bom(product_tmpl_id, qty, uom_id, type):
            return self.MrpBom.create(
                {"product_tmpl_id": product_tmpl_id, "product_qty": qty, "type": type, "product_uom_id": uom_id}
            )
        ...
        self.assertEqual(
            mnf_product_a.product_uom_id.id, self.uom_unit.id, "Wrong unit of measure in manufacturing order."
        )
        ...
        self.assertEqual(
            move.product_uom_qty, 1502.5, "Wrong product quantity in 'To consume line' of manufacturing order."
        )
        ...
        mnf_product_d = self.MrpProduction.search(
            [("product_id", "=", product_d.id), ("move_dest_ids.group_id", "=", order.procurement_group_id.id)],
            order="id desc",
            limit=1,
        )
        ...
        self.assertEqual(
            move.product_uom_qty, 20, "Wrong product quantity in 'To consume line' of manufacturing order."
        )
        self.assertEqual(
            move.product_uom.id, self.uom_kg.id, "Wrong unit of measure in 'To consume line' of manufacturing order."
        )
        self.assertEqual(move.state, "confirmed", "Wrong state in 'To consume line' of manufacturing order.")
        ...
        inventory = self.Inventory.create(
            {"name": "Inventory Product KG", "product_id": product_c.id, "filter": "product"}
        )
        ...
        produce_d = self.ProductProduce.with_context(
            {"active_ids": [mnf_product_d.id], "active_id": mnf_product_d.id}
        ).create({"product_qty": 20})
        ...
        # -------------------------------------------------------
        inventory = self.Inventory.create(
            {"name": "Inventory Product C KG", "product_id": product_c.id, "filter": "product"}
        )
        ...
        self.assertEqual(
            mnf_product_a.availability, "assigned", "Manufacturing order inventory state should be available."
        )
        moves = self.StockMove.search(
        ...
        produce_a = self.ProductProduce.with_context(
            ...
            {"active_ids": [mnf_product_a.id], "active_id": mnf_product_a.id}
        ).create({})
    ...
    def test_01_sale_mrp_delivery_kit(self):
        ...
        self.assertTrue(
            self.so.picking_ids, 'Sale MRP: no picking created for "invoice on delivery" stockable products'
        )
        ...
        self.assertEqual(
            self.so.invoice_status, "no", 'Sale MRP: so invoice_status should be "nothing to invoice" after invoicing'
        )
        ...
        wiz.process()
        self.assertEqual(
            self.so.invoice_status, "no", 'Sale MRP: so invoice_status should be "no" after partial delivery of a kit'
        )
    ...
    def test_02_sale_mrp_anglo_saxon(self):
        ...
        self.MrpBomLine.create({"product_id": self.component1.id, "product_qty": 2.0, "bom_id": self.bom.id})
        self.MrpBomLine.create({"product_id": self.component2.id, "product_qty": 1.0, "bom_id": self.bom.id})

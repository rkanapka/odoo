...
class TestSaleMrpLeadTime(TestStockCommon):
        ...
        self.bom_a = self.env["mrp.bom"].create(
            {
                ...
                "bom_line_ids": [
                    (0, 0, {"product_id": self.product_2.id, "product_qty": 4, "product_uom_id": self.uom_unit.id})
                ],
            }
        )
    ...
    def test_00_product_company_level_delays(self):
        ...
        self.assertTrue(
            abs(date_planned_start - mo_date) <= timedelta(seconds=1),
    ...
    def test_01_product_route_level_delays(self):
        ...
        out_date = (
            fields.Datetime.from_string(order.date_order)
            + timedelta(days=self.product_1.sale_delay)
            - timedelta(days=out.move_lines[0].rule_id.delay)
        )
        self.assertTrue(
        ...
        self.assertTrue(
            abs(pick_min_date - pack_date) <= timedelta(seconds=1),

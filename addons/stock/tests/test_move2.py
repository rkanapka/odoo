...
class TestPickShip(TestStockCommon):
    ...
    def test_mto_moves_return_return(self):
        picking_pick, picking_client = self.create_pick_ship()
        stock_location = self.env["stock.location"].browse(self.stock_location)
        lot = self.env["stock.production.lot"].create({"product_id": self.productA.id, "name": "123456789"})
        ...
        stock_return_picking = self.env["stock.return.picking"].with_context(active_id=return_pick.id).create({})
        stock_return_picking.product_return_moves.quantity = 10.0
        stock_return_picking_action = stock_return_picking.create_returns()
        ...
        lines = self.env["stock.traceability.report"].get_lines(
            model_id=return_return_pick.move_line_ids[0].id, stream="upstream", **vals
        )
        self.assertEqual(
        ...
        lines = self.env["stock.traceability.report"].get_lines(
            model_id=picking_pick.move_line_ids[0].id, stream="downstream", **vals
        )
        self.assertEqual(
            [l.get("res_id") for l in lines],
    ...
    def test_mto_resupply_cancel_ship(self):
        ...
        warehouse_1.write({"default_resupply_wh_id": warehouse_2.id, "resupply_wh_ids": [(6, 0, [warehouse_2.id])]})
        ...
        self.assertEqual(
            len(picking_ship.move_lines.mapped("move_orig_ids")),
            0,
    ...
    def test_edit_done_chained_move(self):
        ...
        self.assertEqual(
            picking_client.move_lines.reserved_availability, 10.0, "Wrong quantity already reserved for client move"
        )
        ...
        self.assertEqual(picking_client.move_lines.product_qty, 10.0, "Wrong initial demand for client move")
        self.assertEqual(
            picking_client.move_lines.reserved_availability, 5.0, "Wrong quantity already reserved for client move"
        )
    ...
    def test_pick_ship_return(self):
        ...
        """
        self.assertTrue(
            return_ship_picking.move_lines in return_pick_picking.move_lines.mapped("move_orig_ids"),
            "The pick return picking's moves should have the ship return picking's moves as origin",
        )

        self.assertTrue(
            return_pick_picking.move_lines in return_ship_picking.move_lines.mapped("move_dest_ids"),
    ...
    def test_pick_pack_ship_return(self):
        ...
        self.assertEqual(
            set(picking_pick.move_lines.move_dest_ids.ids),
            set((picking_pack.move_lines | return_pick_picking.move_lines).ids),
        )

        self.assertEqual(set(picking_pack.move_lines.move_orig_ids.ids), set(picking_pick.move_lines.ids))
        self.assertEqual(
            set(picking_pack.move_lines.move_dest_ids.ids),
            set((picking_ship.move_lines | return_pack_picking.move_lines).ids),
        )
        ...
        self.assertEqual(
            set(return_ship_picking.move_lines.move_dest_ids.ids), set(return_pack_picking.move_lines.ids)
        )

        self.assertEqual(
            set(return_pack_picking.move_lines.move_orig_ids.ids),
            set((picking_pack.move_lines | return_ship_picking.move_lines).ids),
        )
        self.assertEqual(
            set(return_pack_picking.move_lines.move_dest_ids.ids), set(return_pick_picking.move_lines.ids)
        )

        self.assertEqual(
            set(return_pick_picking.move_lines.move_orig_ids.ids),
            set((picking_pick.move_lines | return_pack_picking.move_lines).ids),
        )
    ...
    def test_put_in_pack(self):
        ...
        picking_pick.move_line_ids.filtered(
        ...
        picking_pack.entire_package_ids[0].with_context(
            {"picking_id": picking_pack.id, "destination_location": output_sub_location.id}
        ).action_toggle_processed()
        ...
        quants_of_products_ab = self.env["stock.quant"].search(
            [("product_id", "in", (self.productA.id, self.productB.id))]
        )
    ...
    def test_mto_cancel_move_line(self):
        ...
        self.assertEqual(
            picking_client.move_lines.state,
            "waiting",
        ...
        self.assertEqual(
            picking_client.state, "confirmed", "The picking should be confirmed since all the moves are confirmed."
        )
...
class TestSinglePicking(TestStockCommon):
    ...
    def test_extra_move_4(self):
        ...
        for move in delivery.move_lines:
            self.assertEqual(
                move.quantity_done, move.product_uom_qty, "Initial demand should be equals to quantity done"
            )
    ...
    def test_add_move_when_picking_is_available_1(self):
        ...
        # add a move
        move2 = self.MoveObj.with_context(default_picking_id=delivery_order.id).create(
            {
                "name": self.productA.name,
                "product_id": self.productB.id,
                "product_uom_qty": 1,
    ...
    def test_use_create_lot_use_existing_lot_4(self):
        ...
        with self.assertRaises(UserError):
            self.env["stock.production.lot"].with_context(active_picking_id=delivery_order.id).create(
                {
                    "name": "lot1",
                    "product_id": self.productA.id,
                }
    ...
    def test_merge_moves_1(self):
        ...
        self.assertEqual(
            receipt.move_lines.filtered(lambda m: m.product_id == self.productA).product_uom_qty,
            9,
            "Merged quantity is not correct",
        )
        self.assertEqual(
            receipt.move_lines.filtered(lambda m: m.product_id == self.productB).product_uom_qty,
            5,
    ...
    def test_merge_chained_moves(self):
        ...
        self.assertTrue(
            qc_move.move_orig_ids == input_move, "Move between QC and stock should only have the input move as origin"
        )
...
class TestStockUOM(TestStockCommon):
    ...
    def test_move_product_with_different_uom(self):
        ...
        self.assertTrue(
            float_is_zero(move_line.product_qty - quant.reserved_quantity, precision_rounding=precision_rounding)
        )
    ...
    def test_update_product_move_line_with_different_uom(self):
        ...
        self.assertTrue(
            float_is_zero(
                reserved_on_move_lines_LtDA - quant_LtDA.reserved_quantity, precision_rounding=precision_rounding
            )
        )
        self.assertTrue(
            float_is_zero(
                reserved_on_move_lines_GtDA - quant_GtDA.reserved_quantity, precision_rounding=precision_rounding
            )
        )

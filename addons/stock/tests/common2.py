...
class TestStockCommon(common.TestProductCommon):
    ...
    def _create_move(self, product, src_location, dst_location, **values):
        ...
        move = Move.new(
            {"product_id": product.id, "location_id": src_location.id, "location_dest_id": dst_location.id}
        )
    ...
    def _create_move_in(self, product, warehouse, picking=None, create_picking=False, **values):
        ...
        return self._create_move(
            product, self.env.ref("stock.stock_location_suppliers"), warehouse.lot_stock_id, **values
        )
    ...
    def setUpClass(cls):
        ...
        cls.warehouse_1 = cls.env["stock.warehouse"].create(
            {"name": "Base Warehouse", "reception_steps": "one_step", "delivery_steps": "ship_only", "code": "BWH"}
        )

...
class TestStockFlow(TestStockCommon):
    ...
    def test_00_picking_create_and_transfer_quantity(self):
        ...
        self.assertEqual(
            sum(product_c_qty), 3.0, "Wrong move quantity of product C (%s found instead of 3)" % (product_c_qty)
        )
        ...
        self.assertEqual(
            sum(product_d_qty), 8.0, "Wrong move quantity of product D (%s found instead of 8)" % (product_d_qty)
        )
        ...
        self.assertEqual(
            aval_a_qty, 4.0, "Wrong move quantity availability of product A (%s found instead of 4)" % (aval_a_qty)
        )
        ...
        self.assertEqual(
            aval_b_qty, 5.0, "Wrong move quantity availability of product B (%s found instead of 5)" % (aval_b_qty)
        )
        ...
        self.assertEqual(
            product_a_qty, 8.0, "Wrong move quantity of product A (%s found instead of 8)" % (product_a_qty)
        )
        ...
        self.assertEqual(
            product_d_qty, 4.0, "Wrong move quantity of product D (%s found instead of 4)" % (product_d_qty)
        )
        ...
        self.assertEqual(
            sum(total_qty), 2.0, "Expecting 2.0 Unit , got %.4f Unit on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
        ...
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.productB.id), ("location_id", "=", self.stock_location)]
        )
        ...
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.productC.id), ("location_id", "=", self.stock_location)]
        )
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 2.0, "Expecting 2.0 Unit, got %.4f Unit on location stock!" % (sum(total_qty))
        ...
        quant = self.StockQuantObj.search(
            [("product_id", "=", self.productD.id), ("location_id", "=", self.stock_location)], limit=1
        )
        self.assertEqual(
            quant.quantity, 1.0, "Expecting 1.0 Unit , got %.4f Unit on location stock!" % (quant.quantity)
        )
        self.assertEqual(
        ...
        self.assertEqual(
            back_order_in.state,
            "assigned",
            "Wrong state of incoming shipment back order: %s instead of %s" % (back_order_in.state, "assigned"),
        )
        ...
        packD = self.StockPackObj.search(
            [("product_id", "=", self.productD.id), ("picking_id", "=", back_order_in.id)], order="product_qty"
        )
        ...
        self.assertFalse(
            self.PickingObj.search([("backorder_id", "=", back_order_in.id)]), "Should not create any back order."
        )
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 12.0, "Wrong total stock location quantity (%s found instead of 12)" % (sum(total_qty))
        )
        self.assertEqual(
            self.productA.qty_available,
            12.0,
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 8.0, "Wrong total stock location quantity (%s found instead of 8)" % (sum(total_qty))
        )
        self.assertEqual(
            self.productC.qty_available,
            8.0,
        ...
        # Check quants and available quantity for product D.
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.productD.id), ("location_id", "=", self.stock_location)]
        )
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 9.0, "Wrong total stock location quantity (%s found instead of 9)" % (sum(total_qty))
        )
        self.assertEqual(
            self.productD.qty_available,
            9.0,
    ...
    def test_10_pickings_transfer_with_different_uom(self):
        ...
        self.assertEqual(
            PackSdozAround.product_qty,
            11,
            "Wrong quantity in pack operation (%s found instead of 11)" % (PackSdozAround.product_qty),
        )
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 10, "Expecting 10 Dozen , got %.4f Dozen on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 10.5, "Expecting 10.5 SDozen , got %.4f SDozen on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 11, "Expecting 11 SDozenRound , got %.4f SDozenRound on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 525.3, "Expecting 525.3 gram , got %.4f gram on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
            self.gB.qty_available,
            525.3,
            "Wrong quantity available (%s found instead of 525.3" % (self.gB.qty_available),
        )
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 0.020, "Expecting 0.020 kg , got %.4f kg on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
        ...
        self.assertEqual(
            PackdozA.product_uom_qty,
            120,
            "Wrong quantity in pack operation (%s found instead of 120)" % (PackdozA.product_uom_qty),
        )
        self.assertEqual(
            PackdozA.product_qty,
            10,
            "Wrong real quantity in pack operation (%s found instead of 10)" % (PackdozA.product_qty),
        )
        ...
        self.assertEqual(
            PackSdozA.product_uom_qty,
            1512,
            "Wrong quantity in pack operation (%s found instead of 1512)" % (PackSdozA.product_uom_qty),
        )
        self.assertEqual(
            PackSdozA.product_uom_id.id, self.uom_unit.id, "Wrong uom in pack operation for product SDozA."
        )
        ...
        self.assertEqual(
            PackSdozAround.product_uom_qty,
            1584,
            "Wrong quantity in pack operation (%s found instead of 1584)" % (PackSdozAround.product_uom_qty),
        )
        self.assertEqual(
            PackSdozAround.product_uom_id.id, self.uom_unit.id, "Wrong uom in pack operation for product SDozARound."
        )
        ...
        self.assertEqual(
            packgB.product_uom_qty,
            0.525,
            "Wrong quantity in pack operation (%s found instead of 0.525)" % (packgB.product_uom_qty),
        )
        self.assertEqual(
            packgB.product_qty,
            525,
            "Wrong real quantity in pack operation (%s found instead of 525)" % (packgB.product_qty),
        )
        self.assertEqual(
            packgB.product_uom_id.id, packgB.move_id.product_uom.id, "Wrong uom in pack operation for product kgB."
        )
        ...
        self.assertEqual(
            packkgB.product_uom_qty,
            20.0,
            "Wrong quantity in pack operation (%s found instead of 20)" % (packkgB.product_uom_qty),
        )
        ...
        self.assertEqual(
            moves_SDozARound.product_uom.id, self.uom_unit.id, "Wrong uom in move for product SDozARound."
        )
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 18, "Expecting 18 Dozen , got %.4f Dozen on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
            self.DozA.qty_available,
            18,
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 21, "Expecting 21 SDozen , got %.4f SDozen on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
            self.SDozA.qty_available,
            21,
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 22, "Expecting 22 SDozenRound , got %.4f SDozenRound on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
            self.SDozARound.qty_available,
            22,
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 0.040, "Expecting 0.040 kg , got %.4f kg on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
            self.kgB.qty_available,
            0.040,
        ...
        self.assertEqual(
            SDozARound_qty, 3, "Wrong move quantity availability (%s found instead of 3)" % (SDozARound_qty)
        )
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 13.5, "Expecting 13.5 Dozen , got %.4f Dozen on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
            self.DozA.qty_available,
            13.5,
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 19, "Expecting 19 SDozen , got %.4f SDozen on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
            self.SDozA.qty_available,
            19,
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 19, "Expecting 19 SDozRound , got %.4f SDozRound on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
            self.SDozARound.qty_available,
            19,
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            round(sum(total_qty), 1), 548.3, "Expecting 547.6 g , got %.4f g on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
            round(self.gB.qty_available, 1),
            548.3,
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 0.019, "Expecting 0.019 kg , got %.4f kg on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
            self.kgB.qty_available,
            0.019,
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 15.5, "Expecting 15.5 Dozen , got %.4f Dozen on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
            self.DozA.qty_available,
            15.5,
        ...
        productKG = self.ProductObj.create(
            {"name": "Product KG", "uom_id": self.uom_kg.id, "uom_po_id": self.uom_kg.id, "type": "product"}
        )
        picking_in = self.PickingObj.create(
            {
                "partner_id": self.partner_delta_id,
        ...
        self.assertEqual(
            packKG.product_qty,
            1000,
            "Wrong product real quantity in pack operation (%s found instead of 1000)" % (packKG.product_qty),
        )
        self.assertEqual(
            packKG.product_uom_qty,
            1,
            "Wrong product quantity in pack operation (%s found instead of 1)" % (packKG.product_uom_qty),
        )
        ...
        # Check incoming shipment state.
        self.assertEqual(
            picking_in.state, "done", "Incoming shipment state: %s instead of %s" % (picking_in.state, "done")
        )
        # Check incoming shipment move lines state.
        for move in picking_in.move_lines:
            self.assertEqual(move.state, "done", "Wrong state of move lines.")
        ...
        self.assertEqual(
            sum(total_qty), 999.9995, "Expecting 999.9995 kg , got %.4f kg on location stock!" % (sum(total_qty))
        )
        ...
        # Check back order created with correct quantity and uom or not.
        self.assertEqual(
            moves_KG.product_uom_qty, 2.0, "Wrong move quantity (%s found instead of 2.0)" % (moves_KG.product_uom_qty)
        )
        ...
        # Check total quantity stock location.
        self.assertEqual(
            sum(total_qty), 999.9990, "Expecting 999.9990 kg , got %.4f kg on location stock!" % (sum(total_qty))
        )
        ...
        self.assertEqual(
            moves_KG.product_uom_qty, 1.5, "Wrong move quantity (%s found instead of 1.5)" % (moves_KG.product_uom_qty)
        )
        self.assertEqual(moves_KG.product_uom.id, self.uom_gm.id, "Wrong uom in move for product KG.")
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 999.9985, "Expecting 999.9985 kg , got %.4f kg on location stock!" % (sum(total_qty))
        )
        ...
        self.assertEqual(
            moves_KG.product_uom_qty, 1, "Wrong move quantity (%s found instead of 1.0)" % (moves_KG.product_uom_qty)
        )
        self.assertEqual(moves_KG.product_uom.id, self.uom_gm.id, "Wrong uom in move for product KG.")
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 999.9980, "Expecting 999.9980 kg , got %.4f kg on location stock!" % (sum(total_qty))
        )
        ...
        self.assertEqual(
            moves_KG.product_uom_qty, 0.5, "Wrong move quantity (%s found instead of 0.5)" % (moves_KG.product_uom_qty)
        )
        self.assertEqual(moves_KG.product_uom.id, self.uom_gm.id, "Wrong uom in move for product KG.")
        ...
        self.assertAlmostEqual(
            sum(total_qty), 999.9975, msg="Expecting 999.9975 kg , got %.4f kg on location stock!" % (sum(total_qty))
        )
    ...
    def test_20_create_inventory_with_different_uom(self):
        ...
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.UnitA.id), ("location_id", "=", self.stock_location)]
        )
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 120, "Expecting 120 Units , got %.4f Units on location stock!" % (sum(total_qty))
        )
        self.assertEqual(
            self.UnitA.qty_available,
            120,
            "Expecting 120 Units , got %.4f Units of quantity available!" % (self.UnitA.qty_available),
        )
        # Create Inventory again for product UnitA.
        inventory = self.InvObj.create({"name": "Test", "product_id": self.UnitA.id, "filter": "product"})
        inventory.action_start()
        self.assertEqual(len(inventory.line_ids), 1, "One inventory line should be created.")
        inventory_line = self.InvLineObj.search(
            [("product_id", "=", self.UnitA.id), ("inventory_id", "=", inventory.id)], limit=1
        )
        ...
        total_qty = [quant.quantity for quant in quants]
        self.assertEqual(
            sum(total_qty), 144, "Expecting 144 Units , got %.4f Units on location stock!" % (sum(total_qty))
        )
        self.UnitA._compute_quantities()
        self.assertEqual(
            self.UnitA.qty_available,
            144,
            "Expecting 144 Units , got %.4f Units of quantity available!" % (self.UnitA.qty_available),
        )
        ...
        inventory = self.InvObj.create(
            {"name": "Inventory Product KG", "product_id": productKG.id, "filter": "product"}
        )
        inventory.action_start()
        self.assertFalse(inventory.line_ids, "Inventory line should not created.")
        inventory_line = self.InvLineObj.create(
        ...
        self.assertEqual(
            productKG.qty_available,
            5000,
            "Expecting 5000 kg , got %.4f kg of quantity available!" % (productKG.qty_available),
        )
        ...
        self.assertEqual(len(inventory.line_ids), 1, "One inventory line should be created.")
        inventory_line = self.InvLineObj.search(
            [("product_id", "=", productKG.id), ("inventory_id", "=", inventory.id)], limit=1
        )
        ...
        self.assertEqual(
            productKG.qty_available,
            4000,
            "Expecting 4000 kg , got %.4f of quantity available!" % (productKG.qty_available),
        )
        ...
        packproduct = self.ProductObj.create(
            {"name": "Pack Product", "uom_id": self.uom_unit.id, "uom_po_id": self.uom_unit.id, "type": "product"}
        )
        lotproduct = self.ProductObj.create(
            {"name": "Lot Product", "uom_id": self.uom_unit.id, "uom_po_id": self.uom_unit.id, "type": "product"}
        )
        inventory = self.InvObj.create(
            {"name": "Test Partial and Pack", "filter": "partial", "location_id": self.stock_location}
        )
        inventory.action_start()
        ...
        line_vals += [
            {
                ...
                "product_qty": 10,
                "product_uom_id": packproduct.uom_id.id,
            }
        ]
        line_vals += [
            {
        ...
        line_vals += [
            {
                ...
                "product_qty": 30,
                "product_uom_id": lotproduct.uom_id.id,
                "prod_lot_id": lot1.id,
            }
        ]
        line_vals += [
        ...
        self.assertEqual(
            total_qty,
            20,
            "Expecting 20 units on package 1 of packproduct, but we got %.4f on location stock!" % (total_qty),
        )
        ...
        inventory2 = self.InvObj.create(
            {"name": "Test Partial Lot and Pack2", "filter": "partial", "location_id": self.stock_location}
        )
        ...
        total_qty = sum([quant.quantity for quant in quants])
        self.assertEqual(
            total_qty, 10, "Expecting 0 units lot of lotproduct, but we got %.4f on location stock!" % (total_qty)
        )
        quants = self.StockQuantObj.search(
        ...
        total_qty = sum([quant.quantity for quant in quants])
        self.assertEqual(
            total_qty, 0, "Expecting 0 units lot of lotproduct, but we got %.4f on location stock!" % (total_qty)
        )
        ...
        self.assertEqual(
            inventory5_lines_count,
            inventory4_lines_count + 1,
            "The new product is not taken into account in the inventory valuation.",
        )
        self.assertTrue(
            product.id in inventory5.line_ids.mapped("product_id").ids,
            "The new product is not take into account in the inventory valuation.",
        )
    ...
    def test_50_create_in_out_with_product_pack_lines(self):
        ...
        packout2 = (
            picking_out.move_line_ids[0].with_context(bypass_reservation_update=True).copy({"product_uom_qty": 0})
        )
        packout1.qty_done = 2
        ...
        self.assertEqual(
            neg_quants.location_id.id, self.supplier_location, "There shoud be 1 negative quants for supplier!"
        )

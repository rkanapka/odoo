...
class TestWarehouse(TestStockCommon):
    ...
    def test_inventory_product(self):
        ...
        self.assertEqual(
            inventory.move_ids.location_dest_id, self.env.ref("stock.location_inventory")
        )  # Inventory loss
        ...
        self.assertEqual(
            self.env["stock.quant"]._gather(self.product_1, self.warehouse_1.view_location_id).quantity, 35.0
        )
    ...
    def test_basic_move(self):
        ...
        receive_move = self._create_move(
            product, self.env.ref("stock.stock_location_suppliers"), self.warehouse_1.lot_stock_id, product_uom_qty=15
        )
        ...
        customer_move_2 = self._create_move(
            product, self.warehouse_1.lot_stock_id, self.env.ref("stock.stock_location_customers"), product_uom_qty=2
        )

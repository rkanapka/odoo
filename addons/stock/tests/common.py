...
class TestStockCommon(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.uom_kg = self.UomObj.create({"name": "Test-KG", "category_id": self.categ_kgm, "rounding": 0.000001})
        ...
        # Check Unit
        self.uom_unit = self.UomObj.create({"name": "Test-Unit", "category_id": self.categ_unit, "rounding": 1.0})
        ...
        self.DozA = self.ProductObj.create(
            {"name": "Dozon-A", "type": "product", "uom_id": self.uom_dozen.id, "uom_po_id": self.uom_dozen.id}
        )
        self.SDozA = self.ProductObj.create(
            {"name": "SuperDozon-A", "type": "product", "uom_id": self.uom_sdozen.id, "uom_po_id": self.uom_sdozen.id}
        )
        self.SDozARound = self.ProductObj.create(
            {
                "name": "SuperDozenRound-A",
                "type": "product",
                "uom_id": self.uom_sdozen_round.id,
                "uom_po_id": self.uom_sdozen_round.id,
            }
        )
        self.UnitA = self.ProductObj.create({"name": "Unit-A", "type": "product"})
        self.kgB = self.ProductObj.create(
            {"name": "kg-B", "type": "product", "uom_id": self.uom_kg.id, "uom_po_id": self.uom_kg.id}
        )
        self.gB = self.ProductObj.create(
            {"name": "g-B", "type": "product", "uom_id": self.uom_gm.id, "uom_po_id": self.uom_gm.id}
        )

...
class StockQuant(models.Model):
    ...
    product_id = fields.Many2one("product.product", "Product", ondelete="restrict", readonly=True, required=True)
    ...
    product_uom_id = fields.Many2one("product.uom", "Unit of Measure", readonly=True, related="product_id.uom_id")
    company_id = fields.Many2one(related="location_id.company_id", string="Company", store=True, readonly=True)
    location_id = fields.Many2one(
        "stock.location", "Location", auto_join=True, ondelete="restrict", readonly=True, required=True
    )
    lot_id = fields.Many2one("stock.production.lot", "Lot/Serial Number", ondelete="restrict", readonly=True)
    package_id = fields.Many2one(
        "stock.quant.package", "Package", help="The package containing this quant", readonly=True, ondelete="restrict"
    )
    ...
    def _compute_name(self):
        self.name = "%s: %s%s" % (
            self.lot_id.name or self.product_id.code or "",
            self.quantity,
    ...
    def _gather(self, product_id, location_id, lot_id=None, package_id=None, owner_id=None, strict=False):
        ...
        query_str = (
            'SELECT "%s".id FROM ' % self._table + from_clause + where_str + " ORDER BY " + removal_strategy_order
        )
    ...
    def _update_available_quantity(
        ...
        for quant in quants:
            try:
                with self._cr.savepoint():
                    self._cr.execute(
                        "SELECT 1 FROM stock_quant WHERE id = %s FOR UPDATE NOWAIT", [quant.id], log_exceptions=False
                    )
                    quant.write(
                        {
        ...
        return self._get_available_quantity(
            ...
            strict=False,
            allow_negative=True,
        ), fields.Datetime.from_string(in_date)
...
class QuantPackage(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company", "Company", compute="_compute_package_info", search="_search_company", index=True, readonly=True
    )
    owner_id = fields.Many2one(
        "res.partner", "Owner", compute="_compute_package_info", search="_search_owner", index=True, readonly=True
    )
    ...
    def _compute_current_picking_info(self):
        ...
        for package in self:
            ...
            package.is_processed = not bool(
                package.current_picking_move_line_ids.filtered(lambda ml: ml.qty_done < ml.product_uom_qty)
            )

...
class MrpStockReport(models.TransientModel):
    ...
    def get_move_lines_downstream(self, move_lines):
        ...
        while lines_todo:
            ...
            if move_line.move_id.move_dest_ids:
                lines = (
                    move_line.move_id.move_dest_ids.mapped("move_line_ids").filtered(
                        lambda m: m.lot_id == move_line.lot_id
                    )
                    - lines_seen
                )
    ...
    def get_lines(self, line_id=None, **kw):
        ...
        if context.get("active_id") and not context.get("model") or context.get("model") == "stock.production.lot":
            ...
            move_ids = (
                ...
                .move_lines.mapped("move_line_ids")
                .filtered(lambda m: m.lot_id and m.state == "done")
            )
            res = self._lines(
                line_id,
                model_id=model_id,
            ...
            res = self._lines(
                ...
                parent_quant=parent_quant,
                stream=stream,
                obj_ids=move_line_ids,
            )
        else:
            res = self._lines(
                line_id, model_id=model_id, model=model, level=level, parent_quant=parent_quant, stream=stream
            )
        reverse_sort = True
        if stream == "downstream":
            reverse_sort = False
            ...
            data = [
                {
                    ...
                    % (
                        self._quantity_to_str(
                            move_line.product_uom_id, move_line.product_id.uom_id, move_line.quantity
                        ),
                        move_line.product_id.uom_id.name,
                    ),
    ...
    @api.model
    def upstream_traceability(
        self, level, stream=False, line_id=False, model=False, model_obj=False, parent_quant=False
    ):
        final_vals = []
        if model == "stock.move.line":
        ...
        for move in moves:
            ...
            if move.consume_line_ids:
                unfoldable = True
            final_vals += self.make_dict_move(
                level, stream=stream, parent_id=line_id, move_line=move, unfoldable=unfoldable
            )
        return final_vals

    @api.model
    def downstream_traceability(
        self, level, stream=False, line_id=False, model=False, model_obj=False, parent_quant=False
    ):
        final_vals = []
        if model == "stock.move.line":
            ...
            lines.append(
                {
                    ...
                    "columns": [
                        data.get("reference_id", False) or data.get("product_id", False),
    ...
    @api.model
    def _lines(
        self, line_id=None, model_id=False, model=False, level=0, parent_quant=False, stream=False, obj_ids=[], **kw
    ):
        final_vals = []
        if model and line_id:
            model_obj = self.env[model].browse(model_id)
            if stream == "downstream":
                final_vals += self.downstream_traceability(
                    level,
                    stream="downstream",
                    line_id=line_id,
                    model=model,
                ...
                if model == "stock.move.line":
                    if model_obj.produce_line_ids:
                        final_vals += self.get_produced_or_consumed_vals(
                            model_obj.produce_line_ids, level, model=model, stream=stream, parent_id=line_id
                        )
                    else:
                        final_vals = (
                            self.make_dict_move(level, stream=stream, parent_id=line_id, move_line=model_obj)
                            + final_vals
                        )
            else:
                final_vals += self.upstream_traceability(
                    level,
                    stream="upstream",
                    line_id=line_id,
                    model=model,
                ...
                if model == "stock.move.line":
                    if model_obj.consume_line_ids:
                        final_vals += self.get_produced_or_consumed_vals(
                            model_obj.consume_line_ids, level, model=model, stream=stream, parent_id=line_id
                        )
                    else:
                        final_vals = (
                            self.make_dict_move(level, stream=stream, parent_id=line_id, move_line=model_obj)
            ...
            for move_line in obj_ids:
                final_vals += self.make_dict_head(
                    level, stream=stream, parent_id=line_id, model=model or "stock.pack.operation", move_line=move_line
                )
        return final_vals
    ...
    def get_pdf_lines(self, line_data=[]):
        ...
        for line in line_data:
            ...
            if line.get("unfoldable"):
                final_vals += self.make_dict_head(
                    line["level"], model=line["model_name"], parent_id=line["id"], move_line=model
                )
            else:
            ...
            lines.append(
                {
                    ...
                    "columns": [
                        data.get("reference_id") or data.get("product_id"),
                        data.get("lot_id"),
                        data.get("date"),
                        data.get("product_qty_uom", 0),
                        data.get("location"),

...
class Warehouse(models.Model):
    ...
    view_location_id = fields.Many2one(
        "stock.location", "View Location", domain=[("usage", "=", "view")], required=True
    )
    lot_stock_id = fields.Many2one(
        "stock.location", "Location Stock", domain=[("usage", "=", "internal")], required=True
    )
    ...
    resupply_wh_ids = fields.Many2many(
        "stock.warehouse", "stock_wh_resupply_table", "supplied_wh_id", "supplier_wh_id", "Resupply Warehouses"
    )
    ...
    default_resupply_wh_id = fields.Many2one(
        "stock.warehouse", "Default Resupply Warehouse", help="Goods will always be resupplied from this warehouse"
    )
    ...
    def write(self, vals):
        ...
        if "default_resupply_wh_id" in vals:
            if vals.get("default_resupply_wh_id") and any(
                vals["default_resupply_wh_id"] == warehouse.id for warehouse in warehouses
            ):
        ...
        if vals.get("resupply_wh_ids") and not vals.get("resupply_route_ids"):
            for warehouse in warehouses:
                ...
                if to_remove:
                    Route.search(
                        [("supplied_wh_id", "=", warehouse.id), ("supplier_wh_id", "in", to_remove.ids)]
                    ).write({"active": False})
    ...
    def _update_partner_data(self, partner_id, company_id):
        ...
        if company_id:
            ...
            self.env["res.partner"].browse(partner_id).with_context(force_company=company_id).write(
    ...
    def _create_or_update_reception_route(self, routes_data):
        ...
        for warehouse in self:
            ...
            push_rules_list, pull_rules_list = warehouse._get_push_pull_rules_values(
                routings,
                values={"active": True, "route_id": reception_route.id},
                push_values=None,
                pull_values={"procure_method": "make_to_order"},
            )
    ...
    def _create_or_update_crossdock_route(self, routes_data):
        ...
        for warehouse in self:
            if warehouse.crossdock_route_id:
                ...
                crossdock_route.write(
                    {"active": warehouse.reception_steps != "one_step" and warehouse.delivery_steps != "ship_only"}
                )
            else:
    ...
    def create_resupply_routes(self, supplier_warehouses, default_resupply_wh):
        ...
        for supplier_wh in supplier_warehouses:
            transit_location = (
                internal_transit_location if supplier_wh.company_id == self.company_id else external_transit_location
            )
            ...
            if supplier_wh.delivery_steps == "ship_only":
                Pull.create(
                    supplier_wh._get_mto_pull_rules_values(
    ...
    def _get_transit_locations(self):
        return (
            ...
            self.env.ref("stock.stock_location_inter_wh", raise_if_not_found=False) or self.env["stock.location"],
        )
    ...
    def _get_route_name(self, route_type):
        names = {
            ...
            "two_steps": _("Receipt in 2 steps"),
            "three_steps": _("Receipt in 3 steps"),
            "crossdock": _("Cross-Dock"),
            "ship_only": _("Ship Only"),
            "pick_ship": _("Pick + Ship"),
    ...
    def get_routes_dict(self):
        ...
        return {
            warehouse.id: {
                ...
                "crossdock": [
                    self.Routing(
                        warehouse.wh_input_stock_loc_id, warehouse.wh_output_stock_loc_id, warehouse.int_type_id
                    ),
                ...
                "pick_pack_ship": [
                    ...
                    self.Routing(
                        warehouse.wh_pack_stock_loc_id, warehouse.wh_output_stock_loc_id, warehouse.pack_type_id
                    ),
                    self.Routing(warehouse.wh_output_stock_loc_id, customer_loc, warehouse.out_type_id),
                ],
    ...
    def _get_mto_pull_rules_values(self, route_values):
        ...
        dummy, pull_rules_list = self._get_push_pull_rules_values(
            route_values,
            pull_values={"route_id": mto_route.id, "procure_method": "make_to_order", "active": True},
            name_suffix=_("MTO"),
        )
    ...
    def _get_supply_pull_rules_values(self, route_values, values=None):
        ...
        for pull_rules in pull_rules_list:
            pull_rules["procure_method"] = (
                self.lot_stock_id.id != pull_rules["location_src_id"] and "make_to_order" or "make_to_stock"
    ...
    def _check_delivery_resupply(self, new_location, change_to_multiple):
        ...
        if not change_to_multiple:
            ...
            routings = [
                self.Routing(self.lot_stock_id, location, self.out_type_id) for location in pulls.mapped("location_id")
            ]
    ...
    def _check_reception_resupply(self, new_location):
        ...
        self.env["procurement.rule"].search(
            ["&", ("route_id", "in", routes.ids), ("location_src_id.usage", "=", "transit")]
    ...
    def _update_location_reception(self, new_reception_step):
        ...
        if switch_warehouses:
            (switch_warehouses.mapped("wh_input_stock_loc_id") + switch_warehouses.mapped("wh_qc_stock_loc_id")).write(
                {"active": False}
            )
    ...
    def _get_sequence_values(self):
        return {
            ...
            "pick_type_id": {
                ...
                "prefix": self.code + "/PICK/",
                "padding": 5,
                "company_id": self.company_id.id,
            },
            "int_type_id": {
                "name": self.name + " " + _("Sequence internal"),
                "prefix": self.code + "/INT/",
                "padding": 5,
                "company_id": self.company_id.id,
            },
        }
...
class Orderpoint(models.Model):
    ...
    product_id = fields.Many2one(
        "product.product", "Product", domain=[("type", "=", "product")], ondelete="cascade", required=True
    )
        ...
        if self.lead_type == "supplier":
            ...
            days += (
                self.product_id._select_seller(
                    quantity=product_qty, date=fields.Date.to_string(start_date), uom_id=self.product_uom
                ).delay
                or 0.0
            )

...
class Location(models.Model):
    ...
    posx = fields.Integer(
        "Corridor (X)", default=0, help="Optional localization details, for information purpose only"
    )
    ...
    company_id = fields.Many2one(
        "res.company",
        "Company",
        default=lambda self: self.env["res.company"]._company_default_get("stock.location"),
        index=True,
    ...
    return_location = fields.Boolean(
        "Is a Return Location?", help="Check this box to allow using this location as a return location."
    )
    ...
    def write(self, values):
        ...
        if "usage" in values or "scrap_location" in values:
            ...
            modified_locations = self.filtered(
                lambda l: any(l[f] != values[f] if f in values else False for f in {"usage", "scrap_location"})
            )
...
class Route(models.Model):
    ...
    product_selectable = fields.Boolean(
        ...
        default=True,
        help="When checked, the route will be selectable in the Inventory tab of the Product form.  It will take priority over the Warehouse route. ",
    )
    product_categ_selectable = fields.Boolean(
        "Applicable on Product Category",
        help="When checked, the route will be selectable on the Product Category.  It will take priority over the Warehouse route. ",
    )
    warehouse_selectable = fields.Boolean(
        "Applicable on Warehouse",
        help="When a warehouse is selected for this route, this route should be seen as the default route when products pass through this warehouse.  This behaviour can be overridden by the routes on the Product/Product Categories or by the Preferred Routes on the Procurement",
    )
    ...
    company_id = fields.Many2one(
        "res.company",
        "Company",
        default=lambda self: self.env["res.company"]._company_default_get("stock.location.route"),
        index=True,
    ...
    categ_ids = fields.Many2many(
        "product.category", "stock_location_route_categ", "route_id", "categ_id", "Product Categories"
    )
    warehouse_ids = fields.Many2many(
        "stock.warehouse", "stock_route_warehouse", "route_id", "warehouse_id", "Warehouses"
    )
...
class PushedFlow(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company",
        "Company",
        default=lambda self: self.env["res.company"]._company_default_get("stock.location.path"),
        index=True,
    )
    ...
    propagate = fields.Boolean(
        ...
        default=True,
        help="If checked, when the previous move is cancelled or split, the move generated by this move will too",
    )
    ...
    def _apply(self, move):
        new_date = (
            datetime.strptime(move.date_expected, DEFAULT_SERVER_DATETIME_FORMAT)
            + relativedelta.relativedelta(days=self.delay)

...
class ProcurementRule(models.Model):
    ...
    name = fields.Char(
        "Name", required=True, translate=True, help="This field will fill the packing origin and the name of its moves"
    )
    active = fields.Boolean(
        "Active", default=True, help="If unchecked, it will allow you to hide the rule without removing it."
    )
    ...
    action = fields.Selection(selection=[("move", "Move From Another Location")], string="Action", required=True)
    ...
    procure_method = fields.Selection(
        [("make_to_stock", "Take From Stock"), ("make_to_order", "Create Procurement")],
    ...
    def _run_move(self, product_id, product_qty, product_uom, location_id, name, origin, values):
        ...
        data = self._get_stock_move_values(
    ...
    def _get_stock_move_values(
        ...
        """
        date_expected = (
            datetime.strptime(values["date_planned"], DEFAULT_SERVER_DATETIME_FORMAT)
        ...
        return {
            ...
            or self.location_id.company_id.id
            or values["company_id"].id,
            "product_id": product_id.id,
            "product_uom": product_uom.id,
            "product_uom_qty": qty_left,
            "partner_id": self.partner_address_id.id
            or (values.get("group_id", False) and values["group_id"].partner_id.id)
            or False,
...
class ProcurementGroup(models.Model):
    ...
    move_type = fields.Selection(
        [("direct", "Partial"), ("one", "All at once")], string="Delivery Type", default="direct", required=True
    )
    ...
    def _search_rule(self, product_id, values, domain):
        ...
        if values.get("route_ids", False):
            res = Pull.search(
                expression.AND([[("route_id", "in", values["route_ids"].ids)], domain]),
    ...
    def _procure_orderpoint_confirm(self, use_new_cursor=False, company_id=False):
        ...
        orderpoints_noprefetch = (
            OrderPoint.with_context(prefetch_fields=False)
            .search(domain, order=self._procurement_from_orderpoint_get_order())
            .ids
        )
                    ...
                    for orderpoint in location_orderpoints:
                        try:
                            ...
                            if (
                                ...
                                if (
                                    float_compare(remainder, 0.0, precision_rounding=orderpoint.product_uom.rounding)
                                    > 0
                                ):
                                ...
                                if qty_rounded > 0:
                                    values = orderpoint._prepare_procurement_values(
                                        qty_rounded, **group["procurement_values"]
                                    )
                                    try:
                                        with self._cr.savepoint():
                                        ...
                                        self.env["procurement.rule"]._log_next_activity(
                                            orderpoint.product_id, error.name
                                        )

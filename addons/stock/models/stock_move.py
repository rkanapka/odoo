...
class StockMove(models.Model):
    ...
    product_tmpl_id = fields.Many2one(
        "product.template", "Product Template", related="product_id.product_tmpl_id", help="Technical: used in views"
    )
    ...
    procure_method = fields.Selection(
        [("make_to_stock", "Default: Take From Stock"), ("make_to_order", "Advanced: Apply Procurement Rules")],
    ...
    push_rule_id = fields.Many2one(
        "stock.location.path", "Push Rule", ondelete="restrict", help="The push rule that created this stock move"
    )
    ...
    origin_returned_move_id = fields.Many2one(
        "stock.move", "Origin return move", copy=False, help="Move that created the return move"
    )
    ...
    restrict_partner_id = fields.Many2one(
        "res.partner",
        "Owner ",
        help="Technical field used to depict a restriction on the ownership of quants to consider when marking this move as 'done'",
    )
    ...
    def _compute_show_details_visible(self):
        ...
        for move in self:
            ...
            if (
                ...
                and (
                    multi_locations_enabled
                    or move.has_tracking != "none"
                    or len(move.move_line_ids) > 1
                    or has_package
                    or consignment_enabled
                )
            ):
    ...
    def _compute_reserved_availability(self):
        ...
        """
        result = {
            data["move_id"][0]: data["product_qty"]
    ...
    def _compute_product_availability(self):
        ...
        if self.state == "done":
            ...
            total_availability = (
                self.env["stock.quant"]._get_available_quantity(self.product_id, self.location_id)
                if self.product_id
                else 0.0
            )
    ...
    def init(self):
        self._cr.execute(
            "SELECT indexname FROM pg_indexes WHERE indexname = %s", ("stock_move_product_location_index",)
        )
    ...
    def write(self, vals):
        ...
        if "product_uom_qty" in vals:
            for move in self.filtered(lambda m: m.state not in ("done", "draft") and m.picking_id):
                if float_compare(
                    vals["product_uom_qty"], move.product_uom_qty, precision_rounding=move.product_uom.rounding
                ):
            ...
            if self.env.context.get("do_not_unreserve") is None:
                move_to_unreserve = self.filtered(
                    ...
                    and float_compare(
                        m.reserved_availability, vals.get("product_uom_qty"), precision_rounding=m.product_uom.rounding
                    )
                    == 1
                )
                    ...
                    if propagated_date_field:
                        ...
                        if abs(delta_days) >= move.company_id.propagation_minimum_delta:
                            old_move_date = datetime.strptime(
                                move.move_dest_ids[0].date_expected, DEFAULT_SERVER_DATETIME_FORMAT
                            )
                            new_move_date = (
                                old_move_date + relativedelta.relativedelta(days=delta_days or 0)
        ...
        if track_pickings:
            ...
            initial_values = {picking.id: {"state": picking.state} for picking in pickings}
    ...
    def action_show_details(self):
        ...
        return {
            ...
            "context": dict(
                ...
                and picking_type_id.use_create_lots
        ...
        for move in self:
            ...
            if not rules:
                ...
                if move.warehouse_id:
                    rules = Push.search(
                        domain + [("route_id", "in", move.warehouse_id.route_ids.ids)],
            ...
            if rules and (
                ...
                or move.origin_returned_move_id.location_dest_id.id != rules.location_dest_id.id
            ):
                rules._apply(move)
    ...
    def _merge_moves_fields(self):
        ...
        return {
            ...
            "date_expected": (
                min(self.mapped("date_expected"))
                if self.mapped("picking_id").move_type == "direct"
                else max(self.mapped("date_expected"))
            ),
    ...
    def _merge_moves(self, merge_into=False):
        ...
        for candidate_moves in candidate_moves_list:
            ...
            for k, g in groupby(
                sorted(candidate_moves, key=self._prepare_merge_move_sort_method), key=itemgetter(*distinct_fields)
            ):
                moves = (
                    self.env["stock.move"].concat(*g).filtered(lambda m: m.state not in ("done", "cancel", "draft"))
                )
    ...
    def _get_relevant_state_among_moves(self):
        ...
        moves_todo = self.filtered(lambda move: move.state not in ["cancel", "done"]).sorted(
            key=lambda move: (sort_map.get(move.state, 0), move.product_uom_qty)
        )
    ...
    def _action_confirm(self, merge=True, merge_into=False):
        ...
        for move in move_create_proc:
            ...
            origin = (
                move.group_id
                and move.group_id.name
                or (move.rule_id and move.rule_id.name or move.origin or move.picking_id.name or "/")
            )
            ...
            if move.location_id.should_bypass_reservation() or move.product_id.type == "consu":
                ...
                if not move.move_orig_ids:
                    ...
                    taken_quantity = move._update_reserved_quantity(
                        need, available_quantity, move.location_id, strict=False
                    )
                    ...
                    for k, g in groupby(
                        sorted(move_lines_out_done, key=_keys_out_sorted), key=itemgetter(*keys_out_groupby)
                    ):
                        qty_done = 0
                        for ml in g:
                            qty_done += ml.product_uom_id._compute_quantity(ml.qty_done, ml.product_id.uom_id)
                    ...
                    for k, g in groupby(
                        sorted(move_lines_out_reserved, key=_keys_out_sorted), key=itemgetter(*keys_out_groupby)
                    ):
                    ...
                    for (location_id, lot_id, package_id, owner_id), quantity in available_move_lines.items():
                        ...
                        if float_is_zero(available_quantity, precision_rounding=move.product_id.uom_id.rounding):
                            continue
                        taken_quantity = move._update_reserved_quantity(
                            need, min(quantity, available_quantity), location_id, lot_id, package_id, owner_id
                        )
                        if float_is_zero(taken_quantity, precision_rounding=move.product_id.uom_id.rounding):
                            continue
    ...
    def _create_extra_move(self):
        ...
        if float_compare(self.quantity_done, self.product_uom_qty, precision_rounding=rounding) > 0:
            ...
            extra_move_quantity = float_round(
                self.quantity_done - self.product_uom_qty, precision_rounding=rounding, rounding_method="HALF-UP"
            )
    ...
    def _split(self, qty, restrict_partner_id=False):
        ...
        new_product_qty = float_round(
            new_product_qty, precision_digits=self.env["decimal.precision"].precision_get("Product Unit of Measure")
        )

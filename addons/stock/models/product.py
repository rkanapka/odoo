...
class Product(models.Model):
    ...
    def _compute_quantities_dict(self, lot_id, owner_id, package_id, from_date=False, to_date=False):
        ...
        quants_res = {
            ...
            for item in Quant.read_group(domain_quant, ["product_id", "quantity"], ["product_id"], orderby="id")
        }
        if dates_in_the_past:
    ...
    def onchange_tracking(self):
        ...
        if products:
            unassigned_quants = self.env["stock.quant"].search_count(
                [("product_id", "in", products.ids), ("lot_id", "=", False), ("location_id.usage", "=", "internal")]
            )
...
class ProductTemplate(models.Model):
    ...
    warehouse_id = fields.Many2one(
        "stock.warehouse", "Warehouse", store=False, search=lambda operator, operand, vals: []
    )
    ...
    route_from_categ_ids = fields.Many2many(
        relation="stock.location.route", string="Category Routes", related="categ_id.total_route_ids"
    )
    ...
    def action_view_routes(self):
        routes = (
            self.mapped("route_ids")
            | self.mapped("categ_id").mapped("total_route_ids")
            | self.env["stock.location.route"].search([("warehouse_selectable", "=", True)])
        )
...
class ProductCategory(models.Model):
    ...
    total_route_ids = fields.Many2many(
        "stock.location.route", string="Total routes", compute="_compute_total_route_ids", readonly=True
    )
    ...
    def write(self, values):
        ...
        if "factor" in values or "factor_inv" in values or "category_id" in values:
            changed = self.filtered(
                lambda u: any(u[f] != values[f] if f in values else False for f in {"factor", "factor_inv"})
            ) + self.filtered(
                lambda u: any(u[f].id != int(values[f]) if f in values else False for f in {"category_id"})
            )
            if changed:

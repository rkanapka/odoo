...
class ResConfigSettings(models.TransientModel):
    ...
    module_procurement_jit = fields.Selection(
        [(1, "Immediately after sales order confirmation"), (0, "Manually or based on automatic scheduler")],
        "Reservation",
    ...
    group_stock_production_lot = fields.Boolean("Lots & Serial Numbers", implied_group="stock.group_production_lot")
    group_stock_tracking_lot = fields.Boolean("Delivery Packages", implied_group="stock.group_tracking_lot")
    group_stock_tracking_owner = fields.Boolean("Consignment", implied_group="stock.group_tracking_owner")
    ...
    propagation_minimum_delta = fields.Integer(
        related="company_id.propagation_minimum_delta", string="No Rescheduling Propagation"
    )
    ...
    group_stock_multi_warehouses = fields.Boolean(
        "Multi-Warehouses", implied_group="stock.group_stock_multi_warehouses"
    )
    ...
    def get_values(self):
        ...
        res.update(
            use_propagation_minimum_delta=self.env["ir.config_parameter"]
            .sudo()
            .get_param("stock.use_propagation_minimum_delta")
        )
        return res
    ...
    def set_values(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "stock.use_propagation_minimum_delta", self.use_propagation_minimum_delta
        )

...
class PickingType(models.Model):
    ...
    show_entire_packs = fields.Boolean(
        "Allow moving packs",
        help="If checked, this shows the packs to be moved as a whole in the Operations tab all the time, even if there was no entire pack reserved.",
    )
    ...
    show_reserved = fields.Boolean(
        "Show Reserved",
        default=True,
        help="If this checkbox is ticked, Odoo will show which products are reserved (lot/serial number, source location, source package).",
    )
    ...
    def _compute_last_done_picking(self):
        ...
        for picking in self.env["stock.picking"].search(
            [("picking_type_id", "=", self.id), ("state", "=", "done")], order="date_done desc", limit=10
        ):
    ...
    def _compute_picking_count(self):
        ...
        for record in self:
            record.rate_picking_late = (
                record.count_picking and record.count_picking_late * 100 / record.count_picking or 0
            )
            record.rate_picking_backorders = (
                record.count_picking and record.count_picking_backorders * 100 / record.count_picking or 0
            )
...
class Picking(models.Model):
    ...
    group_id = fields.Many2one(
        "procurement.group", "Procurement Group", readonly=True, related="move_lines.group_id", store=True
    )
    ...
    picking_type_id = fields.Many2one(
        "stock.picking.type", "Operation Type", required=True, readonly=True, states={"draft": [("readonly", False)]}
    )
    ...
    partner_id = fields.Many2one(
        "res.partner", "Partner", states={"done": [("readonly", True)], "cancel": [("readonly", True)]}
    )
    ...
    show_validate = fields.Boolean(
        compute="_compute_show_validate", help="Technical field used to compute whether the validate should be shown."
    )
    ...
    def _compute_show_operations(self):
        for picking in self:
            ...
            if picking.picking_type_id.show_operations:
                if (
                    picking.state == "draft" and not self.env.context.get("planned_picking")
    ...
    def _has_scrap_move(self):
        ...
        self.has_scrap_move = bool(
            self.env["stock.move"].search_count([("picking_id", "=", self.id), ("scrapped", "=", True)])
        )
    ...
    def _compute_entire_package_ids(self):
        ...
        for picking in self:
            ...
            packages_to_check = picking.move_line_ids.filtered(
                lambda ml: ml.result_package_id and ml.package_id.id == ml.result_package_id.id
            ).mapped("package_id")
            for package_to_check in packages_to_check:
                if picking.state in ("done", "cancel") or picking._check_move_lines_map_quant_package(
                    package_to_check
                ):
    ...
    def _compute_show_check_availability(self):
        for picking in self:
            has_moves_to_reserve = any(
                ...
                and float_compare(move.product_uom_qty, 0, precision_rounding=move.product_uom.rounding)
    ...
    def create(self, vals):
        ...
        if (
            ...
            vals["name"] = (
                ...
                .browse(vals.get("picking_type_id", defaults.get("picking_type_id")))
    ...
    def action_confirm(self):
        ...
        self.filtered(
            lambda picking: picking.location_id.usage in ("supplier", "inventory", "production")
    ...
    def _check_move_lines_map_quant_package(self, package):
        ...
        if any(
            not float_is_zero(grouped_quants.get(key, 0) - grouped_ops.get(key, 0), precision_digits=precision_digits)
    ...
    def _get_overprocessed_stock_moves(self):
        ...
        return self.move_lines.filtered(
            lambda move: move.product_uom_qty != 0
            and float_compare(move.quantity_done, move.product_uom_qty, precision_rounding=move.product_uom.rounding)
            == 1
        )
        ...
        for picking in self:
            ...
            if moves_to_backorder:
                backorder_picking = picking.copy(
                    {"name": "/", "move_lines": [], "move_line_ids": [], "backorder_id": picking.id}
                )
            ...
            if operations:
                ...
                for operation in operations:
                    if (
                        ...
                        new_operation = operation.copy(default={"product_uom_qty": 0, "qty_done": operation.qty_done})

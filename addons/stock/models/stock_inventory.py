...
class Inventory(models.Model):
    ...
    def _selection_filter(self):
        r"""Get the list of filter allowed according to the options checked
...
class InventoryLine(models.Model):
    ...
    product_name = fields.Char("Product Name", related="product_id.name", store=True, readonly=True, compute_sudo=True)
    ...
    prodlot_name = fields.Char("Serial Number Name", related="prod_lot_id.name", store=True, readonly=True)
    company_id = fields.Many2one(
    ...
    def create(self, values):
        ...
        if existings:
            raise UserError(
                _(
                    "You cannot have two inventory adjustements in state 'in Progress' with the same product "
        ...
        for line in self:
            if line.product_id.type != "product":
                raise ValidationError(
                    _("You can only adjust stockable products.")
        ...
        for line in self:
            if (
                float_utils.float_compare(
                    line.theoretical_qty, line.product_qty, precision_rounding=line.product_id.uom_id.rounding
                )
                == 0
            ):
                continue
            ...
            if diff < 0:  # found more than expected
                vals = line._get_move_values(
                    abs(diff), line.product_id.property_stock_inventory.id, line.location_id.id, False
                )
            else:
                vals = line._get_move_values(
                    abs(diff), line.location_id.id, line.product_id.property_stock_inventory.id, True
                )

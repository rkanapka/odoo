...
class StockMoveLine(models.Model):
    ...
    picking_id = fields.Many2one(
        "stock.picking", "Stock Picking", help="The stock operation where the packing has been made"
    )
    move_id = fields.Many2one("stock.move", "Stock Move", help="Change to a better name", index=True)
    product_id = fields.Many2one("product.product", "Product", ondelete="cascade")
    product_uom_id = fields.Many2one("product.uom", "Unit of Measure", required=True)
    product_qty = fields.Float(
        "Real Reserved Quantity", digits=0, compute="_compute_product_qty", inverse="_set_product_qty", store=True
    )
    product_uom_qty = fields.Float(
        "Reserved", default=0.0, digits=dp.get_precision("Product Unit of Measure"), required=True
    )
    ...
    def _compute_location_description(self):
        for operation, operation_sudo in izip(self, self.sudo()):
            ...
            operation.to_loc = "%s%s" % (
                operation_sudo.location_dest_id.name,
                operation_sudo.result_package_id.name or "",
            )
    ...
    def _compute_product_qty(self):
        self.product_qty = self.product_uom_id._compute_quantity(
            self.product_uom_qty, self.product_id.uom_id, rounding_method="HALF-UP"
        )
        ...
        for line in self:
            if line.lot_id and line.product_id != line.lot_id.sudo().product_id:
                raise ValidationError(
                    _(
                        "This lot %s is incompatible with this product %s"
    ...
    def _compute_in_entire_package(self):
        ...
        for ml in self:
            ...
            ml.in_entire_package = (
                picking_id
                and picking_id.picking_type_entire_packs
                and picking_id.state != "done"
                and ml.result_package_id
                and ml.result_package_id in picking_id.entire_package_ids
            )
                ...
                new_move = self.env["stock.move"].create(
                    {
                        ...
                        "location_dest_id": "location_dest_id" in vals
                        and vals["location_dest_id"]
    ...
    def unlink(self):
        ...
        for ml in self:
            ...
            if (
                ...
                and not float_is_zero(ml.product_qty, precision_digits=precision)
            ):
                try:
    ...
    def _action_done(self):
        ...
        for ml in self - ml_to_delete:
            if ml.product_id.type == "product":
                ...
                if (
                    not ml.location_id.should_bypass_reservation()
                    and float_compare(ml.qty_done, ml.product_uom_qty, precision_rounding=rounding) > 0
                ):
    ...
    def _log_message(self, record, move, template, vals):
        ...
        record.message_post_with_view(
            template, values={"move": move, "vals": dict(vals, **data)}, subtype_id=self.env.ref("mail.mt_note").id
        )

    def _free_reservation(
        self, product_id, location_id, quantity, lot_id=None, package_id=None, owner_id=None, ml_to_ignore=None
    ):

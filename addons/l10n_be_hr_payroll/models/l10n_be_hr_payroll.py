...
class HrContract(models.Model):
    ...
    public_transport_reimbursed_amount = fields.Monetary(
        string="Reimbursed amount", compute="_compute_public_transport_reimbursed_amount", readonly=False, store=True
    )
    ...
    additional_net_amount = fields.Monetary(
        string="Net Supplements", track_visibility="onchange", help="Monthly net amount the employee receives."
    )
    ...
    @api.depends(
        "wage", "fuel_card", "representation_fees", "transport_employer_cost", "internet", "mobile", "mobile_plus"
    )
...
class HrEmployee(models.Model):
    ...
    other_senior_dependent = fields.Integer(
        "# seniors (>=65)",
        help="Number of seniors dependent on the employee, including the disabled ones",
        groups="hr.group_hr_user",
    )
    ...
    other_juniors_dependent = fields.Integer(
        "# people (<65)",
        help="Number of juniors dependent on the employee, including the disabled ones",
        groups="hr.group_hr_user",
    )
    ...
    dependent_seniors = fields.Integer(
        compute="_compute_dependent_people", string="Considered number of dependent seniors", groups="hr.group_hr_user"
    )
    dependent_juniors = fields.Integer(
        compute="_compute_dependent_people", string="Considered number of dependent juniors", groups="hr.group_hr_user"
    )
    ...
    def _check_spouse_revenue(self):
        for employee in self:
            if (
                employee.spouse_fiscal_status == "with income"
                and not employee.spouse_net_revenue
                and not employee.spouse_other_net_revenue
            ):
                raise ValidationError(

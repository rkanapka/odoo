...
class MailActivityType(models.Model):
    ...
    days = fields.Integer(
        "# Days", default=0, help="Number of days before executing the action. It allows to plan the action deadline."
    )
    ...
    previous_type_ids = fields.Many2many(
        "mail.activity.type", "mail_activity_rel", "recommended_id", "activity_id", string="Preceding Activities"
    )
...
class MailActivity(models.Model):
    ...
    res_model_id = fields.Many2one("ir.model", "Related Document Model", index=True, ondelete="cascade", required=True)
    res_model = fields.Char(
        "Related Document Model", index=True, related="res_model_id.model", store=True, readonly=True
    )
    ...
    state = fields.Selection(
        [("overdue", "Overdue"), ("today", "Today"), ("planned", "Planned")], "State", compute="_compute_state"
    )

...
class MailTracking(models.Model):
    ...
    def create_tracking_values(self, initial_value, new_value, col_name, col_info):
        ...
        if col_info["type"] in ["integer", "float", "char", "text", "datetime", "monetary"]:
            values.update(
                {"old_value_%s" % col_info["type"]: initial_value, "new_value_%s" % col_info["type"]: new_value}
            )
            ...
            values.update(
                {
                    ...
                    and datetime.strftime(
                        datetime.combine(
                            datetime.strptime(initial_value, tools.DEFAULT_SERVER_DATE_FORMAT), datetime.min.time()
                        ),
                    ...
                    and datetime.strftime(
                        datetime.combine(
                            datetime.strptime(new_value, tools.DEFAULT_SERVER_DATE_FORMAT), datetime.min.time()
                        ),
                        tools.DEFAULT_SERVER_DATETIME_FORMAT,
                    )
                    or False,
            ...
            if record.field_type in ["integer", "float", "char", "text", "monetary"]:
                ...
                if record["%s_value_datetime" % type]:
                    new_date = datetime.strptime(
                        record["%s_value_datetime" % type], tools.DEFAULT_SERVER_DATETIME_FORMAT
                    ).date()

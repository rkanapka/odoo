...
class MailTemplate(models.Model):
    ...
    user_signature = fields.Boolean(
        "Add Signature", help="If checked, the user's signature will be appended to the text version " "of the message"
    )
    ...
    mail_server_id = fields.Many2one(
        ...
        readonly=False,
        help="Optional preferred server for outgoing mails. If not set, the highest " "priority one will be used.",
    )
    ...
    attachment_ids = fields.Many2many(
        ...
        "attachment_id",
        "Attachments",
        help="You may attach files to this template, to be added to all " "emails created from this template",
    )
    auto_delete = fields.Boolean(
        "Auto Delete", default=True, help="Permanently delete this email after sending it, to save space"
    )
    ...
    copyvalue = fields.Char(
        "Placeholder Expression", help="Final placeholder expression, to be copy-pasted in the desired template field."
    )
    ...
    def _replace_local_links(self, html):
        ...
        if not len(root) and root.text is None and root.tail is None:
            html = "<div>%s</div>" % tools.ustr(html)
            root = lxml.html.fromstring(html)

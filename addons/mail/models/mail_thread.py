...
class MailThread(models.AbstractModel):
    ...
    message_ids = fields.One2many(
        "mail.message", "res_id", string="Messages", domain=lambda self: [("model", "=", self._name)], auto_join=True
    )
    ...
    message_unread = fields.Boolean(
        "Unread Messages", compute="_get_message_unread", help="If checked new messages require your attention."
    )
    message_unread_counter = fields.Integer(
        "Unread Messages Counter", compute="_get_message_unread", help="Number of unread messages"
    )
    message_needaction = fields.Boolean(
        "Action Needed",
        compute="_get_message_needaction",
        search="_search_message_needaction",
        help="If checked, new messages require your attention.",
    )
    message_needaction_counter = fields.Integer(
        "Number of Actions", compute="_get_message_needaction", help="Number of messages which requires an action"
    )
    ...
    def _search_follower_channels(self, operator, operand):
        ...
        followers = (
            self.env["mail.followers"]
            .sudo()
            .search([("res_model", "=", self._name), ("channel_id", operator, operand)])
        )
        # using read() below is much faster than followers.mapped('res_id')
        return [("id", "in", [res["res_id"] for res in followers.read(["res_id"])])]
    ...
    def _get_message_unread(self):
        res = {res_id: 0 for res_id in self.ids}
        partner_id = self.env.user.partner_id.id
    ...
    def create(self, values):
        ...
        if not self._context.get("mail_create_nosubscribe"):
            ...
            message_follower_ids += self.env["mail.followers"]._add_follower_command(
                self._name, [], {self.env.user.partner_id.id: None}, {}, force=True
            )[0]
            values["message_follower_ids"] = message_follower_ids
        thread = super().create(values)
    ...
    def write(self, values):
        ...
        if tracked_fields:
            initial_values = {
                record.id: {key: getattr(record, key) for key in tracked_fields} for record in track_self
            }
        ...
        if alias:
            ...
            if add_arrow:
                return "<p class='oe_view_nocontent_create'>%(dyn_help)s</p>%(static_help)s" % {
                    ...
                    % {"document": document_name, "email_link": email_link},
                }
    ...
    def _message_track_post_template(self, tracking):
        ...
        }  # Use clean_context on further versions
        ...
        for record in self:
            ...
            if not self._context.get("mail_track_log_only"):
                subtype_xmlid = record._track_subtype(
                    {col_name: initial_values[record.id][col_name] for col_name in changes}
                )
    ...
    def _notification_link_helper(self, link_type, **kwargs):
        ...
        if kwargs.get("message_id"):
            base_params = {"message_id": kwargs["message_id"]}
        else:
            base_params = {
    ...
    def _message_notification_recipients(self, message, recipients):
        ...
        default_groups = [
            ...
            (
                "portal",
                lambda partner: bool(partner.user_ids) and all(user.share for user in partner.user_ids),
                {
        ...
        for group_name, group_func, group_data in groups:
            ...
            group_data.setdefault("button_access", {"url": access_link, "title": view_title})
    ...
    def message_get_reply_to(self, res_ids, default=None):
        ...
        if alias_domain:
            if model_name and model_name != "mail.thread" and res_ids:
                ...
                doc_names.update(
                    {ng_res[0]: ng_res[1] for ng_res in self.env[model_name].sudo().browse(aliases).name_get()}
                )
            ...
            if left_ids:
                ...
                if catchall_alias:
                    aliases.update({res_id: "%s@%s" % (catchall_alias, alias_domain) for res_id in left_ids})
        ...
        left_ids = set(res_ids).difference(set(aliases))
        if left_ids:
            res.update({res_id: default for res_id in res_ids})
        return res
    ...
    def message_get_email_values(self, notif_mail=None):
        ...
        return {"headers": repr({"X-Odoo-Objects": "%s-%s" % (self._name, self.id), "X-Odoo-db-uuid": database_uuid})}
    ...
    def message_get_recipient_values(self, notif_message=None, recipient_ids=None):
        ...
        return {"recipient_ids": [(4, pid) for pid in recipient_ids]}
    ...
    def _routing_warn(self, error_message, warn_suffix, message_id, route, raise_exception):
        ...
        full_message = "Routing mail with Message-Id %s: route %s: %s" % (message_id, route, error_message)
...
<p>The following email sent to %s cannot be accepted because this is a private email address.
   Only allowed people can contact us at this address.</p>
        ...
        if not model:
            ...
            if thread_id:
                self._routing_warn(
                    _("posting a message without model should be with a null res_id (private message), received %s")
                    % thread_id,
                    _("resetting thread_id"),
                    message_id,
                    route,
                    assert_model,
                )
            ...
            if not message_dict.get("parent_id"):
                self._routing_warn(
                    _("posting a message without model should be with a parent_id (private message)"),
                    _("skipping"),
                    message_id,
                    route,
                    assert_model,
        ...
        if thread_id:
            if not record_set.exists() and create_fallback:
                self._routing_warn(
                    _("reply to missing document (%s,%s), fall back on new document creation") % (model, thread_id),
                    "",
                    message_id,
                    route,
                    False,
                ...
                self._routing_warn(
                    _("model %s does not accept document update, fall back on document creation") % model,
                    "",
                    message_id,
                    route,
                    False,
            ...
            if not record_set.exists():
                self._routing_warn(
                    _("reply to missing document (%s,%s)") % (model, thread_id),
                    _("skipping"),
                    message_id,
                    route,
                    assert_model,
                ...
                self._routing_warn(
                    _("model %s does not accept document update") % model,
                    _("skipping"),
                    message_id,
                    route,
                    assert_model,
        ...
        if not thread_id and model and not hasattr(record_set, "message_new"):
            self._routing_warn(
                _("model %s does not accept document creation") % model, _("skipping"), message_id, route, assert_model
            )
            return False
        ...
        if alias:
            ...
            if check_result is not True:
                self._routing_warn(
                    _("alias %s: %s") % (alias.alias_name, check_result.get("error_message", _("unknown error"))),
                    _("skipping"),
                    message_id,
                    route,
                    False,
                ...
                self._routing_create_bounce_email(
                    email_from, check_result.get("error_template", _generic_bounce_body_html), message
                )
                return False
    ...
    def message_route(self, message, message_dict, model=None, thread_id=None, custom_values=None):
        ...
        if bounce_alias and any(email.startswith(bounce_alias) for email in email_to_localparts):
            ...
            if bounce_match:
                bounced_mail_id, bounced_model, bounced_thread_id = (
                    bounce_match.group(1),
                    bounce_match.group(2),
                    bounce_match.group(3),
                )

                email_part = next(
                    (part for part in message.walk() if part.get_content_type() == "message/rfc822"), None
                )
                dsn_part = next(
                ...
                if partners and mail_message:
                    notifications = (
                        ...
                        .sudo()
                        .search([("mail_message_id", "=", mail_message.id), ("res_partner_id", "in", partners.ids)])
                    )
                    notifications.write({"email_status": "bounce"})

                if (
                    bounced_model in self.env
                    and hasattr(self.env[bounced_model], "message_receive_bounce")
        ...
        msg_references = [
            ref for ref in tools.mail_header_msgid_re.findall(thread_references) if "reply_to" not in ref
        ]
        ...
        if rcpt_tos_localparts:
            ...
            if dest_aliases:
                ...
                for alias in dest_aliases:
                    ...
                    route = self.message_route_verify(
                        message, message_dict, route, update_author=True, assert_model=True, create_fallback=True
                    )
                    if route:
                        _logger.info(
    ...
    def message_route_process(self, message, message_dict, routes):
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            if model:
                ...
                MessageModel = Model.sudo(user_id if self.env.user._is_admin() else None).with_context(
    ...
    @api.model
    def message_process(
        self, model, message, custom_values=None, save_original=False, strip_attachments=False, thread_id=None
    ):
            ...
            for part in message.walk():
                ...
                if part.get_content_type() == "text/plain" and (not alternative or not body):
                    body = tools.append_content_to_html(
                        body, tools.ustr(part.get_payload(decode=True), encoding, errors="replace"), preserve=True
                    )
                # 3) text/html -> raw
    ...
    def _find_partner_from_emails(
        self, emails, res_model=None, res_id=None, check_followers=True, force_create=False, exclude_aliases=True
    ):
    ...
    def _message_preprocess_attachments(self, attachments, attachment_ids, attach_model, attach_res_id):
        ...
        """
        return self._message_post_process_attachments(
            attachments, attachment_ids, {"model": attach_model, "res_id": attach_res_id}
        )
    ...
    def message_post(
        ...
        **kwargs
        ...
        if parent_id and not model:
            ...
            private_followers = {partner.id for partner in parent_message.partner_ids}
            if parent_message.author_id:
        ...
        if self._context.get("mail_post_autofollow") and self.ids and partner_ids:
            ...
            if self._context.get("mail_post_autofollow_partner_ids"):
                partner_to_subscribe = [
                    p for p in partner_ids if p in self._context.get("mail_post_autofollow_partner_ids")
                ]
        ...
        if (
            self._context.get("mail_save_message_last_post")
            and model
    ...
    def message_post_with_template(self, template_id, **kwargs):
        ...
        if template_id:
            update_values = composer.onchange_template_id(template_id, kwargs["composition_mode"], self._name, res_id)[
                "value"
            ]
    ...
    def message_subscribe_users(self, user_ids=None, subtype_ids=None):
        ...
        return self.message_subscribe(
            self.env["res.users"].browse(user_ids).mapped("partner_id").ids, subtype_ids=subtype_ids
        )
    ...
    def message_subscribe(self, partner_ids=None, channel_ids=None, subtype_ids=None, force=True):
        ...
        # TDE CHECK THIS
        if not channel_ids and partner_ids and set(partner_ids) == {self.env.user.partner_id.id}:
            try:
        ...
        partner_data = {pid: subtype_ids for pid in partner_ids}
        channel_data = {cid: subtype_ids for cid in channel_ids}
    ...
    def message_unsubscribe(self, partner_ids=None, channel_ids=None):
        ...
        if not channel_ids and set(partner_ids) == {user_pid}:
            self.check_access_rights("read")
            self.check_access_rule("read")
        else:
            self.check_access_rights("write")
    ...
    def _message_get_auto_subscribe_fields(self, updated_fields, auto_follow_fields=None):
        ...
        for name, field in self._fields.items():
            if (
                name in auto_follow_fields
                and name in updated_fields
                and getattr(field, "track_visibility", False)
    ...
    def message_auto_subscribe(self, updated_fields, values=None):
        ...
        to_add_users = (
            self.env["res.users"]
            .sudo()
            .browse([values[name] for name in user_field_lst if values.get(name)])
        ...
        user_pids = [
            user.partner_id.id for user in to_add_users if user != self.env.user and user.notification_type == "email"
        ]
    ...
    def message_change_thread(self, new_thread):
        ...
        msg_comment = MailMessage.search(
            [("model", "=", self._name), ("res_id", "=", self.id), ("subtype_id", "=", subtype_comment.id)]
        )

...
class Message(models.Model):
    ...
    parent_id = fields.Many2one(
        "mail.message", "Parent Message", index=True, ondelete="set null", help="Initial thread message."
    )
    ...
    message_type = fields.Selection(
        [("email", "Email"), ("comment", "Comment"), ("notification", "System notification")],
        "Type",
        required=True,
        default="email",
    ...
    needaction = fields.Boolean(
        "Need Action", compute="_get_needaction", search="_search_needaction", help="Need Action"
    )
    channel_ids = fields.Many2many("mail.channel", "mail_message_mail_channel_rel", string="Channels")
    # notifications
    ...
    reply_to = fields.Char(
        "Reply-To", help="Reply email address. Setting the reply_to bypasses the automatic thread creation."
    )
    ...
    def set_message_done(self):
        ...
        notifications = (
            ...
            .sudo()
            .search(
                [("mail_message_id", "in", self.ids), ("res_partner_id", "=", partner_id.id), ("is_read", "=", False)]
            )
        )
    ...
    def _message_read_dict_postprocess(self, messages, message_tree):
        ...
        for message in message_tree.values():
            ...
            if message.subtype_id and message.partner_ids:  # take notified people of message with a subtype
                ...
                not message.subtype_id and message.partner_ids
        ...
        partner_tree = {partner[0]: partner for partner in partners_names}
        ...
        for message_dict in messages:
            ...
            if message.subtype_id:
                partner_ids = [
                    partner_tree[partner.id] for partner in message.partner_ids if partner.id in partner_tree
                ]
            else:
                partner_ids = [
                    partner_tree[partner.id] for partner in message.partner_ids if partner.id in partner_tree
            ...
            message_dict.update(
                {
                    ...
                    "customer_email_status": (all(d[2] == "sent" for d in customer_email_data) and "sent")
                    or (any(d[2] == "exception" for d in customer_email_data) and "exception")
                    or (any(d[2] == "bounce" for d in customer_email_data) and "bounce")
                    or "ready",
    ...
    def message_format(self):
        ...
        message_tree = {m.id: m for m in self.sudo()}
        ...
        subtypes_dict = {subtype["id"]: subtype for subtype in subtypes}
        ...
        notifs = (
            self.env["mail.notification"]
            .sudo()
            .search([("mail_message_id", "in", list(mid for mid in message_tree)), ("is_read", "=", False)])
        )
        for notif in notifs:
        ...
        for message in message_values:
            ...
            message["subtype_description"] = (
                message["subtype_id"] and subtypes_dict[message["subtype_id"][0]]["description"]
            )
    ...
    def check_access_rule(self, operation):
        ...
        message_values = {res_id: {} for res_id in self.ids}
        ...
        if operation == "create":
            ...
            parent_ids = [message.get("parent_id") for message in message_values.values() if message.get("parent_id")]
            self._cr.execute(
            ...
            notified_ids += [
                mid for mid, message in message_values.items() if message.get("parent_id") in not_parent_ids
            ]
        ...
        if "body" in values:
            ...
            def base64_to_boundary(match):
                ...
                if not data_to_url.get(key):
                    ...
                    data_to_url[key] = [
                        "/web/image/%s?access_token=%s" % (attachment.id, attachment.access_token),
                        name,
                    ]
        ...
        if self_sudo.subtype_id and self.model and self.res_id:
            ...
            if self_sudo.subtype_id.internal:
                followers = followers.filtered(
                    lambda fol: fol.channel_id
                    or (fol.partner_id.user_ids and group_user in fol.partner_id.user_ids[0].mapped("groups_id"))
                )

...
from odoo.tools import formataddr, ormcache
...
class Channel(models.Model):
    ...
    channel_type = fields.Selection(
        [("chat", "Chat Discussion"), ("channel", "Channel")], "Channel Type", default="channel"
    )
    ...
    channel_partner_ids = fields.Many2many(
        "res.partner", "mail_channel_partner", "channel_id", "partner_id", string="Listeners"
    )
    ...
    public = fields.Selection(
        [("public", "Everyone"), ("private", "Invited people only"), ("groups", "Selected group of users")],
        "Privacy",
        required=True,
        default="groups",
    ...
    def _subscribe_users(self):
        for mail_channel in self:
            mail_channel.write(
                {
                    "channel_partner_ids": [
                        (4, pid) for pid in mail_channel.mapped("group_ids").mapped("users").mapped("partner_id").ids
                    ]
                }
            )
    ...
    def _action_unfollow(self, partner):
        ...
        if not self.email_send:
            ...
            self.sudo().message_post(
                body=notification, message_type="notification", subtype="mail.mt_comment", author_id=partner.id
            )
        return result
    ...
    def message_receive_bounce(self, email, partner, mail_id=None):
        ...
        return super().message_receive_bounce(email, partner, mail_id=mail_id)
        ...
        if self.alias_domain and self.alias_name:
            return {
                "email_to": ",".join(
                    ...
                    for partner in self.env["res.partner"].sudo().browse(recipient_ids)
                ),
                "recipient_ids": [],
            }
        return super().message_get_recipient_values(notif_message=notif_message, recipient_ids=recipient_ids)
    ...
    def _alias_check_contact(self, message, message_dict, alias):
        if alias.alias_contact == "followers" and self.ids:
            ...
            return True
        return super()._alias_check_contact(message, message_dict, alias)

    @api.model_cr
    def init(self):
        self._cr.execute(
            "SELECT indexname FROM pg_indexes WHERE indexname = %s", ("mail_channel_partner_seen_message_id_idx",)
        )
        ...
        for channel in self:
            ...
            if channel.channel_type == "chat":
                info["direct_partner"] = (
                    ...
                    .channel_partner_ids.filtered(lambda p: p.id != self.env.user.partner_id.id)
    ...
    def channel_minimize(self, uuid, minimized=True):
        values = {"fold_state": minimized and "open" or "closed", "is_minimized": minimized}
        domain = [("partner_id", "=", self.env.user.partner_id.id), ("channel_id.uuid", "=", uuid)]
    ...
    def channel_fetch_slot(self):
        ...
        pinned_channels = (
            self.env["mail.channel.partner"]
            .search([("partner_id", "=", my_partner_id), ("is_pinned", "=", True)])
            .mapped("channel_id")
        )
    ...
    def get_mention_suggestions(self, search, limit=8):
        ...
        domain = expression.AND(
            [
                ...
                [("channel_type", "=", "channel")],
                expression.OR(
                    [[("public", "!=", "private")], [("channel_partner_ids", "in", [self.env.user.partner_id.id])]]
                ),
            ]
        )
    ...
    def channel_fetch_preview(self):
        ...
        channels_preview = {r["message_id"]: r for r in self._cr.dictfetchall()}
    ...
    def _define_command_who(self):
        return {"channel_types": ["channel", "chat"], "help": _("List users in the current channel")}

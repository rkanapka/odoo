...
class MailMail(models.Model):
    ...
    references = fields.Text(
        "References", help="Message references, such as identifiers of previous messages", readonly=1
    )
    headers = fields.Text("Headers", copy=False)
    # Auto-detected based on create() - if 'mail_message_id' was passed then this mail is a notification
    # and during unlink() we will not cascade delete the parent and its attachments
    notification = fields.Boolean(
        "Is Notification", help="Mail has been created to notify people of an existing mail.message"
    )
    # recipients
    ...
    def send(self, auto_commit=False, raise_exception=False):
        ...
        for server_id, batch_ids in self._split_by_server():
            ...
            try:
                ...
                _logger.info("Sent batch %s emails via mail server ID #%s", len(batch_ids), server_id)
            finally:
                if smtp_session:
                    smtp_session.quit()
    ...
    def _send(self, auto_commit=False, raise_exception=False, smtp_session=None):
        ...
        for mail_id in self.ids:
            try:
                ...
                _logger.exception(
                    "Exception while processing mail with ID %r and Msg-Id %r.", mail.id, mail.message_id
                )
                raise

...
class Partner(models.Model):
    ...
    channel_ids = fields.Many2many(
        "mail.channel", "mail_channel_partner", "partner_id", "channel_id", string="Channels", copy=False
    )
    ...
    def message_get_default_recipients(self):
        return {res_id: {"partner_ids": [res_id], "email_to": False, "email_cc": False} for res_id in self.ids}
    ...
    def _notify_prepare_template_context(self, message):
        ...
        if user.company_id.website:
            website_url = (
                "http://%s" % user.company_id.website
                if not user.company_id.website.lower().startswith(("http:", "https:"))
    ...
    def _notify_send(self, body, subject, recipients, **mail_values):
        ...
        for email_chunk in split_every(50, recipients.ids):
            ...
            if (
                ...
                and message.res_id
                and message.model in self.env
                and hasattr(self.env[message.model], "message_get_recipient_values")
            ):
    ...
    def _notify(self, message, force_send=False, send_after_commit=True, user_signature=True):
        ...
        if force_send and recipients_nbr < recipients_max and (not self.pool._init or test_mode):

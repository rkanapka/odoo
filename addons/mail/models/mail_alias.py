...
class Alias(models.Model):
    ...
    alias_user_id = fields.Many2one(
        "res.users",
        "Owner",
        defaults=lambda self: self.env.user,
    ...
    alias_contact = fields.Selection(
        [("everyone", "Everyone"), ("partners", "Authenticated Partners"), ("followers", "Followers only")],
        default="everyone",
...
class AliasMixin(models.AbstractModel):
    ...
    def _init_column(self, name):
        ...
        for record in child_model.search([("alias_id", "=", False)]):
            ...
            _logger.info("Mail alias created for %s %s (id %s)", record._name, record.display_name, record.id)
    ...
    def _alias_check_contact_on_record(self, record, message, message_dict, alias):
        ...
        if alias.alias_contact == "followers":
            ...
            accepted_partner_ids = record.message_partner_ids | record.message_channel_ids.mapped(
                "channel_partner_ids"
            )
            ...
            return {"error_message": _("restricted to known authors")}
        return True

...
class Followers(models.Model):
    ...
    def _add_follower_command(self, res_model, res_ids, partner_data, channel_data, force=True):
        ...
        default_subtypes, _internal_subtypes, external_subtypes = self.env["mail.message.subtype"].default_subtypes(
            res_model
        )
        ...
        if force_mode:
            employee_pids = (
                self.env["res.users"]
                .sudo()
                .search([("partner_id", "in", list(partner_data)), ("share", "=", False)])
                .mapped("partner_id")
                .ids
            )
            ...
            generic.append(
                [
                    ...
                    0,
                    {
                        "res_model": res_model,
                        "partner_id": pid,
                        "subtype_ids": [(6, 0, partner_data.get(pid) or default_subtypes.ids)],
                    },
                ]
            )
            ...
            generic.append(
                [
                    ...
                    0,
                    {
                        "res_model": res_model,
                        "channel_id": cid,
                        "subtype_ids": [(6, 0, channel_data.get(cid) or default_subtypes.ids)],
                    },
                ]
            )
        ...
        if not force_mode:
            for res_id in res_ids:
                ...
                new_pids = (
                    set(partner_data)
                    - {sub.partner_id.id for sub in doc_followers if sub.partner_id}
                    - set(gen_new_pids)
                )
                new_cids = (
                    set(channel_data)
                    - {sub.channel_id.id for sub in doc_followers if sub.channel_id}
                    - set(gen_new_cids)
                )
    ...
    def create(self, vals):
        res = super().create(vals)._check_rights()

...
def configure_parser():
    ...
    parser.add_option(
        ...
        default=config.OPENERP_DEFAULT_USER_ID,
        type="int",
    )
    parser.add_option(
        "-p", "--password", dest="password", help="Odoo user password", default=config.OPENERP_DEFAULT_PASSWORD
    )
...
def main():
    ...
    try:
        ...
        sys.stderr.write(
            "Failed to deliver email to Odoo Server, sending error notification to %s\n" % config.MAIL_ADMINS
        )

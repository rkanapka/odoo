...
class MailComposer(models.TransientModel):
    ...
    composition_mode = fields.Selection(
        selection=_get_composition_mode_selection, string="Composition mode", default="comment"
    )
    partner_ids = fields.Many2many(
        "res.partner", "mail_compose_message_res_partner_rel", "wizard_id", "partner_id", "Additional Contacts"
    )
    use_active_domain = fields.Boolean("Use active domain")
    active_domain = fields.Text("Active domain", readonly=True)
    attachment_ids = fields.Many2many(
        "ir.attachment", "mail_compose_message_ir_attachments_rel", "wizard_id", "attachment_id", "Attachments"
    )
    ...
    subtype_id = fields.Many2one(
        default=lambda self: self.sudo().env.ref("mail.mt_comment", raise_if_not_found=False).id
    )
    ...
    def check_access_rule(self, operation):
        ...
        if operation == "create" and self._uid != SUPERUSER_ID:
            ...
            message_values = {}
            self._cr.execute(
                'SELECT DISTINCT id, model, res_id FROM "%s" WHERE id = ANY (%%s) AND res_id = 0' % self._table,
                (self.ids,),
            )
            ...
            author_ids = [
                mid for mid, message in message_values.items() if message.get("model") and not message.get("res_id")
            ]
                ...
                for res_id, mail_values in all_mail_values.items():
                    if wizard.composition_mode == "mass_mail":
                        ...
                        ActiveModel.browse(res_id).message_post(
                            message_type=wizard.message_type, subtype_id=subtype_id, **mail_values
                        )
    ...
    def get_mail_values(self, res_ids):
        ...
        if mass_mail_mode and not self.no_auto_thread:
            ...
            reply_to_value = (
                ...
                .with_context(thread_model=self.model)
                .message_get_reply_to(res_ids, default=self.email_from)
            )
        ...
        for res_id in res_ids:
            ...
            if mass_mail_mode and self.model:
                ...
                mail_values.update(
                    notification=not self.auto_delete_message, model=self.model, res_id=res_id, record_name=False
                )
                ...
                mail_values["attachments"] = [
                    (name, base64.b64decode(enc_cont)) for name, enc_cont in email_dict.pop("attachments", list())
                ]
                attachment_ids = []
                for attach_id in mail_values.pop("attachment_ids"):
                    new_attach_id = (
                        self.env["ir.attachment"].browse(attach_id).copy({"res_model": self._name, "res_id": self.id})
                    )
                    attachment_ids.append(new_attach_id.id)
                mail_values["attachment_ids"] = self.env["mail.thread"]._message_preprocess_attachments(
                    mail_values.pop("attachments", []), attachment_ids, "mail.message", 0
                )
    ...
    def onchange_template_id_wrapper(self):
        self.ensure_one()
        values = self.onchange_template_id(self.template_id.id, self.composition_mode, self.model, self.res_id)[
            "value"
        ]
    ...
    def onchange_template_id(self, template_id, composition_mode, model, res_id):
        ...
        if template_id and composition_mode == "mass_mail":
            ...
            default_values = self.with_context(
                default_composition_mode=composition_mode, default_model=model, default_res_id=res_id
            ).default_get(
                [
                    "composition_mode",
                    "model",

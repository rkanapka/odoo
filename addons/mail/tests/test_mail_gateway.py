...
class TestMailgateway(TestMail):
    ...
    def test_message_process_alias_basic(self):
        ...
        self.assertEqual(
            msg.message_type, "email", "message_process: newly created group should have an email as first message"
        )
        self.assertEqual(
            msg.subtype_id,
    ...
    def test_message_process_alias_user_id(self):
        ...
        res = new_groups.get_metadata()[0].get("create_uid") or [None]
        self.assertEqual(
            res[0], self.user_employee.id, "message_process: group should have been created by alias_user_id"
        )
    ...
    def test_message_process_email_email_from(self):
        ...
        self.assertIn(
            "test.sylvie.lelitre@agrolait.com",
            new_groups.message_ids[0].email_from,
            "message_process: unrecognized email -> email_from",
        )

        self.assertEqual(
            len(new_groups.message_partner_ids), 0, "message_process: newly create group should not have any follower"
        )
    ...
    def test_message_process_email_author(self):
        ...
        new_groups = self.format_and_process(
            ...
            to="groups@example.com, valid.other@gmail.com",
        )

        self.assertEqual(
            new_groups.message_ids[0].author_id, self.partner_1, "message_process: recognized email -> author_id"
        )
        self.assertIn(
    ...
    def test_message_process_bounce(self):
        ...
        new_groups = self.format_and_process(
            ...
            % (
                self.bounce_alias,
                self.fake_email.id,
                self.fake_email.model,
                self.fake_email.res_id,
    ...
    def test_message_process_alias_partners_bounce(self):
        ...
        self.assertEqual(
            len(self._mails), 1, "message_process: incoming email on Partners alias should send a bounce email"
        )
        self.assertIn(
            "New Frogs",
            self._mails[0].get("subject"),
            "message_process: bounce email on Partners alias should contain the original subject",
        )
        self.assertIn(
            "whatever-2a840@postmaster.twitter.com",
            self._mails[0].get("email_to"),
    ...
    def test_message_process_alias_followers_bounce(self):
        ...
        self.assertEqual(
            len(self._mails), 1, "message_process: incoming email on Followers alias should send a bounce email"
        )
    ...
    def test_message_process_alias_followers(self):
        ...
        new_groups = self.format_and_process(
            MAIL_TEMPLATE,
            email_from="Valid Lelitre <valid.lelitre@agrolait.com>",
            to="groups@example.com, other6@gmail.com",
        )
    ...
    def test_message_process_alias_update(self):
        ...
        new_groups = self.format_and_process(
            ...
            to="groups@example.com>",
            subject="Re: cats",
        )

        # Test: no new group + new message
        self.assertEqual(
            len(new_groups), 0, "message_process: reply on Frogs should not have created a new group with new subject"
        )
    ...
    def test_message_process_alias_forward_domain_confusion(self):
        ...
        new_rec = self.format_and_process(
            ...
            email_from="valid.other@gmail.com",
            to="%s@%s, %s@%s"
            % (self.alias.alias_name, self.catchall_domain, self.alias_2.alias_name, self.catchall_domain),
            msg_id="<whatever.JavaMail.diff1@agrolait.com>",
    ...
    def test_message_process_alias_forward_domain_confusion_no_domain(self):
        ...
        new_rec = self.format_and_process(
            ...
            email_from="valid.other@gmail.com",
            to="%s@%s, %s@%s"
            % (self.alias.alias_name, self.catchall_domain, self.alias_2.alias_name, "another.domain.com"),
            msg_id="<whatever.JavaMail.diff1@agrolait.com>",
            target_model=self.alias_2.alias_model_id.model,
        )
        # Test: one channel (alias 2) created
    ...
    def test_message_process_in_reply_to(self):
        ...
        self.assertEqual(len(self.test_public.message_ids), 2, "message_process: group should contain one new message")
        self.assertEqual(
            len(self.fake_email.child_ids), 1, "message_process: new message should be children of the existing one"
        )
    ...
    def test_message_process_references_forward(self):
        ...
        self.assertEqual(
            len(self.fake_email.child_ids), 0, "message_process: original email should not contain childs"
        )
    ...
    def test_message_process_duplicate(self):
        ...
        self.alias.write(
            {
                "alias_force_thread_id": self.test_public.id,
            }
        )
        ...
        self.assertEqual(
            len(frog_groups), 0, "message_process: reply on Frogs should not have created a new group with new subject"
        )

        # Test: no new message
        self.assertEqual(
            len(self.test_public.message_ids),
            2,
        ...
        self.assertEqual(
            no_of_msg, 1, "message_process: message with already existing message_id should not have been duplicated"
        )
    ...
    def test_message_process_partner_find(self):
        ...
        self.format_and_process(
            MAIL_TEMPLATE, to="public@example.com", msg_id="<1>", email_from="Brice Denisse <from.test@example.com>"
        )
        self.assertEqual(
            self.test_public.message_ids[0].author_id, from_1, "message_process: email_from -> author_id wrong"
        )
        ...
        from_2 = (
            ...
            .create({"name": "B", "login": "B", "email": "from.test@example.com"})
        )

        self.format_and_process(
            MAIL_TEMPLATE, to="public@example.com", msg_id="<2>", email_from="Brice Denisse <from.test@example.com>"
        )
        self.assertEqual(
            self.test_public.message_ids[0].author_id,
            from_2.partner_id,
        ...
        self.format_and_process(
            MAIL_TEMPLATE, to="public@example.com", msg_id="<3>", email_from="Brice Denisse <from.test@example.com>"
        )
        self.assertEqual(
            self.test_public.message_ids[0].author_id, from_3, "message_process: email_from -> author_id wrong"
        )
    ...
    def test_private_discussion(self):
        ...
        msg1 = (
            ...
            .with_context({"thread_model": "mail.test"})
            .sudo(self.user_employee)
            .message_post(partner_ids=msg1_pids, subtype="mail.mt_comment")
        )
        ...
        self.assertEqual(
            msg.partner_ids,
            self.env.user.partner_id | self.partner_1,
        ...
        self.assertEqual(
            msg2.partner_ids,
            self.user_employee.partner_id | self.env.user.partner_id,
    ...
    def test_forward_parent_id(self):
        ...
        fw_message = MAIL_TEMPLATE.format(
            to="groups@example.com",
            cc="",
            subject="FW: Re: 1",

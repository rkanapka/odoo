...
class TestMailTemplate(TestMail):
    ...
    def test_composer_template_onchange(self):
        ...
        values = composer.onchange_template_id(self.email_template.id, "comment", "mail.test", self.test_pigs.id)[
            "value"
        ]
        ...
        test_recipients = (
            self.env["res.partner"].search([("email", "in", ["test1@example.com", "test2@example.com"])])
            | self.partner_1
            | self.partner_2
        ...
        self.assertEqual(set(attachments.mapped("res_model")), {"res.partner"})
        self.assertEqual(set(attachments.mapped("res_id")), {self.user_admin.partner_id.id})
    ...
    def test_composer_template_send(self):
        self.test_pigs.with_context(use_template=False).message_post_with_template(
        ...
        self.assertEqual(set(message.attachment_ids.mapped("res_model")), {"mail.test"})
        self.assertEqual(set(message.attachment_ids.mapped("res_id")), {self.test_pigs.id})
    ...
    def test_composer_template_mass_mailing(self):
        ...
        values = composer.onchange_template_id(self.email_template.id, "mass_mail", "mail.test", self.test_pigs.id)[
            "value"
        ]
    ...
    def test_message_compose_template_save(self):
        ...
        ).create({"subject": "Forget me subject", "body": "<p>Dummy body</p>"}).with_context(
            {"default_model": "mail.test"}
        ).save_as_template()
        # Test: email_template subject, body_html, model
        last_template = self.env["mail.template"].search(
            [("model", "=", "mail.test"), ("subject", "=", "Forget me subject")], limit=1
        )

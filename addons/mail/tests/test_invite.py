...
class TestInvite(TestMail):
    ...
    def test_invite_email(self):
        mail_invite = (
            ...
            .with_context({"default_res_model": "mail.test", "default_res_id": self.test_pigs.id})
            .sudo(self.user_employee.id)
            .create({"partner_ids": [(4, self.user_portal.partner_id.id), (4, self.partner_1.id)], "send_mail": True})
        )
        ...
        self.assertEqual(
            self.test_pigs.message_follower_ids.mapped("channel_id"),
        ...
        self.assertEqual(
            self._mails[1].get("subject"),
            "Invitation to follow %s: Pigs" % self.env["mail.test"]._description,
            "invite wizard: subject of invitation email is incorrect",
        )
        self.assertIn(
            "%s invited you to follow %s document: Pigs"
            % (self.user_employee.name, self.env["mail.test"]._description),

...
class TestMailMessage(TestMail):
    ...
    def test_mail_message_values_document_no_alias(self):
        ...
        msg = (
            self.env["mail.message"]
            .sudo(self.user_employee)
            .create({"model": "mail.test", "res_id": self.test_pigs.id})
        )
        self.assertIn(
            "-openerp-%d-mail.test" % self.test_pigs.id,
            msg.message_id.split("@")[0],
            'mail_message: message_id for a void message should be a "private" one',
        )
        self.assertEqual(msg.reply_to, '"%s" <%s>' % (self.user_employee.name, self.user_employee.email))
    ...
    def test_mail_message_values_document_alias(self):
        ...
        self.assertEqual(
            msg.reply_to,
            '"%s %s" <%s@%s>'
            % (self.env.user.company_id.name, self.test_pigs.name, self.test_pigs.alias_name, alias_domain),
        )
        self.assertEqual(msg.email_from, '"%s" <%s>' % (self.user_employee.name, self.user_employee.email))
    ...
    def test_mail_message_base64_image(self):
        ...
        body = body % (
            msg.attachment_ids[0].id,
            msg.attachment_ids[0].access_token,
            msg.attachment_ids[0].id,
            msg.attachment_ids[0].access_token,
        )
...
class TestMailMessageAccess(TestMail):
    ...
    def setUpClass(cls):
        super().setUpClass()
        Channel = cls.env["mail.channel"].with_context({"mail_create_nolog": True, "mail_create_nosubscribe": True})
        # Pigs: base group for tests
        cls.group_pigs = Channel.create(
            {"name": "Pigs", "public": "groups", "group_public_id": cls.env.ref("base.group_user").id}
        )
        # Jobs: public group
        cls.group_public = Channel.create({"name": "Jobs", "description": "NotFalse", "public": "public"})
        # Private: private gtroup
        cls.group_private = Channel.create({"name": "Private", "public": "private"})
    ...
    def test_mail_message_access_search(self):
        ...
        msg7 = (
            self.env["mail.message"]
            .sudo(self.user_employee)
            .create({"subject": "_Test", "body": "B", "subtype_id": self.ref("mail.mt_comment")})
        )
        msg8 = (
            self.env["mail.message"]
            .sudo(self.user_employee)
        ...
        messages = (
            self.env["mail.message"]
            .sudo(self.user_employee)
            .search([("subject", "like", "_Test"), ("body", "ilike", "A")])
        )
    ...
    def test_mail_message_access_read_notification(self):
        attachment = self.env["ir.attachment"].create(
            {"datas": base64.b64encode(b"My attachment"), "name": "doc.txt", "datas_fname": "doc.txt"}
        )
    ...
    def test_mark_all_as_read(self):
        ...
        group_private.message_post(
            body="Test", message_type="comment", subtype="mail.mt_comment", partner_ids=[emp_partner.id]
        )
    ...
    def test_mark_all_as_read_share(self):
        ...
        # mark all as read clear needactions
        self.group_pigs.message_post(
            body="Test", message_type="comment", subtype="mail.mt_comment", partner_ids=[portal_partner.id]
        )

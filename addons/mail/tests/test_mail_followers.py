...
class TestMailFollowers(TestMail):
    ...
    def setUp(self):
        ...
        self.default_group_subtypes = Subtype.search(
            [("default", "=", True), "|", ("res_model", "=", "mail.test"), ("res_model", "=", False)]
        )
    ...
    def test_m2o_command_new(self):
        ...
        self.assertEqual({item["res_model"] for item in items}, {"mail.test"})
        self.assertEqual({item["channel_id"] for item in items if item.get("channel_id")}, {test_channel.id})
        self.assertEqual(
            {item["partner_id"] for item in items if item.get("partner_id")}, {self.user_employee.partner_id.id}
        )
        self.assertEqual(
            set(items[0]["subtype_ids"][0][2] + items[1]["subtype_ids"][0][2]),
    ...
    def test_m2o_command_update_selective(self):
        ...
        self.env["mail.followers"].create(
            {"partner_id": self.user_employee.partner_id.id, "res_model": "mail.test", "res_id": self.test_pigs.id}
        )
        generic, specific = self.env["mail.followers"]._add_follower_command(
            "mail.test",
            groups.ids,
        ...
        self.assertEqual(set(specific[self.test_public.id][0][2]["subtype_ids"][0][2]), {self.mt_mg_nodef.id})
    ...
    def test_followers_subtypes_default_internal(self):
        mt_mg_def_int = self.env["mail.message.subtype"].create(
            {"name": "mt_mg_def", "default": True, "res_model": "mail.test", "internal": True}
        )
        self.test_pigs.message_subscribe_users(user_ids=[self.user_employee.id])
        follower = self.env["mail.followers"].search(
            [
        ...
        self.assertEqual(
            follower.subtype_ids, self.default_group_subtypes.filtered(lambda subtype: not subtype.internal)
        )
    ...
    def test_followers_multiple_subscription(self):
        ...
        self.test_pigs.sudo(self.user_employee).message_subscribe_users(
    ...
    def test_no_DID(self):
        ...
        """
        test_record = self.env["mail.channel"].create({"name": "I used to be schizo, but now we are alright."})

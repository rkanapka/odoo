...
class TestTracking(TestMail):
    ...
    def test_message_track(self):
        ...
        test_channel = self.env["mail.channel"].create(
            {"name": "Test", "channel_partner_ids": [(4, self.user_employee.partner_id.id)]}
        )
        ...
        mt_group_public_unset = Subtype.create(
            {"name": "group_public_unset", "description": "Group_public field unset"}
        )
        Data.create(
            {
        ...
        self.assertEqual(set(last_msg.tracking_value_ids.mapped("field")), {"name", "public"})
        self.assertEqual(set(last_msg.tracking_value_ids.mapped("field_desc")), {"Name", "Privacy"})
        self.assertEqual(
            set(last_msg.tracking_value_ids.mapped("old_value_char")), {"my_name", "Selected group of users"}
        )
        self.assertEqual(
            set(last_msg.tracking_value_ids.mapped("new_value_char")), {"supername", "Invited people only"}
        )
        ...
        self.assertEqual(set(last_msg.tracking_value_ids.mapped("field")), {"group_public_id", "public", "name"})
        self.assertEqual(
            set(last_msg.tracking_value_ids.mapped("field_desc")), {"Authorized Group", "Privacy", "Name"}
        )
        self.assertEqual(
        ...
        self.assertEqual(
            set(last_msg.tracking_value_ids.mapped("new_value_char")),
            {group_system.name_get()[0][1], "Everyone", "supername"},
        )
        self.assertEqual(set(last_msg.tracking_value_ids.mapped("old_value_integer")), {0, group_user.id})
        self.assertEqual(set(last_msg.tracking_value_ids.mapped("new_value_integer")), {0, group_system.id})

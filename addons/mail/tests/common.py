...
class TestMail(BaseFunctionalTest):
    ...
    def setUpClass(cls):
        ...
        cls.partner_1 = cls.env["res.partner"].create({"name": "Valid Lelitre", "email": "valid.lelitre@agrolait.com"})
        cls.partner_2 = cls.env["res.partner"].create({"name": "Valid Poilvache", "email": "valid.other@gmail.com"})
        ...
        cls.test_public = TestModel.create(
            {"name": "Public", "description": "NotFalse", "alias_name": "public", "alias_contact": "everyone"}
        )

        cls.env["mail.followers"].search(
            [("res_model", "=", "mail.test"), ("res_id", "in", (cls.test_public | cls.test_pigs).ids)]
        ).unlink()

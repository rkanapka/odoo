...
class TestPortal(TestMail):
    ...
    def test_mail_compose_access_rights(self):
        test_channel = self.env["mail.channel"].create(
            {"name": "Pigs", "public": "groups", "group_public_id": self.env.ref("base.group_portal").id}
        )
        port_msg = test_channel.message_post(body="Message")

        # Do: Chell comments Pigs, ok because can write on it (public group)
        test_channel.sudo(self.user_portal).message_post(
            body="I love Pigs", message_type="comment", subtype="mail.mt_comment"
        )
        ...
        compose = (
            ...
            .sudo(self.user_portal)
            .create({"subject": "Subject", "body": "Body text", "partner_ids": []})
        )
        compose.send_mail()
        ...
        compose = (
            self.env["mail.compose.message"]
            .with_context({"default_composition_mode": "comment", "default_parent_id": port_msg.id})
            .sudo(self.user_portal)
            .create({"subject": "Subject", "body": "Body text"})
        )
        compose.send_mail()
    ...
    def test_invite_email_portal(self):
        ...
        mail_invite = (
            ...
            .with_context({"default_res_model": "mail.test", "default_res_id": test_record.id})
            .create({"partner_ids": [(4, partner_carine.id)], "send_mail": True})
        )
        ...
        for sent_email in self._mails:
            self.assertEqual(
                ...
                "Invitation to follow %s: Pigs" % self.env["mail.test"]._description,
                "invite: subject of invitation email is incorrect",
            )
            self.assertIn(
                "Administrator invited you to follow %s document: Pigs" % self.env["mail.test"]._description,
                sent_email.get("body"),

...
class TestMailFeatures(TestMail):
    ...
    def test_alias_setup(self):
        alias = (
            self.env["mail.alias"].with_context(alias_model_name="mail.test").create({"alias_name": "b4r+_#_R3wl$$"})
        )
...
class TestMessagePost(TestMail):
    ...
    def test_post_subscribe_recipients(self):
        original = self.test_pigs.message_follower_ids
        self.test_pigs.sudo(self.user_employee).with_context(
        ...
        self.assertEqual(
            self.test_pigs.message_follower_ids.mapped("partner_id"),
    ...
    def test_post_subscribe_recipients_partial(self):
        ...
        self.test_pigs.sudo(self.user_employee).with_context(
            {
                ...
                "mail_post_autofollow_partner_ids": [self.partner_2.id],
            }
        ).message_post(
            body="Test Body",
            message_type="comment",
        ...
        self.assertEqual(
            self.test_pigs.message_follower_ids.mapped("partner_id"), original.mapped("partner_id") | self.partner_2
        )
        self.assertEqual(self.test_pigs.message_follower_ids.mapped("channel_id"), original.mapped("channel_id"))
    ...
    def test_post_notifications(self):
        ...
        _attachments = [("List1", b"My first attachment"), ("List2", b"My second attachment")]
        _attach_1 = (
        ...
        # attachments
        self.assertEqual(
            set(msg.attachment_ids.mapped("res_model")),
            {"mail.test"},
            "message_post: all atttachments should be linked to the mail.test model",
        )
        self.assertEqual(
            set(msg.attachment_ids.mapped("res_id")),
            {self.test_pigs.id},
        ...
        self.assertTrue(
            {_attach_1.id, _attach_2.id}.issubset(msg.attachment_ids.ids),
            "message_post: mail.message attachments duplicated",
        )
        # notifications
        self.assertFalse(
            self.env["mail.mail"].search([("mail_message_id", "=", msg.message_id)]),
            "message_post: mail.mail notifications should have been auto-deleted",
        )

        # notification emails: followers + recipients - author (user_employee)
        self.assertEqual(
            {m["email_from"] for m in self._mails},
        ...
        self.assertEqual(
            {m["email_to"][0] for m in self._mails},
            {
                '"%s" <%s>' % (self.partner_1.name, self.partner_1.email),
                '"%s" <%s>' % (self.partner_2.name, self.partner_2.email),
                '"%s" <%s>' % (self.env.user.name, self.env.user.email),
            },
        )
        self.assertFalse(
            any(len(m["email_to"]) != 1 for m in self._mails),
            "message_post: notification email should be sent to one partner at a time",
        )
        self.assertEqual(
            {m["reply_to"] for m in self._mails},
            {
                '"%s %s" <%s@%s>'
                % (self.env.user.company_id.name, self.test_pigs.name, self.test_pigs.alias_name, _domain)
            },
    ...
    def test_post_answer(self):
        ...
        parent_msg = self.test_pigs.sudo(self.user_employee).message_post(
            body=_body, subject=_subject, message_type="comment", subtype="mt_comment"
        )
        ...
        new_msg = self.test_pigs.sudo(self.user_employee).message_post(
            body=_body, subject=_subject, message_type="comment", subtype="mt_comment", parent_id=msg.id
        )
    ...
    def test_post_portal(self):
        ...
        new_msg = self.test_pigs.sudo(self.user_portal).message_post(
            body="<p>Test</p>", subject="Subject", message_type="comment", subtype="mt_comment"
        )
    ...
    def test_post_internal(self):
        ...
        msg = self.test_pigs.sudo(self.user_employee).message_post(
            body="My Body", subject="My Subject", message_type="comment", subtype="mt_note"
        )
    ...
    def test_message_compose(self):
        composer = (
            ...
            .sudo(self.user_employee)
            .create({"body": "<p>Test Body</p>", "partner_ids": [(4, self.partner_1.id), (4, self.partner_2.id)]})
        )
    ...
    def test_message_compose_mass_mail(self):
        ...
        composer.with_context(
            {"default_res_id": -1, "active_ids": [self.test_pigs.id, self.test_public.id]}
        ).send_mail()

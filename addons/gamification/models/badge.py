...
class BadgeUser(models.Model):
    ...
    challenge_id = fields.Many2one(
        "gamification.challenge", string="Challenge originating", help="If this badge was rewarded through a challenge"
    )
...
class GamificationBadge(models.Model):
    ...
    image = fields.Binary(
        "Image", attachment=True, help="This field holds the image used for the badge, limited to 256x256"
    )
    ...
    rule_auth_user_ids = fields.Many2many(
        "res.users", "rel_badge_auth_users", string="Authorized Users", help="Only these people can give this badge"
    )
    ...
    rule_max = fields.Boolean(
        "Monthly Limited Sending", help="Check to set a monthly limit per person of sending this badge"
    )
    rule_max_number = fields.Integer(
        "Limitation Number", help="The maximum number of time this badge can be sent per month per person."
    )
    ...
    def _get_badge_user_stats(self):
        ...
        for badge in self:
            ...
            badge.stat_my_this_month = sum(
                o.user_id == self.env.user and o.create_date >= first_month_day for o in owners
            )
            badge.stat_my_monthly_sending = sum(
                o.create_uid == self.env.user and o.create_date >= first_month_day for o in owners
            )

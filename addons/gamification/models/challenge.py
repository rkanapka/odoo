...
class Challenge(models.Model):
    ...
    report_template_id = fields.Many2one(
        "mail.template", default=lambda self: self._get_report_template(), string="Report Template", required=True
    )
    remind_update_delay = fields.Integer(
        "Non-updated manual goals will be reminded after", help="Never reminded if no value or zero is specified."
    )
        ...
        for challenge in self:
            ...
            for line in challenge.line_ids:
                ...
                if user_squating_challenge_ids:
                    ...
                    Goals.search(
                        [("challenge_id", "=", challenge.id), ("user_id", "in", list(user_squating_challenge_ids))]
                    ).unlink()
    ...
    def report_progress(self, users=(), subset_goals=False):
        ...
        if challenge.visibility_mode == "ranking":
            ...
            body_html = MailTemplates.with_context(challenge_lines=lines_boards).render_template(
                challenge.report_template_id.body_html, "gamification.challenge", challenge.id
            )

            # send to every follower and participant of the challenge
            challenge.message_post(
                body=body_html, partner_ids=challenge.mapped("user_ids.partner_id.id"), subtype="mail.mt_comment"
            )
            ...
            for user in users or challenge.user_ids:
                ...
                body_html = (
                    MailTemplates.sudo(user)
                    .with_context(challenge_lines=lines)
                    .render_template(challenge.report_template_id.body_html, "gamification.challenge", challenge.id)
                )

                # send message only to users, not on the challenge
                self.env["gamification.challenge"].message_post(
                    body=body_html, partner_ids=[(4, user.partner_id.id)], subtype="mail.mt_comment"
                )
                if challenge.report_message_group_id:
                    challenge.report_message_group_id.message_post(body=body_html, subtype="mail.mt_comment")
        ...
        for challenge in self:
            ...
            if challenge_ended:
                ...
                if rewarded_users:
                    ...
                    message_body += _("<br/>Reward (badge %s) for every succeeding user was sent to %s.") % (
                        challenge.reward_id.name,
                        ", ".join(name for (user_id, name) in user_names),
                    )
                else:
                    message_body += _(
                ...
                challenge.message_post(
                    partner_ids=[user.partner_id.id for user in challenge.user_ids], body=message_body
                )
                if commit:
                    commit()

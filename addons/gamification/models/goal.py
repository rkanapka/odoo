...
class GoalDefinition(models.Model):
    ...
    monetary = fields.Boolean(
        "Monetary Value", default=False, help="The target and current value are defined in the company currency."
    )
    ...
    action_id = fields.Many2one(
        "ir.actions.act_window", string="Action", help="The action that will be called to update the goal value."
    )
    ...
    def _check_domain_validity(self):
        ...
        for definition in self:
            ...
            try:
                domain = safe_eval(definition.domain, {"user": self.env.user.sudo(self.env.user)})
...
class Goal(models.Model):
    ...
    def _check_remind_delay(self):
        ...
        self.env["mail.thread"].message_post(
            body=body_html, partner_ids=[self.user_id.partner_id.id], subtype="mail.mt_comment"
        )
                        ...
                        for goal in [g for g in goals if g.id in query_goals]:
                            for user_value in user_values:
                                ...
                                if (
                                    ...
                                    and isinstance(queried_value[0], pycompat.integer_types)
                                ):
    ...
    def get_action(self):
        ...
        if self.definition_id.action_id:
            ...
            if self.definition_id.res_id_field:
                ...
                action["res_id"] = safe_eval(self.definition_id.res_id_field, {"user": current_user})

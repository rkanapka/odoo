    ...
    def compute_OP(self, modeFull=True):
        ...
        if self.OE.found and self.GG.found:
            ...
            if self.OE.status != self.GG.status and is_owner:
                self.OP = Delete(
                    (self.OE.status and "OE") or (self.GG.status and "GG"),
                ...
                if self.OE.update.split(".")[0] != self.GG.update.split(".")[0]:
                    ...
                    if self[tmpSrc].isRecurrence:
                        if self[tmpSrc].status:
                            ...
                            self.OP = Exclude(
                                tmpSrc, "Need to Exclude (Me = First event from recurrence) from recurrence"
                            )
                    ...
                    if not self.OE.synchro or self.OE.synchro.split(".")[0] < self.OE.update.split(".")[0]:
                        self.OP = Update(
                            "OE", "Event already updated by another user, but not synchro with my google calendar"
                        )
                    else:
...
class GoogleCalendar(models.AbstractModel):
    ...
    def get_calendar_primary_id(self):
        ...
        """
        params = {"fields": "id", "access_token": self.get_token()}
        headers = {"Content-type": "application/json", "Accept": "text/plain"}
    ...
    def update_recurrent_event_exclu(self, instance_id, event_ori_google_id, event_new):
        ...
        data.update(
            ...
            originalStartTime=event_new.recurrent_id_date,
    ...
    def create_from_google(self, event, partner_id):
        ...
        attendee_record = self.env["calendar.attendee"].search(
            [("partner_id", "=", partner_id), ("event_id", "=", res)]
        )
        attendee_record.with_context(context_tmp).write(
            {"oe_synchro_date": meeting.oe_update_date, "google_internal_event_id": event.GG.event["id"]}
        )
        if meeting.recurrency:
            attendees = (
                self.env["calendar.attendee"]
                .sudo()
                .search([("google_internal_event_id", "=ilike", r"%s\_%%" % event.GG.event["id"])])
            )
            excluded_recurrent_event_ids = {attendee.event_id for attendee in attendees}
            for event in excluded_recurrent_event_ids:
                event.write(
                    {"recurrent_id": meeting.id, "recurrent_id_date": event.start, "user_id": meeting.user_id.id}
                )
        return event
    ...
    def update_from_google(self, event, single_event_dict, type):
        ...
        if self.get_need_synchro_attendee():
            for google_attendee in single_event_dict.get("attendees", []):
                ...
                if type == "write":
                    for oe_attendee in event["attendee_ids"]:
                        if oe_attendee.email == google_attendee["email"]:
                            oe_attendee.write(
                                {
                                    "state": google_attendee["responseStatus"],
                ...
                attendee = ResPartner.search(
                    [("email", "=ilike", google_attendee["email"]), ("user_ids", "!=", False)], limit=1
                )
                if not attendee:
        ...
        if single_event_dict.get("start") and single_event_dict.get("end"):  # If not cancelled
            ...
            result.update({"start": date, "stop": stop, "allday": allday})
        result.update(
            {
        ...
        if self.env.context.get("curr_attendee"):
            self.env["calendar.attendee"].with_context(no_mail_to_attendees=True).browse(
                [self.env.context["curr_attendee"]]
            ).write({"oe_synchro_date": update_date, "google_internal_event_id": single_event_dict.get("id", False)})
        return res
    ...
    def synchronize_events_cron(self):
        ...
        if self.env.context.get("last_sync_hours"):
            ...
            domain = expression.AND(
                [domain, [("google_calendar_last_sync_date", "<=", fields.Datetime.to_string(last_sync_date))]]
            )
    ...
    def synchronize_events(self, lastSync=True):
        ...
        if current_user.google_calendar_cal_id:
            if current_google != current_user.google_calendar_cal_id:
                return {
                    "status": "need_reset",
                    "info": {"old_name": current_user.google_calendar_cal_id, "new_name": current_google},
                    "url": "",
                }
            ...
            if lastSync and recs.get_last_sync_date() and not recs.get_disable_since_synchro():
                ...
                _logger.info(
                    "[%s] Calendar Synchro - MODE SINCE_MODIFIED : %s !",
                    user_to_sync,
                    fields.Datetime.to_string(lastSync),
                )
            else:
                lastSync = False
        ...
        return {"status": res and "need_refresh" or "no_new_event_from_google", "url": ""}
    ...
    def create_new_events(self):
        ...
        for att in my_attendees:
            other_google_ids = [
                other_att.google_internal_event_id
                for other_att in att.event_id.attendee_ids
                if other_att.google_internal_event_id
                and other_att.id != att.id
                and not other_att.google_internal_event_id.startswith("_")
            ]
            for other_google_id in other_google_ids:
                ...
                if not att.event_id.recurrent_id or att.event_id.recurrent_id == 0:
                    ...
                    if status_response(status):
                        ...
                        _logger.warning(
                            "Impossible to create event %s. [%s] Enable DEBUG for response detail.",
                            att.event_id.id,
                            status,
                        )
    ...
    def bind_recurring_events_to_google(self):
        ...
        my_attendees = CalendarAttendee.with_context(context_norecurrent).search(
            [("partner_id", "=", my_partner_id), ("google_internal_event_id", "=", False)]
        )
        for att in my_attendees:
            new_google_internal_event_id = False
            source_event_record = self.env["calendar.event"].browse(att.event_id.recurrent_id)
            source_attendee_record = CalendarAttendee.search(
                [("partner_id", "=", my_partner_id), ("event_id", "=", source_event_record.id)], limit=1
            )
            if not source_attendee_record:
                continue

            if (
                att.event_id.recurrent_id_date
                and source_event_record.allday
                and source_attendee_record.google_internal_event_id
            ):
                new_google_internal_event_id = (
                    source_attendee_record.google_internal_event_id
                    + "_"
                    + att.event_id.recurrent_id_date.split(" ")[0].replace("-", "")
                )
            elif att.event_id.recurrent_id_date and source_attendee_record.google_internal_event_id:
                new_google_internal_event_id = (
                    source_attendee_record.google_internal_event_id
                    + "_"
                    + att.event_id.recurrent_id_date.replace("-", "").replace(" ", "T").replace(":", "")
                    + "Z"
                )
            ...
            if new_google_internal_event_id:
                ...
                try:
                    status, response, ask_time = self.update_recurrent_event_exclu(
                        new_google_internal_event_id, source_attendee_record.google_internal_event_id, att.event_id
                    )
                    if status_response(status):
    ...
    def update_events(self, lastSync=False):
        ...
        if lastSync:
            ...
            my_google_attendees = CalendarAttendee.with_context(context_novirtual).search(
                [("partner_id", "=", my_partner_id), ("google_internal_event_id", "in", list(all_event_from_google))]
            )
            ...
            my_odoo_attendees = CalendarAttendee.with_context(context_novirtual).search(
                [
                    ...
                    (
                        ...
                        ">",
                        lastSync
                        and fields.Datetime.to_string(lastSync)
                        or self.get_minTime().fields.Datetime.to_string(),
                    ),
                    ("google_internal_event_id", "!=", False),
                ]
            )
            ...
            if self.get_print_log():
                _logger.info(
                    "Calendar Synchro -  \n\nUPDATE IN GOOGLE\n%s\n\nRETRIEVE FROM OE\n%s\n\nUPDATE IN OE\n%s\n\nRETRIEVE FROM GG\n%s\n\n",
                    all_event_from_google,
                    my_google_att_ids,
                    my_odoo_attendees.ids,
            ...
            for gi_record in my_odoo_googleinternal_records:
                ...
                if gi_record.get("event_id"):
                    active = (
                        CalendarEvent.with_context(context_novirtual).browse(int(gi_record.get("event_id")[0])).active
                    )

                if (
                    gi_record.get("google_internal_event_id")
                    and not all_event_from_google.get(gi_record.get("google_internal_event_id"))
                    and active
                ):
        ...
        for base_event in event_to_synchronize:
            event_to_synchronize[base_event] = sorted(
                event_to_synchronize[base_event].items(), key=operator.itemgetter(0)
            )
            for current_event in event_to_synchronize[base_event]:
                self.env.cr.commit()
                    ...
                    if actSrc == "OE":
                        ...
                        if event.GG.status:
                            ...
                            if event_to_synchronize[base_event][0][1].OE.event_id:
                                parent_event["id"] = "%s-%s" % (
                                    event_to_synchronize[base_event][0][1].OE.event_id,
                                    new_google_event_id,
                                )
                                recs.update_from_google(parent_event, event.GG.event, "copy")
                            else:
                            ...
                            if parent_oe_id:
                                CalendarEvent.browse("%s-%s" % (parent_oe_id, new_google_event_id)).with_context(
                                    curr_attendee=event.OE.attendee_id
                                ).unlink(can_be_deleted=True)
    ...
    def check_and_sync(self, oe_event, google_event):
        if datetime.strptime(oe_event.oe_update_date, "%Y-%m-%d %H:%M:%S.%f") > datetime.strptime(
            google_event["updated"], "%Y-%m-%dT%H:%M:%S.%fz"
        ):
            self.update_to_google(oe_event, google_event)
        elif datetime.strptime(oe_event.oe_update_date, "%Y-%m-%d %H:%M:%S.%f") < datetime.strptime(
            google_event["updated"], "%Y-%m-%dT%H:%M:%S.%fz"
        ):
            self.update_from_google(oe_event, google_event, "write")

    def get_sequence(self, instance_id):
        params = {"fields": "sequence", "access_token": self.get_token()}
        headers = {"Content-type": "application/json"}
    ...
    def get_token(self):
        current_user = self.env.user
        if not current_user.google_calendar_token_validity or fields.Datetime.from_string(
            current_user.google_calendar_token_validity.split(".")[0]
    ...
    def get_last_sync_date(self):
        current_user = self.env.user
        return (
            current_user.google_calendar_last_sync_date
            and fields.Datetime.from_string(current_user.google_calendar_last_sync_date) + timedelta(minutes=0)
            or False
        )
    ...
    def authorize_google_uri(self, from_url="http://www.odoo.com"):
        url = self.env["google.service"]._get_authorize_uri(
            from_url, self.STR_SERVICE, scope=self.get_calendar_scope()
        )
        return url

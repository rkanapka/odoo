...
class AccountInvoice(models.Model):
    ...
    def _get_printed_report_name(self):
        ...
        if self.company_id.country_id.code != "IN":
            return super()._get_printed_report_name()
        return (
            self.type == "out_invoice"
            and self.state == "draft"
            and _("Draft %s") % (self.journal_id.name)
            or self.type == "out_invoice"
            and self.state in ("open", "paid")
            and "%s - %s" % (self.journal_id.name, self.number)
    ...
    def _invoice_line_tax_values(self):
        ...
        for line in self.mapped("invoice_line_ids"):
            ...
            tax_lines = line.invoice_line_tax_ids.compute_all(

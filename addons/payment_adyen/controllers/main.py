...
class AdyenController(http.Controller):
    ...
    def adyen_notification(self, **post):
        tx = post.get("merchantReference") and request.env["payment.transaction"].sudo().search(
        ...
        if post.get("eventCode") in ["AUTHORISATION"] and tx:
            ...
            if (post.get("success") == "true" and tx.state == "done") or (
                post.get("success") == "false" and tx.state in ["cancel", "error"]
            ):

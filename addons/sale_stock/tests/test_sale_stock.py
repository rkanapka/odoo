...
class TestSaleStock(TestSale):
    def test_00_sale_stock_invoice(self):
        ...
        self.assertTrue(
            self.so.picking_ids, 'Sale Stock: no picking created for "invoice on delivery" stockable products'
        )
        # invoice on order
        ...
        self.assertEqual(
            del_qties, del_qties_truth, "Sale Stock: delivered quantities are wrong after partial delivery"
        )
        ...
        self.assertIsNone(
            pick_2.button_validate(), "Sale Stock: second picking should be final without need for a backorder"
        )
        self.assertEqual(
            self.so.invoice_status,
            "to invoice",
        ...
        self.assertEqual(
            del_qties, del_qties_truth, "Sale Stock: delivered quantities are wrong after complete delivery"
        )
    ...
    def test_01_sale_stock_order(self):
        ...
        self.assertTrue(
            self.so.picking_ids, 'Sale Stock: no picking created for "invoice on order" stockable products'
        )
        ...
        self.assertEqual(
            inv.amount_untaxed, self.so.amount_untaxed * 5.0 / 100.0, "Sale Stock: deposit invoice is wrong"
        )
        self.assertEqual(
            self.so.invoice_status, "to invoice", "Sale Stock: so should be to invoice after invoicing deposit"
        )
        # invoice on order: everything should be invoiced
        self.so.action_invoice_create(final=True)
        self.assertEqual(
            self.so.invoice_status, "invoiced", "Sale Stock: so should be fully invoiced after second invoice"
        )
    ...
    def test_02_sale_stock_return(self):
        ...
        self.assertEqual(
            len(inv_1_id), 1, 'Sale Stock: only one invoice instead of "%s" should be created' % len(inv_1_id)
        )
        ...
        self.assertAlmostEqual(
            self.inv_2.invoice_line_ids[0].quantity,
            2.0,
    ...
    def test_03_sale_stock_delivery_partial(self):
        ...
        self.so.action_done()
        self.assertEqual(
            self.so.invoice_status, "invoiced", 'Sale Stock: so invoice_status should be "invoiced" when set to done'
        )

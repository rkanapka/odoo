...
class SaleOrderLine(models.Model):
    ...
    route_id = fields.Many2one(
        "stock.location.route", string="Route", domain=[("sale_selectable", "=", True)], ondelete="restrict"
    )
    ...
    def write(self, values):
        ...
        if "product_uom_qty" in values:
            ...
            lines = self.filtered(
                ...
                and float_compare(r.product_uom_qty, values["product_uom_qty"], precision_digits=precision) == -1
            )
        ...
        if self.product_id.type == "product":
            ...
            if float_compare(product.virtual_available, product_qty, precision_digits=precision) == -1:
                ...
                if not is_available:
                    ...
                    if (
                        float_compare(
                            product.virtual_available, self.product_id.virtual_available, precision_digits=precision
                        )
                        == -1
                    ):
                    ...
                    warning_mess = {"title": _("Not enough inventory!"), "message": message}
    ...
    def _prepare_procurement_values(self, group_id=False):
        ...
        self.ensure_one()
        date_planned = (
            datetime.strptime(self.order_id.confirmation_date, DEFAULT_SERVER_DATETIME_FORMAT)
    ...
    def _check_package(self):
        ...
        if (
            qty
            and q
            and float_compare(qty / q, float_round(qty / q, precision_rounding=1.0), precision_rounding=0.001) != 0
        ):
            newqty = qty - (qty % q) + q
            return {
                "warning": {
        ...
        if not is_available:
            for pull_rule in product_routes.mapped("pull_ids"):
                if (
                    ...
                    and pull_rule.picking_type_id.sudo().default_location_dest_id.usage == "customer"
                ):
                    is_available = True
                    break
    ...
    def _update_line_quantity(self, values):
        ...
        if (
            ...
            and float_compare(values["product_uom_qty"], max(self.mapped("qty_delivered")), precision_digits=precision)
            == -1
        ):
            raise UserError(

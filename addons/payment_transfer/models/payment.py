...
class TransferPaymentAcquirer(models.Model):
    ...
    def _create_missing_journal_for_acquirers(self, company=None):
        ...
        return super()._create_missing_journal_for_acquirers(company=company)
...
<p>Please use the order name as communication reference.</p>
    ...
    def write(self, values):
        ...
        if (
            all(not acquirer.post_msg and acquirer.provider != "transfer" for acquirer in self)
            and values.get("provider") == "transfer"
        ):
            values["post_msg"] = self._format_transfer_data()

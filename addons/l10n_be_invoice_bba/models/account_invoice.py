...
class AccountInvoice(models.Model):
    ...
    def _get_reference_type(self):
        ...
        res = super()._get_reference_type()
        res[[i for i, x in enumerate(res) if x[0] == "none"][0]] = ("none", _("Free Communication"))
        res.append(("bba", "BBA Structured Communication"))
        return res

    reference_type = fields.Selection("_get_reference_type", string="Payment Reference", required=True, readonly=True)
    ...
    def _onchange_partner_id(self):
        ...
        if self.partner_id:
            if self.type == "out_invoice":
                ...
                if reference_type:
                    reference = self.generate_bbacomm(self.type, reference_type, self.partner_id.id, "")["value"][
                        "reference"
                    ]
                        ...
                        if (len(partner_ref_nr) < 3) or (len(partner_ref_nr) > 7):
                            ...
                            invoices = self.search(
                                [
                                    ("type", "=", "out_invoice"),
                                    ("reference_type", "=", "bba"),
                                    ("reference", "like", "+++%s/%s/%%" % (partner_ref_nr[:3], partner_ref_nr[3:])),
                                ],
                                order="reference",
                            )
                            if invoices:
        ...
        for invoice in self:
            ...
            if reference_type == "bba" and "reference" in vals:
                if self.check_bbacomm(vals["reference"]):
                    reference = re.sub(r"\D", "", vals["reference"])
                    vals["reference"] = "+++" + reference[0:3] + "/" + reference[3:7] + "/" + reference[7:] + "+++"
                    same_ids = self.search(
                        [
                            ("id", "!=", invoice.id),
    ...
    def copy(self, default=None):
        ...
        if self.type in ["out_invoice"]:
            ...
            if reference_type == "bba":
                default["reference"] = self.generate_bbacomm(self.type, reference_type, self.partner_id.id, "")[
                    "value"
                ]["reference"]

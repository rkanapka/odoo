...
class PaymentTransaction(models.Model):
    ...
    def confirm_invoice_token(self):
        ...
        if self.payment_token_id and self.partner_id == self.account_invoice_id.partner_id:
            try:
                ...
                _logger.warning(
                    _("<%s> transaction (%s) failed : <%s>") % (self.acquirer_id.provider, self.id, str(e))
                )
    ...
    def _confirm_invoice(self):
        ...
        if not float_compare(self.amount, self.account_invoice_id.amount_total, 2) == 0:
            ...
            self.account_invoice_id.message_post(
                ...
                body=_(
                    "The invoice was not confirmed despite response from the acquirer (%s): invoice amount is %r but acquirer replied with %r."
                )
                % (
                    self.acquirer_id.provider,
                    self.account_invoice_id.amount_total,
        ...
        if self.state == "authorized" and self.acquirer_id.capture_manually:
            ...
            _logger.warning(
                "<%s> transaction MISMATCH for invoice %s (ID %s)",
                self.acquirer_id.provider,
                self.account_invoice_id.number,
                self.account_invoice_id.id,
            )
    ...
    def _pay_invoice(self):
        ...
        ctx_company = {
            "company_id": self.account_invoice_id.company_id.id,
        ...
        if not self.acquirer_id.journal_id:
            ...
            if not default_journal:
                _logger.warning(
                    "<%s> transaction completed, could not auto-generate payment for invoice %s (ID %s) (no journal set on acquirer)",
                    self.acquirer_id.provider,
                    self.account_invoice_id.number,
                    self.account_invoice_id.id,
                )
        ...
        _logger.info(
            "<%s> transaction <%s> completed, reconciled invoice %s (ID %s))",
            self.acquirer_id.provider,
            self.ids[0],
            invoice.number,
            invoice.id,
    ...
    def render_invoice_button(self, invoice, return_url, submit_txt=None, render_values=None):
        ...
        return (
            self.acquirer_id.with_context(submit_class="btn btn-primary", submit_txt=submit_txt or _("Pay Now"))
            .sudo()
            .render(
                self.reference,
    ...
    def _check_or_create_invoice_tx(self, invoice, acquirer, payment_token=None, tx_type="form", add_tx_values=None):
        ...
        if tx and tx.state in ["error", "cancel"]:  # filter incorrect states
            tx = False
        if (tx and acquirer and tx.acquirer_id != acquirer) or (
            tx and tx.account_invoice_id != invoice
        ):  # filter unmatching
            tx = False
        if (
            tx and tx.payment_token_id and payment_token and payment_token != tx.payment_token_id

...
class Task(models.Model):
    ...
    def rating_apply(self, rate, token=None, feedback=None, subtype=None):
        return super().rating_apply(rate, token=token, feedback=feedback, subtype="rating_project.mt_task_rating")
...
class Project(models.Model):
    ...
    def _compute_percentage_satisfaction_project(self):
        ...
        for project in self:
            ...
            project.percentage_satisfaction_project = (
                activity["great"] * 100 / sum(activity.values()) if sum(activity.values()) else -1
            )
    ...
    def _compute_percentage_satisfaction_task(self):
        ...
        self.percentage_satisfaction_task = (
            activity["great"] * 100 / sum(activity.values()) if sum(activity.values()) else -1
        )
    ...
    def action_view_all_rating(self):
        ...
        action = self.env["ir.actions.act_window"].for_xml_id(
            "rating_project", "rating_rating_action_view_project_rating"
        )

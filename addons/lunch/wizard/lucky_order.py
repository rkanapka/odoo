...
class LunchOrderLineLucky(models.TransientModel):
    ...
    supplier_ids = fields.Many2many(
        ...
        string="Vendor",
        domain=lambda self: [("id", "in", self.env["lunch.product"].search([]).mapped("supplier").ids)],
    )
    is_max_budget = fields.Boolean(
        "I'm not feeling rich", help="Enable this option to set a maximal budget for your lucky order.", store=True
    )
    ...
    def random_pick(self):
        ...
        if self.is_max_budget:
            products_obj = self.env["lunch.product"].search(
                [("supplier", "in", self.supplier_ids.ids), ("price", "<=", self.max_budget)]
            )
        else:

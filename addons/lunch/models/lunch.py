...
class LunchOrder(models.Model):
    ...
    def _default_previous_order_ids(self):
        prev_order = self.env["lunch.order.line"].search(
            [("user_id", "=", self.env.uid), ("product_id.active", "!=", False)], limit=20, order="id desc"
        )
        ...
        return list({(order.product_id, order.note): order.id for order in prev_order}.values())

    user_id = fields.Many2one(
        "res.users", "User", readonly=True, states={"new": [("readonly", False)]}, default=lambda self: self.env.uid
    )
    date = fields.Date(
        "Date", required=True, readonly=True, states={"new": [("readonly", False)]}, default=fields.Date.context_today
    )
    ...
    state = fields.Selection(
        [("new", "New"), ("confirmed", "Received"), ("cancelled", "Cancelled")],
        "Status",
        readonly=True,
        index=True,
        copy=False,
    ...
    def _compute_previous_order(self):
        ...
        prev_order = self.env["lunch.order.line"].search(
            [("user_id", "=", self.env.uid), ("product_id.active", "!=", False)], limit=20, order="date desc, id desc"
        )
        # If we use prev_order.ids, we will have duplicates (identical orders).
        # Therefore, this following part removes duplicates based on product_id and note.
        self.previous_order_ids = list({(order.product_id, order.note): order.id for order in prev_order}.values())
...
class LunchOrderLine(models.Model):
    ...
    supplier = fields.Many2one(
        "res.partner", string="Vendor", related="product_id.supplier", readonly=True, store=True
    )
    ...
    state = fields.Selection(
        [("new", "New"), ("confirmed", "Received"), ("ordered", "Ordered"), ("cancelled", "Cancelled")],
        "Status",
        readonly=True,
        index=True,
        default="new",
...
class LunchAlert(models.Model):
    ...
    alert_type = fields.Selection(
        [("specific", "Specific Day"), ("week", "Every Week"), ("days", "Every Day")],
        string="Recurrence",
        required=True,
        index=True,
        default="specific",

...
class MassMailingContact(models.Model):
    ...
    list_ids = fields.Many2many(
        "mail.mass_mailing.list", "mail_mass_mailing_contact_list_rel", "contact_id", "list_id", string="Mailing Lists"
    )
    opt_out = fields.Boolean(
        string="Opt Out", help="The contact has chosen not to receive mails anymore from this list"
    )
    unsubscription_date = fields.Datetime(string="Unsubscription Date")
    message_bounce = fields.Integer(
        string="Bounced", help="Counter of the number of bounced emails for this contact.", default=0
    )
    ...
    def message_get_default_recipients(self):
        return {record.id: {"partner_ids": [], "email_to": record.email, "email_cc": False} for record in self}
...
class MassMailingCampaign(models.Model):
    ...
    tag_ids = fields.Many2many(
        "mail.mass_mailing.tag", "mail_mass_mailing_tag_rel", "tag_id", "campaign_id", string="Tags"
    )
    ...
    def _compute_clicks_ratio(self):
        ...
        mapped_data = {c["id"]: 100 * c["nb_clicks"] / c["nb_mails"] for c in campaign_data}
    ...
    def _compute_total_mailings(self):
        campaign_data = self.env["mail.mass_mailing"].read_group(
            [("mass_mailing_campaign_id", "in", self.ids)], ["mass_mailing_campaign_id"], ["mass_mailing_campaign_id"]
        )
        mapped_data = {c["mass_mailing_campaign_id"][0]: c["mass_mailing_campaign_id_count"] for c in campaign_data}
        for campaign in self:
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        if groupby and groupby[0] == "stage_id":
            ...
            read_group_res = super().read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby)
...
class MassMailing(models.Model):
    ...
    email_from = fields.Char(
        string="From", required=True, default=lambda self: self.env["mail.message"]._get_default_from()
    )
    create_date = fields.Datetime(string="Creation Date")
    ...
    attachment_ids = fields.Many2many(
        "ir.attachment", "mass_mailing_ir_attachments_rel", "mass_mailing_id", "attachment_id", string="Attachments"
    )
    ...
    mailing_model_real = fields.Char(
        compute="_compute_model", string="Recipients Real Model", default="mail.mass_mailing.contact", required=True
    )
    ...
    mailing_model_name = fields.Char(
        related="mailing_model_id.model", string="Recipients Model Name", readonly=True, related_sudo=True
    )
    ...
    def _compute_model(self):
        for record in self:
            record.mailing_model_real = (
                ...
                and record.mailing_model_name
                or "mail.mass_mailing.contact"
            )
    ...
    def _onchange_model_and_list(self):
        if self.mailing_model_name == "mail.mass_mailing.list":
            ...
            and "opt_out" in self.env[self.mailing_model_name]._fields
            and not self.mailing_domain
        ):
    ...
    def _get_blacklist(self):
        ...
        if "opt_out" in target._fields:
            ...
            blacklist = {m[0] for m in self._cr.fetchall()}
            _logger.info("Mass-mailing %s targets %s, blacklist: %s emails", self, target._name, len(blacklist))
        else:
    ...
    def _get_seen_list(self):
        ...
        self.ensure_one()
        target = self.env[self.mailing_model_real]
        if {"email", "email_from"} & set(target._fields):
            mail_field = "email" if "email" in target._fields else "email_from"
            # avoid loading a large number of records in memory
            # + use a basic heuristic for extracting emails
            query = """
        ...
        _logger.info("Mass-mailing %s has already reached %s %s emails", self, len(seen_list), target._name)
        return seen_list
    ...
    def get_remaining_recipients(self):
        ...
        already_mailed = self.env["mail.mail.statistics"].search_read(
            [("model", "=", self.mailing_model_real), ("res_id", "in", res_ids), ("mass_mailing_id", "=", self.id)],
            ["res_id"],
        )
    ...
    def convert_links(self):
        ...
        for mass_mailing in self:
            utm_mixin = (
                mass_mailing.mass_mailing_campaign_id if mass_mailing.mass_mailing_campaign_id else mass_mailing
            )

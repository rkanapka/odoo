...
class TestMassMailingShortener(common.TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch(
            "odoo.addons.link_tracker.models.link_tracker.link_tracker._compute_favicon", wraps=_compute_favicon
        )
        patcher2 = patch(
            "odoo.addons.link_tracker.models.link_tracker.link_tracker._get_title_from_url", wraps=_get_title_from_url
        )
        patcher.start()
        patcher2.start()
...
Email: <a id="url4" href="mailto:test@odoo.com">test@odoo.com</h1>
        ...
        self.assertEqual(mailing_list_A.contact_nbr, len(sent_messages), "Some message has not been sent")

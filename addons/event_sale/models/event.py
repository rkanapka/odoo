...
class EventType(models.Model):
    ...
    event_ticket_ids = fields.One2many(
        "event.event.ticket", "event_type_id", string="Tickets", default=_get_default_event_ticket_ids
    )
...
class Event(models.Model):
    ...
    event_ticket_ids = fields.One2many("event.event.ticket", "event_id", string="Event Ticket", copy=True)
...
class EventTicket(models.Model):
    ...
    price_reduce = fields.Float(
        string="Price Reduce", compute="_compute_price_reduce", digits=dp.get_precision("Product Price")
    )
    ...
    def _get_price_reduce_tax(self):
        for record in self:
            ...
            taxes = tax_ids.compute_all(
                record.price_reduce, record.event_id.company_id.currency_id, 1.0, product=record.product_id
            )
...
class EventRegistration(models.Model):
    ...
    def _prepare_attendee_values(self, registration):
        ...
        att_data = super()._prepare_attendee_values(registration)
        if line_id:
            att_data.update(
                {

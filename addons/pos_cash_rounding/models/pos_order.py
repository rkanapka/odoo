...
class PosOrder(models.Model):
    ...
    def _get_amount_receivable(self, move_lines):
        if self.config_id.cash_rounding:
            ...
            return super()._get_amount_receivable(move_lines)

    def _prepare_account_move_and_lines(self, session=None, move=None):
        res = super()._prepare_account_move_and_lines(session, move)
        ...
        if unpaid_order:
            ...
            if config_id.cash_rounding and config_id.rounding_method:
                ...
                different_currency = (
                    config_id.currency_id if config_id.currency_id.id != company_id.currency_id.id else False
                )
                for order in unpaid_order:
                    ...
                    if config_id.currency_id.id != company_id.currency_id.id:
                        converted_paid = different_currency._convert(
                            order.amount_paid, company_id.currency_id, company_id, order.date_order
                        )
                        converted_total = different_currency._convert(
                            order.amount_total, company_id.currency_id, company_id, order.date_order
                        )
                ...
                if difference:
                    ...
                    if float_compare(0.0, difference, precision_rounding=config_id.currency_id.rounding) > 0:
                        ...
                        if different_currency:
                            difference_move_line.update(
                                {"currency_id": different_currency.id, "amount_currency": -difference}
                            )
                        grouped_data_key = (
                            "difference_rounding",
                            False,

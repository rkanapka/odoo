...
class Lead2OpportunityMassConvert(models.TransientModel):
    ...
    deduplicate = fields.Boolean(
        "Apply deduplication", default=True, help="Merge with existing leads/opportunities of each partner"
    )
    action = fields.Selection(
        [("each_exist_or_create", "Use existing partner or create"), ("nothing", "Do not link to a customer")],
        "Related Customer",
        required=True,
    )
    ...
    def _onchange_deduplicate(self):
        ...
        partner_ids = [
            (lead.partner_id.id, lead.partner_id and lead.partner_id.email or lead.email_from) for lead in active_leads
        ]
    ...
    def _convert_opportunity(self, vals):
        ...
        return super()._convert_opportunity(vals)

...
class MergePartnerAutomatic(models.TransientModel):
    ...
    def _update_foreign_keys(self, src_partners, dst_partner):
        ...
        """
        _logger.debug(
            "_update_foreign_keys for dst_partner: %s for src_partners: %s", dst_partner.id, str(src_partners.ids)
        )
    ...
    def _update_reference_fields(self, src_partners, dst_partner):
        ...
        """
        _logger.debug(
            "_update_reference_fields for dst_partner: %s for src_partners: %r", dst_partner.id, src_partners.ids
        )
    ...
    def _update_values(self, src_partners, dst_partner):
        ...
        if parent_id and parent_id != dst_partner.id:
            try:
                ...
                _logger.info(
                    "Skip recursive partner hierarchies for parent_id %s of partner: %s", parent_id, dst_partner.id
                )
    ...
    def _merge(self, partner_ids, dst_partner=None):
        ...
        if SUPERUSER_ID != self.env.uid and len({partner.email for partner in partner_ids}) > 1:
            raise UserError(
                _(
        ...
        dst_partner.message_post(
            ...
            % (
                ...
                ", ".join("%s <%s> (ID %s)" % (p.name, p.email or "n/a", p.id) for p in src_partners),
            )
        )

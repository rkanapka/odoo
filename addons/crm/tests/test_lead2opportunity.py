...
class TestLead2opportunity2win(TestCrmCases):
    ...
    def test_lead2opportunity2win(self):
        ...
        mass = (
            CrmLead2OpportunityPartnerMass.sudo(self.crm_salemanager.id)
            .with_context(
                {
        ...
        self.assertEqual(
            ...
            expected_partner,
            "Partner mismatch! %s vs %s" % (crm_case_13.partner_id.name, expected_partner),
        )
        ...
        self.env.ref("calendar.calendar_event_4").with_context({"active_model": "calendar.event"}).write(
            {"state": "open"}
        )
    ...
    def test_lead2opportunity_assign_salesmen(self):
        ...
        test_res_user_01 = self.env["res.users"].create(
            {"name": "Test user A", "login": "tua@example.com", "new_password": "tua"}
        )
        test_res_user_02 = self.env["res.users"].create(
            {"name": "Test user B", "login": "tub@example.com", "new_password": "tub"}
        )
        test_res_user_03 = self.env["res.users"].create(
            {"name": "Test user C", "login": "tuc@example.com", "new_password": "tuc"}
        )
        test_res_user_04 = self.env["res.users"].create(
            {"name": "Test user D", "login": "tud@example.com", "new_password": "tud"}
        )
        ...
        test_crm_lead_04 = LeadSaleman.create(
            {"type": "lead", "name": "Test lead 4", "email_from": "Fabrice Lepoilu", "stage_id": default_stage_id}
        )
        test_crm_lead_05 = LeadSaleman.create(
            {"type": "lead", "name": "Test lead 5", "email_from": "Fabrice Lepoilu", "stage_id": default_stage_id}
        )
        ...
        mass = (
            CrmLead2OpportunityPartnerMass.sudo(self.crm_salesman.id)
        ...
        for opp in opps:
            ...
            self.assertEqual(
                opp.user_id.id,
                salesmen_ids[i],
                "Salesman mismatch: expected salesman %r, got %r" % (salesmen_ids[i], opp.user_id.id),
            )

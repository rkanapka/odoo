...
class TestCRMLead(TestCrmCases):
    ...
    def test_crm_lead_message(self):
        ...
        lead = (
            self.env["crm.lead"]
            .sudo(self.crm_salesman)
            .search([("email_from", "=", "Mr. John Right <info@customer.com>")], limit=1)
        )
    ...
    def test_crm_message_default_alias(self):
        ...
        msg = self.env["mail.message"].create({"model": "crm.lead", "res_id": lead.id})
    ...
    def test_crm_lead_merge(self):
        ...
        merged_lead = self.env["crm.lead"].search(
            [("name", "=", "Test opportunity 1"), ("partner_id", "=", self.env.ref("base.res_partner_3").id)], limit=1
        )
        self.assertTrue(merged_lead, "Fail to create merge opportunity wizard")
        self.assertEqual(
            merged_lead.description,
            "This is the description of the test opp 1.\n\nThis is the description of the test lead first.\n\nThis is the description of the test lead second.",
            "Description mismatch: when merging leads/opps with different text values, these values should get concatenated and separated with line returns",
        )
        self.assertEqual(
            merged_lead.type,
            "opportunity",
            "Type mismatch: when at least one opp in involved in the merge, the result should be a new opp (instead of %s)"
            % merged_lead.type,
        )

        # The other (tailing) leads/opps shouldn't exist anymore
        self.assertFalse(
            test_crm_lead_01.exists(), "This tailing lead (id %s) should not exist anymore" % test_crm_lead_02.id
        )
        self.assertFalse(
            test_crm_lead_02.exists(), "This tailing opp (id %s) should not exist anymore" % test_crm_opp_01.id
        )
        ...
        merged_lead = self.env["crm.lead"].search(
            [("name", "=", "Test lead 3"), ("partner_id", "=", self.env.ref("base.res_partner_1").id)], limit=1
        )
        self.assertTrue(merged_lead, "Fail to create merge opportunity wizard")
        ...
        self.assertEqual(
            merged_lead.type,
            "lead",
            "Type mismatch: when leads get merged together, the result should be a new lead (instead of %s)"
            % merged_lead.type,
        )
        self.assertFalse(
            test_crm_lead_04.exists(), "This tailing lead (id %s) should not exist anymore" % test_crm_lead_04.id
        )
        ...
        additionnal_context = {
            ...
            "active_ids": opportunity_ids,
            "active_id": opportunity_ids[0],
        }
        ...
        merged_opportunity = self.env["crm.lead"].search(
            [("name", "=", "Test opportunity 2"), ("partner_id", "=", self.env.ref("base.res_partner_3").id)], limit=1
        )
        ...
        self.assertEqual(
            ...
            "opportunity",
            "Type mismatch: when opps get merged together, the result should be a new opp (instead of %s)"
            % merged_opportunity.type,
        )
        self.assertFalse(
            test_crm_opp_03.exists(), "This tailing opp (id %s) should not exist anymore" % test_crm_opp_03.id
        )

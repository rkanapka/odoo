...
class Lead(models.Model):
    ...
    priority = fields.Selection(
        crm_stage.AVAILABLE_PRIORITIES, string="Priority", index=True, default=crm_stage.AVAILABLE_PRIORITIES[0][0]
    )
    ...
    company_currency = fields.Many2one(
        string="Currency", related="company_id.currency_id", readonly=True, relation="res.currency"
    )
    ...
    company_id = fields.Many2one(
        "res.company", string="Company", index=True, default=lambda self: self.env.user.company_id.id
    )
            ...
            if message:
                return {
                    "effect": {
                        ...
                        "message": message,
                        "img_url": (
                            "/web/image/%s/%s/image" % (self.team_id.user_id._name, self.team_id.user_id.id)
                            if self.team_id.user_id.image
    ...
    def _mail_body(self, fields):
        ...
        fields = self.env["ir.model.fields"].search(
            [("name", "in", fields or []), ("model_id.model", "=", self._name)]
        )
        for field in fields:
            ...
            if field.ttype == "selection":
                ...
                if value:
                    value = ",".join(val.name_get()[0][1] for val in value.sudo())
    ...
    def _merge_opportunity_history(self, opportunities):
        ...
        for opportunity in opportunities:
            for message in opportunity.message_ids:
                message.write({"res_id": self.id, "subject": _("From %s : %s") % (opportunity.name, message.subject)})
        return True
    ...
    def redirect_lead_view(self):
        ...
        return {
            ...
            "res_id": self.id,
            "view_id": False,
            "views": [(form_view.id, "form"), (tree_view.id, "tree"), (False, "calendar"), (False, "graph")],
            "type": "ir.actions.act_window",
        }
    ...
    def _notification_recipients(self, message, groups):
        ...
        if self.type == "lead":
            ...
            salesman_actions = [{"url": won_action, "title": _("Won")}, {"url": lost_action, "title": _("Lost")}]

        new_group = (
            "group_sale_salesman",
            lambda partner: bool(partner.user_ids)
            and any(user.has_group("sales_team.group_sale_salesman") for user in partner.user_ids),
            {
    ...
    def message_get_reply_to(self, res_ids, default=None):
        ...
        aliases = self.env["crm.team"].message_get_reply_to(
            leads.mapped(lambda x: x.team_id.id or None), default=default
        )
    ...
    def get_formview_id(self, access_uid=None):
        if self.type == "opportunity":
            ...
            view_id = super().get_formview_id()
        return view_id

    @api.multi
    def message_get_default_recipients(self):
        return {r.id: {"partner_ids": [], "email_to": r.email_from, "email_cc": False} for r in self.sudo()}

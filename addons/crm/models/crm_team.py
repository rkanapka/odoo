...
class Team(models.Model):
    ...
    use_opportunities = fields.Boolean(
        "Pipeline", help="Check this box to manage a presales process with opportunities."
    )
    ...
    unassigned_leads_count = fields.Integer(
        compute="_compute_unassigned_leads_count", string="Unassigned Leads", readonly=True
    )
    opportunities_count = fields.Integer(
        compute="_compute_opportunities", string="Number of open opportunities", readonly=True
    )
    opportunities_amount = fields.Integer(
        compute="_compute_opportunities", string="Amount of quotations to invoice", readonly=True
    )
    ...
    def _onchange_team_type(self):
        if self.team_type == "sales":
            ...
            self.use_leads = False
        return super()._onchange_team_type()
    ...
    def _check_graph_model(self):
        if not self.use_opportunities and self.dashboard_graph_model == "crm.opportunity.report":
            raise ValidationError(
                _(
                    "Dashboard graph content cannot be Pipeline if the sales channel doesn't use it. (Pipeline is unchecked.)"
                )
            )
    ...
    as a member of one of the sales channel.
            ...
            if user_team_id:
                action[
                    "help"
                ] += "<p>As you don't belong to any sales channel, Odoo opens the first one by default.</p>"
    ...
    def _graph_get_dates(self, today):
        ...
        return super()._graph_get_dates(today)

    def _get_graph(self):
        graph_datas = super()._get_graph()
        if self.dashboard_graph_model == "crm.opportunity.report" and self.dashboard_graph_group_pipeline == "stage":
            ...
            graph_datas[0]["values"] = sorted(
                graph_datas[0]["values"], key=lambda el: stage_data[el["label"]]["sequence"]
            )

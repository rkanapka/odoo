...
class WebsiteSaleDigitalConfirmation(WebsiteSale):
    ...
    def payment_confirmation(self, **post):
        response = super().payment_confirmation(**post)
...
class WebsiteSaleDigital(CustomerPortal):
    ...
    def portal_order_page(self, order=None, **post):
        response = super().portal_order_page(order=order, **post)
        ...
        invoiced_lines = (
            ...
            .sudo()
            .search([("invoice_id", "in", order.invoice_ids.ids), ("invoice_id.state", "=", "paid")])
        )
    ...
    def download_attachment(self, attachment_id):
        ...
        attachment = (
            ...
            .sudo()
            .search_read(
                [("id", "=", int(attachment_id))], ["name", "datas", "file_type", "res_model", "res_id", "type", "url"]
            )
        )
        ...
        # Also check for attachments in the product templates
            ...
            template_ids = (
                request.env["product.product"].sudo().browse(purchased_products).mapped("product_tmpl_id").ids
            )

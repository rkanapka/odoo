...
class SaleTimesheetController(http.Controller):
    ...
    def _prepare_plan_values(self, domain):
        ...
        for data in dashboard_data:
            ...
            # rates
            dashboard_values["rates"][billable_type] = (
                dashboard_total_hours and round(data.get("unit_amount") / dashboard_total_hours * 100, 2) or 0
            )
            dashboard_values["rates"]["total"] += (
                dashboard_total_hours and round(data.get("unit_amount") / dashboard_total_hours * 100, 2) or 0
            )
        ...
        dashboard_values["money_amount"]["invoiced"] = sum(
            [
                inv_line.currency_id.with_context(date=inv_line.invoice_id.date_invoice).compute(
                    inv_line.price_unit * inv_line.quantity, currency
                )
                for inv_line in invoice_lines.filtered(lambda line: line.invoice_id.state in ["open", "paid"])
            ]
        )
        dashboard_values["money_amount"]["to_invoice"] = sum(
            [
                sol.currency_id.compute(
                    sol.price_unit * (1 - (sol.discount or 0.0) / 100.0) * sol.qty_to_invoice, currency
                )
                for sol in so_lines
            ]
        ) + sum(
            [
                i.currency_id.with_context(date=i.invoice_id.date_invoice).compute(i.price_unit * i.quantity, currency)
                for i in invoice_lines.filtered(lambda line: line.invoice_id.state == "draft")
            ]
        )
        dashboard_values["money_amount"]["cost"] = sum(values["timesheet_lines"].mapped("amount"))
        dashboard_values["money_amount"]["total"] = sum(
            [dashboard_values["money_amount"][item] for item in dashboard_values["money_amount"].keys()]
        )
    ...
    def _plan_get_stat_button(self, timesheet_lines):
        ...
        stat_task_domain = [
            ("project_id", "in", stat_project_ids),
            "|",
            ("stage_id", "=", False),
            ("stage_id.fold", "=", False),
        ]

...
class TestSaleTimesheet(CommonTest):
    ...
    def test_timesheet_order(self):
        ...
        self.assertEqual(
            sale_order.tasks_count, 2, "Two tasks (1 per SO line) should have been created on SO confirmation"
        )
        ...
        self.assertTrue(
            float_is_zero(invoice.amount_total - so_line_ordered_task_new_project.price_unit * 3, precision_digits=2),
            "Sale: invoice generation on timesheets product is wrong",
        )
        self.assertEqual(
            sale_order.project_project_id,
            task_serv3.project_id,
    ...
    def test_timesheet_delivery(self):
        ...
        self.assertEqual(
            task_serv2.project_id, self.project_global, "Sale Timesheet: task should be created in global project"
        )
        self.assertTrue(
            task_serv2, "Sale Timesheet: on SO confirmation, a task should have been created in global project"
        )
        self.assertTrue(
            task_serv3, "Sale Timesheet: on SO confirmation, a task should have been created in a new project"
        )
        self.assertEqual(
            sale_order.invoice_status,
            "no",
        ...
        invoice = self.env["account.invoice"].browse(invoice_id)
        self.assertTrue(
            float_is_zero(invoice.amount_total - so_line_deliver_global_project.price_unit * 10.5, precision_digits=2),
            "Sale: invoice generation on timesheets product is wrong",
        )
    ...
    def test_timesheet_manual(self):
        ...
        self.assertEqual(
            timesheet1.timesheet_invoice_type, "billable_fixed", "Milestone timesheet goes in billable fixed category"
        )
        self.assertTrue(

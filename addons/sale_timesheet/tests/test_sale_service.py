...
class TestSaleService(CommonTest):
    ...
    def test_sale_service(self):
        ...
        self.assertEqual(
            sale_order.invoice_status, "no", "Sale Service: there should be nothing to invoice after validation"
        )

        # check task creation
        project = self.project_global
        task = project.task_ids.filtered(
            lambda t: t.name == "%s:%s" % (sale_order.name, self.product_delivery_timesheet2.name)
        )
        self.assertTrue(task, "Sale Service: task is not created")
        self.assertEqual(
            task.partner_id, sale_order.partner_id, "Sale Service: customer should be the same on task and on SO"
        )
        self.assertEqual(
            task.email_from,
            sale_order.partner_id.email,
        ...
        self.assertTrue(
            line.product_uom_qty == line.qty_delivered == line.qty_invoiced,
        ...
        self.assertEqual(
            sale_order.tasks_count, 2, "Adding a new service line on a confirmer SO should create a new task."
        )
    ...
    def test_task_so_line_assignation(self):
        ...
        self.assertTrue(
            all([billing_type == "non_billable" for billing_type in timesheets.mapped("timesheet_invoice_type")]),
            "Timesheet to a non billable task should be non billable too",
        )
        ...
        self.assertTrue(
            all([billing_type == "billable_time" for billing_type in timesheets.mapped("timesheet_invoice_type")]),
            "Timesheet to a billable time task should be billable",
        )

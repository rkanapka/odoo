...
class TestSaleTimesheet(TestSale):
    ...
    def test_revenue(self):
        ...
        self.assertEqual(
            timesheet1.company_currency_id,
            sale_order.currency_id,
            "Currencies should not differ (%s vs %s)"
            % (timesheet1.company_currency_id.name, sale_order.currency_id.name),
        )
        ...
        self.assertNotEqual(timesheet1.unit_amount, 10)
        ...
        self.assertNotEqual(timesheet1.unit_amount, 15)
        self.assertNotEqual(timesheet3.unit_amount, 15)
    ...
    def test_revenue_multi_currency(self):
        ...
        self.assertEqual(
            timesheet4.timesheet_invoice_type,
            "billable_fixed",
            "Billable type on task from ordered service should be 'billabe fixed'",
        )
        self.assertEqual(timesheet1.timesheet_invoice_id, self.env["account.invoice"])
        ...
        # check concrete revenue
        self.assertEqual(
            float_repr(timesheet1.timesheet_revenue, precision_digits=2),
            "385.85",
            "Revenue computation on invoice validation does not return the correct revenue !",
        )
        self.assertEqual(
            float_repr(timesheet2.timesheet_revenue, precision_digits=2),
            "154.35",
            "Revenue computation on invoice validation does not return the correct revenue !",
        )
        self.assertEqual(
            float_repr(timesheet3.timesheet_revenue, precision_digits=2),
            "114.50",
            "Revenue computation on invoice validation does not return the correct revenue !",
        )
        self.assertEqual(
            float_repr(timesheet4.timesheet_revenue, precision_digits=2),
            "152.68",
            "Revenue computation on invoice validation does not return the correct revenue !",
        )

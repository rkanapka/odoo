...
class Project(models.Model):
    ...
    sale_line_id = fields.Many2one(
        ...
        "Sales Order Line",
        readonly=True,
        help="Sale order line from which the project has been created. Used for tracability.",
    )
...
class ProjectTask(models.Model):
    ...
    def write(self, values):
        ...
        if "sale_line_id" in values:
            ...
            self.sudo().mapped("child_ids").write({"so_line": values["sale_line_id"]})
            self.sudo().mapped("timesheet_ids").write({"so_line": values["sale_line_id"]})
        return result
    ...
    def unlink(self):
        if any(task.sale_line_id for task in self):
            raise ValidationError(
                _("You cannot delete a task related to a Sales Order. You can only archive this task.")
            )
        return super().unlink()

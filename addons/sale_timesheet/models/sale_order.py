...
class SaleOrder(models.Model):
    ...
    timesheet_count = fields.Float(
        string="Timesheet activities", compute="_compute_timesheet_ids", groups="hr_timesheet.group_hr_timesheet_user"
    )
    ...
    project_project_id = fields.Many2one(
        "project.project", compute="_compute_project_project_id", string="Project associated to this sale"
    )
        ...
        for order in self:
            if order.analytic_account_id:
                order.timesheet_ids = self.env["account.analytic.line"].search(
                    [("so_line", "in", order.order_line.ids), ("amount", "<=", 0.0), ("project_id", "!=", False)]
                )
            else:
                order.timesheet_ids = []
        ...
        if len(self.project_ids) == 1:
            if self.env.user.has_group("hr_timesheet.group_hr_timesheet_user"):
                ...
                action["context"] = safe_eval(
                    action.get("context", "{}"), {"active_id": self.project_ids.id, "active_ids": self.project_ids.ids}
                )
        else:
...
class SaleOrderLine(models.Model):
    ...
    is_service = fields.Boolean(
        ...
        store=True,
        compute_sudo=True,
        help="Sales Order item should generate a task and/or a project, depending on the product settings.",
    )
    ...
    def _timesheet_find_project(self):
        ...
        if not project:
            ...
            if not project:
                project_name = (
                    "%s (%s)" % (account.name, self.order_partner_id.ref)
                    if self.order_partner_id.ref
                    else account.name
                )
                ...
                if not project.sale_line_id and self.product_id.service_tracking in [
                    "task_new_project",
                    "project_only",
                ]:

...
class AccountInvoice(models.Model):
    ...
    timesheet_ids = fields.One2many(
        "account.analytic.line", "timesheet_invoice_id", string="Timesheets", readonly=True, copy=False
    )
    ...
    def _compute_timesheet_revenue(self):
        for invoice in self:
            for invoice_line in invoice.invoice_line_ids.filtered(
            ...
            ).sorted(key=lambda inv_line: (inv_line.invoice_id, inv_line.id)):
                ...
                if uninvoiced_timesheet_lines:
                    ...
                    if invoice_line.product_id.invoice_policy == "delivery":
                        invoiced_price_per_hour = invoice_line.currency_id.round(
                            invoice_line.price_subtotal / float(sum(uninvoiced_timesheet_lines.mapped("unit_amount")))
                        )
                        # invoicing analytic lines of different currency
                        total_revenue_per_currency = dict.fromkeys(
                            uninvoiced_timesheet_lines.mapped("company_currency_id").ids, 0.0
                        )
                        for index, timesheet_line in enumerate(
                            uninvoiced_timesheet_lines.sorted(key=lambda ts: (ts.date, ts.id))
                        ):
                            if index + 1 != len(uninvoiced_timesheet_lines):
                                line_revenue = (
                                    invoice_line.currency_id.compute(
                                        invoiced_price_per_hour, timesheet_line.company_currency_id
                                    )
                                    * timesheet_line.unit_amount
                                )
                                ...
                                total_revenue = sum(
                                    [
                                        ...
                                        .browse(currency_id)
                                        .compute(amount, timesheet_line.company_currency_id)
                                        for currency_id, amount in total_revenue_per_currency.items()
                                    ]
                                )
                                line_revenue = (
                                    invoice_line.currency_id.compute(
                                        invoice_line.price_subtotal, timesheet_line.company_currency_id
                                    )
                                    - total_revenue
                                )
                    ...
                    # ordered : update revenue with the prorata of theorical revenue
                        ...
                        zero_timesheet_revenue = uninvoiced_timesheet_lines.filtered(
                            lambda line: line.timesheet_revenue == 0.0
                        )
                        no_zero_timesheet_revenue = uninvoiced_timesheet_lines.filtered(
                            lambda line: line.timesheet_revenue != 0.0
                        )
                        ...
                        # invoicing analytic lines of different currency
                        total_revenue_per_currency = dict.fromkeys(
                            no_zero_timesheet_revenue.mapped("company_currency_id").ids, 0.0
                        )

                        for index, timesheet_line in enumerate(
                            no_zero_timesheet_revenue.sorted(key=lambda ts: (ts.date, ts.id))
                        ):
                            if index + 1 != len(no_zero_timesheet_revenue):
                                price_subtotal_inv = invoice_line.currency_id.compute(
                                    invoice_line.price_subtotal, timesheet_line.company_currency_id
                                )
                                price_subtotal_sol = timesheet_line.so_line.currency_id.compute(
                                    timesheet_line.so_line.price_subtotal, timesheet_line.company_currency_id
                                )
                                if not float_is_zero(
                                    price_subtotal_sol, precision_rounding=timesheet_line.company_currency_id.rounding
                                ):
                                    line_revenue = (
                                        timesheet_line.timesheet_revenue * price_subtotal_inv / price_subtotal_sol
                                    )
                                    total_revenue_per_currency[timesheet_line.company_currency_id.id] += line_revenue
                                else:
                                ...
                                last_price_subtotal_inv = invoice_line.currency_id.compute(
                                    invoice_line.price_subtotal, timesheet_line.company_currency_id
                                )
                                total_revenue = sum(
                                    [

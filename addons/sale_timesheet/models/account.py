...
class AccountAnalyticLine(models.Model):
    ...
    timesheet_invoice_id = fields.Many2one(
        "account.invoice", string="Invoice", readonly=True, copy=False, help="Invoice created from the timesheet"
    )
    timesheet_revenue = fields.Monetary(
        "Revenue", default=0.0, readonly=True, currency_field="company_currency_id", copy=False
    )
    ...
    def write(self, values):
        ...
        if self.sudo().filtered(lambda aal: aal.so_line.product_id.invoice_policy == "delivery") and self.filtered(
            lambda timesheet: timesheet.timesheet_invoice_id
        ):
            if any(
                [
                ...
                raise UserError(
                    _(
                        "You can not modify already invoiced timesheets (linked to a Sales order items invoiced on Time and material)."
                    )
                )
    ...
    def _timesheet_compute_theorical_revenue_values(self):
        ...
        if timesheet.task_id and so_line.product_id.type == "service":
            ...
            if so_line.product_id.invoice_policy == "delivery":
                values["timesheet_revenue"] = analytic_account.currency_id.round(
                    unit_amount * sale_price * (1 - (so_line.discount / 100))
                )
                values["timesheet_invoice_type"] = (
                    "billable_time" if so_line.product_id.service_type == "timesheet" else "billable_fixed"
                )
                ...
                total_revenue_so = analytic_account.currency_id.round(
                    quantity_hour * sale_price * (1 - (so_line.discount / 100))
                )

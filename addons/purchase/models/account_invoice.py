...
class AccountInvoice(models.Model):
    ...
    def purchase_order_change(self):
        ...
        if vendor_ref and (
            not self.reference or (vendor_ref + ", " not in self.reference and not self.reference.endswith(vendor_ref))
        ):
    ...
    def _onchange_partner_id(self):
        ...
        if (
            not self.env.context.get("default_journal_id")
            and self.partner_id
            ...
            for line in res:
                if line.get("invl_id", 0) == i_line.id and reference_account_id == line["account_id"]:
                    ...
                    if invoice_currency != currency_valuation_price_unit:
                        valuation_price_unit_invoice_currency = currency_valuation_price_unit.with_context(
                    ...
                    if (
                        acc
                        and float_compare(
                            line["price_unit"], i_line.price_unit, precision_rounding=invoice_currency.rounding
                        )
                        == 0
                        and float_compare(
                        ...
                        if line["tax_ids"]:
                            ...
                            price_unit = taxes.compute_all(price_unit, currency=invoice_currency, quantity=1.0)[
                                "total_excluded"
                            ]
                            for tax in taxes:
                        ...
                        line.update(
                            {"price": invoice_currency.round(valuation_price_unit_invoice_currency * line["quantity"])}
                        )
                        diff_res.append(
                            {
                                "type": "src",
    ...
    def write(self, vals):
        ...
        for invoice in self:
            ...
            if purchase:
                message = _("This vendor bill has been modified from: %s") % (
                    ",".join(
                        [
                            "<a href=# data-oe-model=purchase.order data-oe-id="
                            + str(order.id)

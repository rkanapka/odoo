...
class ResConfigSettings(models.TransientModel):
    ...
    lock_confirmed_po = fields.Boolean(
        "Lock Confirmed Orders", default=lambda self: self.env.user.company_id.po_lock == "lock"
    )
    po_lock = fields.Selection(related="company_id.po_lock", string="Purchase Order Modification *")
    po_order_approval = fields.Boolean(
        "Order Approval", default=lambda self: self.env.user.company_id.po_double_validation == "two_step"
    )
    ...
    po_double_validation_amount = fields.Monetary(
        related="company_id.po_double_validation_amount", string="Minimum Amount", currency_field="company_currency_id"
    )
    ...
    group_manage_vendor_price = fields.Boolean("Vendor Pricelists", implied_group="purchase.group_manage_vendor_price")
    ...
    group_analytic_account_for_purchases = fields.Boolean(
        "Analytic accounting for purchases", implied_group="purchase.group_analytic_accounting"
    )

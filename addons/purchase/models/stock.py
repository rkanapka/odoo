...
class StockPicking(models.Model):
    ...
    purchase_id = fields.Many2one(
        "purchase.order", related="move_lines.purchase_line_id.order_id", string="Purchase Orders", readonly=True
    )
...
class StockMove(models.Model):
    ...
    purchase_line_id = fields.Many2one(
        "purchase.order.line", "Purchase Order Line", ondelete="set null", index=True, readonly=True
    )
    created_purchase_line_id = fields.Many2one(
        "purchase.order.line", "Created Purchase Order Line", ondelete="set null", readonly=True, copy=False
    )
    ...
    def _get_price_unit(self):
        ...
        if self.purchase_line_id and self.product_id.id == self.purchase_line_id.product_id.id:
            ...
            if order.currency_id != order.company_id.currency_id:
                ...
                price_unit = order.currency_id.with_context(date=fields.Date.context_today(self)).compute(
                ...
                self.env["mail.activity"].sudo().create(
                    {
                        ...
                        "note": _(
                            "A sale order that generated this purchase order has been deleted. Check if an action is needed."
                        ),
...
class StockWarehouse(models.Model):
    ...
    buy_to_resupply = fields.Boolean(
        "Purchase to resupply this warehouse",
        default=True,
    ...
    def create_routes(self):
        res = super().create_routes()  # super applies ensure_one()
    ...
    def _get_all_routes(self):
        routes = super().get_all_routes_for_wh()
        routes |= (
            self.filtered(lambda self: self.buy_to_resupply and self.buy_pull_id and self.buy_pull_id.route_id)
            .mapped("buy_pull_id")
            .mapped("route_id")
        )
        return routes

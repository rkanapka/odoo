...
from odoo.addons.base.res.res_partner import WARNING_HELP, WARNING_MESSAGE
...
class PurchaseOrder(models.Model):
    ...
    @api.depends(
        "order_line.move_ids.returned_move_ids", "order_line.move_ids.state", "order_line.move_ids.picking_id"
    )
    ...
    invoice_ids = fields.Many2many(
        "account.invoice", compute="_compute_invoice", string="Bills", copy=False, store=True
    )
    ...
    picking_count = fields.Integer(
        compute="_compute_picking", string="Receptions", default=0, store=True, compute_sudo=True
    )
    picking_ids = fields.Many2many(
        "stock.picking", compute="_compute_picking", string="Receptions", copy=False, store=True, compute_sudo=True
    )
    ...
    def onchange_partner_id(self):
        if not self.partner_id:
            ...
            self.currency_id = (
                self.partner_id.property_purchase_currency_id.id or self.env.user.company_id.currency_id.id
            )
        return {}
        ...
        for line in self.order_line:
            ...
            if partner not in line.product_id.seller_ids.mapped("name") and len(line.product_id.seller_ids) <= 10:
                ...
                supplierinfo = {
                    "name": partner.id,
                    "sequence": (
                        max(line.product_id.seller_ids.mapped("sequence")) + 1 if line.product_id.seller_ids else 1
                    ),
                    "min_qty": 0.0,
                    "price": price,
...
class PurchaseOrderLine(models.Model):
    ...
    def write(self, values):
        ...
        if "date_planned" in values:
            self.env["stock.move"].search([("purchase_line_id", "in", self.ids), ("state", "!=", "done")]).write(
    ...
    product_id = fields.Many2one(
        "product.product", string="Product", domain=[("purchase_ok", "=", True)], change_default=True, required=True
    )
    ...
    move_ids = fields.One2many(
        "stock.move", "purchase_line_id", string="Reservation", readonly=True, ondelete="set null", copy=False
    )
    ...
    invoice_lines = fields.One2many(
        "account.invoice.line", "purchase_line_id", string="Bill Lines", readonly=True, copy=False
    )
    ...
    def _get_stock_move_price_unit(self):
        ...
        if order.currency_id != order.company_id.currency_id:
            price_unit = order.currency_id.with_context(date=order.date_approve).compute(
    ...
    def _onchange_quantity(self):
        ...
        price_unit = (
            self.env["account.tax"]._fix_tax_included_price_company(
                seller.price, self.product_id.supplier_taxes_id, self.taxes_id, self.company_id
            )
            if seller
            else 0.0
        )
...
class ProcurementGroup(models.Model):
    ...
    def _get_exceptions_domain(self):
        return super()._get_exceptions_domain() + [("created_purchase_line_id", "=", False)]
...
class ProcurementRule(models.Model):
    ...
    def _get_purchase_order_date(self, product_id, product_qty, product_uom, values, partner, schedule_date):
        ...
        seller = product_id.with_context(force_company=values["company_id"].id)._select_seller(
            partner_id=partner, quantity=product_qty, date=fields.Date.to_string(schedule_date), uom_id=product_uom
        )
    ...
    def _update_purchase_order_line(self, product_id, product_qty, product_uom, values, line, partner):
        ...
        price_unit = (
            self.env["account.tax"]._fix_tax_included_price_company(
                seller.price, line.product_id.supplier_taxes_id, line.taxes_id, values["company_id"]
            )
            if seller
            else 0.0
        )
    ...
    def _prepare_purchase_order_line(self, product_id, product_qty, product_uom, values, po, supplier):
        ...
        price_unit = (
            self.env["account.tax"]._fix_tax_included_price_company(
                seller.price, product_id.supplier_taxes_id, taxes_id, values["company_id"]
            )
            if seller
            else 0.0
        )
    ...
    def _prepare_purchase_order(self, product_id, product_qty, product_uom, origin, values, partner):
        ...
        fpos = (
            self.env["account.fiscal.position"]
            .with_context(force_company=values["company_id"].id)
        ...
        return {
            ...
            "currency_id": partner.with_context(force_company=values["company_id"].id).property_purchase_currency_id.id
...
class ProductTemplate(models.Model):
    ...
    purchase_line_warn = fields.Selection(
        WARNING_MESSAGE, "Purchase Order Line", help=WARNING_HELP, required=True, default="no-message"
    )

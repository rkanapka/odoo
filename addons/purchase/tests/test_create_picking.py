...
class TestCreatePicking(common.TestProductCommon):
    ...
    def test_02_check_mto_chain(self):
        ...
        self.assertEqual(
            purchase_order_line.product_id, product, "The product on the purchase order line is not correct."
        )
        self.assertEqual(
            purchase_order_line.price_unit,
            seller.price,
            "The purchase order line price should be the same than the seller.",
        )
        self.assertEqual(
            purchase_order_line.product_qty,
            customer_move.product_uom_qty,
            "The purchase order line qty should be the same than the move.",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(product, stock_location), 100.0, "Wrong quantity in stock."
        )

        customer_move._action_assign()
        self.assertEqual(
            customer_move.state, "assigned", "Reservation should work with the new quantity provided by the PO."
        )
    ...
    def test_04_mto_multiple_po(self):
        ...
        self.assertEqual(
            sum(customer_picking.move_lines.mapped("reserved_availability")),
            100.0,

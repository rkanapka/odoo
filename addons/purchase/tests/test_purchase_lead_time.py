...
class TestPurchaseLeadTime(TestPurchase):
    ...
    def test_00_product_company_level_delays(self):
        ...
        order_date = (
            fields.Datetime.from_string(date_planned)
            - timedelta(days=company.po_lead)
            - timedelta(days=self.product_1.seller_ids.delay)
        )
    ...
    def test_01_product_level_delay(self):
        ...
        self.assertEqual(
            purchase1, purchase2, "Purchase orders should be same for the two different product with same vendor."
        )
    ...
    def test_02_product_route_level_delays(self):
        ...
        # Check scheduled date of In Type shipment
        incoming_shipment = purchase.picking_ids.filtered(
            lambda r: r.picking_type_id == self.warehouse_1.in_type_id
            and r.location_dest_id == self.warehouse_1.wh_input_stock_loc_id
        )
        self.assertEqual(
            incoming_shipment.scheduled_date,
            po_schedule_date,
            "Schedule date of In type shipment should be same as schedule date of purchase order.",
        )
        ...
        incoming_shipment1 = self.env["stock.picking"].search(
            [
                ("move_lines.product_id", "in", (self.product_1.id, self.product_2.id)),
        ...
        self.assertEqual(
            ...
            incoming_shipment1_schedule_date,
        ...
        incoming_shipment2_date = incoming_shipment1_date + timedelta(
            days=incoming_shipment2.move_lines[0].push_rule_id.delay
        )
        incoming_shipment2_schedule_date = fields.Datetime.to_string(incoming_shipment2_date)
        self.assertEqual(
            incoming_shipment2.scheduled_date,
            incoming_shipment2_schedule_date,

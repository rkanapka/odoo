...
class TestPurchaseOrder(AccountingTestCase):
    ...
    def test_00_purchase_order_flow(self):
        ...
        if self.partner_id in self.product_id_2.seller_ids.mapped("name"):
            id_to_remove = (
                self.product_id_2.seller_ids.filtered(lambda r: r.name == self.partner_id).ids[0]
                if self.product_id_2.seller_ids.filtered(lambda r: r.name == self.partner_id)
                else False
            )
            if id_to_remove:
        ...
        self.assertFalse(
            self.product_id_2.seller_ids.filtered(lambda r: r.name == self.partner_id),
        ...
        self.assertEqual(
            self.po.order_line.mapped("qty_received"), [0.0, 0.0], 'Purchase: no product should be received"'
        )
        self.assertEqual(
            self.po.order_line.mapped("qty_invoiced"), [0.0, 0.0], 'Purchase: no product should be invoiced"'
        )

        self.po.button_confirm()
        self.assertEqual(self.po.state, "purchase", 'Purchase: PO state should be "Purchase"')
        self.assertEqual(
            self.po.invoice_status, "to invoice", 'Purchase: PO invoice_status should be "Waiting Invoices"'
        )
        ...
        seller = self.product_id_2._select_seller(
            partner_id=self.partner_id, quantity=2.0, date=self.po.date_planned, uom_id=self.product_id_2.uom_po_id
        )
        ...
        self.assertEqual(
            self.po.order_line.mapped("qty_received"), [5.0, 5.0], 'Purchase: all products should be received"'
        )

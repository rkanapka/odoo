...
class AccountAnalyticLine(models.Model):
    ...
    def _sale_postprocess(self, values, additional_so_lines=None):
        if "so_line" not in values:  # allow to force a False value for so_line
            ...
            self.filtered(lambda aal: aal.amount <= 0).with_context(
                sale_analytic_norecompute=True
    ...
    def _sale_determine_order(self):
        ...
        for analytic_line in self.sudo().filtered(
            ...
            sale_order = self.env["sale.order"].search(
                [("analytic_account_id", "=", analytic_line.account_id.id), ("state", "=", "sale")], limit=1
            )
            if not sale_order:
                sale_order = self.env["sale.order"].search(
                    [("analytic_account_id", "=", analytic_line.account_id.id)], limit=1
                )
            if not sale_order:
                continue
    ...
    def _sale_determine_order_line(self):
        ...
        for analytic_line in self.sudo().filtered(
            ...
            if (
                so_line
            ):  # if so line found or created, then update AAL (this will trigger the recomputation of qty delivered on SO line)

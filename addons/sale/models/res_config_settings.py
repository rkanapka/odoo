...
class ResConfigSettings(models.TransientModel):
    ...
    use_sale_note = fields.Boolean(string="Default Terms & Conditions", oldname="default_use_sale_note")
    ...
    portal_confirmation_options = fields.Selection(
        [("sign", "Signature"), ("pay", "Payment")], string="Online Signature & Payment options"
    )
    module_sale_payment = fields.Boolean(
        "Online Signature & Payment", help="Technical field implied by user choice of online_confirmation"
    )
    ...
    def get_values(self):
        ...
        res.update(
            auth_signup_uninvited=(
                "b2c" if ICPSudo.get_param("auth_signup.allow_uninvited", "False").lower() == "true" else "b2b"
            ),
            ...
            multi_sales_price_method=sale_pricelist_setting in ["percentage", "formula"]
            and sale_pricelist_setting
            or False,
            ...
            portal_confirmation_options=(
                sale_portal_confirmation_options if sale_portal_confirmation_options in ("pay", "sign") else False
            ),
        )
        return res
    ...
    def set_values(self):
        ...
        ICPSudo.set_param(
            ...
            self.portal_confirmation_options if self.portal_confirmation_options in ("pay", "sign") else "none",
        )

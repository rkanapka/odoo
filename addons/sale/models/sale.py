...
class SaleOrder(models.Model):
        ...
        for order in self:
            ...
            if domain_inv:
                refund_ids = self.env["account.invoice"].search(
                    expression.AND([["&", ("type", "=", "out_refund"), ("origin", "!=", False)], domain_inv])
                )
            else:
    ...
    create_date = fields.Datetime(
        string="Creation Date", readonly=True, index=True, help="Date on which sales order is created."
    )
    ...
    user_id = fields.Many2one(
        "res.users", string="Salesperson", index=True, track_visibility="onchange", default=lambda self: self.env.user
    )
    ...
    invoice_ids = fields.Many2many(
        "account.invoice", string="Invoices", compute="_get_invoiced", readonly=True, copy=False
    )
    ...
    amount_untaxed = fields.Monetary(
        string="Untaxed Amount", store=True, readonly=True, compute="_amount_all", track_visibility="onchange"
    )
    ...
    company_id = fields.Many2one(
        "res.company", "Company", default=lambda self: self.env["res.company"]._company_default_get("sale.order")
    )
    team_id = fields.Many2one(
        "crm.team", "Sales Channel", change_default=True, default=_get_default_team, oldname="section_id"
    )
        ...
        if self._context.get("sale_show_partner_name"):
            if operator in ("ilike", "like", "=", "=like", "=ilike"):
                domain = expression.AND(
                    [args or [], ["|", ("name", operator, name), ("partner_id.name", operator, name)]]
                )
    ...
    def action_invoice_create(self, grouped=False, final=False):
        ...
        for group_key in invoices:
            invoices[group_key].write(
                {"name": ", ".join(invoices_name[group_key]), "origin": ", ".join(invoices_origin[group_key])}
            )
        ...
        for order in self:
            if any(
                [
                    ...
                    for expense_policy in order.order_line.mapped("product_id.expense_policy")
                ]
            ):
    ...
    def _get_tax_amount_by_group(self):
        ...
        for line in self.order_line:
            ...
            taxes = line.tax_id.compute_all(
                price_reduce, quantity=line.product_uom_qty, product=line.product_id, partner=self.partner_shipping_id
            )["taxes"]
    ...
    def get_portal_confirmation_action(self):
        return (
            self.env["ir.config_parameter"].sudo().get_param("sale.sale_portal_confirmation_options", default="none")
        )
...
class SaleOrderLine(models.Model):
    ...
    def _compute_qty_delivered_updateable(self):
        for line in self:
            line.qty_delivered_updateable = (
                (line.order_id.state == "sale")
                and (line.product_id.service_type == "manual")
    ...
    def _compute_invoice_amount(self):
        ...
        for line in self:
            ...
            refund_invoice_lines = (
                refund_invoices.mapped("invoice_line_ids") + refund_lines - refund_lines_product
            ...
            if (
                line.invoice_lines
                and line.invoice_lines[0].currency_id
                and line.invoice_lines[0].currency_id != line.currency_id
            ):
                invoiced_amount_total = sum(
                    [
                        inv_line.currency_id.with_context({"date": inv_line.invoice_id.date}).compute(
    ...
    def write(self, values):
        if "product_uom_qty" in values:
            ...
            self.filtered(
                ...
                and float_compare(r.product_uom_qty, values["product_uom_qty"], precision_digits=precision) != 0
    ...
    order_id = fields.Many2one(
        "sale.order", string="Order Reference", required=True, ondelete="cascade", index=True, copy=False
    )
    ...
    product_updatable = fields.Boolean(
        compute="_compute_product_updatable", string="Can Edit Product", readonly=True, default=True
    )
    ...
    qty_delivered = fields.Float(
        string="Delivered", copy=False, digits=dp.get_precision("Product Unit of Measure"), default=0.0
    )
    ...
    amt_to_invoice = fields.Monetary(
        string="Amount To Invoice", compute="_compute_invoice_amount", compute_sudo=True, store=True
    )
    amt_invoiced = fields.Monetary(
        string="Amount Invoiced", compute="_compute_invoice_amount", compute_sudo=True, store=True
    )
    ...
    def _get_display_price(self, product):
        ...
        base_price, currency_id = self.with_context(product_context)._get_real_price_currency(
            product, rule_id, self.product_uom_qty, self.product_uom, self.order_id.pricelist_id.id
        )
    ...
    def name_search(self, name="", args=None, operator="ilike", limit=100):
        if operator in ("ilike", "like", "=", "=like", "=ilike"):
            args = expression.AND([args or [], ["|", ("order_id.name", operator, name), ("name", operator, name)]])
        return super().name_search(name, args, operator, limit)
    ...
    def _get_protected_fields(self):
        return ["product_id", "name", "price_unit", "product_uom", "product_uom_qty", "tax_id", "analytic_tag_ids"]
    ...
    def _onchange_discount(self):
        ...
        new_list_price, currency_id = self.with_context(product_context)._get_real_price_currency(
            product, rule_id, self.product_uom_qty, self.product_uom, self.order_id.pricelist_id.id
        )
        ...
        if new_list_price != 0:
            if self.order_id.pricelist_id.currency_id.id != currency_id:
                ...
                new_list_price = (
                    ...
                    .browse(currency_id)
                    .with_context(product_context)
                    .compute(new_list_price, self.order_id.pricelist_id.currency_id)
                )
    ...
    def _analytic_compute_delivered_quantity(self):
        ...
        data = self.env["account.analytic.line"].read_group(
            domain, ["so_line", "unit_amount", "product_uom_id"], ["product_uom_id", "so_line"], lazy=False
        )

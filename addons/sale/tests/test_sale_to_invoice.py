...
class TestSale(TestMail):
    ...
    def test_sale_to_invoice(self):
        ...
        account_rev_id = account_obj.create(
            {"code": "X2020", "name": "Sales - Test Sales Account", "user_type_id": user_type_id, "reconcile": True}
        )
        user_type_id = IrModelData.xmlid_to_res_id("account.data_account_type_receivable")
        account_recv_id = account_obj.create(
            {"code": "X1012", "name": "Sales - Test Reicv Account", "user_type_id": user_type_id, "reconcile": True}
        )
        ...
        user = self.env["res.users"].create(
            {"name": "Test User", "login": "test@test.com", "company_id": 1, "groups_id": [(6, 0, [group_id])]}
        )

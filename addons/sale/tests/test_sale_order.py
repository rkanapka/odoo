...
class TestSaleOrder(TestSale):
    def test_sale_order(self):
        ...
        self.assertEqual(
            so.amount_total, sum([2 * p.list_price for p in self.products.values()]), "Sale: total amount is wrong"
        )
        ...
        self.assertEqual(
            inv.amount_total,
            sum([2 * p.list_price if p.invoice_policy == "order" else 0 for p in self.products.values()]),
        ...
        self.assertEqual(
            inv.amount_total,
            sum([2 * p.list_price if p.invoice_policy == "delivery" else 0 for p in self.products.values()]),
            "Sale: second invoice total amount is wrong",
        )
        self.assertTrue(
            so.invoice_status == "invoiced", 'Sale: SO status after invoicing everything should be "invoiced"'
        )
        ...
        self.assertEqual(
            inv.amount_total, 8 * self.products["serv_order"].list_price, "Sale: second invoice total amount is wrong"
        )
        self.assertTrue(
    ...
    def test_so_create_multicompany(self):
        ...
        self.assertEqual(
            set(so_1.order_line.tax_id.ids),
            {tax_company_1.id},
    ...
    def test_multi_currency_discount(self):
        ...
        other_company = self.env["res.company"].create({"name": "Test", "currency_id": curr_eur.id})

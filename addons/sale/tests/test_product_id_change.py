...
class TestOnchangeProductId(TransactionCase):
    ...
    def test_onchange_product_id(self):
        ...
        tax_include_id = self.tax_model.create(
            dict(name="Include tax", amount="21.00", price_include=True, type_tax_use="sale")
        )
        tax_exclude_id = self.tax_model.create(dict(name="Exclude tax", amount="0.00", type_tax_use="sale"))

        product_tmpl_id = self.product_tmpl_model.create(
            dict(name="Voiture", list_price=121, taxes_id=[(6, 0, [tax_include_id.id])])
        )
    ...
    def test_pricelist_with_other_currency(self):
        ...
        partner = self.res_partner_model.create(dict(name="George"))
        categ_unit_id = self.ref("product.product_uom_unit")
        other_currency = self.env["res.currency"].create({"name": "other currency", "symbol": "other"})
        ...
        self.env["res.currency.rate"].search([("currency_id", "=", self.env.user.company_id.currency_id.id)]).unlink()
        new_uom = self.env["product.uom"].create(
            {"name": "10 units", "factor_inv": 10, "uom_type": "bigger", "rounding": 1.0, "category_id": categ_unit_id}
        )

...
class PurchaseRequisition(models.Model):
    ...
    name = fields.Char(
        ...
        required=True,
        copy=False,
        default=lambda self: self.env["ir.sequence"].next_by_code("purchase.order.requisition"),
    )
    ...
    purchase_ids = fields.One2many(
        "purchase.order", "requisition_id", string="Purchase Orders", states={"done": [("readonly", True)]}
    )
    ...
    def action_done(self):
        ...
        """
        if any(
            purchase_order.state in ["draft", "sent", "to approve"] for purchase_order in self.mapped("purchase_ids")
        ):
...
class PurchaseRequisitionLine(models.Model):
    ...
    company_id = fields.Many2one(
        ...
        store=True,
        readonly=True,
        default=lambda self: self.env["res.company"]._company_default_get("purchase.requisition.line"),
    )
...
class ProductTemplate(models.Model):
    ...
    purchase_requisition = fields.Selection(
        [("rfq", "Create a draft purchase order"), ("tenders", "Propose a call for tenders")],
        string="Procurement",
        default="rfq",
    )
...
class ProcurementGroup(models.Model):
    ...
    def _get_exceptions_domain(self):
        return super()._get_exceptions_domain() + [("requistion_line_ids", "=", False)]

...
class CrmTeam(models.Model):
    ...
    def _get_default_team_id(self, user_id=None):
        ...
        if not team_id:
            ...
            if default_team_id:
                ...
                if (
                    self.env.context.get("default_type") != "lead"
                    or default_team_id.use_leads
                    and default_team_id.active
                ):
    ...
    active = fields.Boolean(
        default=True,
        help="If the active field is set to false, it will allow you to hide the sales channel without removing it.",
    )
    company_id = fields.Many2one(
        "res.company", string="Company", default=lambda self: self.env["res.company"]._company_default_get("crm.team")
    )
    ...
    dashboard_graph_model = fields.Selection(
        [], string="Content", help="The graph this channel will display in the Dashboard.\n"
    )
    ...
    def _compute_dashboard_graph(self):
        for team in self.filtered("dashboard_graph_model"):
            if (
                team.dashboard_graph_group in (False, "user")
                or team.dashboard_graph_period == "week"
    ...
    def _get_graph(self):
        ...
        if self.dashboard_graph_group == "day":
            ...
            for data_item in graph_data:
                values.append(
                    {
                        x_field: self.env["res.users"].browse(data_item.get("x_value")).name or _("Not Defined"),

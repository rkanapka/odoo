...
class GoogleDrive(models.Model):
  ...
  <entry>
    ...
    <gs:cell row="60" col="15" inputValue="{config}"/>
            ...
            key=spreadsheet_key, formula=misc.html_escape(formula), config=misc.html_escape(config_formula)
        )

        try:
            req = requests.post(
                "https://spreadsheets.google.com/feeds/cells/%s/od6/private/full/batch?%s"
                % (spreadsheet_key, werkzeug.url_encode({"v": 3, "access_token": access_token})),
                data=request,
    ...
    def set_spreadsheet(self, model, domain, groupbys, view_id):
        try:
            config_id = self.env["ir.model.data"].get_object_reference(
                "google_spreadsheet", "google_spreadsheet_template"
            )[1]
        except ValueError:
            raise

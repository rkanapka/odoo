...
class WebsiteLivechat(http.Controller):
    ...
    def channel_rating(self, channel, **kw):
        ...
        for grade in repartition:
            percentage[grade] = (
                round(repartition[grade] * 100.0 / sum(repartition.values()), 1) if sum(repartition.values()) else 0
            )

        # the value dict to render the template
        values = {"channel": channel, "ratings": ratings, "team": channel.sudo().user_ids, "percentage": percentage}

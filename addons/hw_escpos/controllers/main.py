...
class EscposDriver(Thread):
    ...
    def connected_usb_devices(self):
        ...
        for printer in printers:
            try:
                description = (
                    usb.util.get_string(printer, 256, printer.iManufacturer)
                    + " "
                    + usb.util.get_string(printer, 256, printer.iProduct)
                )
            except Exception as e:
                _logger.error("Can not get printer description: %s" % e)
                description = "Unknown printer"
            connected.append({"vendor": printer.idVendor, "product": printer.idProduct, "name": description})
    ...
    def print_status(self, eprint):
        ...
        ssid = (
            subprocess.check_output("iwconfig 2>&1 | grep 'ESSID:\"' | sed 's/.*\"\\(.*\\)\"/\\1/'", shell=True)
            .decode("utf-8")
            .rstrip()
        )
        mac = get_mac()
        ...
        for line in receipt["orderlines"]:
            ...
            if line["discount"] == 0 and line["unit_name"] == "Unit(s)" and line["quantity"] == 1:
                ...
                if line["unit_name"] == "Unit(s)":
                    eprint.text(
                        printline(
                            quantity(line["quantity"]) + " x " + price(line["price"]), pricestr, ratio=0.6, indent=2
                        )
                    )
                else:
                    eprint.text(
                        printline(
                            quantity(line["quantity"]) + line["unit_name"] + " x " + price(line["price"]),
                            pricestr,
                            ratio=0.6,
                            indent=2,
                        )

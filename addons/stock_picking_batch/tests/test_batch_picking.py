...
class TestBatchPicking(TransactionCase):
    ...
    def setUp(self):
        ...
        self.batch = self.env["stock.picking.batch"].create(
            {"name": "Batch 1", "picking_ids": [(4, self.picking_client_1.id), (4, self.picking_client_2.id)]}
        )
    ...
    def test_simple_batch_with_wizard(self):
        ...
        immediate_transfer_wizard = self.env[(immediate_transfer_wizard_dict.get("res_model"))].browse(
            immediate_transfer_wizard_dict.get("res_id")
        )
    ...
    def test_batch_with_backorder_wizard(self):
        ...
        back_order_wizard = self.env[(back_order_wizard_dict.get("res_model"))].browse(
            back_order_wizard_dict.get("res_id")
        )

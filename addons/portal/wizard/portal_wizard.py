...
class PortalWizard(models.TransientModel):
    ...
    welcome_message = fields.Text(
        "Invitation Message", help="This text is included in the email sent to new users of the portal."
    )
    ...
    def get_error_messages(self):
        ...
        if partners_error_empty:
            error_msg.append(
                "%s\n- %s"
                % (
                    _("Some contacts don't have a valid email: "),
        ...
        if partners_error_emails:
            error_msg.append(
                "%s\n- %s"
                % (_("Several contacts have the same email: "), "\n- ".join(partners_error_emails.mapped("email")))
            )
        if partners_error_user:
            error_msg.append(
                "%s\n- %s"
                % (
                    _("Some contacts have the same email as an existing portal user:"),
        ...
        if partners_error_internal_user:
            error_msg.append(
                "%s\n- %s"
                % (
                    _("Some contacts are already internal users:"),

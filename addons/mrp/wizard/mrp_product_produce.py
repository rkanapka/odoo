    ...
    def default_get(self, fields):
        ...
        if self._context and self._context.get("active_id"):
            ...
            if "produce_line_ids" in fields:
                qty_by_bom = production.product_uom_id._compute_quantity(
                    todo_quantity, production.bom_id.product_uom_id
                )
                lines = []
                for move in production.move_raw_ids.filtered(
                    lambda x: (x.product_id.tracking != "none") and x.state not in ("done", "cancel") and x.bom_line_id
                ):
                    qty_to_consume = float_round(
                        qty_by_bom / move.bom_line_id.bom_id.product_qty * move.bom_line_id.product_qty,
    ...
    def check_finished_move_lots(self):
        ...
        if produce_move and produce_move.product_id.tracking != "none":
            ...
            if existing_move_line:
                ...
                location_dest_id = (
                    produce_move.location_dest_id.get_putaway_strategy(self.product_id).id

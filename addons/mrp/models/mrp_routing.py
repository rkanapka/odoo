...
class MrpRouting(models.Model):
    ...
    operation_ids = fields.One2many(
        "mrp.routing.workcenter", "routing_id", "Operations", copy=True, oldname="workcenter_lines"
    )
    ...
    company_id = fields.Many2one(
        "res.company", "Company", default=lambda self: self.env["res.company"]._company_default_get("mrp.routing")
    )
...
class MrpRoutingWorkcenter(models.Model):
    ...
    sequence = fields.Integer(
        "Sequence", default=100, help="Gives the sequence order when displaying a list of routing Work Centers."
    )
    ...
    batch = fields.Selection(
        [("no", "Once all products are processed"), ("yes", "Once a minimum number of products is processed")],
    ...
    def _compute_time_cycle(self):
        ...
        for operation in self - manual_ops:
            data = self.env["mrp.workorder"].read_group(
                [("operation_id", "=", operation.id), ("state", "=", "done")],
                ["operation_id", "duration", "qty_produced"],
                ["operation_id"],
                limit=operation.time_mode_batch,
            )
            count_data = {item["operation_id"][0]: (item["duration"], item["qty_produced"]) for item in data}
            if count_data.get(operation.id) and count_data[operation.id][1]:
                operation.time_cycle = (count_data[operation.id][0] / count_data[operation.id][1]) * (
                    operation.workcenter_id.capacity or 1.0
                )
            else:
                operation.time_cycle = operation.time_cycle_manual

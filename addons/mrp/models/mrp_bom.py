...
class MrpBom(models.Model):
    ...
    type = fields.Selection(
        [("normal", "Manufacture this product"), ("phantom", "Kit")], "BoM Type", default="normal", required=True
    )
    product_tmpl_id = fields.Many2one(
        "product.template", "Product", domain="[('type', 'in', ['product', 'consu'])]", required=True
    )
    ...
    ready_to_produce = fields.Selection(
        [("all_available", "All components available"), ("asap", "The components of 1st operation")],
    ...
    def name_get(self):
        return [
            (bom.id, "%s%s" % (bom.code and "%s: " % bom.code or "", bom.product_tmpl_id.display_name)) for bom in self
        ]
...
class MrpBomLine(models.Model):
    ...
    product_qty = fields.Float(
        "Product Quantity", default=1.0, digits=dp.get_precision("Product Unit of Measure"), required=True
    )
    product_uom_id = fields.Many2one(
        "product.uom",
        "Product Unit of Measure",
    ...
    attribute_value_ids = fields.Many2many(
        "product.attribute.value", string="Variants", help="BOM Product Variants needed form apply this line."
    )

...
class ProcurementRule(models.Model):
    ...
    def _run_manufacture(self, product_id, product_qty, product_uom, location_id, name, origin, values):
        ...
        origin_production = (
            values.get("move_dest_ids") and values["move_dest_ids"][0].raw_material_production_id or False
        )
    ...
    def _get_matching_bom(self, product_id, values):
        ...
        return (
            self.env["mrp.bom"]
            .with_context(company_id=values["company_id"].id, force_company=values["company_id"].id)
            ._bom_find(product=product_id, picking_type=self.picking_type_id)

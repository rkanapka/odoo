...
class MrpWorkorder(models.Model):
    ...
    name = fields.Char(
        "Work Order", required=True, states={"done": [("readonly", True)], "cancel": [("readonly", True)]}
    )
    ...
    working_state = fields.Selection(
        "Workcenter Status", related="workcenter_id.working_state", help="Technical: used in views only"
    )
    ...
    production_state = fields.Selection(
        "Production State", readonly=True, related="production_id.state", help="Technical: used in views only."
    )
    product_tracking = fields.Selection(
        "Product Tracking", related="production_id.product_id.tracking", help="Technical: used in views only."
    )
    qty_production = fields.Float("Original Production Quantity", readonly=True, related="production_id.product_qty")
    qty_remaining = fields.Float(
        "Quantity To Be Produced", compute="_compute_qty_remaining", digits=dp.get_precision("Product Unit of Measure")
    )
    ...
    date_planned_start = fields.Datetime(
        "Scheduled Date Start", states={"done": [("readonly", True)], "cancel": [("readonly", True)]}
    )
    date_planned_finished = fields.Datetime(
        "Scheduled Date Finished", states={"done": [("readonly", True)], "cancel": [("readonly", True)]}
    )
    date_start = fields.Datetime(
        "Effective Start Date", states={"done": [("readonly", True)], "cancel": [("readonly", True)]}
    )
    date_finished = fields.Datetime(
        "Effective End Date", states={"done": [("readonly", True)], "cancel": [("readonly", True)]}
    )
    ...
    duration_percent = fields.Integer(
        "Duration Deviation (%)", compute="_compute_duration", group_operator="avg", readonly=True, store=True
    )
    ...
    def _compute_is_produced(self):
        ...
        self.is_produced = (
            float_compare(self.qty_produced, self.production_id.product_qty, precision_rounding=rounding) >= 0
        )
    ...
    def _compute_color(self):
        late_orders = self.filtered(
            lambda x: x.production_id.date_planned_finished
    ...
    def _onchange_qty_producing(self):
        ...
        moves = self.move_raw_ids.filtered(
            ...
            and move.product_id.id != self.production_id.product_id.id
        )
        for move in moves:
            ...
            if move.product_id.tracking == "lot":
                ...
                if float_compare(qty_todo, 0.0, precision_rounding=rounding) > 0:
                    ...
                    for move_lot in move_lots:
                        ...
                        if (
                            not move_lot.lot_id
                            and float_compare(qty_todo, move_lot.qty_done, precision_rounding=rounding) >= 0
                        ):
    ...
    def _generate_lot_ids(self):
        ...
        tracked_moves = self.move_raw_ids.filtered(
            lambda move: move.state not in ("done", "cancel")
            and move.product_id.tracking != "none"
            and move.product_id != self.production_id.product_id
    ...
    def _assign_default_final_lot_id(self):
        self.final_lot_id = self.env["stock.production.lot"].search(
            [("use_next_on_work_order_id", "=", self.id)], order="create_date, id", limit=1
        )
    ...
    def record_production(self):
        ...
        for move in self.move_raw_ids:
            if (
                ...
                and move.bom_line_id
                and move.unit_factor
                and not move.move_line_ids.filtered(lambda ml: not ml.done_wo)
            ):
                rounding = move.product_uom.rounding
        ...
        for move_line in self.active_move_line_ids:
            ...
            lots = self.move_line_ids.filtered(
                lambda x: (x.lot_id.id == move_line.lot_id.id)
                and (not x.lot_produced_id)
                and (not x.done_move)
                and (x.product_id == move_line.product_id)
            )
            if lots:
        ...
        ).write({"lot_produced_id": self.final_lot_id.id, "lot_produced_qty": self.qty_producing})
        ...
        if not self.next_work_order_id:
            ...
            if production_move.product_id.tracking != "none":
                ...
                if move_line:
                    ...
                    location_dest_id = (
                        production_move.location_dest_id.get_putaway_strategy(self.product_id).id
                        or production_move.location_dest_id.id
                    )
                    move_line.create(
                        {
            ...
            for by_product_move in self.production_id.move_finished_ids.filtered(
                ...
                if by_product_move.has_tracking != "serial":
                    values = self._get_byproduct_move_line(
                        by_product_move, self.qty_producing * by_product_move.unit_factor
                    )
        ...
        if float_compare(self.qty_produced, self.production_id.product_qty, precision_rounding=rounding) >= 0:
            ...
            self.qty_producing = float_round(
                self.production_id.product_qty - self.qty_produced, precision_rounding=rounding
            )
            self._generate_lot_ids()

        if (
            self.next_work_order_id
            and self.next_work_order_id.state not in ["done", "cancel"]

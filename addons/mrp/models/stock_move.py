    ...
    def write(self, vals):
        for move_line in self:
            if move_line.move_id.production_id and "lot_id" in vals:
                move_line.production_id.move_raw_ids.mapped("move_line_ids").filtered(
                    lambda r: r.done_wo and not r.done_move and r.lot_produced_id == move_line.lot_id
...
class StockMove(models.Model):
    ...
    production_id = fields.Many2one("mrp.production", "Production Order for finished products")
    raw_material_production_id = fields.Many2one("mrp.production", "Production Order for raw materials")
    unbuild_id = fields.Many2one("mrp.unbuild", "Disassembly Order")
    consume_unbuild_id = fields.Many2one("mrp.unbuild", "Consumed Disassembly Order")
    operation_id = fields.Many2one("mrp.routing.workcenter", "Operation To Consume")  # TDE FIXME: naming
    workorder_id = fields.Many2one("mrp.workorder", "Work Order To Consume")
    # Quantities to process, in normalized UoMs
    active_move_line_ids = fields.One2many(
        "stock.move.line", "move_id", domain=[("done_wo", "=", True)], string="Lots"
    )
    bom_line_id = fields.Many2one("mrp.bom.line", "BoM Line")
    unit_factor = fields.Float("Unit Factor")
    is_done = fields.Boolean("Done", compute="_compute_is_done", store=True, help="Technical Field to order moves")
    ...
    def _unreserve_initial_demand(self, new_move):
        ...
        self.filtered(lambda m: m.production_id or m.raw_material_production_id).mapped("move_line_ids").filtered(
            lambda ml: ml.qty_done == 0.0
    ...
    def _action_cancel(self):
        if any(move.quantity_done and (move.raw_material_production_id or move.production_id) for move in self):
            raise exceptions.UserError(
                _(
                    "You cannot cancel a manufacturing order if you have already consumed material.\
    ...
    def _generate_consumed_move_line(self, qty_to_add, final_lot, lot=False):
        ...
        if (
            ...
            and lot in self.move_line_ids.filtered(lambda ml: ml.qty_done).mapped("lot_id")
        ):
            raise UserError(
        ...
        for ml in move_lines:
            ...
            if (
                float_compare(
                    ml.product_uom_id._compute_quantity(new_quantity_done, ml.product_id.uom_id),
                    ml.product_qty,

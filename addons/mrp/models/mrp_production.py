...
class MrpProduction(models.Model):
    ...
    def _get_default_location_src_id(self):
        ...
        if self._context.get("default_picking_type_id"):
            location = (
                self.env["stock.picking.type"]
                .browse(self.env.context["default_picking_type_id"])
    ...
    origin = fields.Char(
        "Source", copy=False, help="Reference of the document that generated this production order request."
    )
    ...
    product_tmpl_id = fields.Many2one(
        "product.template", "Product Template", related="product_id.product_tmpl_id", readonly=True
    )
    ...
    picking_type_id = fields.Many2one(
        "stock.picking.type", "Operation Type", default=_get_default_picking_type, required=True
    )
    ...
    workorder_ids = fields.One2many(
        "mrp.workorder", "production_id", "Work Orders", copy=False, oldname="workcenter_lines", readonly=True
    )
    ...
    post_visible = fields.Boolean(
        "Allowed to Post Inventory", compute="_compute_post_visible", help="Technical field to check when we can post"
    )
    ...
    production_location_id = fields.Many2one(
        "stock.location", "Production Location", related="product_id.property_stock_production"
    )
    ...
    def _compute_workorder_done_count(self):
        data = self.env["mrp.workorder"].read_group(
            [("production_id", "in", self.ids), ("state", "=", "done")], ["production_id"], ["production_id"]
        )
        count_data = {item["production_id"][0]: item["production_id_count"] for item in data}
        for production in self:
    ...
    def _compute_availability(self):
        for order in self:
            ...
            if order.bom_id.ready_to_produce == "all_available":
                order.availability = (
                    any(move.state not in ("assigned", "done", "cancel") for move in order.move_raw_ids)
                    and "waiting"
                    or "assigned"
                )
            else:
                ...
                order.availability = (
                    (all(assigned_list) and "assigned") or (any(partial_list) and "partially_available") or "waiting"
                )
    ...
    def _compute_post_visible(self):
        for order in self:
            if order.product_tmpl_id._is_cost_method_standard():
                ...
                order.post_visible = order.is_locked and any(
                    (x.quantity_done > 0 and x.state not in ["done", "cancel"]) for x in order.move_finished_ids
                )
            ...
            order.consumed_less_than_planned = any(
                order.move_raw_ids.filtered(
                    lambda move: float_compare(
                        move.quantity_done, move.product_uom_qty, precision_rounding=move.product_uom.rounding
                    )
                    == -1
                )
            )
    ...
    def onchange_picking_type(self):
        ...
        self.location_src_id = (
            self.routing_id.location_id.id or self.picking_type_id.default_location_src_id.id or location.id
        )
    ...
    def create(self, values):
        if not values.get("name", False) or values["name"] == _("New"):
            if values.get("picking_type_id"):
                values["name"] = (
                    self.env["stock.picking.type"].browse(values["picking_type_id"]).sequence_id.next_by_id()
                )
            else:
    ...
    def _generate_moves(self):
        for production in self:
            ...
            factor = (
                production.product_uom_id._compute_quantity(production.product_qty, production.bom_id.product_uom_id)
    ...
    def button_plan(self):
        ...
        for order in orders_to_plan:
            quantity = (
                order.product_uom_id._compute_quantity(order.product_qty, order.bom_id.product_uom_id)

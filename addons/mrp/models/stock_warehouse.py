...
class StockWarehouse(models.Model):
    ...
    manu_type_id = fields.Many2one(
        "stock.picking.type", "Manufacturing Operation Type", domain=[("code", "=", "mrp_operation")]
    )

    def create_sequences_and_picking_types(self):
        res = super().create_sequences_and_picking_types()
        self._create_manufacturing_picking_type()
        return res

    @api.multi
    def get_routes_dict(self):
        result = super().get_routes_dict()

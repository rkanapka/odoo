...
class MrpWorkcenter(models.Model):
    ...
    sequence = fields.Integer(
        "Sequence", default=1, required=True, help="Gives the sequence order when displaying a list of work centers."
    )
    ...
    blocked_time = fields.Float(
        "Blocked Time", compute="_compute_blocked_time", help="Blocked hour(s) over the last month", digits=(16, 2)
    )
    ...
    @api.depends(
        "order_ids.duration_expected", "order_ids.workcenter_id", "order_ids.state", "order_ids.date_planned_start"
    )
        ...
        data = self.env["mrp.workcenter.productivity"].read_group(
            [
                (
                    "date_start",
                    ">=",
                    fields.Datetime.to_string(datetime.datetime.now() - relativedelta.relativedelta(months=1)),
                ),
                ("workcenter_id", "in", self.ids),
                ("date_end", "!=", False),
    ...
    def _compute_duration(self):
        for blocktime in self:
            if blocktime.date_end:
                ...
                diff = d2 - d1
                if (
                    blocktime.loss_type not in ("productive", "performance")

...
class TestMrpOrder(TestMrpCommon):
    ...
    def test_basic(self):
        ...
        self.assertEqual(
            first_move.product_qty, test_quantity / self.bom_1.product_qty * self.product_4.uom_id.factor_inv * 2
        )
        first_move = man_order.move_raw_ids.filtered(lambda move: move.product_id == self.product_1)
        self.assertEqual(
            first_move.product_qty, test_quantity / self.bom_1.product_qty * self.product_4.uom_id.factor_inv * 4
        )
    ...
    def test_explode_from_order(self):
        ...
        self.assertEqual(
            consume_qty_2, 24.0, "Consume material quantity of Wood should be 24 instead of %s" % str(consume_qty_2)
        )
        consume_qty_3 = product_3_consume_moves.product_uom_qty
        self.assertEqual(
            consume_qty_3, 12.0, "Consume material quantity of Stone should be 12 instead of %s" % str(consume_qty_3)
        )
        ...
        for consume_moves in product_4_consume_moves:
            ...
            self.assertIn(
                consume_qty_4,
                [8.0, 16.0],
                "Consume material quantity of Stick should be 8 or 16 instead of %s" % str(consume_qty_4),
            )
        ...
        self.assertEqual(
            kit_wo.duration_expected,
            80,
            "Workorder duration should be 80 instead of %s." % str(kit_wo.duration_expected),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            door_wo_2.duration_expected,
            20,
            "Workorder duration should be 20 instead of %s." % str(door_wo_2.duration_expected),
        )
    ...
    def test_production_avialability(self):
        ...
        self.assertEqual(
            production_2.availability, "waiting", "Production order should be availability for waiting state"
        )
        ...
        self.assertEqual(
            production_2.availability, "assigned", "Production order should be availability for assigned state"
        )
    ...
    def test_empty_routing(self):
        ...
        self.assertEqual(
            production.move_raw_ids[0].location_id.id,
            self.warehouse_1.wh_input_stock_loc_id.id,
            "Raw moves start location should have altered.",
        )
    ...
    def test_multiple_post_inventory(self):
        ...
        product_charger = self.env["product.product"].create(
            {"name": "Charger", "type": "product", "uom_id": unit, "uom_po_id": unit}
        )
        product_keybord = self.env["product.product"].create(
            {"name": "Usb Keybord", "type": "product", "uom_id": unit, "uom_po_id": unit}
        )
        bom_custom_laptop = self.env["mrp.bom"].create(
            {
                ...
                "product_uom_id": unit,
                "bom_line_ids": [
                    (0, 0, {"product_id": product_charger.id, "product_qty": 1, "product_uom_id": unit}),
                    (0, 0, {"product_id": product_keybord.id, "product_qty": 1, "product_uom_id": unit}),
                ],
            }
        )
        ...
        mo_custom_laptop = self.env["mrp.production"].create(
            {"product_id": custom_laptop.id, "product_qty": 2, "product_uom_id": unit, "bom_id": bom_custom_laptop.id}
        )
    ...
    def test_product_produce_1(self):
        ...
        self.assertEqual(
            len(product_produce.produce_line_ids),
            0,
            "You should not have any produce lines since the consumed products are not tracked.",
        )
    ...
    def test_product_produce_2(self):
        ...
        self.assertEqual(
            len(product_produce.produce_line_ids), 2, "You should have 2 produce lines. One for each serial to consume"
        )
        ...
        self.assertEqual(
            len(product_produce.produce_line_ids),
            1,
            "You should have 1 produce lines since one has already be consumed.",
        )
    ...
    def test_product_produce_3(self):
        ...
        self.assertEqual(
            len(product_produce.produce_line_ids),
            3,
            "You should have 3 produce lines. lot 1 shelf_1, lot 1 shelf_2, lot2",
        )
    ...
    def test_product_produce_7(self):
        ...
        categ_test = self.env["product.uom.categ"].create({"name": "Volume Test"})

        uom_L = self.env["product.uom"].create(
            {"name": "Test Liters", "category_id": categ_test.id, "uom_type": "reference", "rounding": 0.001}
        )

        uom_ml = self.env["product.uom"].create(
            {
                "name": "Test ml",
        ...
        test_bom = self.env["mrp.bom"].create(
            {
                ...
                "type": "normal",
                "bom_line_ids": [
                    (0, 0, {"product_id": product_comp.id, "product_qty": 375.00, "product_uom_id": uom_ml.id})
                ],
            }
        )

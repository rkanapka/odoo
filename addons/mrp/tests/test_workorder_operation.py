...
class TestWorkOrderProcess(common.TransactionCase):
    ...
    def test_00b_workorder_process(self):
        ...
        bom.bom_line_ids.filtered(lambda p: p.product_id == product_table_sheet).operation_id = (
            bom.routing_id.operation_ids[0]
        )
        bom.bom_line_ids.filtered(lambda p: p.product_id == product_table_leg).operation_id = (
            bom.routing_id.operation_ids[1]
        )
        bom.bom_line_ids.filtered(lambda p: p.product_id == product_bolt).operation_id = bom.routing_id.operation_ids[
            2
        ]
        ...
        wiz = self.env["change.production.qty"].create({"mo_id": production_table.id, "product_qty": 1.0})
        wiz.change_prod_qty()
        # ---------------------------------------------------------------
        # Check consume quants and produce quants after posting inventory
        # ---------------------------------------------------------------
        ...
        self.assertEqual(
            sum(move_table_sheet.mapped("quantity_done")),
            1,
            "Wrong quantity of consumed product %s" % move_table_sheet.product_id.name,
        )
        self.assertEqual(
            sum(move_leg.mapped("quantity_done")),
            4,
            "Wrong quantity of consumed product %s" % move_leg.product_id.name,
        )
        self.assertEqual(
            sum(move_table_bolt.mapped("quantity_done")),
            4,
            "Wrong quantity of consumed product %s" % move_table_bolt.product_id.name,
        )
    ...
    def test_01_without_workorder(self):
        ...
        product_charger = self.env["product.product"].create(
            {"name": "Charger", "type": "product", "tracking": "lot", "uom_id": unit, "uom_po_id": unit}
        )
        product_keybord = self.env["product.product"].create(
            {"name": "Usb Keybord", "type": "product", "tracking": "lot", "uom_id": unit, "uom_po_id": unit}
        )
        ...
        bom_custom_laptop = self.env["mrp.bom"].create(
            {
                ...
                "product_uom_id": unit,
                "bom_line_ids": [
                    (0, 0, {"product_id": product_charger.id, "product_qty": 20, "product_uom_id": unit}),
                    (0, 0, {"product_id": product_keybord.id, "product_qty": 20, "product_uom_id": unit}),
                ],
            }
        )
        ...
        mo_custom_laptop = self.env["mrp.production"].create(
            {"product_id": custom_laptop.id, "product_qty": 10, "product_uom_id": unit, "bom_id": bom_custom_laptop.id}
        )
        ...
        for move in mo_custom_laptop.move_raw_ids:
            self.assertEqual(
                move.quantity_done, 12, "Wrong produced quantity on raw material %s" % (move.product_id.name)
            )
        ...
        charger_done_move = mo_custom_laptop.move_raw_ids.filtered(
            lambda x: x.product_id.id == product_charger.id and x.state == "done"
        )
        keybord_done_move = mo_custom_laptop.move_raw_ids.filtered(
            lambda x: x.product_id.id == product_keybord.id and x.state == "done"
        )
    ...
    def test_02_different_uom_on_bomlines(self):
        ...
        product_B = self.env["product.product"].create(
            {"name": "Product B", "type": "product", "tracking": "lot", "uom_id": dozen, "uom_po_id": dozen}
        )
        product_C = self.env["product.product"].create(
            {"name": "Product C", "type": "product", "tracking": "lot", "uom_id": kg, "uom_po_id": kg}
        )
        ...
        bom_a = self.env["mrp.bom"].create(
            {
                ...
                "product_uom_id": unit,
                "bom_line_ids": [
                    (0, 0, {"product_id": product_B.id, "product_qty": 4, "product_uom_id": unit}),
                    (0, 0, {"product_id": product_C.id, "product_qty": 600, "product_uom_id": gm}),
                ],
            }
        )
        ...
        mo_custom_product = self.env["mrp.production"].create(
            {"product_id": product_A.id, "product_qty": 10, "product_uom_id": unit, "bom_id": bom_a.id}
        )
    ...
    def test_03_test_serial_number_defaults(self):
        ...
        bom_laptop = self.env["mrp.bom"].create(
            {
                ...
                "bom_line_ids": [
                    (0, 0, {"product_id": graphics_card.id, "product_qty": 1, "product_uom_id": unit.id})
                ],
        ...
        mo_laptop = self.env["mrp.production"].create(
            {"product_id": laptop.id, "product_qty": 3, "product_uom_id": unit.id, "bom_id": bom_laptop.id}
        )

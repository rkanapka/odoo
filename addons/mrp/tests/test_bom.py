...
class TestBoM(TestMrpCommon):
    ...
    def test_multi_level_variants(self):
        ...
        self.assertEqual(set((test_bom_2_l1 | test_bom_2_l4 | self.bom_2.bom_line_ids).ids), {l[0].id for l in lines})

        # check sequence priority
        test_bom_1.write({"sequence": 1})
        boms, lines = test_bom_2.explode(self.product_7_1, 4)
        self.assertEqual(set((test_bom_2 | test_bom_1).ids), {b[0].id for b in boms})
        self.assertEqual(set((test_bom_2_l1 | test_bom_2_l4 | test_bom_1.bom_line_ids).ids), {l[0].id for l in lines})

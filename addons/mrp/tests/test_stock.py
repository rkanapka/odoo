...
class TestWarehouse(common.TestMrpCommon):
    def setUp(self):
        ...
        def new_mo_laptop():
            return self.env["mrp.production"].create(
                {"product_id": self.laptop.id, "product_qty": 1, "product_uom_id": unit.id, "bom_id": bom_laptop.id}
            )
    ...
    def test_manufacturing_scrap(self):
        ...
        self.env["stock.scrap"].with_context(active_model="mrp.production", active_id=production_3.id).create(
            {
                "product_id": self.product_2.id,
                "scrap_qty": 1.0,
                "product_uom_id": self.product_2.uom_id.id,

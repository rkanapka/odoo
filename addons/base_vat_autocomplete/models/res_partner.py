...
class ResPartner(models.Model):
    ...
    def vies_vat_change(self):
        def _check_city(lines, country="BE"):
            if country == "GB":
                ukzip = r"[A-Z]{1,2}[0-9][0-9A-Z]?\s?[0-9][A-Z]{2}"
                if re.match(ukzip, lines[-1]):
                    cp = lines.pop()
                    city = lines.pop()
                    return (cp, city)
            elif country == "SE":
                result = re.match(r"([0-9]{3}\s?[0-9]{2})\s?([A-Z]+)", lines[-1])
                if result:
                    lines.pop()
                    return (result.group(1), result.group(2))
            else:
                result = re.match(r"((?:L-|AT-)?[0-9\-]+[A-Z]{,2}) (.+)", lines[-1])
                if result:
                    lines.pop()
                    return (result.group(1), result.group(2))
            return False
        ...
        for partner in self:
            ...
            # wipe it anyway
            non_set_address_fields = {"street2", "city", "zip", "state_id", "country_id"}
            if not partner.vat:
                return {}
            if len(partner.vat) > 5 and partner.vat[:2].lower() in stdnum_vat.country_codes:
                ...
                vals = partner._split_street_with_params(
                    ", ".join(lines.pop(0).rsplit(" ", 1)), "%(street_name)s, %(street_number)s/%(street_number2)s"
                )
                partner.update(vals)

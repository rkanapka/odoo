...
from odoo.addons.http_routing.models.ir_http import _guess_mimetype, slugify
...
class Website(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company", string="Company", default=lambda self: self.env.ref("base.main_company").id
    )
    language_ids = fields.Many2many(
        "res.lang", "website_lang_rel", "website_id", "lang_id", "Languages", default=_active_languages
    )
    ...
    user_id = fields.Many2one(
        "res.users", string="Public User", required=True, default=lambda self: self.env.ref("base.public_user").id
    )
    ...
    favicon = fields.Binary(
        string="Website Favicon", help="This field holds the image used to display a favicon on the website."
    )
    ...
    def _compute_menu(self):
        ...
        for website in self:
            website.menu_id = Menu.search(
                [("parent_id", "=", False), ("website_id", "=", website.id)], order="id", limit=1
            ).id
    ...
    def new_page(self, name=False, add_menu=False, template="website.default_page", ispage=True, namespace=None):
        ...
        if ispage:
            page = self.env["website.page"].create(
                {"url": page_url, "website_ids": [(6, None, [self.get_current_website().id])], "view_id": view.id}
            )
    ...
    def page_search_dependencies(self, page_id=False):
        ...
        for view in views:
            ...
            dependencies[view_key].append(
                {
                    "text": _("Template <b>%s (id:%s)</b> contains a link to this page")
    ...
    def enumerate_pages(self, query_string=None, force=False):
        ...
        for rule in router.iter_rules():
            if "sitemap" in rule.endpoint.routing:
                ...
                if func is False:
                    continue
                yield from func(self.env, rule, query_string)
                continue
...
class Page(models.Model):
    ...
    def _compute_visible(self):
        self.is_visible = self.website_published and (
            not self.date_publish or self.date_publish < fields.Datetime.now()
        )
    ...
    def clone_page(self, page_id, clone_menu=True):
        ...
        if clone_menu:
            ...
            if menu:
                ...
                new_menu = menu.copy()
                new_menu.write(
                    {"url": new_page.url, "name": "%s %s" % (menu.name, _("(copy)")), "page_id": new_page.id}
                )
    ...
    def unlink(self):
        ...
        for page in self:
            ...
            pages_linked_to_iruiview = self.search([("view_id", "=", page.view_id.id), ("id", "!=", page.id)])
    ...
    def save(self, website_id, data):
        ...
        for menu in data["data"]:
            ...
            page = self.env["website.page"].search(
                ["|", ("url", "=", menu["url"]), ("url", "=", "/" + menu["url"])], limit=1
            )
            if page:
...
class WebsiteRedirect(models.Model):
    ...
    type = fields.Selection(
        [("301", "Moved permanently"), ("302", "Moved temporarily")], string="Redirection Type", default="301"
    )

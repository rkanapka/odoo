...
class ResConfigSettings(models.TransientModel):
    ...
    default_lang_id = fields.Many2one(
        string="Default language", related="website_id.default_lang_id", relation="res.lang", required=True
    )
    ...
    google_management_client_secret = fields.Char(
        "Google Client Secret", related="website_id.google_management_client_secret"
    )
    ...
    def get_values(self):
        ...
        res.update(
            auth_signup_uninvited=(
                "b2c" if get_param("auth_signup.allow_uninvited", "False").lower() == "true" else "b2b"
            ),
    ...
    def open_template_user(self):
        ...
        action["res_id"] = literal_eval(
            self.env["ir.config_parameter"].sudo().get_param("auth_signup.template_user_id", "False")
        )

...
class TestViewSaving(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.view_id = self.env["ir.ui.view"].create(
            {"name": "Test View", "type": "qweb", "arch": ET.tostring(self.arch, encoding="unicode")}
        )
    ...
    def test_embedded_save(self):
        embedded = h.SPAN("+00 00 000 00 0 000", attrs(model="res.company", id=1, field="phone", type="char"))
    ...
    def test_fixup_arch(self):
        ...
        self.eq(
            result,
            h.DIV(
                h.DIV(h.H3("I am the greatest title alive!"), h.UL(h.LI("Item 1"), h.LI("Item 2"), h.LI("Item 3"))),
                h.DIV(
                    h.H3("Column 2"),
                    h.UL(
                        h.LI("Item 1"),
    ...
    def test_save(self):
        ...
        self.eq(
            ...
            h.DIV(
                ...
                h.DIV(
                    h.H3("Column 2"),
                    h.UL(h.LI("wob wob wob"), h.LI(h.SPAN({"t-field": "bob"})), h.LI(h.SPAN({"t-field": "edmund"}))),
                ),
            ),
        )
    ...
    def test_save_escaped_text(self):
        ...
        self.assertIn(replacement.replace("&", "&amp;"), view.arch, "inline script should be escaped server side")
        self.assertIn(replacement, view.render().decode("utf-8"), "inline script should not be escaped when rendering")
        ...
        self.assertIn(
            replacement,
            view.render().decode("utf-8").replace("&", "&amp;"),
    ...
    def test_field_tail(self):
        ...
        replacement = ET.tostring(
            h.LI(
                h.SPAN(
                    "+12 3456789", attrs(model="res.company", id=1, type="char", field="phone", expression="edmund")
                ),
                "whop whop",
            ),
            encoding="utf-8",

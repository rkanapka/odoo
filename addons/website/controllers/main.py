...
from odoo.addons.http_routing.models.ir_http import _guess_mimetype, slug
...
class Website(Home):
    ...
    def index(self, **kw):
        ...
        if (
            homepage
            and (homepage.sudo().is_visible or request.env.user.has_group("base.group_user"))
            and homepage.url != "/"
        ):
    ...
    def website_info(self):
        ...
        values = {"apps": apps, "modules": modules, "version": odoo.service.common.exp_version()}
    ...
    def pages_management(self, page=1, sortby="name", search="", **kw):
        ...
        pager = portal_pager(url="/website/pages", url_args={"sortby": sortby}, total=pages_count, page=page, step=50)

...
class MailGroup(http.Controller):
    ...
    def _to_date(self, dt):
        ...
        """
        return (
            datetime.strptime(dt, tools.DEFAULT_SERVER_DATETIME_FORMAT)
    ...
    def get_alias_info(self, group, **post):
        return {
            "alias_name": group.alias_id
            and group.alias_id.alias_name
            and group.alias_id.alias_domain
            and "%s@%s" % (group.alias_id.alias_name, group.alias_id.alias_domain)
            or False
        }
    ...
    def confirm_unsubscribe(self, channel, partner_id, token, **kw):
        ...
        if not subscriber:
            ...
            try:
                response = request.render("website_mail_channel.not_subscribed", {"partner_id": partner})

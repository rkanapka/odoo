...
class TestConfirmUnsubscribe(common.HttpCase):
    def setUp(self):
        ...
        self.partner = env["res.partner"].create({"name": "Bob", "email": "bob@bob.bob"})
    ...
    def test_not_subscribed_no_template(self):
        ...
        self.assertEqual(
            ...
            0,
            "XID for template should have been deleted",
        )
        self.env2.cr.release()

        self._unsubscribe_check(
            "The address %s is already unsubscribed or was never subscribed to any mailing list" % self.partner.email
        )

    def test_wrong_token(self):
        self.mailing_list.sudo().write({"channel_partner_ids": [(4, self.partner.id, False)]})
        self.env2.cr.release()
        self.token = "XXX"
    ...
    def _unsubscribe_check(self, text):
        url = "/groups/unsubscribe/{}/{}/{}".format(self.mailing_list.id, self.partner.id, self.token)
        r = self.url_open(url)
        body = ustr(r.content)

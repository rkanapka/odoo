...
class TestIntervals(TestResourceCommon):
    ...
    def setUp(self):
        ...
        self.intervals = [
            self.calendar._interval_new(
                Datetime.from_string("2013-02-04 09:00:00"), Datetime.from_string("2013-02-04 11:00:00")
            ),
            self.calendar._interval_new(
                Datetime.from_string("2013-02-04 08:00:00"), Datetime.from_string("2013-02-04 12:00:00")
            ),
            self.calendar._interval_new(
                Datetime.from_string("2013-02-04 11:00:00"), Datetime.from_string("2013-02-04 14:00:00")
            ),
            self.calendar._interval_new(
                Datetime.from_string("2013-02-04 17:00:00"), Datetime.from_string("2013-02-04 21:00:00")
            ),
            self.calendar._interval_new(
                Datetime.from_string("2013-02-03 08:00:00"), Datetime.from_string("2013-02-03 10:00:00")
            ),
            self.calendar._interval_new(
                Datetime.from_string("2013-02-04 18:00:00"), Datetime.from_string("2013-02-04 19:00:00")
            ),
        ]
    ...
    def test_interval_merge(self):
        ...
        self.assertEqual(
            cleaned_intervals[0][:2],
            (Datetime.from_string("2013-02-03 08:00:00"), Datetime.from_string("2013-02-03 10:00:00")),
        )
        # Second interval: 04, 08-14, combining 08-12 and 11-14, 09-11 being inside 08-12
        self.assertEqual(
            cleaned_intervals[1][:2],
            (Datetime.from_string("2013-02-04 08:00:00"), Datetime.from_string("2013-02-04 14:00:00")),
        )
        # Third interval: 04, 17-21, 18-19 being inside 17-21
        self.assertEqual(
            cleaned_intervals[2][:2],
            (Datetime.from_string("2013-02-04 17:00:00"), Datetime.from_string("2013-02-04 21:00:00")),
        )

    def test_interval_and(self):
        self.assertEqual(
            self.env["resource.calendar"]._interval_and(self.intervals[0], self.intervals[1]),
            self.calendar._interval_new(
                Datetime.from_string("2013-02-04 09:00:00"), Datetime.from_string("2013-02-04 11:00:00")
            ),
        )
        self.assertEqual(self.env["resource.calendar"]._interval_and(self.intervals[2], self.intervals[3]), None)

    def test_interval_remove(self):
        working_interval = self.calendar._interval_new(
            Datetime.from_string("2013-02-04 08:00:00"), Datetime.from_string("2013-02-04 18:00:00")
        )
    ...
    def test_interval_schedule_hours(self):
        ...
        self.assertEqual(
            result[0][:2], (Datetime.from_string("2013-02-03 08:00:00"), Datetime.from_string("2013-02-03 10:00:00"))
        )
    ...
    def test_interval_schedule_hours_backwards(self):
        ...
        # First interval: 03, 8-10 untouched
        self.assertEqual(
            result[1][:2], (Datetime.from_string("2013-02-04 17:00:00"), Datetime.from_string("2013-02-04 21:00:00"))
        )
        # First interval: 04, 08-11:30
        self.assertEqual(
...
class TestCalendarBasics(TestResourceCommon):
    ...
    def test_calendar_working_day_intervals_no_leaves(self):
        ...
        self.assertEqual(
            intervals[0][:2],
            (Datetime.from_string("2013-02-12 09:08:07"), Datetime.from_string("2013-02-12 16:00:00")),
        )
        ...
        self.assertEqual(
            intervals[0][:2],
            (Datetime.from_string("2013-02-19 10:30:00"), Datetime.from_string("2013-02-19 16:00:00")),
        )
        self.assertEqual(intervals[0][2]["attendances"], self.att_1)
        ...
        self.assertEqual(
            intervals[0][:2],
            (Datetime.from_string("2013-02-15 10:11:12"), Datetime.from_string("2013-02-15 13:00:00")),
        )
        self.assertEqual(
            intervals[1][:2],
            (Datetime.from_string("2013-02-15 16:00:00"), Datetime.from_string("2013-02-15 23:00:00")),
        )
    ...
    def test_calendar_working_day_intervals_leaves_generic(self):
        ...
        intervals = self.calendar._get_day_work_intervals(
            Date.from_string("2013-02-12"), start_time=time(7, 0, 0), compute_leaves=True
        )
        self.assertEqual(len(intervals), 1)
        self.assertEqual(
            intervals[0][:2],
            (Datetime.from_string("2013-02-12 08:00:00"), Datetime.from_string("2013-02-12 16:00:00")),
        )

        # Test: day0 with leaves: 2 intervals because of leave between 9 and 12, ending at 15:45:30
        intervals = self.calendar._get_day_work_intervals(
            Date.from_string("2013-02-19"), start_time=time(8, 15, 0), end_time=time(15, 45, 30), compute_leaves=True
        )
        self.assertEqual(len(intervals), 2)
        self.assertEqual(
            intervals[0][:2],
            (Datetime.from_string("2013-02-19 08:15:00"), Datetime.from_string("2013-02-19 09:00:00")),
        )
        self.assertEqual(
            intervals[1][:2],
            (Datetime.from_string("2013-02-19 12:00:00"), Datetime.from_string("2013-02-19 15:45:30")),
        )
        self.assertEqual(intervals[0][2]["attendances"], self.att_1)
    ...
    def test_calendar_working_day_intervals_limited_attendances(self):
        ...
        attendance = self.env["resource.calendar.attendance"].search([("name", "=", "Att3")])
        attendance.write(
            {
...
class ResourceWorkingHours(TestResourceCommon):
    ...
    def test_calendar_working_hours_24(self):
        self.att_4 = self.env["resource.calendar.attendance"].create(
            {"name": "Att4", "calendar_id": self.calendar.id, "dayofweek": "2", "hour_from": 0, "hour_to": 24}
        )
        res = self.calendar.get_work_hours_count(
    ...
    def test_calendar_hours_scheduling_backward(self):
        ...
        self.assertEqual(
            res[-1][:2], (Datetime.from_string("2013-02-12 08:00:00"), Datetime.from_string("2013-02-12 09:00:00"))
        )
        # previous days: 5+7 hours / 8 hours / 5+7 hours -> 32 hours
        self.assertEqual(
            res[-2][:2], (Datetime.from_string("2013-02-08 16:00:00"), Datetime.from_string("2013-02-08 23:00:00"))
        )
        self.assertEqual(
            res[-3][:2], (Datetime.from_string("2013-02-08 08:00:00"), Datetime.from_string("2013-02-08 13:00:00"))
        )
        self.assertEqual(
            res[-4][:2], (Datetime.from_string("2013-02-05 08:00:00"), Datetime.from_string("2013-02-05 16:00:00"))
        )
        self.assertEqual(
            res[-5][:2], (Datetime.from_string("2013-02-01 16:00:00"), Datetime.from_string("2013-02-01 23:00:00"))
        )
        self.assertEqual(
            res[-6][:2], (Datetime.from_string("2013-02-01 08:00:00"), Datetime.from_string("2013-02-01 13:00:00"))
        )
        # 7 hours remaining
        self.assertEqual(
            res[-7][:2], (Datetime.from_string("2013-01-29 09:00:00"), Datetime.from_string("2013-01-29 16:00:00"))
        )
    ...
    def test_calendar_hours_scheduling_forward(self):
        res = self.calendar._schedule_hours(40, day_dt=Datetime.from_string("2013-02-12 09:00:00"))
        self.assertEqual(
            res[0][:2], (Datetime.from_string("2013-02-12 09:00:00"), Datetime.from_string("2013-02-12 16:00:00"))
        )
        self.assertEqual(
            res[1][:2], (Datetime.from_string("2013-02-15 08:00:00"), Datetime.from_string("2013-02-15 13:00:00"))
        )
        self.assertEqual(
            res[2][:2], (Datetime.from_string("2013-02-15 16:00:00"), Datetime.from_string("2013-02-15 23:00:00"))
        )
        self.assertEqual(
            res[3][:2], (Datetime.from_string("2013-02-19 08:00:00"), Datetime.from_string("2013-02-19 16:00:00"))
        )
        self.assertEqual(
            res[4][:2], (Datetime.from_string("2013-02-22 08:00:00"), Datetime.from_string("2013-02-22 13:00:00"))
        )
        self.assertEqual(
            res[5][:2], (Datetime.from_string("2013-02-22 16:00:00"), Datetime.from_string("2013-02-22 23:00:00"))
        )
        self.assertEqual(
            res[6][:2], (Datetime.from_string("2013-02-26 08:00:00"), Datetime.from_string("2013-02-26 09:00:00"))
        )
    ...
    def test_calendar_hours_scheduling_timezone(self):
        # user in timezone UTC-9 asks for work hours
        self.env.user.tz = "US/Alaska"
        res = self.calendar.plan_hours(42, to_naive_utc(Datetime.from_string("2013-02-12 09:25:00"), self.env.user))
    ...
    def test_calendar_hours_scheduling_timezone_2(self):
        ...
        for test_date in ["2013-02-15 08:00:00", "2013-02-15 11:00:00"]:
            ...
            self.assertEqual(
                [(start_dt_utc, start_dt_utc.replace(minute=1))],
                res,
    ...
    def test_calendar_hours_scheduling_forward_leaves_resource(self):
        res = self.calendar._schedule_hours(
            40, day_dt=Datetime.from_string("2013-02-12 09:00:00"), compute_leaves=True, resource_id=self.resource1_id
        )
        self.assertEqual(
            res[0][:2], (Datetime.from_string("2013-02-12 09:00:00"), Datetime.from_string("2013-02-12 16:00:00"))
        )
        ...
        self.assertEqual(
            res[3][:2], (Datetime.from_string("2013-02-19 08:00:00"), Datetime.from_string("2013-02-19 09:00:00"))
        )
        self.assertEqual(
            res[4][:2], (Datetime.from_string("2013-02-19 12:00:00"), Datetime.from_string("2013-02-19 16:00:00"))
        )
        self.assertEqual(
            res[5][:2], (Datetime.from_string("2013-02-22 08:00:00"), Datetime.from_string("2013-02-22 09:00:00"))
        )
        self.assertEqual(
            res[6][:2], (Datetime.from_string("2013-02-22 16:00:00"), Datetime.from_string("2013-02-22 23:00:00"))
        )
        self.assertEqual(
            res[7][:2], (Datetime.from_string("2013-03-01 11:30:00"), Datetime.from_string("2013-03-01 13:00:00"))
        )
        self.assertEqual(
            res[8][:2], (Datetime.from_string("2013-03-01 16:00:00"), Datetime.from_string("2013-03-01 22:30:00"))
        )
    ...
    def test_calendar_days_scheduling(self):
        ...
        res = self.calendar.plan_days(
            5, Datetime.from_string("2013-02-12 09:08:07"), compute_leaves=True, resource_id=self.resource1_id
        )
        self.assertEqual(
    ...
    def test_calendar_days_scheduling_timezone(self):
        ...
        self.assertEqual(
            to_naive_user_tz(res, self.env.user).date(), Datetime.from_string("2013-02-26 00:00:00").date()
        )
...
class TestWorkDays(TestResourceCommon):
    ...
    def setUp(self):
        ...
        self.calendar.write({"attendance_ids": [(0, 0, self._make_attendance(i)) for i in range(5)]})
    ...
    def test_trivial_calendar_no_leaves(self):
        ...
        """
        r = self.env["resource.resource"].create({"name": "Trivial Calendar", "calendar_id": self.calendar.id})
    ...
    def test_global_leaves(self):
        ...
        r1 = self.env["resource.resource"].create({"name": "Resource 1", "calendar_id": self.calendar.id})
        r2 = self.env["resource.resource"].create({"name": "Resource 2", "calendar_id": self.calendar.id})
    ...
    def test_mixed_leaves(self):
        r = self.env["resource.resource"].create({"name": "Resource 1", "calendar_id": self.calendar.id})
        self.env["resource.calendar.leaves"].create(
            {
                "calendar_id": self.calendar.id,
                "date_from": "1932-11-09 00:00:00",
...
class TestResourceMixin(TestResourceCommon):
    ...
    def setUp(self):
        ...
        self.test = (
            self.env["resource.test"]
            .with_context(default_resource_calendar_id=self.calendar.id)
            .create({"name": "Test"})
        )

    def test_basics(self):
        self.assertEqual(
            self.env["resource.test"].create({"name": "Test"}).resource_calendar_id,

...
class TestResourceCommon(TransactionCase):
    ...
    def setUp(self):
        ...
        self.calendar = self.env["resource.calendar"].create({"name": "TestCalendar", "attendance_ids": [(5, 0, 0)]})
        self.att_1 = self.env["resource.calendar.attendance"].create(
            {"name": "Att1", "calendar_id": self.calendar.id, "dayofweek": "1", "hour_from": 8, "hour_to": 16}
        )
        self.att_2 = self.env["resource.calendar.attendance"].create(
            {"name": "Att2", "calendar_id": self.calendar.id, "dayofweek": "4", "hour_from": 8, "hour_to": 13}
        )
        self.att_3 = self.env["resource.calendar.attendance"].create(
            {"name": "Att3", "calendar_id": self.calendar.id, "dayofweek": "4", "hour_from": 16, "hour_to": 23}
        )

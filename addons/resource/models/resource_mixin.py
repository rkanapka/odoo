...
class ResourceMixin(models.AbstractModel):
    ...
    resource_id = fields.Many2one(
        "resource.resource", "Resource", auto_join=True, index=True, ondelete="restrict", required=True
    )
    ...
    def create(self, values):
        if not values.get("resource_id"):
            resource = self.env["resource.resource"].create({"name": values.get(self._rec_name)})
    ...
    def get_work_days_data(self, from_datetime, to_datetime, calendar=None):
        ...
        for day_intervals in calendar._iter_work_intervals(
            from_datetime, to_datetime, self.resource_id.id, compute_leaves=True
        ):

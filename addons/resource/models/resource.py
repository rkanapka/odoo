...
def float_to_time(float_hour):
    ...
    return datetime.time(
        int(math.modf(float_hour)[1]), int(float_round(60 * math.modf(float_hour)[0], precision_digits=0)), 0
    )
...
class ResourceCalendar(models.Model):
    ...
    attendance_ids = fields.One2many(
        "resource.calendar.attendance", "calendar_id", "Working Time", copy=True, default=_get_default_attendance_ids
    )
    leave_ids = fields.One2many("resource.calendar.leaves", "calendar_id", "Leaves")
    global_leave_ids = fields.One2many(
        "resource.calendar.leaves", "calendar_id", "Global Leaves", domain=[("resource_id", "=", False)]
    )
    ...
    def _merge_kw(self, kw, kw_ext):
        ...
        new_kw.update(
            attendances=kw.get("attendances", self.env["resource.calendar.attendance"])
            | kw_ext.get("attendances", self.env["resource.calendar.attendance"]),
    ...
    def _interval_exclude_left(self, interval, interval_dst):
        return self._interval_obj(
            ...
            and interval.start_datetime
            or interval_dst.end_datetime,
    ...
    def _interval_exclude_right(self, interval, interval_dst):
        return self._interval_obj(
            ...
            and interval.end_datetime
            or interval_dst.start_datetime,
            self._merge_kw(interval.data, interval_dst.data),
        )
    ...
    def _interval_remove_leaves(self, interval, leave_intervals):
        ...
        for leave in leave_intervals:
            ...
            if current_interval[0] <= leave[1]:
                current_interval = self._interval_exclude_left(interval, leave)
        if (
            current_interval and current_interval[0] < interval[1]
    ...
    def _get_day_attendances(self, day_date, start_time, end_time):
        ...
        for attendance in self.attendance_ids.filtered(
            ...
            and not (att.date_from and fields.Date.from_string(att.date_from) > day_date)
    ...
    @api.multi
    def _get_day_work_intervals(
        self, day_date, start_time=None, end_time=None, compute_leaves=False, resource_id=None
    ):
        ...
        working_intervals = [
            att_interval for att_interval in self._iter_day_attendance_intervals(day_date, start_time, end_time)
        ]
        ...
        return [
            self._interval_new(
                to_naive_utc(interval[0], self.env.user), to_naive_utc(interval[1], self.env.user), interval[2]
            )
            for interval in working_intervals
        ]
    ...
    def _iter_leave_intervals(self, start_dt, end_dt, resource_id):
        ...
        for day in rrule.rrule(
            ...
            intervals = self._get_day_leave_intervals(day.date(), start_time, end_time, resource_id)
        ...
        while float_compare(remaining_hours, 0.0, precision_digits=2) in (1, 0) and iterations < 1000:
            ...
            if backwards:
                current_datetime = datetime.datetime.combine(
                    self._get_previous_work_day(current_datetime), datetime.time(23, 59, 59)
                )
            else:
                current_datetime = datetime.datetime.combine(
    ...
    def _schedule_days(self, days, day_dt, compute_leaves=False, resource_id=None):
        ...
        while planned_days < abs(days) and iterations < 100:
            working_intervals = self._get_day_work_intervals(
                current_datetime.date(), compute_leaves=compute_leaves, resource_id=resource_id
            )
...
class ResourceCalendarLeaves(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company", related="calendar_id.company_id", string="Company", readonly=True, store=True
    )

...
class MailChannel(models.Model):
    ...
    def _channel_message_notifications(self, message):
        ...
        """
        notifications = super()._channel_message_notifications(message)
        ...
        if not message.author_id:
            unpinned_channel_partner = self.mapped("channel_last_seen_partner_ids").filtered(
                lambda cp: not cp.is_pinned
            )
            if unpinned_channel_partner:
                unpinned_channel_partner.write({"is_pinned": True})
                notifications = (
                    self._channel_channel_notifications(unpinned_channel_partner.mapped("partner_id").ids)
                    + notifications
                )
    ...
    def channel_info(self, extra_info=False):
        ...
        :rtype : list(dict)
        """
        channel_infos = super().channel_info(extra_info)
        # add the operator id
        if self.env.context.get("im_livechat_operator_partner_id"):
            partner_name = (
                self.env["res.partner"].browse(self.env.context.get("im_livechat_operator_partner_id")).name_get()[0]
            )
    ...
    def channel_fetch_slot(self):
        values = super().channel_fetch_slot()
        pinned_channels = (
            self.env["mail.channel.partner"]
            .search([("partner_id", "=", self.env.user.partner_id.id), ("is_pinned", "=", True)])
            .mapped("channel_id")
        )
        values["channel_livechat"] = self.search(
            [("channel_type", "=", "livechat"), ("id", "in", pinned_channels.ids)]
        ).channel_info()
        return values
    ...
    def _define_command_history(self):
        return {"channel_types": ["livechat"], "help": _("See 15 last visited pages")}

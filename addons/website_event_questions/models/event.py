...
class EventEvent(models.Model):
    ...
    general_question_ids = fields.One2many(
        "event.question", "event_id", "General Questions", domain=[("is_individual", "=", False)]
    )
    specific_question_ids = fields.One2many(
        "event.question", "event_id", "Specific Questions", domain=[("is_individual", "=", True)]
    )
    ...
    def create(self, vals):
        ...
        if event_id:
            ...
            if event.event_type_id.use_questions and event.event_type_id.question_ids:
                vals["answer_ids"] = vals.get("answer_ids", []) + [
                    (
                        0,
                        0,
                        {

...
class BaseAutomation(models.Model):
    ...
    filter_domain = fields.Char(
        string="Apply on", help="If present, this condition must be satisfied before executing the action rule."
    )
    last_run = fields.Datetime(readonly=True, copy=False)
    on_change_fields = fields.Char(
        string="On Change Fields Trigger", help="Comma-separated list of field names that triggers the onchange."
    )
    ...
    def create(self, vals):
        ...
        base_automation = super().create(vals)
        self._update_cron()
    ...
    def _register_hook(self):
        ...
        for action_rule in self.with_context({}).search([]):
            ...
            if Model is None:
                _logger.warning(
                    "Action rule with ID %d depends on model %s" % (action_rule.id, action_rule.model_name)
                )
                continue

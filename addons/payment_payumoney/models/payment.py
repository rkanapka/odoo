...
class PaymentAcquirerPayumoney(models.Model):
    ...
    payumoney_merchant_key = fields.Char(
        string="Merchant Key", required_if_provider="payumoney", groups="base.group_user"
    )
    payumoney_merchant_salt = fields.Char(
        string="Merchant Salt", required_if_provider="payumoney", groups="base.group_user"
    )
...
class PaymentTransactionPayumoney(models.Model):
    ...
    def _payumoney_form_get_tx_from_data(self, data):
        ...
        if not reference or not pay_id or not shasign:
            raise ValidationError(
                _("PayUmoney: received data with missing reference (%s) or pay_id (%s) or shashign (%s)")
        ...
        if shasign_check.upper() != shasign.upper():
            raise ValidationError(
                _("PayUmoney: invalid shasign, received %s, computed %s, for data %s") % (shasign, shasign_check, data)
            )
        return transaction
    ...
    def _payumoney_form_get_invalid_parameters(self, data):
        ...
        if self.acquirer_reference and data.get("mihpayid") != self.acquirer_reference:
            invalid_parameters.append(("Transaction Id", data.get("mihpayid"), self.acquirer_reference))
        # check what is buyed

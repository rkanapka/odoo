...
class PayuMoneyController(http.Controller):
    @http.route(
        ["/payment/payumoney/return", "/payment/payumoney/cancel", "/payment/payumoney/error"],
        type="http",
        auth="public",
        csrf=False,
    )
    def payu_return(self, **post):
        """PayUmoney."""
        _logger.info("PayUmoney: entering form_feedback with post data %s", pprint.pformat(post))
        return_url = "/"
        if post:

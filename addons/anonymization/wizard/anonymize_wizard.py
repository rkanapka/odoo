...
class IrModelFieldsAnonymizeWizard(models.TransientModel):
    ...
    def _get_summary_value(self):
        ...
        for anon_field in self.env["ir.model.fields.anonymization"].search([("state", "!=", "not_existing")]):
            ...
            if field:
                ...
                summary += (
                    " * %(model_name)s (%(model_code)s) -> %(field_name)s (%(field_code)s): state: (%(state)s)\n"
                    % values
                )
            else:
                summary += "* Missing local model (%s) and field (%s): state: (%s) \n" % (
                    anon_field.model_name,
    ...
    def anonymize_database(self):
        ...
        raise UserError(
            """The Odoo Migration Platform no longer accepts anonymized databases.\n
        ...
        history = self.env["ir.model.fields.anonymization.history"].create(
            {"date": fields.Datetime.now(), "state": "started", "direction": "clear -> anonymized"}
        )
        ...
        if state == "anonymized":
            raise UserError(
                "%s: %s" % (error_type, _("The database is currently anonymized, you cannot anonymize it again."))
            )
        elif state == "unstable":
            raise UserError(
                "%s: %s"
        ...
        for field in ano_fields:
            ...
            for record in self.env.cr.dictfetchall():
                data.append(
                    {"model_id": model_name, "field_id": field_name, "id": record["id"], "value": record[field_name]}
                )
    ...
    def reverse_anonymize_database(self):
        ...
        for line in data:
            ...
            if custom_updates:
                ...
                queries = [
                    (record["query"], record["query_type"]) for record in custom_updates if record["query_type"]
                ]
            elif table_name:
                queries = [
                    (

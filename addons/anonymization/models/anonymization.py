...
ANONYMIZATION_DIRECTION = [
    ("clear -> anonymized", "clear -> anonymized"),
    ("anonymized -> clear", "anonymized -> clear"),
]
...
def group(lst, cols):
    if isinstance(cols, pycompat.string_types):
        cols = [cols]
    return {k: [v for v in itr] for k, itr in groupby(sorted(lst, key=itemgetter(*cols)), itemgetter(*cols))}
...
class IrModelFieldsAnonymization(models.Model):
    ...
    _sql_constraints = [
        (
            "model_id_field_id_uniq",
            "unique (model_name, field_name)",
            "You cannot have two fields with the same name on the same object!",
        ),
    ]

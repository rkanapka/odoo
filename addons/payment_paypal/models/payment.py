...
class AcquirerPaypal(models.Model):
    ...
    paypal_use_ipn = fields.Boolean(
        "Use IPN", default=True, help="Paypal Instant Payment Notification", groups="base.group_user"
    )
        ...
        paypal_tx_values.update(
            {
                ...
                "state": values.get("partner_state")
                and (values.get("partner_state").code or values.get("partner_state").name)
                or "",
                ...
                "custom": (
                    json.dumps({"return_url": "%s" % paypal_tx_values.pop("return_url")})
...
class TxPaypal(models.Model):
    ...
    def _paypal_form_get_invalid_parameters(self, data):
        ...
        if data.get("test_ipn"):
            _logger.warning("Received a notification from Paypal using sandbox"),
        ...
        if float_compare(float(data.get("mc_gross", "0.0")), (self.amount + self.fees), 2) != 0:
            invalid_parameters.append(
                ("mc_gross", data.get("mc_gross"), "%.2f" % self.amount)
        ...
        if (
            ...
            and self.acquirer_id.paypal_seller_account
            and data["receiver_id"] != self.acquirer_id.paypal_seller_account
        ):

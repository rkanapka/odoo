...
class CrmLead(models.Model):
    ...
    def retrieve_sales_dashboard(self):
        res = super().retrieve_sales_dashboard()
        ...
        invoice_data = self.env["account.invoice"].search_read(
            account_invoice_domain, ["date_invoice", "amount_untaxed_signed"]
        )
        ...
        for invoice in invoice_data:
            if invoice["date_invoice"]:
                ...
                elif invoice_date < date_today.replace(day=1) and invoice_date >= date_today.replace(
                    day=1

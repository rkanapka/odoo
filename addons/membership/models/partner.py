    ...
    def _compute_membership_cancel(self):
        for partner in self:
            partner.membership_cancel = (
                self.env["membership.membership_line"]
                .search([("partner", "=", partner.id)], limit=1, order="date_cancel desc")
                .date_cancel
            )
        ...
        for partner in self:
            ...
            if partner.member_lines:
                ...
                if s == 4:
                    for mline in partner.member_lines:
                        if (
                            (mline.date_from or "0000-00-00") < today
                            and (mline.date_to or "0000-00-00") < today
                            and (mline.date_from or "0000-00-00") <= (mline.date_to or "0000-00-00")

...
class TestMembership(TestMembershipCommon):
    ...
    def test_old_membership(self):
        ...
        self.assertEqual(
            self.partner_1.membership_state, "old", "membership: outdated subscription should put member in old state"
        )

    def test_paid_membership(self):
        self.assertEqual(
            self.partner_1.membership_state, "none", "membership: default membership status of partners should be None"
        )
        ...
        self.assertEqual(
            self.partner_1.membership_state, "waiting", "membership: new membership should be in waiting state"
        )
        ...
        self.assertEqual(
            self.partner_2.membership_state, "free", "membership: free member customer should be in free state"
        )
        self.partner_2.write({"free_member": False, "associate_member": self.partner_1.id})
        self.assertEqual(
            self.partner_2.membership_state, "paid", "membership: associated customer should be in paid state"
        )

...
class StockInventory(models.Model):
    ...
    def post_inventory(self):
        ...
        for inventory in acc_inventories:
            res = super(
                StockInventory, inventory.with_context(force_period_date=inventory.accounting_date)
            ).post_inventory()
    ...
    def create(self, vals):
        ...
        if move.state == "done":
            ...
            if move.product_id.valuation == "real_time" and (move._is_in() or move._is_out()):
                move.with_context(
                    force_valuation_amount=correction_value, forced_quantity=res.qty_done
                )._account_entry_move()
        return res
            ...
            for move_id, qty_difference in moves_to_update.items():
                ...
                if move_id.product_id.cost_method in ["standard", "average"]:
                    ...
                    if move_id._is_in():
                        ...
                        move_id.remaining_value + correction_value
                        move_vals["value"] = move_id.value + correction_value
                        ...
                        candidates_receipt = self.env["stock.move"].search(
                            move_id._get_in_domain(), order="date, id desc", limit=1
                        )
                ...
                if move_id.product_id.valuation == "real_time":
                    move_id.with_context(
                        force_valuation_amount=correction_value, forced_quantity=qty_difference
                    )._account_entry_move()
                if qty_difference > 0:
...
class StockMove(models.Model):
    ...
    def _run_fifo(self, move, quantity=None):
        ...
        if new_standard_price and move.product_id.cost_method == "fifo":
            move.product_id.sudo().with_context(force_company=move.company_id.id).standard_price = new_standard_price
        ...
        if qty_to_take_on_candidates == 0:
            move.write(
                {
                    "value": (
                        -tmp_value if not quantity else move.value or -tmp_value
    ...
    def _action_done(self):
        ...
        for move in res.filtered(
            ...
            and (m._is_in() or m._is_out() or m._is_dropshipped() or m._is_dropshipped_returned())
        ):
    ...
    def product_price_update_before_done(self, forced_qty=None):
        ...
        for move in self.filtered(lambda move: move._is_in() and move.product_id.cost_method == "average"):
            ...
            if float_is_zero(product_tot_qty_available, precision_rounding=rounding) or product_tot_qty_available < 0:
                ...
                amount_unit = (
                    std_price_update.get((move.company_id.id, move.product_id.id)) or move.product_id.standard_price
                )
    ...
    def _run_fifo_vacuum(self):
        ...
        fifo_valued_products |= (
            self.env["product.template"].search([("property_cost_method", "=", "fifo")]).mapped("product_variant_ids")
        )
    ...
    def _account_entry_move(self):
        ...
        if self._is_in():
            ...
            if location_from and location_from.usage == "customer":  # goods returned from customer
                self.with_context(force_company=company_to.id)._create_account_move_line(
        ...
        if self._is_out():
            ...
            if location_to and location_to.usage == "supplier":  # goods returned to supplier
                self.with_context(force_company=company_from.id)._create_account_move_line(
        ...
        if self.company_id.anglo_saxon_accounting:
            ...
            if self._is_dropshipped():
                self.with_context(force_company=self.company_id.id)._create_account_move_line(

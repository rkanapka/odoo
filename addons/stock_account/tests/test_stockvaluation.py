...
class TestStockValuation(TransactionCase):
    ...
    def test_fifo_perpetual_1(self):
        ...
        self.assertTrue(
            set(move1.account_move_ids.mapped("line_ids").ids)
            == {
                move1_input_aml.id,
                move1_valuation_aml.id,
                move1_correction_input_aml.id,
                move1_correction_valuation_aml.id,
            }
        )
        ...
        move5_correction_output_aml = move5_correction_account_move.line_ids.filtered(
            lambda ml: ml.account_id == self.stock_output_account
        )
        self.assertEqual(move5_correction_output_aml.debit, 40)
        self.assertEqual(move5_correction_output_aml.credit, 0)
        move5_correction_valuation_aml = move5_correction_account_move.line_ids.filtered(
            lambda ml: ml.account_id == self.stock_valuation_account
        )
        self.assertEqual(move5_correction_valuation_aml.debit, 0)
        self.assertEqual(move5_correction_valuation_aml.credit, 40)
        self.assertTrue(
            set(move5.account_move_ids.mapped("line_ids").ids)
            == {
                move5_valuation_aml.id,
                move5_output_aml.id,
                move5_correction_output_aml.id,
                move5_correction_valuation_aml.id,
            }
        )
        self.assertTrue(set(move6.account_move_ids.line_ids.ids) == {move6_valuation_aml.id, move6_input_aml.id})
        ...
        self.assertTrue(
            set(move6.account_move_ids.mapped("line_ids").ids)
            == {
                move6_input_aml.id,
                move6_valuation_aml.id,
                move6_correction_input_aml.id,
                move6_correction_valuation_aml.id,
            }
        )
        ...
        self.assertTrue(
            ...
            == {
                ...
                move6_correction_input_aml.id,
                move6_correction_valuation_aml.id,
                move6_correction2_input_aml.id,
                move6_correction2_valuation_aml.id,
            }
        )
    ...
    def test_fifo_perpetual_4(self):
        ...
        stock_return_picking = (
            self.env["stock.return.picking"].with_context(active_ids=[out_pick.id], active_id=out_pick.id).create({})
        )
    ...
    def test_fifo_negative_1(self):
        ...
        self.assertTrue(
            set(move1.mapped("account_move_ids.line_ids").ids)
            == {move1_valuation_aml.id, move1_output_aml.id, vacuum1_valuation_aml.id, vacuum1_output_aml.id}
        )
        ...
        self.assertTrue(
            ...
            == {
                move1_valuation_aml.id,
                move1_output_aml.id,
                vacuum1_valuation_aml.id,
                vacuum1_output_aml.id,
                vacuum2_valuation_aml.id,
                vacuum2_output_aml.id,
            }
        )
    ...
    def test_at_date_standard_1(self):
        ...
        self.env["product.price.history"].search(
            [("product_id", "=", self.product1.id)], order="datetime desc, id DESC", limit=1
        ).datetime = date1

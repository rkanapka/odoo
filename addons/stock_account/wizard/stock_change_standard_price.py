...
class StockChangeStandardPrice(models.TransientModel):
    ...
    counterpart_account_id = fields.Many2one(
        "account.account", string="Counter-Part Account", domain=[("deprecated", "=", False)]
    )
    ...
    def default_get(self, fields):
        ...
        if "counterpart_account_id" in fields and "counterpart_account_id" not in res:
            res["counterpart_account_id"] = (
                product_or_template.property_account_expense_id.id
                or product_or_template.categ_id.property_account_expense_categ_id.id
            )

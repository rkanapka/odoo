...
def geo_query_address(street=None, zip=None, city=None, state=None, country=None):
    ...
    return tools.ustr(
        ", ".join(field for field in [street, ("%s %s" % (zip or "", city or "")).strip(), state, country] if field)
    )
    ...
    def geo_localize(self):
        ...
        for partner in self.with_context(lang="en_US"):
            result = partner._geo_localize(
                apikey, partner.street, partner.zip, partner.city, partner.state_id.name, partner.country_id.name
            )
            if result:
                partner.write(
                    {

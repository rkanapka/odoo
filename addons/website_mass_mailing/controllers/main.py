...
class MassMailController(MassMailController):
        ...
        if mailing.exists():
            if mailing.mailing_model_real == "mail.mass_mailing.contact":
                ...
                return request.render(
                    ...
                    {"contacts": contacts, "email": email, "mailing_id": mailing_id},
                )
            else:
                super().mailing(mailing_id, email=email, res_id=res_id, **post)
    ...
    def is_subscriber(self, list_id, **post):
        ...
        if email:
            contacts_count = (
                request.env["mail.mass_mailing.contact"]
                .sudo()
                .search_count([("list_ids", "in", [int(list_id)]), ("email", "=", email), ("opt_out", "=", False)])
            )

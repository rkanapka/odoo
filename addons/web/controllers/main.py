...
from werkzeug.urls import iri_to_uri, url_decode

import odoo
import odoo.modules.registry
from odoo import http
from odoo.api import Environment, call_kw
...
from odoo.tools import crop_image, html_escape, pycompat, topological_sort
from odoo.tools.misc import file_open, str2bool, xlwt
...
def module_installed(environment):
    ...
    modules = OrderedDict((module.name, module.dependencies_id.mapped("name")) for module in Modules.search(domain))
...
def module_installed_bypass_session(dbname):
    try:
        ...
        with registry.cursor() as cr:
            return module_installed(environment=Environment(cr, odoo.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}
...
class WebClient(http.Controller):
            ...
            try:
                return http.Response(
                    werkzeug.wsgi.wrap_file(
                        request.httprequest.environ, file_open("web/static/lib/moment/locale/%s.js" % code, "rb")
                    ),
    ...
    def translations(self, mods=None, lang=None):
        ...
        if mods is None:
            mods = [
                x["name"]
                for x in request.env["ir.module.module"].sudo().search_read([("state", "=", "installed")], ["name"])
            ]
        if lang is None:
...
class Binary(http.Controller):
    ...
    def upload(self, callback, ufile):
        ...
        try:
            data = ufile.read()
            args = [len(data), ufile.filename, ufile.content_type, pycompat.to_text(base64.b64encode(data))]
        except Exception as e:
            args = [False, str(e)]
    ...
    def upload_attachment(self, callback, model, id, ufile):
        ...
        for ufile in files:
            ...
            try:
                ...
                args.append({"filename": clean(filename), "mimetype": ufile.content_type, "id": attachment.id})
        return out % (json.dumps(clean(callback)), json.dumps(args))
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix="", parent_name="", import_compat=True, parent_field_type=None, exclude=None):
        ...
        fields_sequence = sorted(fields.items(), key=lambda field: odoo.tools.ustr(field[1].get("string", "")))
        ...
        for field_name, field in fields_sequence:
            if import_compat:
                ...
                if field.get("readonly"):
                    ...
                    if all(dict(attrs).get("readonly", True) for attrs in field.get("states", {}).values()):
                        continue
    ...
    def namelist(self, model, export_id):
        ...
        return [{"name": field["name"], "label": fields_data[field["name"]]} for field in export_fields_list]
    ...
    def fields_info(self, model, export_fields):
        ...
        for (base, length), subfields in itertools.groupby(
            ...
            if length == 2:
                ...
                info.update(self.graft_subfields(fields[base]["relation"], base, fields[base]["string"], subfields))
            elif base in fields:
    ...
    def graft_subfields(self, model, prefix, prefix_string, fields):
        ...
        return ((prefix + "/" + k, prefix_string + "/" + v) for k, v in self.fields_info(model, export_fields).items())
...
class ExportFormat:
    ...
    def base(self, data, token):
        ...
        if self.max_rows and len(records) > self.max_rows:
            raise UserError(
                _(
                    "There are too many records (%s records, limit: %s) to export as this format. Consider splitting the export."
                )
...
class ExcelExport(ExportFormat, http.Controller):
    ...
    def from_data(self, fields, rows):
        if len(rows) > self.max_rows:
            raise UserError(
                _(
                    "There are too many rows (%s rows, limit: 65535) to export as Excel 97-2003 (.xls) format. Consider splitting the export."
                )
                % len(rows)
            )
...
class Apps(http.Controller):
    ...
    def get_app_url(self, req, app):
        try:
            ...
            action = record.read(
                ["name", "type", "res_model", "view_mode", "view_type", "context", "views", "domain"]
            )[0]
        ...
        return werkzeug.utils.redirect("/web{}#sa={}".format(debug, sakey))
    ...
    def report_barcode(self, type, value, width=600, height=100, humanreadable=0):
        ...
        try:
            barcode = request.env["ir.actions.report"].barcode(
                type, value, width=width, height=height, humanreadable=humanreadable
            )

...
class TestRecruitmentProcess(common.TransactionCase):
    ...
    def test_00_recruitment_process(self):
        ...
        applicant = self.env["hr.applicant"].search(
            [("email_from", "=", "Mr. Richard Anderson <Richard_Anderson@yahoo.com>")], limit=1
        )
        ...
        self.assertEqual(
            applicant.name, "Application for the post of Jr.application Programmer.", "Applicant name does not match."
        )
        self.assertEqual(
            applicant.stage_id,

...
class HrEmployee(models.Model):
    ...
    newly_hired_employee = fields.Boolean(
        "Newly hired employee", compute="_compute_newly_hired_employee", search="_search_newly_hired_employee"
    )

    @api.multi
    def _compute_newly_hired_employee(self):
        read_group_result = self.env["hr.applicant"].read_group(
            [("emp_id", "in", self.ids), ("job_id.state", "=", "recruit")], ["emp_id"], ["emp_id"]
        )
        result = {data["emp_id"]: data["emp_id_count"] > 0 for data in read_group_result}
        for record in self:
    ...
    def _broadcast_welcome(self):
        ...
        if template_new_employee:
            ...
            channel_all_employees.message_post(body=body_html, subject=subject, subtype="mail.mt_comment")
        return True

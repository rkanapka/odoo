...
class RecruitmentSource(models.Model):
    ...
    def create_alias(self):
        ...
        for source in self:
            ...
            source.alias_id = (
                self.with_context(alias_model_name="hr.applicant", alias_parent_model_name="hr.job")
                .env["mail.alias"]
                .create(vals)
            )
...
class RecruitmentStage(models.Model):
    ...
    sequence = fields.Integer(
        "Sequence", default=10, help="Gives the sequence order when displaying a list of stages."
    )
...
class Applicant(models.Model):
    ...
    user_id = fields.Many2one(
        "res.users", "Responsible", track_visibility="onchange", default=lambda self: self.env.uid
    )
    ...
    salary_proposed_extra = fields.Char(
        "Proposed Salary Extra", help="Salary Proposed by the Organisation, extra advantages"
    )
    ...
    availability = fields.Date(
        "Availability", help="The date at which the applicant will be available to start working"
    )
    ...
    emp_id = fields.Many2one(
        "hr.employee", string="Employee", track_visibility="onchange", help="Employee linked to the applicant."
    )
        ...
        if job_id:
            ...
            if not self.stage_id:
                stage_ids = (
                    self.env["hr.recruitment.stage"]
                    .search(
                        ["|", ("job_id", "=", False), ("job_id", "=", job.id), ("fold", "=", False)],
                        order="sequence asc",
                        limit=1,
                    )
                    .ids
    ...
    def get_empty_list_help(self, help):
        return super(
            ...
            self.with_context(
                ...
                empty_list_help_document_name=_("job applicants"),
            ),
    ...
    def action_get_attachment_tree_view(self):
        ...
        action["search_view_id"] = (
            self.env.ref("hr_recruitment.ir_attachment_view_search_inherit_hr_recruitment").id,
        )
        return action
    ...
    def message_get_reply_to(self, ids, default=None):
        ...
        return {
            applicant.id: aliases.get(applicant.job_id and applicant.job_id.id or 0, False) for applicant in applicants
        }
        ...
        for applicant in self:
            if applicant.partner_id:
                applicant._message_add_suggested_recipient(
                    recipients, partner=applicant.partner_id, reason=_("Contact")
                )
            elif applicant.email_from:
                applicant._message_add_suggested_recipient(
                    recipients, email=applicant.email_from, reason=_("Contact Email")
                )
        return recipients
        ...
        for applicant in self:
            ...
            if applicant.job_id and (applicant.partner_name or contact_name):
                ...
                applicant.job_id.message_post(
                    body=(
                        _("New Employee %s Hired") % applicant.partner_name

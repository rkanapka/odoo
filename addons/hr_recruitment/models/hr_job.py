...
class Job(models.Model):
    ...
    address_id = fields.Many2one(
        "res.partner", "Job Location", default=_default_address_id, help="Address where employees are working"
    )
    application_ids = fields.One2many("hr.applicant", "job_id", "Applications")
    application_count = fields.Integer(compute="_compute_application_count", string="Applications")
    manager_id = fields.Many2one(
        "hr.employee", related="department_id.manager_id", string="Department Manager", readonly=True, store=True
    )

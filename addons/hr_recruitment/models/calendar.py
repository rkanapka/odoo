...
class CalendarEvent(models.Model):
    ...
    def default_get(self, fields):
        ...
        if (
            "applicant_id" not in defaults
            and defaults.get("res_id")
            and (defaults.get("res_model") or defaults.get("res_model_id"))
        ):
            if (defaults.get("res_model") and defaults["res_model"] == "hr.applicant") or (
                defaults.get("res_model_id")
                and self.env["ir.model"].sudo().browse(defaults["res_model_id"]).model == "hr.applicant"
            ):

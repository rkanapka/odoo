...
class AccountMove(models.Model):
    ...
    def _get_new_hash(self, secure_seq_number):
        ...
        if prev_move and len(prev_move) != 1:
            raise UserError(
                _(
                    "An error occured when computing the inalterability. Impossible to get the unique previous posted journal entry."
                )
            )
    ...
    def _compute_string_to_hash(self):
        ...
        for move in self:
            ...
            move.l10n_fr_string_to_hash = dumps(
    ...
    def write(self, vals):
        ...
        for move in self:
            if move.company_id._is_accounting_unalterable():
                ...
                if move.state == "posted" and set(vals).intersection(MOVE_FIELDS):
                    raise UserError(
                        _(
                            "According to the French law, you cannot modify a journal entry in order for its posted data to be updated or deleted. Unauthorized field: %s."
                        )
                        % ", ".join(MOVE_FIELDS)
                    )
                # restrict the operation in case we are trying to overwrite existing hash
                if (move.l10n_fr_hash and "l10n_fr_hash" in vals) or (
                    move.l10n_fr_secure_sequence_number and "l10n_fr_secure_sequence_number" in vals
                ):
                    raise UserError(
        ...
        if has_been_posted:
            for move in self.filtered(
                lambda m: m.company_id._is_accounting_unalterable()
                and not (m.l10n_fr_secure_sequence_number or m.l10n_fr_hash)
            ):
    ...
    def _check_hash_integrity(self, company_id):
        ...
        moves = self.search(
            [("state", "=", "posted"), ("company_id", "=", company_id), ("l10n_fr_secure_sequence_number", "!=", 0)],
        ...
        if not moves:
            raise UserError(
                _(
                    "There isn't any journal entry flagged for data inalterability yet for the company %s. This mechanism only runs for journal entries generated after the installation of the module France - Certification CGI 286 I-3 bis."
                )
    ...
    def write(self, vals):
        ...
        if set(vals).intersection(LINE_FIELDS):
            if any(l.company_id._is_accounting_unalterable() and l.move_id.state == "posted" for l in self):
                raise UserError(
                    _(
                        "According to the French law, you cannot modify a journal item in order for its posted data to be updated or deleted. Unauthorized field: %s."
                    )
                    % ", ".join(LINE_FIELDS)
                )
...
class AccountJournal(models.Model):
    ...
    def _onchange_update_posted(self):
        if self.update_posted and self.company_id._is_accounting_unalterable():
            ...
            raise UserError(
                _(
                    "According to the French law, you cannot modify a journal in order for its posted data to be updated or deleted. Unauthorized field: %s."
                )
                % field_string
            )
    ...
    def _is_journal_alterable(self):
        ...
        if self.env["account.move"].search(critical_domain):
            raise UserError(
                _(
                    "It is not permitted to disable the data inalterability in this journal (%s) since journal entries have already been protected."
                )
                % (self.name,)
            )
        return True

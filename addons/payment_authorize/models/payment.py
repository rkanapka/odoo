...
class PaymentAcquirerAuthorize(models.Model):
    ...
    authorize_transaction_key = fields.Char(
        string="API Transaction Key", required_if_provider="authorize", groups="base.group_user"
    )
    authorize_signature_key = fields.Char(
        string="API Signature Key",
        groups="base.group_user",
        compute="_compute_auth_signature_key",
        inverse="_inverse_auth_signature_key",
    )
    ...
    def authorize_form_generate_values(self, values):
        ...
        if values.get("billing_partner_country") and values.get("billing_partner_country") == self.env.ref(
            "base.us", False
        ):
        ...
        if data["cc_expiry"]:
            ...
            try:
                expiry_date = datetime.strptime(
                    "/".join(cc_expiry), "%m/%{}".format("y" if len(cc_expiry[1]) == 2 else "Y")
                ).strftime("%y%m")
...
class TxAuthorize(models.Model):
    ...
    def _authorize_form_get_tx_from_data(self, data):
        ...
        reference, trans_id, fingerprint = (
            ...
            data.get("x_trans_id"),
            data.get("x_SHA2_Hash") or data.get("x_MD5_Hash"),
        )
    ...
    def _authorize_form_validate(self, data):
        ...
        if status_code == self._authorize_valid_tx_status:
            ...
            if (
                ...
                and (self.type == "form_save" or self.acquirer_id.save_token == "always")
            ):
                transaction = AuthorizeAPI(self.acquirer_id)
...
class PaymentToken(models.Model):
    ...
    authorize_profile = fields.Char(
        ...
        help="This contains the unique reference "

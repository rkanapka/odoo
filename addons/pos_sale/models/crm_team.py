                ...
                .search(
                    [
                        (
                            "session_id",
                            "in",
                            team.pos_config_ids.mapped("session_ids").filtered(lambda s: s.state == "opened").ids,
                        )
                    ]
                )
                .mapped("price_total")
        ...
        if self.team_type == "pos":
            ...
            if self.dashboard_graph_group_pos == "pos":
                ...
                for data_point in order_data:
                    result.append(
                        {
                            "x_value": self.env["pos.config"].browse(data_point.get("config_id")[0]).name,
                            "y_value": data_point.get("price_total"),
                        }
                    )
                ...
                if self.dashboard_graph_group_pos == "day":
                    for data_point in order_data:
                        result.append(
                            {
                                "x_value": fields.Date.to_string(
                                    fields.datetime.strptime(data_point.get("date:day"), "%d %b %Y")
                                ),
                                "y_value": data_point.get("price_total"),
                            }
                        )
                    ...
                    for data_point in order_data:
                        result.append(
                            {
                                "x_value": int(data_point.get("date:week")[1:3]),
                                "y_value": data_point.get("price_total"),
                            }
                        )
                    ...
                    for data_point in order_data:
                        result.append(
                            {
                                "x_value": fields.datetime.strptime(data_point.get("date:month"), "%B %Y").month,
                                "y_value": data_point.get("price_total"),
                            }
                        )
            return result

        return super()._graph_data(start_date, end_date)

...
class BarcodeNomenclature(models.Model):
    ...
    def check_ean(self, ean):
        return re.match(r"^\d+$", ean) and self.ean_checksum(ean) == int(ean[-1])

    # returns true if the barcode string is encoded with the provided encoding.
    def check_encoding(self, barcode, encoding):
        if encoding == "ean13":
            return (
                len(barcode) == 13 and re.match(r"^\d+$", barcode) and self.ean_checksum(barcode) == int(barcode[-1])
            )
        elif encoding == "ean8":
            return (
                len(barcode) == 8 and re.match(r"^\d+$", barcode) and self.ean8_checksum(barcode) == int(barcode[-1])
            )
        elif encoding == "upca":
            return (
                len(barcode) == 12
                and re.match(r"^\d+$", barcode)
                and self.ean_checksum("0" + barcode) == int(barcode[-1])
            )
        elif encoding == "any":
            return True
        else:
    ...
    def match_pattern(self, barcode, pattern):
        ...
        barcode = barcode.replace("\\", "\\\\").replace("{", r"\{").replace("}", r"\}").replace(".", r"\.")
        ...
        if numerical_content:  # the pattern encodes a numerical content
            ...
            match["base_code"] = (
                match["base_code"].replace("\\\\", "\\").replace(r"\{", "{").replace(r"\}", "}").replace(r"\.", ".")
            )
            pattern = (
    ...
    def parse_barcode(self, barcode):
        ...
        for rule in rules:
            ...
            if (
                rule["encoding"] == "ean13"
                and self.check_encoding(barcode, "upca")
                and self.upc_ean_conv in ["upc2ean", "always"]
            ):
                cur_barcode = "0" + cur_barcode
            elif (
                rule["encoding"] == "upca"
                and self.check_encoding(barcode, "ean13")
                and barcode[0] == "0"
...
class BarcodeRule(models.Model):
    ...
    sequence = fields.Integer(
        string="Sequence", help="Used to order rules such that rules with a smaller sequence match first"
    )
    ...
    type = fields.Selection(
        [("alias", "Alias"), ("product", "Unit Product")], string="Type", required=True, default="product"
    )
    pattern = fields.Char(
        string="Barcode Pattern", size=32, help="The barcode matching pattern", required=True, default=".*"
    )
    alias = fields.Char(
        string="Alias", size=32, default="0", help="The matched pattern will alias to this barcode", required=True
    )

    @api.one
    @api.constrains("pattern")
    def _check_pattern(self):
        p = self.pattern.replace("\\\\", "X").replace(r"\{", "X").replace(r"\}", "X")
        findall = re.findall("[{]|[}]", p)  # p does not contain escaped { or }
        if len(findall) == 2:
            if not re.search("[{][N]*[D]*[}]", p):
                raise ValidationError(
                    _("There is a syntax error in the barcode pattern ")
                    + self.pattern
                    + _(": braces can only contain N's followed by D's.")
                )
            elif re.search("[{][}]", p):
                raise ValidationError(
                    _("There is a syntax error in the barcode pattern ") + self.pattern + _(": empty braces.")
                )
        elif len(findall) != 0:
            raise ValidationError(
                _("There is a syntax error in the barcode pattern ")
                + self.pattern
                + _(": a rule can only contain one pair of braces.")
            )
        elif p == "*":

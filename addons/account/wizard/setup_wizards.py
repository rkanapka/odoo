...
class FinancialYearOpeningWizard(models.TransientModel):
    ...
    account_setup_fy_data_done = fields.Boolean(
        string="Financial year setup marked as done", compute="_compute_setup_marked_done"
    )
...
class OpeningAccountMoveWizard(models.TransientModel):
    ...
    opening_move_id = fields.Many2one(
        string="Opening Journal Entry", comodel_name="account.move", related="company_id.account_opening_move_id"
    )
    ...
    journal_id = fields.Many2one(
        string="Journal", comodel_name="account.journal", required=True, related="opening_move_id.journal_id"
    )

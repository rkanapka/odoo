...
class AccountInvoiceReport(models.Model):
    ...
    def _compute_amounts_in_user_currency(self):
        ...
        currency_rate_id = self.env["res.currency.rate"].search(
            [("rate", "=", 1), "|", ("company_id", "=", self.env.user.company_id.id), ("company_id", "=", False)],
            limit=1,
        )
        ...
        for record in self:
            ...
            record.user_currency_price_total = base_currency_id.with_context(ctx).compute(
                record.price_total, user_currency_id
            )
            record.user_currency_price_average = base_currency_id.with_context(ctx).compute(
                record.price_average, user_currency_id
            )
            record.user_currency_residual = base_currency_id.with_context(ctx).compute(
                record.residual, user_currency_id
            )
    ...
    payment_term_id = fields.Many2one(
        "account.payment.term", string="Payment Terms", oldname="payment_term", readonly=True
    )
    fiscal_position_id = fields.Many2one(
        "account.fiscal.position", oldname="fiscal_position", string="Fiscal Position", readonly=True
    )
    ...
    user_currency_price_total = fields.Float(
        string="Total Without Tax", compute="_compute_amounts_in_user_currency", digits=0
    )
    price_average = fields.Float(string="Average Price", readonly=True, group_operator="avg")
    user_currency_price_average = fields.Float(
        string="Average Price", compute="_compute_amounts_in_user_currency", digits=0
    )
    currency_rate = fields.Float(
        string="Currency Rate", readonly=True, group_operator="avg", groups="base.group_multi_currency"
    )
    ...
    account_id = fields.Many2one(
        "account.account", string="Account", readonly=True, domain=[("deprecated", "=", False)]
    )
    account_line_id = fields.Many2one(
        "account.account", string="Account Line", readonly=True, domain=[("deprecated", "=", False)]
    )
    ...
    user_currency_residual = fields.Float(
        string="Total Residual", compute="_compute_amounts_in_user_currency", digits=0
    )

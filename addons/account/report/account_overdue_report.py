...
class ReportOverdue(models.AbstractModel):
    ...
    def _get_account_move_lines(self, partner_ids):
        ...
        self.env.cr.execute(
            ...
            "WHERE l.partner_id IN %s AND at.type IN ('receivable', 'payable') AND l.full_reconcile_id IS NULL GROUP BY l.date, l.name, l.ref, l.date_maturity, l.partner_id, at.type, l.blocked, l.amount_currency, l.currency_id, l.move_id, m.name",
    ...
    def get_report_values(self, docids, data=None):
        ...
        for partner_id in docids:
            ...
            for line_tmp in lines[partner_id]:
                line = line_tmp.copy()
                currency = (
                    line["currency_id"] and self.env["res.currency"].browse(line["currency_id"]) or company_currency
                )
                if currency not in lines_to_display[partner_id]:
                    lines_to_display[partner_id][currency] = []
                    totals[partner_id][currency] = {fn: 0.0 for fn in ["due", "paid", "mat", "total"]}

...
from odoo.tools import float_is_zero, float_repr, float_round, pycompat
...
class AccountBankStatement(models.Model):
    ...
    def _is_difference_zero(self):
        for bank_stmt in self:
            bank_stmt.is_difference_zero = float_is_zero(
                bank_stmt.difference, precision_digits=bank_stmt.currency_id.decimal_places
            )
    ...
    balance_start = fields.Monetary(
        string="Starting Balance", states={"confirm": [("readonly", True)]}, default=_default_opening_balance
    )
    ...
    total_entry_encoding = fields.Monetary(
        "Transactions Subtotal", compute="_end_balance", store=True, help="Total of transaction lines."
    )
    ...
    move_line_ids = fields.One2many(
        "account.move.line", "statement_id", string="Entry lines", states={"confirm": [("readonly", True)]}
    )
    ...
    is_difference_zero = fields.Boolean(
        compute="_is_difference_zero", string="Is zero", help="Check if difference is zero."
    )
...
class AccountBankStatementLine(models.Model):
    ...
    statement_id = fields.Many2one(
        "account.bank.statement", string="Statement", index=True, required=True, ondelete="cascade"
    )
    journal_id = fields.Many2one(
        "account.journal", related="statement_id.journal_id", string="Journal", store=True, readonly=True
    )
    ...
    company_id = fields.Many2one(
        "res.company", related="statement_id.company_id", string="Company", store=True, readonly=True
    )
    journal_entry_ids = fields.One2many(
        "account.move.line", "statement_line_id", "Journal Items", copy=False, readonly=True
    )
    amount_currency = fields.Monetary(
        help="The amount expressed in an optional other currency if it is a multi-currency entry."
    )
    currency_id = fields.Many2one(
        "res.currency", string="Currency", help="The optional other currency if it is a multi-currency entry."
    )
    ...
    def get_data_for_reconciliation_widget(self, excluded_ids=None):
        ...
        for st_line in self:
            ...
            target_currency = (
                st_line.currency_id or st_line.journal_id.currency_id or st_line.journal_id.company_id.currency_id
            )
            rp = aml_recs.prepare_move_lines_for_reconciliation_widget(
                target_currency=target_currency, target_date=st_line.date
            )
            excluded_ids += [move_line["id"] for move_line in rp]
            ret.append(
                {"st_line": st_line.get_statement_line_for_reconciliation_widget(), "reconciliation_proposition": rp}
            )
        return ret
    ...
    def get_statement_line_for_reconciliation_widget(self):
        ...
        data = {
            ...
            "account_id": [
                ...
                self.journal_id.default_debit_account_id.display_name,
            ],
    ...
    def get_move_lines_for_reconciliation_widget(
        ...
        aml_recs = self.get_move_lines_for_reconciliation(
            partner_id=partner_id, excluded_ids=excluded_ids, str=str, offset=offset, limit=limit
        )
        target_currency = self.currency_id or self.journal_id.currency_id or self.journal_id.company_id.currency_id
        return aml_recs.prepare_move_lines_for_reconciliation_widget(
            target_currency=target_currency, target_date=self.date
        )
    ...
    def _get_common_sql_query(self, overlook_partner=False, excluded_ids=None, split=False):
        acc_type = (
            ...
            if (self.partner_id or overlook_partner)
    ...
    def get_reconciliation_proposition(self, excluded_ids=None):
        ...
        sql_query = (
            ...
            + " AND ("
            + field
            + " = %(amount)s::numeric OR (acc.internal_type = 'liquidity' AND "
            + liquidity_field
            + " = "
    ...
    def process_reconciliations(self, data):
        ...
        for st_line, datum in pycompat.izip(self, data_copy):
            ...
            st_line.with_context(ctx).process_reconciliation(
                datum.get("counterpart_aml_dicts", []), payment_aml_rec, datum.get("new_aml_dicts", [])
            )
    ...
    def process_reconciliation(self, counterpart_aml_dicts=None, payment_aml_rec=None, new_aml_dicts=None):
        ...
        for aml_rec in payment_aml_rec:
            ...
            total -= aml_currency.with_context(date=aml_rec.date, company_id=aml_rec.company_id.id).compute(
                balance, currency
            )
            ...
            for aml_dict in to_create:
                ...
                if st_line_currency.id != company_currency.id:
                    ...
                    if self.currency_id and statement_currency.id == company_currency.id and st_line_currency_rate:
                        ...
                        aml_dict["credit"] = st_line_currency.with_context(ctx).compute(
                            aml_dict["credit"], company_currency
                        )

...
class account_journal(models.Model):
    ...
    def get_line_graph_datas(self):
        ...
        last_bank_stmt = self.env["account.bank.statement"].search(
            [("journal_id", "in", self.ids), ("date", "<=", last_month.strftime(DF))],
            ...
            if i == 0:
                ...
                if start_week.month == end_week.month:
                    label = (
                        ...
                        + str(end_week.day)
                        + " "
                        + format_date(end_week, "MMM", locale=self._context.get("lang") or "en_US")
                    )
                else:
                    label = (
    ...
    def get_journal_dashboard_datas(self):
        ...
        if self.type in ["bank", "cash"]:
            last_bank_stmt = self.env["account.bank.statement"].search(
                [("journal_id", "in", self.ids)], order="date desc, id desc", limit=1
            )
    ...
    def _get_open_bills_to_pay_query(self):
        ...
        """
        return (
            """SELECT state, amount_total, currency_id AS currency, type
                  FROM account_invoice
                  WHERE journal_id = %(journal_id)s AND state = 'open';""",
            {"journal_id": self.id},
        )

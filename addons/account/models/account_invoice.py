...
class AccountInvoice(models.Model):
    ...
    def _compute_residual(self):
        ...
        for line in self._get_aml_for_amount_residual():
            ...
            if line.currency_id == self.currency_id:
                ...
                if line.currency_id:
                    residual += line.currency_id.with_context(date=line.date).compute(
                        line.amount_residual_currency, self.currency_id
                    )
                else:
                    residual += line.company_id.currency_id.with_context(date=line.date).compute(
    ...
    sent = fields.Boolean(
        readonly=True, default=False, copy=False, help="It indicates that the invoice has been sent."
    )
    ...
    refund_invoice_ids = fields.One2many(
        "account.invoice", "refund_invoice_id", string="Refund Invoices", readonly=True
    )
    ...
    amount_untaxed = fields.Monetary(
        string="Untaxed Amount", store=True, readonly=True, compute="_compute_amount", track_visibility="always"
    )
    ...
    payment_move_line_ids = fields.Many2many(
        "account.move.line", string="Payment Move Lines", compute="_compute_payments", store=True
    )
    ...
    sequence_number_next = fields.Char(
        string="Next Number", compute="_get_sequence_number_next", inverse="_set_sequence_next"
    )
    ...
    def _get_printed_report_name(self):
        ...
        return (
            ...
            and _("Invoice - %s") % (self.number)
            or self.type == "out_refund"
            and self.state == "draft"
            and _("Credit Note")
            or self.type == "out_refund"
            and _("Credit Note - %s") % (self.number)
            or self.type == "in_invoice"
            and self.state == "draft"
            and _("Vendor Bill")
            or self.type == "in_invoice"
            and self.state in ("open", "paid")
            and _("Vendor Bill - %s") % (self.number)
            or self.type == "in_refund"
            and self.state == "draft"
            and _("Vendor Credit Note")
            or self.type == "in_refund"
            and _("Vendor Credit Note - %s") % (self.number)
        )
            ...
            if not self.currency_id.is_zero(rounding_amount):
                if self.cash_rounding_id.strategy == "biggest_tax":
                    ...
                    if rounding_amount > 0.0:
                        ...
                        self.cash_rounding_id._get_profit_account_id().id
    ...
    def get_taxes_values(self):
        ...
        for line in self.invoice_line_ids:
            ...
            taxes = line.invoice_line_tax_ids.compute_all(
                price_unit, self.currency_id, line.quantity, line.product_id, self.partner_id
            )["taxes"]
            for tax in taxes:
                ...
                res.append(
                    {
                        ...
                        "invoice_id": self.id,
                        "tax_ids": (
                            [(6, 0, list(done_taxes))] if done_taxes and tax_line.tax_id.include_base_amount else []
                        ),
                    }
                )
        ...
        for inv in self:
            ...
            if inv.payment_term_id:
                totlines = (
                    ...
                    .payment_term_id.with_context(currency_id=company_currency.id)
        ...
        for inv in self:
            if inv.cash_rounding_id:
                ...
                if rounding_amount != 0.0:
                    raise UserError(
                        _(
                            "The cash rounding cannot be computed because the difference must "
    ...
    def _prepare_payment_vals(self, pay_journal, pay_amount=None, date=None, writeoff_acc=None, communication=None):
        ...
        if payment_type == "inbound":
            ...
            pay_journal.outbound_payment_method_ids
    ...
    def pay_and_reconcile(self, pay_journal, pay_amount=None, date=None, writeoff_acc=None):
        ...
        payment_vals = self._prepare_payment_vals(
            pay_journal, pay_amount=pay_amount, date=date, writeoff_acc=writeoff_acc
        )
...
class AccountInvoiceLine(models.Model):
    ...
    uom_id = fields.Many2one(
        "product.uom", string="Unit of Measure", ondelete="set null", index=True, oldname="uos_id"
    )
    ...
    price_subtotal = fields.Monetary(
        string="Amount", store=True, readonly=True, compute="_compute_price", help="Total amount without taxes"
    )
    price_total = fields.Monetary(
        string="Amount", store=True, readonly=True, compute="_compute_price", help="Total amount with taxes"
    )
    ...
    quantity = fields.Float(
        string="Quantity", digits=dp.get_precision("Product Unit of Measure"), required=True, default=1
    )
    ...
    company_id = fields.Many2one(
        "res.company", string="Company", related="invoice_id.company_id", store=True, readonly=True, related_sudo=False
    )
    partner_id = fields.Many2one(
        "res.partner", string="Partner", related="invoice_id.partner_id", store=True, readonly=True, related_sudo=False
    )
    currency_id = fields.Many2one("res.currency", related="invoice_id.currency_id", store=True, related_sudo=False)
    company_currency_id = fields.Many2one(
        "res.currency", related="invoice_id.company_currency_id", readonly=True, related_sudo=False
    )
    ...
    def _set_currency(self):
        ...
        if company and currency:
            if company.currency_id != currency:
                self.price_unit = (
                    self.price_unit
                    * currency.with_context(dict(self._context or {}, date=self.invoice_id.date_invoice)).rate
                )
    ...
    def _set_taxes(self):
        ...
        if self.invoice_id.type in ("in_invoice", "in_refund"):
            ...
            if (
                not self.price_unit
                or float_compare(self.price_unit, self.product_id.standard_price, precision_digits=prec) == 0
            ):
...
class AccountInvoiceTax(models.Model):
    ...
    account_id = fields.Many2one(
        "account.account", string="Tax Account", required=True, domain=[("deprecated", "=", False)]
    )
    account_analytic_id = fields.Many2one("account.analytic.account", string="Analytic account")
    ...
    company_id = fields.Many2one(
        "res.company", string="Company", related="account_id.company_id", store=True, readonly=True
    )
...
class AccountPaymentTerm(models.Model):
    ...
    line_ids = fields.One2many(
        "account.payment.term.line", "payment_id", string="Terms", copy=True, default=_default_line_ids
    )
    company_id = fields.Many2one(
...
class AccountPaymentTermLine(models.Model):
    ...
    value_amount = fields.Float(
        string="Value", digits=dp.get_precision("Payment Terms"), help="For percent enter a ratio between 0-100."
    )
    ...
    payment_id = fields.Many2one(
        "account.payment.term", string="Payment Terms", required=True, index=True, ondelete="cascade"
    )

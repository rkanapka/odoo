...
def migrate_set_tags_and_taxes_updatable(cr, registry, module):
    ...
    xml_record_ids = (
        env["ir.model.data"]
        .search([("model", "in", ["account.tax.template", "account.account.tag"]), ("module", "like", module)])
        .ids
    )
    if xml_record_ids:
...
class AccountAccountTemplate(models.Model):
    ...
    tax_ids = fields.Many2many(
        "account.tax.template", "account_account_template_tax_rel", "account_id", "tax_id", string="Default Taxes"
    )
...
class AccountChartTemplate(models.Model):
    ...
    property_account_receivable_id = fields.Many2one(
        "account.account.template", string="Receivable Account", oldname="property_account_receivable"
    )
    property_account_payable_id = fields.Many2one(
        "account.account.template", string="Payable Account", oldname="property_account_payable"
    )
    property_account_expense_categ_id = fields.Many2one(
        "account.account.template", string="Category of Expense Account", oldname="property_account_expense_categ"
    )
    property_account_income_categ_id = fields.Many2one(
        "account.account.template", string="Category of Income Account", oldname="property_account_income_categ"
    )
    property_account_expense_id = fields.Many2one(
        "account.account.template", string="Expense Account on Product Template", oldname="property_account_expense"
    )
    property_account_income_id = fields.Many2one(
        "account.account.template", string="Income Account on Product Template", oldname="property_account_income"
    )
    ...
    def _install_template(
        self, company, code_digits=None, transfer_account_id=None, obj_wizard=None, acc_ref=None, taxes_ref=None
    ):
    ...
    def create_record_with_xmlid(self, company, template, model, vals):
        ...
        return ir_model_data._update(
            model, template_xmlid.module, vals, xml_id=new_xml_id, store=True, noupdate=True, mode="init", res_id=False
        )
    ...
    def _prepare_reconcile_model_vals(self, company, account_reconcile_model, acc_template_ref, tax_template_ref):
        ...
        return {
            ...
            "second_account_id": account_reconcile_model.second_account_id
            and acc_template_ref[account_reconcile_model.second_account_id.id]
            or False,
            ...
            "second_tax_id": account_reconcile_model.second_tax_id
            and tax_template_ref[account_reconcile_model.second_tax_id.id]
            or False,
        }
...
class AccountTaxTemplate(models.Model):
    ...
    sequence = fields.Integer(
        required=True, default=1, help="The sequence field is used to define order in which the tax lines are applied."
    )
    ...
    tag_ids = fields.Many2many(
        "account.account.tag", string="Account tag", help="Optional tags you may want to assign for custom reporting"
    )
    ...
    def _generate_tax(self, company):
        ...
        for tax in self:
            ...
            new_tax = self.env["account.chart.template"].create_record_with_xmlid(
                company, tax, "account.tax", vals_tax
            )
...
class AccountFiscalPositionTemplate(models.Model):
    ...
    country_id = fields.Many2one(
        "res.country", string="Country", help="Apply only if delivery or invoicing country match."
    )
...
class AccountFiscalPositionTaxTemplate(models.Model):
    ...
    position_id = fields.Many2one(
        "account.fiscal.position.template", string="Fiscal Position", required=True, ondelete="cascade"
    )
...
class AccountFiscalPositionAccountTemplate(models.Model):
    ...
    position_id = fields.Many2one(
        "account.fiscal.position.template", string="Fiscal Mapping", required=True, ondelete="cascade"
    )
...
class WizardMultiChartsAccounts(models.TransientModel):
    ...
    currency_id = fields.Many2one(
        "res.currency", string="Currency", help="Currency as per company's country.", required=True
    )
    ...
    def onchange_chart_template_id(self):
        ...
        if self.chart_template_id:
            currency_id = (
                ...
                and self.chart_template_id.currency_id.id
        ...
        if chart_templates:
            ...
            if context.get("default_charts"):
                model_data = self.env["ir.model.data"].search_read(
                    [("model", "=", "account.chart.template"), ("module", "=", context.get("default_charts"))],
                    ["res_id"],
                )
                if model_data:
            ...
            if "chart_template_id" in fields:
                res.update({"only_one_chart_template": len(chart_templates) == 1, "chart_template_id": chart_id})
            if "sale_tax_id" in fields:
                sale_tax = tax_templ_obj.search(
                    [("chart_template_id", "in", chart_hierarchy_ids), ("type_tax_use", "=", "sale")],
                    limit=1,
                    order="sequence",
                )
            ...
            if "purchase_tax_id" in fields:
                purchase_tax = tax_templ_obj.search(
                    [("chart_template_id", "in", chart_hierarchy_ids), ("type_tax_use", "=", "purchase")],
                    limit=1,
                    order="sequence",
                )
    ...
    def _create_tax_templates_from_rates(self, company_id):
        ...
        if not self.chart_template_id.complete_tax_set:
            ...
            ref_taxs = obj_tax_temp.search(
                [("type_tax_use", "=", "sale"), ("chart_template_id", "in", all_parents)],
            ...
            ref_taxs = obj_tax_temp.search(
                [("type_tax_use", "=", "purchase"), ("chart_template_id", "in", all_parents)],
                order="sequence, id desc",
                limit=1,
            )
            ref_taxs.write({"amount": value, "name": _("Tax %.2f%%") % value, "description": "%.2f%%" % value})
    ...
    def execute(self):
        ...
        acc_template_ref, taxes_ref = self.chart_template_id._install_template(
            company, code_digits=self.code_digits, transfer_account_id=self.transfer_account_id
        )
        ...
        if self.sale_tax_id and taxes_ref:
            IrDefault.sudo().set(
                "product.template", "taxes_id", [taxes_ref[self.sale_tax_id.id]], company_id=company.id
            )
        if self.purchase_tax_id and taxes_ref:
            IrDefault.sudo().set(
                "product.template", "supplier_taxes_id", [taxes_ref[self.purchase_tax_id.id]], company_id=company.id
            )
...
class AccountBankAccountsWizard(models.TransientModel):
    ...
    bank_account_id = fields.Many2one(
        "wizard.multi.charts.accounts", string="Bank Account", required=True, ondelete="cascade"
    )
    currency_id = fields.Many2one(
        "res.currency",
        string="Account Currency",
...
class AccountReconcileModelTemplate(models.Model):
    ...
    account_id = fields.Many2one(
        "account.account.template", string="Account", ondelete="cascade", domain=[("deprecated", "=", False)]
    )
    label = fields.Char(string="Journal Item Label")
    amount_type = fields.Selection(
        [("fixed", "Fixed"), ("percentage", "Percentage of balance")], required=True, default="percentage"
    )
    amount = fields.Float(
        digits=0,
        required=True,
    ...
    tax_id = fields.Many2one(
        "account.tax.template", string="Tax", ondelete="restrict", domain=[("type_tax_use", "=", "purchase")]
    )
    second_account_id = fields.Many2one(
        "account.account.template", string="Second Account", ondelete="cascade", domain=[("deprecated", "=", False)]
    )
    ...
    second_tax_id = fields.Many2one(
        "account.tax.template", string="Second Tax", ondelete="restrict", domain=[("type_tax_use", "=", "purchase")]
    )

...
class account_abstract_payment(models.AbstractModel):
    ...
    journal_id = fields.Many2one(
        "account.journal", string="Payment Journal", required=True, domain=[("type", "in", ("bank", "cash"))]
    )
    ...
    def _compute_hide_payment_method(self):
        for payment in self:
            ...
            payment.hide_payment_method = (
                len(journal_payment_methods) == 1 and journal_payment_methods[0].code == "manual"
            )
...
class account_payment(models.Model):
    ...
    destination_account_id = fields.Many2one(
        "account.account", compute="_compute_destination_account_id", readonly=True
    )
    ...
    destination_journal_id = fields.Many2one(
        "account.journal", string="Transfer To", domain=[("type", "in", ("bank", "cash"))]
    )
    ...
    writeoff_account_id = fields.Many2one(
        "account.account", string="Difference Account", domain=[("deprecated", "=", False)], copy=False
    )
    ...
    def _create_payment_entry(self, amount):
        ...
        if self.payment_difference_handling == "reconcile" and self.payment_difference:
            ...
            debit_wo, credit_wo, amount_currency_wo, currency_id = aml_obj.with_context(
                date=self.payment_date
            )._compute_amount_fields(self.payment_difference, self.currency_id, self.company_id.currency_id)
    ...
    def _create_transfer_entry(self, amount):
        ...
        amount_currency = (
            ...
            and self.currency_id.with_context(date=self.payment_date).compute(
                amount, self.destination_journal_id.currency_id
            )
            or 0
        )
    ...
    def _get_shared_move_line_vals(self, debit, credit, amount_currency, move_id, invoice_id=False):
        ...
        return {
            "partner_id": self.payment_type in ("inbound", "outbound")
    ...
    def _get_liquidity_move_line_vals(self, amount):
        ...
        if self.journal_id.currency_id and self.currency_id != self.journal_id.currency_id:
            ...
            debit, credit, amount_currency, dummy = (
                ...
                ._compute_amount_fields(amount, self.journal_id.currency_id, self.company_id.currency_id)
            )
            vals.update(
                {
    ...
    def _get_invoice_payment_amount(self, inv):
        ...
        """
        self.ensure_one()
        return sum([data["amount"] for data in inv._get_payments_vals() if data["account_payment_id"] == self.id])

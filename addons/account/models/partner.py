...
class AccountFiscalPosition(models.Model):
    ...
    country_group_id = fields.Many2one(
        "res.country.group", string="Country Group", help="Apply only if delivery or invocing country match the group."
    )
...
class AccountFiscalPositionAccount(models.Model):
    ...
    account_src_id = fields.Many2one(
        "account.account", string="Account on Product", domain=[("deprecated", "=", False)], required=True
    )
    account_dest_id = fields.Many2one(
        "account.account", string="Account to Use Instead", domain=[("deprecated", "=", False)], required=True
    )
...
class ResPartner(models.Model):
    ...
    def _credit_debit_get(self):
        tables, where_clause, where_params = (
            ...
            .with_context(state="posted", company_id=self.env.user.company_id.id)
            ._query_get()
        )
    ...
    def mark_as_reconciled(self):
        ...
        return (
            self.sudo()
            .with_context(company_id=self.env.user.company_id.id)
            .write({"last_time_entries_checked": time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)})
        )
    ...
    total_invoiced = fields.Monetary(
        compute="_invoice_total", string="Total Invoiced", groups="account.group_account_invoice"
    )
    ...
    ref_company_ids = fields.One2many(
        "res.company", "partner_id", string="Companies that refers to partner", oldname="ref_companies"
    )

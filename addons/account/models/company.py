...
class ResCompany(models.Model):
    ...
    property_stock_account_input_categ_id = fields.Many2one(
        "account.account", string="Input Account for Stock Valuation", oldname="property_stock_account_input_categ"
    )
    property_stock_account_output_categ_id = fields.Many2one(
        "account.account", string="Output Account for Stock Valuation", oldname="property_stock_account_output_categ"
    )
...
Best Regards,"""
    ...
    account_setup_company_data_done = fields.Boolean(
        string="Company Setup Marked As Done", help="Technical field holding the status of the company setup step."
    )
    account_setup_bank_data_done = fields.Boolean(
        "Bank Setup Marked As Done", help="Technical field holding the status of the bank setup step."
    )
    ...
    account_setup_bar_closed = fields.Boolean(
        string="Setup Bar Closed", help="Technical field set to True when setup bar has been closed by the user."
    )
    ...
    def _check_lock_dates(self, vals):
        ...
        * The new lock date for advisors must be set after the previous lock date.
        ...
        """
        period_lock_date = vals.get("period_lock_date") and time.strptime(
            vals["period_lock_date"], DEFAULT_SERVER_DATE_FORMAT
        )
        fiscalyear_lock_date = vals.get("fiscalyear_lock_date") and time.strptime(
            vals["fiscalyear_lock_date"], DEFAULT_SERVER_DATE_FORMAT
        )
        ...
        for company in self:
            old_fiscalyear_lock_date = company.fiscalyear_lock_date and time.strptime(
                company.fiscalyear_lock_date, DEFAULT_SERVER_DATE_FORMAT
            )
    ...
    def get_opening_move_differences(self, opening_move_lines):
        ...
        debit_diff = (
            (debits_sum > credits_sum) and float_round(difference, precision_rounding=currency.rounding) or 0.0
        )
        credit_diff = (
            (debits_sum < credits_sum) and float_round(difference, precision_rounding=currency.rounding) or 0.0
        )

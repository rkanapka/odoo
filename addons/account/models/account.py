...
class AccountAccount(models.Model):
    ...
    tax_ids = fields.Many2many(
        "account.tax", "account_account_tax_default_rel", "account_id", "tax_id", string="Default Taxes"
    )
        ...
        for record in self:
            ...
            if record.company_id.account_opening_move_id:
                for line in self.env["account.move.line"].search(
                    [("account_id", "=", record.id), ("move_id", "=", record.company_id.account_opening_move_id.id)]
                ):
    ...
    def write(self, vals):
        ...
        if vals.get("reconcile"):
            ...
            if len(move_lines):
                raise UserError(
                    _("You cannot change the value of the reconciliation on this account as it already has some moves")
                )
...
class AccountJournal(models.Model):
    ...
    type_control_ids = fields.Many2many(
        "account.account.type", "account_journal_type_rel", "journal_id", "type_id", string="Account Types Allowed"
    )
    ...
    currency_id = fields.Many2one(
        "res.currency", help="The currency used to enter statement", string="Currency", oldname="currency"
    )
    company_id = fields.Many2one(
        "res.company",
        string="Company",
    ...
    belongs_to_company = fields.Boolean(
        "Belong to the user's current company",
    ...
    bank_statements_source = fields.Selection(
        [("undefined", "Undefined Yet"), ("manual", "Record Manually")], string="Bank Feeds", default="undefined"
    )
    ...
    def _check_currency(self):
        if self.currency_id:
            if self.currency_id == self.company_id.currency_id:
                raise ValidationError(
                    _(
                        "Currency field should only be set if the journal's currency is different from the company's. Leave the field blank to use company currency."
                    )
                )
            if (
                self.default_credit_account_id
                and not self.default_credit_account_id.currency_id.id == self.currency_id.id
            ):
                raise ValidationError(
                    _(
            ...
            if (
                ...
                and not self.default_debit_account_id.currency_id.id == self.currency_id.id
            ):
                raise ValidationError(
                    _(
    ...
    def copy(self, default=None):
        default = dict(default or {})
        default.update(code=_("%s (copy)") % (self.code or ""), name=_("%s (copy)") % (self.name or ""))
        return super().copy(default)
        ...
        if vals.get("type") in ("bank", "cash"):
            ...
            if not default_account:
                ...
                account_vals = self._prepare_liquidity_account(
                    vals.get("name"), company, vals.get("currency_id"), vals.get("type")
                )
...
class AccountTax(models.Model):
    ...
    children_tax_ids = fields.Many2many(
        "account.tax", "account_tax_filiation_rel", "parent_tax", "child_tax", string="Children Taxes"
    )
    ...
    def onchange_amount(self):
        if self.amount_type in ("percent", "division") and self.amount != 0.0 and not self.description:
            self.description = "{:.4g}%".format(self.amount)
    ...
    def json_friendly_compute_all(self, price_unit, currency_id=None, quantity=1.0, product_id=None, partner_id=None):
        ...
        return self.compute_all(
            price_unit, currency=currency_id, quantity=quantity, product=product_id, partner=partner_id
        )
...
class AccountReconcileModel(models.Model):
    ...
    account_id = fields.Many2one(
        "account.account", string="Account", ondelete="cascade", domain=[("deprecated", "=", False)]
    )
    ...
    second_account_id = fields.Many2one(
        "account.account", string="Second Account", ondelete="cascade", domain=[("deprecated", "=", False)]
    )
    ...
    second_tax_id = fields.Many2one(
        "account.tax", string="Second Tax", ondelete="restrict", domain=[("type_tax_use", "=", "purchase")]
    )
    second_analytic_account_id = fields.Many2one(
        "account.analytic.account", string="Second Analytic Account", ondelete="set null"
    )

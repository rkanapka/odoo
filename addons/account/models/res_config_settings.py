...
class ResConfigSettings(models.TransientModel):
    ...
    chart_template_id = fields.Many2one("account.chart.template", string="Template", domain="[('visible','=', True)]")
    code_digits = fields.Integer(
        string="# of Digits *", related="company_id.accounts_code_digits", help="No. of digits to use for account code"
    )
    ...
    default_sale_tax_id = fields.Many2one(
        "account.tax", string="Default Sale Tax", company_dependent=True, oldname="default_sale_tax"
    )
    default_purchase_tax_id = fields.Many2one(
        "account.tax", string="Default Purchase Tax", company_dependent=True, oldname="default_purchase_tax"
    )
    ...
    tax_cash_basis_journal_id = fields.Many2one(
        "account.journal", related="company_id.tax_cash_basis_journal_id", string="Tax Cash Basis Journal"
    )
    account_hide_setup_bar = fields.Boolean(
    ...
    def get_values(self):
        ...
        default_sale_tax_id = IrDefault.get(
            "product.template", "taxes_id", company_id=self.company_id.id or self.env.user.company_id.id
        )
        default_purchase_tax_id = IrDefault.get(
            "product.template", "supplier_taxes_id", company_id=self.company_id.id or self.env.user.company_id.id
        )
        res.update(
    ...
    def set_values(self):
        ...
        IrDefault.set(
            "product.template", "supplier_taxes_id", self.default_purchase_tax_id.ids, company_id=self.company_id.id
        )
    ...
    def _onchange_tax_exigibility(self):
        ...
        tax = self.env["account.tax"].search(
            [("company_id", "=", self.env.user.company_id.id), ("tax_exigibility", "=", "on_payment")], limit=1
        )

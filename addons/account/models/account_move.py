...
class AccountMove(models.Model):
    ...
    date = fields.Date(
        required=True, states={"posted": [("readonly", True)]}, index=True, default=fields.Date.context_today
    )
    ...
    line_ids = fields.One2many(
        "account.move.line", "move_id", string="Journal Items", states={"posted": [("readonly", True)]}, copy=True
    )
    partner_id = fields.Many2one(
        "res.partner", compute="_compute_partner_id", string="Partner", store=True, readonly=True
    )
    amount = fields.Monetary(compute="_amount_compute", store=True)
    narration = fields.Text(string="Internal Note")
    company_id = fields.Many2one(
        "res.company", related="journal_id.company_id", string="Company", store=True, readonly=True
    )
    ...
    dummy_account_id = fields.Many2one(
        "account.account", related="line_ids.account_id", string="Account", store=False, readonly=True
    )
    ...
    def fields_view_get(self, view_id=None, view_type="form", toolbar=False, submenu=False):
        ...
        if self._context.get("vat_domain"):
            res["fields"]["line_ids"]["views"]["tree"]["fields"]["tax_line_id"]["domain"] = [
                ("tag_ids", "in", [self.env.ref(self._context.get("vat_domain")).id])
            ]
        return res
    ...
    def create(self, vals):
        move = super(
            AccountMove, self.with_context(check_move_validity=False, partner_id=vals.get("partner_id"))
        ).create(vals)
    ...
    def update_lines_tax_exigibility(self):
        if all(
            account.user_type_id.type not in {"payable", "receivable"}
            for account in self.mapped("line_ids.account_id")
        ):
            ...
            base_lines_caba = self.line_ids.filtered(
                lambda x: any(
                    tax.tax_exigibility == "on_payment"
                    or (tax.amount_type == "group" and "on_payment" in tax.mapped("children_tax_ids.tax_exigibility"))
    ...
    def _check_lock_date(self):
        for move in self:
            lock_date = max(
                move.company_id.period_lock_date or "0000-00-00", move.company_id.fiscalyear_lock_date or "0000-00-00"
            )
    ...
    def _reverse_move(self, date=None, journal_id=None):
        ...
        with self.env.norecompute():
            ...
            for acm_line in reversed_move.line_ids.with_context(check_move_validity=False):
                acm_line.write(
                    {"debit": acm_line.credit, "credit": acm_line.debit, "amount_currency": -acm_line.amount_currency}
                )
        self.recompute()
...
class AccountMoveLine(models.Model):
    ...
    def _amount_residual(self):
        ...
        for line in self:
            ...
            if not line.debit and not line.credit and line.amount_currency and line.currency_id:
                ...
                sign = (
                    1
                    if float_compare(line.amount_currency, 0, precision_rounding=line.currency_id.rounding) == 1
                    else -1
                )
                ...
                if line.currency_id:
                    if partial_line.currency_id and partial_line.currency_id == line.currency_id:
                        ...
                        if line.balance and line.amount_currency:
                            ...
                            date = (
                                partial_line.credit_move_id.date
            ...
            if (line.matched_debit_ids or line.matched_credit_ids) and float_is_zero(
                amount, precision_rounding=digits_rounding_precision
            ):
            ...
            line.amount_residual_currency = (
                line.currency_id and line.currency_id.round(amount_residual_currency * sign) or 0.0
            )
    ...
    credit_cash_basis = fields.Monetary(
        currency_field="company_currency_id", compute="_compute_cash_basis", store=True
    )
    balance_cash_basis = fields.Monetary(
    ...
    payment_id = fields.Many2one(
        "account.payment", string="Originator Payment", help="Payment that created this entry", copy=False
    )
    ...
    journal_id = fields.Many2one(
        "account.journal", related="move_id.journal_id", string="Journal", index=True, store=True, copy=False
    )  # related is required
    ...
    date = fields.Date(
        related="move_id.date", string="Date", index=True, store=True, copy=False
    )  # related is required
    analytic_line_ids = fields.One2many(
        "account.analytic.line", "move_id", string="Analytic lines", oldname="analytic_lines"
    )
    ...
    company_id = fields.Many2one(
        "res.company", related="account_id.company_id", string="Company", store=True, readonly=True
    )
    ...
    is_unaffected_earnings_line = fields.Boolean(
    ...
    _sql_constraints = [
        ...
        (
            "credit_debit2",
            "CHECK (credit+debit>=0)",
            "Wrong credit or debit value in accounting entry! Credit and debit should be positive.",
        ),
    ]
    ...
    def default_get(self, fields):
        ...
        currency = (
            ...
            and self.env["account.journal"].browse(self._context["journal_id"]).company_id.currency_id
        )
        if currency:
    ...
    def _check_currency_and_amount(self):
        for line in self:
            if line.amount_currency and not line.currency_id:
                raise ValidationError(
                    _(
                        "You cannot create journal items with a secondary currency without filling both 'currency' and 'amount currency' field."
                    )
                )
    ...
    def get_reconciliation_proposition(self, account_id, partner_id=False):
        ...
        if move_line_id:
            ...
            partner_id_condition = (
                partner_id_condition and partner_id_condition + " AND b.partner_id = %(partner_id)s" or ""
            )
            query = """
                    SELECT a.id, b.id
    ...
    def domain_move_lines_for_reconciliation(self, str):
        ...
        def _domain_range_amount(field, amount, signed=False):
            def build_for_amount(amount):
                return expression.AND([[(field, ">=", amount - epsilon)], [(field, "<=", amount + epsilon)]])
        ...
        for line in self:
            ...
            if target_currency != company_currency:
                if line_currency == target_currency:
                    ...
                    total_amount = company_currency.with_context(ctx).compute(
                        (line.debit - line.credit), target_currency
                    )
            ...
            ret_line["amount_currency_str"] = (
                amount_currency and formatLang(self.env, abs(amount_currency), currency_obj=line_currency) or ""
            )
            ret_line["total_amount_currency_str"] = (
                total_amount_currency
                and formatLang(self.env, abs(total_amount_currency), currency_obj=line_currency)
                or ""
            )
            ret.append(ret_line)
        return ret
    ...
    def _get_pair_to_reconcile(self):
        ...
        field = (
            (account_curreny_id and company_currency_id != account_curreny_id)
            and "amount_residual_currency"
            or "amount_residual"
        )
            ...
            if sm_debit_move.currency_id == sm_credit_move.currency_id:
                if sm_debit_move.currency_id.id:
                    ...
                    amount_reconcile_currency = min(
                        sm_debit_move.amount_residual_currency, -sm_credit_move.amount_residual_currency
                    )
            else:
                if not sm_debit_move.currency_id or not sm_credit_move.currency_id:
                    ...
                    amount_reconcile_currency = company_currency.with_context(date=currency_date).compute(
                        amount_reconcile, currency
                    )
    ...
    def reconcile(self, writeoff_acc_id=False, writeoff_journal_id=False):
        ...
        if writeoff_acc_id and writeoff_journal_id and remaining_moves:
            ...
            writeoff_vals = {"account_id": writeoff_acc_id.id, "journal_id": writeoff_journal_id.id}
    ...
    def _create_writeoff(self, vals):
        ...
        writeoff_currency = self[0].account_id.currency_id or company_currency
        if (
            not self._context.get("skip_full_reconcile_check") == "amount_currency_excluded"
            and "amount_currency" not in vals
    ...
    def force_full_reconcile(self):
        ...
        if aml_to_balance_currency and any([residual for dummy, residual in aml_to_balance_currency.values()]):
            exchange_move = self.env["account.move"].create(
                self.env["account.full.reconcile"]._prepare_exchange_diff_move(
                    move_date=maxdate, company=self[0].company_id
                )
            )
        ...
        for currency, values in aml_to_balance_currency.items():
            ...
            if total_amount_currency:
                ...
                aml_recs, partial_recs = self.env["account.partial.reconcile"].create_exchange_rate_entry(
                    aml_to_balance, 0.0, total_amount_currency, currency, exchange_move
                )
    ...
    def remove_move_reconcile(self):
        ...
        for account_move_line in self:
            for invoice in account_move_line.payment_id.invoice_ids:
                if (
                    invoice.id == self.env.context.get("invoice_id")
    ...
    def create(self, vals):
        ...
        if (
            account.currency_id
            and "amount_currency" not in vals
            and account.currency_id.id != account.company_id.currency_id.id
        ):
    ...
    def write(self, vals):
        ...
        for record in self:
            if "statement_line_id" in vals and record.payment_id:
                ...
                if all(
                    line.statement_id
                    for line in record.payment_id.move_line_ids.filtered(
            ...
            if not matched_percentage_per_move.get(line.move_id.id, False):
                ...
                if not all_same_currency:
                    ...
                    if total_amount_currency == 0.0:
                        ...
                        matched_percentage_per_move[line.move_id.id] = (
                            total_reconciled_currency / total_amount_currency
                        )
    ...
    def _prepare_analytic_line(self):
        ...
        return {
            ...
            "amount": (
                self.company_currency_id.with_context(date=self.date or fields.Date.context_today(self)).compute(
                    amount, self.analytic_account_id.currency_id
                )
...
class AccountPartialReconcile(models.Model):
        ...
        for aml in aml_to_fix:
            ...
            aml_model.with_context(check_move_validity=False).create(
                {
                    ...
                    "debit": amount_diff > 0 and aml.amount_residual or 0.0,
                    "credit": amount_diff < 0 and -aml.amount_residual or 0.0,
                    "account_id": amount_diff > 0
                    and exchange_journal.default_debit_account_id.id
                    or exchange_journal.default_credit_account_id.id,
                    "move_id": move.id,
                    "currency_id": currency.id,
                    "amount_currency": diff_in_currency and aml.amount_residual_currency or 0.0,
                    "partner_id": aml.partner_id.id,
            ...
            partial_rec |= self.with_context(skip_full_reconcile_check=True).create(
                self._prepare_exchange_diff_partial_reconcile(
                    aml=aml, line_to_reconcile=line_to_rec, currency=currency
                )
            )
        ...
        with self.env.norecompute():
                    ...
                    if not line.tax_exigible:
                                ...
                                .create(
                                    {
                                        ...
                                        "tax_exigible": True,
                                        "amount_currency": line.amount_currency
                                        and line.currency_id.round(-line.amount_currency * amount / line.balance)
                                        or 0.0,
                        ...
                        if any([tax.tax_exigibility == "on_payment" for tax in line.tax_ids]):
                            ...
                            for tax in line.tax_ids:
                                ...
                                cash_basis_amount_currency_dict[key] += (
                                    line.currency_id.round(line.amount_currency * amount / line.balance)
                                    if line.currency_id and self.amount_currency
                                    else 0.0
                                )
            ...
            if cash_basis_amount_dict:
                ...
                self._create_tax_cash_basis_base_line(
                    cash_basis_amount_dict, cash_basis_amount_currency_dict, newly_created_move
                )
        self.recompute()
    ...
    def _create_tax_basis_move(self):
        ...
        if not self.company_id.tax_cash_basis_journal_id:
            raise UserError(
                _(
                    "There is no tax cash basis journal defined "
        ...
        move_vals = {
            ...
            "ref": (
                self.credit_move_id.move_id.name if self.credit_move_id.payment_id else self.debit_move_id.move_id.name
            ),
        }
    ...
    def _compute_partial_lines(self):
            ...
            for aml in [partial_rec.debit_move_id, partial_rec.credit_move_id]:
                if aml not in aml_set:
                    ...
                    if aml.currency_id and aml.currency_id == currency:
                        ...
                        total_amount_currency += (
                            aml.balance > 0 and partial_rec.amount_currency or -partial_rec.amount_currency
                        )
        ...
        if (currency and float_is_zero(total_amount_currency, precision_rounding=currency.rounding)) or (
            (currency is None or more_than_1_currency)
            and float_compare(total_debit, total_credit, precision_rounding=digits_rounding_precision) == 0
        ):
            exchange_move_id = False
            if aml_to_balance:
                exchange_move = self.env["account.move"].create(
                    self.env["account.full.reconcile"]._prepare_exchange_diff_move(
                        move_date=maxdate, company=aml_to_balance[0].company_id
                    )
                )
                # eventually create a journal entry to book the difference due to foreign currency's exchange rate that fluctuates
                rate_diff_amls, rate_diff_partial_rec = self.create_exchange_rate_entry(
                    aml_to_balance,
                    total_debit - total_credit,
                    total_amount_currency,
                    currency or aml_to_balance[0].currency_id,
                    exchange_move,
                )
    ...
    def create(self, vals):
        ...
        tax_cash_basis_entry = not self.env.context.get("skip_tax_cash_basis_entry") and lines[
            0

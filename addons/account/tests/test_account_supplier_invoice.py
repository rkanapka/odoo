...
class TestAccountSupplierInvoice(AccountingTestCase):
    ...
    def test_supplier_invoice(self):
        ...
        invoice_account = (
            ...
            .search([("user_type_id", "=", self.env.ref("account.data_account_type_receivable").id)], limit=1)
            .id
        )
        invoice_line_account = (
            self.env["account.account"]
            .search([("user_type_id", "=", self.env.ref("account.data_account_type_expenses").id)], limit=1)
            .id
        )
    ...
    def test_vendor_bill_refund(self):
        ...
        if self.env.ref("base.main_partner").bank_account_count > 0:
            bank = self.env["res.partner.bank"].search(
                [("partner_id", "=", self.env.ref("base.main_partner").id)], limit=1
            )

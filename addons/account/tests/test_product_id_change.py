...
class TestProductIdChange(AccountingTestCase):
    ...
    def test_product_id_change(self):
        ...
        tax_include_sale = self.tax_model.create(
            dict(name="Include tax", type_tax_use="sale", amount="21.00", price_include=True)
        )
        tax_include_purchase = self.tax_model.create(
            dict(name="Include tax", type_tax_use="purchase", amount="21.00", price_include=True)
        )
        ...
        fp_tax_sale = self.fiscal_position_tax_model.create(
            dict(position_id=fp.id, tax_src_id=tax_include_sale.id, tax_dest_id=tax_exclude_sale.id)
        )
        fp_tax_purchase = self.fiscal_position_tax_model.create(
            dict(position_id=fp.id, tax_src_id=tax_include_purchase.id, tax_dest_id=tax_exclude_purchase.id)
        )
        ...
        self.assertEqual(100, out_line.price_unit, "The included tax must be subtracted to the price")
        in_line._onchange_product_id()
        self.assertEqual(200, in_line.price_unit, "The included tax must be subtracted to the price")

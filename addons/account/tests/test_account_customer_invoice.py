...
class TestAccountCustomerInvoice(AccountTestUsers):
    ...
    def test_customer_invoice(self):
        ...
        self.account_invoice_customer0 = self.account_invoice_obj.sudo(self.account_user.id).create(
            dict(
        ...
        self.account_invoice_customer0.pay_and_reconcile(
            self.env["account.journal"].search([("type", "=", "bank")], limit=1), 10050.0
        )
        ...
        self.assertEqual(
            total_after_confirm - total_before_confirm, self.account_invoice_customer0.amount_untaxed_signed
        )

        # I created a credit note Using Add Credit Note Button
        invoice_refund_obj = self.env["account.invoice.refund"]
        self.account_invoice_refund_0 = invoice_refund_obj.create(
            dict(description="Credit Note for China Export", date=datetime.date.today(), filter_refund="refund")
        )

...
class TestReconciliation(AccountingTestCase):
    ...
    def setUp(self):
        ...
        self.account_rcv = partner_agrolait.property_account_receivable_id or self.env["account.account"].search(
            [("user_type_id", "=", self.env.ref("account.data_account_type_receivable").id)], limit=1
        )
        self.account_rsa = partner_agrolait.property_account_payable_id or self.env["account.account"].search(
        ...
        self.bank_journal_usd = self.env["account.journal"].create(
            {"name": "Bank US", "type": "bank", "code": "BNK68", "currency_id": self.currency_usd_id}
        )
        self.account_usd = self.bank_journal_usd.default_debit_account_id

        self.diff_income_account = (
            self.env["res.users"].browse(self.env.uid).company_id.income_currency_exchange_account_id
        )
        self.diff_expense_account = (
            self.env["res.users"].browse(self.env.uid).company_id.expense_currency_exchange_account_id
        )
        ...
        self.account_invoice_line_model.create(
            {
                ...
                .search([("user_type_id", "=", self.env.ref("account.data_account_type_revenue").id)], limit=1)
                .id,
            }
        )
        if auto_validate:
    ...
    def check_results(self, move_line_recs, aml_dict):
        ...
        for move_line in move_line_recs:
            ...
            if "currency_diff" in aml_dict[move_line.account_id.id]:
                ...
                self.assertTrue(
                    currency_diff_move, "The full reconciliation should have created an exchange rate journal entry"
                )
                for currency_diff_line in currency_diff_move.line_ids:
                    if aml_dict[move_line.account_id.id].get("currency_diff") == 0:
                        if currency_diff_line.account_id.id == move_line.account_id.id:
                            self.assertAlmostEqual(
                                currency_diff_line.amount_currency,
                                aml_dict[move_line.account_id.id].get("amount_currency_diff"),
                            )
                    if aml_dict[move_line.account_id.id].get("currency_diff") == 0:
                        if currency_diff_line.account_id.id == move_line.account_id.id:
                            self.assertAlmostEqual(
                    ...
                    if aml_dict[move_line.account_id.id].get("currency_diff") > 0:
                        if currency_diff_line.account_id.id == move_line.account_id.id:
                            self.assertAlmostEqual(
                                currency_diff_line.debit, aml_dict[move_line.account_id.id].get("currency_diff")
                            )
                        else:
                            self.assertAlmostEqual(
                                currency_diff_line.credit, aml_dict[move_line.account_id.id].get("currency_diff")
                            )
                            self.assertIn(
                                currency_diff_line.account_id.id,
                                [self.diff_expense_account.id, self.diff_income_account.id],
                                "The difference accounts should be used correctly. ",
                            )
                    else:
                        if currency_diff_line.account_id.id == move_line.account_id.id:
                            self.assertAlmostEqual(
                                currency_diff_line.credit, abs(aml_dict[move_line.account_id.id].get("currency_diff"))
                            )
                        else:
                            self.assertAlmostEqual(
                                currency_diff_line.debit, abs(aml_dict[move_line.account_id.id].get("currency_diff"))
                            )
                            self.assertIn(
                                currency_diff_line.account_id.id,
                                [self.diff_expense_account.id, self.diff_income_account.id],
    ...
    def make_customer_and_supplier_flows(
        self, invoice_currency_id, invoice_amount, bank_journal, amount, amount_currency, transaction_currency_id
    ):
        # we create an invoice in given invoice_currency
        invoice_record = self.create_invoice(
            type="out_invoice", invoice_amount=invoice_amount, currency_id=invoice_currency_id
        )
        # we encode a payment on it, on the given bank_journal with amount, amount_currency and transaction_currency given
        bank_stmt = self.make_payment(
            invoice_record,
            bank_journal,
            amount=amount,
            amount_currency=amount_currency,
        ...
        invoice_record = self.create_invoice(
            type="in_invoice", invoice_amount=invoice_amount, currency_id=invoice_currency_id
        )
        # we encode a payment on it, on the given bank_journal with amount, amount_currency and transaction_currency given
        bank_stmt = self.make_payment(
            invoice_record,
    ...
    def test_statement_usd_invoice_eur_transaction_eur(self):
        customer_move_lines, supplier_move_lines = self.make_customer_and_supplier_flows(
            self.currency_euro_id, 30, self.bank_journal_usd, 42, 30, self.currency_euro_id
        )
        self.check_results(
            customer_move_lines,
            {
    ...
    def test_statement_usd_invoice_usd_transaction_usd(self):
        customer_move_lines, supplier_move_lines = self.make_customer_and_supplier_flows(
            self.currency_usd_id, 50, self.bank_journal_usd, 50, 0, False
        )
        self.check_results(
            customer_move_lines,
            {
    ...
    def test_statement_usd_invoice_usd_transaction_eur(self):
        customer_move_lines, supplier_move_lines = self.make_customer_and_supplier_flows(
            self.currency_usd_id, 50, self.bank_journal_usd, 50, 40, self.currency_euro_id
        )
        self.check_results(
            customer_move_lines,
            {
    ...
    def test_statement_usd_invoice_chf_transaction_chf(self):
        customer_move_lines, supplier_move_lines = self.make_customer_and_supplier_flows(
            self.currency_swiss_id, 50, self.bank_journal_usd, 42, 50, self.currency_swiss_id
        )
        self.check_results(
            customer_move_lines,
            {
    ...
    def test_statement_eur_invoice_usd_transaction_usd(self):
        customer_move_lines, supplier_move_lines = self.make_customer_and_supplier_flows(
            self.currency_usd_id, 50, self.bank_journal_euro, 40, 50, self.currency_usd_id
        )
        self.check_results(
            customer_move_lines,
            {
                self.account_euro.id: {
                    "debit": 40.0,
                    "credit": 0.0,
                    "amount_currency": 50,
                    "currency_id": self.currency_usd_id,
    ...
    def test_statement_eur_invoice_usd_transaction_eur(self):
        customer_move_lines, supplier_move_lines = self.make_customer_and_supplier_flows(
            self.currency_usd_id, 50, self.bank_journal_euro, 40, 0.0, False
        )
        self.check_results(
            customer_move_lines,
            {
    ...
    def test_statement_euro_invoice_usd_transaction_chf(self):
        customer_move_lines, supplier_move_lines = self.make_customer_and_supplier_flows(
            self.currency_usd_id, 50, self.bank_journal_euro, 42, 50, self.currency_swiss_id
        )
        self.check_results(
            customer_move_lines,
            {
    ...
    def test_statement_euro_invoice_usd_transaction_euro_full(self):
        ...
        bank_stmt_line.process_reconciliation(
            ...
            new_aml_dicts=[
                {"debit": 0.0, "credit": 7.3, "name": "exchange difference", "account_id": self.diff_income_account.id}
            ],
        )
        self.check_results(
            bank_stmt.move_line_ids,
            {
    ...
    def test_balanced_exchanges_gain_loss(self):
        ...
        self.assertTrue(
            counterpart_exchange_loss_line,
            'There should be one move line of 0.01 EUR on account "Foreign Exchange Loss"',
        )
    ...
    def test_manual_reconcile_wizard_opw678153(self):
        ...
        wizard = (
            self.env["account.move.line.reconcile.writeoff"]
            .with_context(active_ids=[x.id for x in aml_recs])
            .create({"journal_id": self.bank_journal_usd.id, "writeoff_acc_id": self.account_rsa.id})
        )
    ...
    def test_reconcile_bank_statement_with_payment_and_writeoff(self):
        ...
        bank_stmt_line.process_reconciliation(
            payment_aml_rec=bank_move_line,
            new_aml_dicts=[
                {
    ...
    def test_partial_reconcile_currencies_01(self):
        ...
        dest_journal_id = self.env["account.journal"].search(
            [("type", "=", "purchase"), ("company_id", "=", self.env.ref("base.main_company").id)], limit=1
        )
        ...
        self.bank_journal_euro.write(
            {"default_debit_account_id": self.account_rsa.id, "default_credit_account_id": self.account_rsa.id}
        )
        dest_journal_id.write(
            {"default_debit_account_id": self.account_rsa.id, "default_credit_account_id": self.account_rsa.id}
        )
        ...
        invoice_a = self.account_invoice_model.create(
            {
                "partner_id": self.partner_agrolait_id,
                "reference_type": "none",
                "currency_id": self.currency_usd_id,
        ...
        invoice_b = self.account_invoice_model.create(
            {
                "partner_id": self.partner_agrolait_id,
                "reference_type": "none",
                "currency_id": self.currency_usd_id,
        ...
        payment_c = self.env["account.payment"].create(
            {
                "payment_type": "outbound",
                "amount": 25,
                "currency_id": self.currency_usd_id,
        ...
        debit_line_a = payment_a.move_line_ids.filtered(
            lambda l: l.debit and l.account_id == dest_journal_id.default_debit_account_id
        )
        debit_line_b = payment_b.move_line_ids.filtered(
            lambda l: l.debit and l.account_id == dest_journal_id.default_debit_account_id
        )
        debit_line_c = payment_c.move_line_ids.filtered(
            lambda l: l.debit and l.account_id == dest_journal_id.default_debit_account_id
        )
        ...
        for inv in invoice_a + invoice_b:
            ...
            for aml in (inv.payment_move_line_ids + inv.move_id.line_ids).filtered(
                lambda l: l.account_id == self.account_rsa
            ):
    ...
    def test_partial_reconcile_currencies_02(self):
        ...
        old_rates = self.env["res.currency.rate"].search([("currency_id", "=", self.currency_usd_id)])
        old_rates.unlink()
    ...
    def test_revert_payment_and_reconcile(self):
        ...
        bank_line = payment.move_line_ids.filtered(
            lambda l: l.account_id.id == self.bank_journal_usd.default_debit_account_id.id
        )
        ...
        reversed_bank_line = reversed_move.line_ids.filtered(
            lambda l: l.account_id.id == self.bank_journal_usd.default_debit_account_id.id
        )
    ...
    def test_aged_report(self):
        ...
        self.env.cr.execute(
            "UPDATE account_partial_reconcile SET create_date = %(date)s WHERE id = %(partial_id)s",
            {"date": report_date_to + " 00:00:00", "partial_id": statement_partial_id.id},
        )
    ...
    def test_revert_payment_and_reconcile_exchange(self):
        ...
        def _determine_debit_credit_line(move):
            line_ids_reconciliable = move.line_ids.filtered(
                lambda l: l.account_id.reconcile or l.account_id.internal_type == "liquidity"
            )
            return line_ids_reconciliable.filtered(lambda l: l.debit), line_ids_reconciliable.filtered(
                lambda l: l.credit
            )
        ...
        reverted_payment_move = self.env["account.move"].browse(
            payment_move.reverse_moves(time.strftime("%Y") + "-08-01")
        )
        ...
        reverted_exchange_move = self.env["account.move"].search(
            [("journal_id", "=", exchange_move.journal_id.id), ("ref", "ilike", exchange_move.name)], limit=1
        )
    ...
    def test_aged_report_future_payment(self):
        ...
        self.env.cr.execute(
            "UPDATE account_partial_reconcile SET create_date = %(date)s WHERE id = %(partial_id)s",
            {"date": invoice.date_invoice + " 00:00:00", "partial_id": statement_partial_id.id},
        )
        ...
        move_balance = self.env["account.move"].create(
            {
                ...
                "line_ids": [
                    (
                        0,
                        False,
                        {"credit": 160.16, "account_id": inv1_receivable.account_id.id, "name": "Balance WriteOff"},
                    ),
                    (
                        0,
                        False,
                        {"debit": 160.16, "account_id": self.diff_expense_account.id, "name": "Balance WriteOff"},
                    ),
                ],
            }
        )
    ...
    def test_inv_refund_foreign_payment_writeoff_domestic3(self):
        ...
        Reconciliation should be full, without exhange difference
        ...
        move_balance = self.env["account.move"].create(
            {
                ...
                "line_ids": [
                    (
                        0,
                        False,
                        {"credit": 17.78, "account_id": inv1_receivable.account_id.id, "name": "Balance WriteOff"},
                    ),
                    (
                        0,
                        False,
                        {"debit": 17.78, "account_id": self.diff_expense_account.id, "name": "Balance WriteOff"},
                    ),
                ],
            }
        )
    ...
    def test_inv_refund_foreign_payment_writeoff_domestic4(self):
        ...
        move_balance = self.env["account.move"].create(
            {
                ...
                "line_ids": [
                    (
                        0,
                        False,
                        {"credit": 83.31, "account_id": inv1_receivable.account_id.id, "name": "Balance WriteOff"},
                    ),
                    (
                        0,
                        False,
                        {"debit": 83.31, "account_id": self.diff_expense_account.id, "name": "Balance WriteOff"},
                    ),
                ],
            }
        )
        ...
        exchange_rcv = inv1_receivable.full_reconcile_id.exchange_move_id.line_ids.filtered(
        ...
        move_balance = self.env["account.move"].create(
            {
                ...
                "line_ids": [
                    (
                        0,
                        False,
                        {"credit": 0.05, "account_id": inv1_receivable.account_id.id, "name": "Balance WriteOff"},
                    ),
                    (
                        0,
                        False,
                        {"debit": 0.05, "account_id": self.diff_expense_account.id, "name": "Balance WriteOff"},
                    ),
                ],
            }
        )

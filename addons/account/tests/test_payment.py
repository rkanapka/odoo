...
class TestPayment(AccountingTestCase):
    ...
    def reconcile(self, liquidity_aml, amount=0.0, amount_currency=0.0, currency_id=None):
        ...
        currency_id and amount_currency or amount
    ...
    def check_journal_items(self, aml_recs, aml_dicts):
        def compare_rec_dict(aml_rec, aml_dict):
            return (
                aml_rec.account_id.id == aml_dict["account_id"]
    ...
    def test_full_payment_process(self):
        ...
        self.assertTrue(
            payment.move_line_ids.filtered(lambda l: l.account_id == inv_1.account_id)[0].full_reconcile_id
        )
    ...
    def test_internal_transfer_journal_usd_journal_eur(self):
        ...
        self.check_journal_items(
            ...
            [
                {
                    "account_id": self.transfer_account.id,
                    "debit": 32.70,
                    "credit": 0.0,
                    "amount_currency": 50,
                    "currency_id": self.currency_usd_id,
            ...
            [
                {
                    ...
                    "debit": 0.0,
                    "credit": 38.21,
                    "amount_currency": -58.42,
                    "currency_id": self.currency_usd_id,
                },
                {
                    "account_id": self.partner_china_exp.property_account_payable_id.id,
                    "debit": 38.21,
                    "credit": 0.0,
                    "amount_currency": 50,
                    "currency_id": self.currency_chf_id,
    ...
    def test_multiple_payments_00(self):
        ...
        inv_4 = self.create_invoice(
            amount=50, currency_id=self.currency_eur_id, partner=self.partner_agrolait.id, type="in_invoice"
        )
    ...
    def test_multiple_payments_01(self):
        ...
        inv_2 = self.create_invoice(
            amount=100, currency_id=self.currency_eur_id, partner=self.partner_agrolait.id, type="out_refund"
        )
    ...
    def test_partial_payment(self):
        ...
        # Test Vendor Bill
        inv_2 = self.create_invoice(
            amount=500, currency_id=self.currency_eur_id, type="in_invoice", partner=self.partner_china_exp.id
        )
        ids = [inv_2.id]
        register_payments = self.register_payments_model.with_context(active_ids=ids).create(
            {
    ...
    def test_payment_and_writeoff_in_other_currency_1(self):
        ...
        invoice = self.create_invoice(
            amount=25, type="out_invoice", currency_id=self.currency_eur_id, partner=self.partner_agrolait.id
        )
        ...
        # Check that all the aml are correctly created.
        invoice = self.create_invoice(
            amount=25, type="in_invoice", currency_id=self.currency_eur_id, partner=self.partner_agrolait.id
        )
        # register payment on invoice
        payment = self.payment_model.create(
            {
    ...
    def test_payment_and_writeoff_out_refund(self):
        ...
        # Check that all the aml are correctly created.
        invoice = self.create_invoice(
            amount=100, type="out_refund", currency_id=self.currency_eur_id, partner=self.partner_agrolait.id
        )
        # register payment on invoice
        payment = self.payment_model.create(
            {
    ...
    def test_payment_and_writeoff_in_other_currency_2(self):
        ...
        self.env["res.currency.rate"].create(
            {"currency_id": self.currency_usd_id, "rate": 0.895, "name": time.strftime("%Y") + "-06-26"}
        )
        self.env["res.currency.rate"].create(
            {"currency_id": self.currency_usd_id, "rate": 0.88, "name": time.strftime("%Y") + "-07-15"}
        )
        invoice = self.create_invoice(
            amount=5325.6, type="in_invoice", currency_id=self.currency_usd_id, partner=self.partner_agrolait.id
        )
        self.check_journal_items(
    ...
    def test_payment_and_writeoff_in_other_currency_3(self):
        ...
        self.env["res.currency.rate"].create(
            {"currency_id": self.currency_usd_id, "rate": 1, "name": time.strftime("%Y") + "-06-26"}
        )
        self.env["res.currency.rate"].create(
            {"currency_id": self.currency_eur_id, "rate": 948, "name": time.strftime("%Y") + "-06-26"}
        )
        invoice = self.create_invoice(
            amount=247590.4, type="out_invoice", currency_id=self.currency_eur_id, partner=self.partner_agrolait.id
        )
        self.check_journal_items(
            invoice.move_id.line_ids,
            [
        ...
        self.assertEqual(invoice.state, "paid")
        self.assertTrue(
            invoice.move_id.line_ids.filtered(lambda l: l.account_id == self.account_receivable)[0].full_reconcile_id
        )

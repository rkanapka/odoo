...
class TestFiscalPosition(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.jc = self.res_partner.create(dict(name="JCVD", vat="BE0477472701", country_id=be.id))
        self.ben = self.res_partner.create(dict(name="BP", country_id=be.id))
        self.george = self.res_partner.create(dict(name="George", vat="BE0477472701", country_id=fr.id))
        self.alberto = self.res_partner.create(dict(name="Alberto", vat="BE0477472701", country_id=mx.id))
        self.be_nat = self.fp.create(
            dict(name="BE-NAT", auto_apply=True, country_id=be.id, vat_required=False, sequence=10)
        )
        self.fr_b2c = self.fp.create(
            dict(name="EU-VAT-FR-B2C", auto_apply=True, country_id=fr.id, vat_required=False, sequence=40)
        )
        self.fr_b2b = self.fp.create(
            dict(name="EU-VAT-FR-B2B", auto_apply=True, country_id=fr.id, vat_required=True, sequence=50)
        )

    def test_10_fp_country(self):
        def assert_fp(partner, expected_pos, message):
            self.assertEqual(self.fp.get_fiscal_position(partner.id), expected_pos.id, message)
        ...
        self.eu_intra_b2b = self.fp.create(
            dict(name="EU-INTRA B2B", auto_apply=True, country_group_id=self.eu.id, vat_required=True, sequence=20)
        )

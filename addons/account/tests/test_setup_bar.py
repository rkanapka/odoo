...
class TestSetupBar(AccountingTestCase):
    ...
    def test_initial_balance_step(self):
        ...
        initial_setup_wizard = (
            self.env["account.opening"].create({"company_id": company.id}).with_context({"check_move_validity": False})
        )
        ...
        self.assertEqual(
            len(initial_setup_wizard.opening_move_line_ids),
            2,
            "The wizard should contain 2 lines: 1 manually created, and 1 automatic adjustment.",
        )
        automatic_line = initial_setup_wizard.opening_move_line_ids.filtered(lambda x: x != test_line_1)
        self.assertEqual(
            automatic_line.account_id.user_type_id,
            unaffected_earnings_type,
            "Automatic adjustement line should be of type 'current year earnings'.",
        )
        ...
        self.assertEqual(
            len(initial_setup_wizard.opening_move_line_ids),
            3,
            "The wizard should contain 3 lines: 2 manually created, and 1 automatic adjustment.",
        )
        self.assertTrue(
            automatic_line in initial_setup_wizard.opening_move_line_ids,
            "Automatic line should stay the same when adding a new line.",
        )
        ...
        self.assertEqual(
            len(initial_setup_wizard.opening_move_line_ids),
            3,
            "The wizard should contain 3 lines: 3 manually created, and 0 automatic adjustment.",
        )
        self.assertFalse(
            automatic_line in initial_setup_wizard.opening_move_line_ids,
            "Automatic adjustment line should be removed when useless.",
        )

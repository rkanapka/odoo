...
class AccountTestUsers(AccountingTestCase):
    ...
    def setUp(self):
        ...
        self.account_user = self.res_user_model.with_context({"no_reset_password": True}).create(
            dict(
                name="Accountant",
        ...
        self.account_manager = self.res_user_model.with_context({"no_reset_password": True}).create(
            dict(
                name="Adviser",
                company_id=self.main_company.id,
                login="fm",

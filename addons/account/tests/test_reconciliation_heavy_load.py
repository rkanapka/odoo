...
class TestReconciliationHeavyLoad(AccountingTestCase):
    ...
    def _get_values_account_move_line(self, account, journal, name, move, credit=0, debit=0, date=fields.Date.today()):
        return {
            "journal_id": journal.id,
            "name": name,
    ...
    def test_heavy_load_reconciliation(self):
        ...
        for i in range(nb_lines):
            ...
            values = self._get_values_account_move_line(self.account, self.journal, name, move, credit=amount_per_line)
            line_ids.append((0, False, values))

        values = self._get_values_account_move_line(self.account, self.journal, "Move line Debit", move, debit=total)
        line_ids.append((0, False, values))

...
class TestTax(AccountTestUsers):
    ...
    def setUp(self):
        ...
        self.group_tax = self.tax_model.create(
            {
                ...
                "amount": 0,
                "sequence": 5,
                "children_tax_ids": [(4, self.fixed_tax.id, 0), (4, self.percent_tax.id, 0)],
            }
        )
        ...
        self.group_of_group_tax = self.tax_model.create(
            {
                ...
                "amount": 0,
                "sequence": 7,
                "children_tax_ids": [(4, self.group_tax.id, 0), (4, self.group_tax_bis.id, 0)],
            }
        )
        self.bank_journal = self.env["account.journal"].search(
            [("type", "=", "bank"), ("company_id", "=", self.account_manager.company_id.id)]
        )[0]
        ...
        )  # Should be done by onchange later
    ...
    def test_tax_move_lines_creation(self):
        ...
        aml_with_taxes = move.line_ids.filtered(
            lambda l: set(l.tax_ids.ids) == {self.group_tax.id, self.fixed_tax_bis.id}
        )

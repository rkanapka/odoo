...
class TestAccountInvoiceRounding(AccountingTestCase):
    ...
    def test_rounding_add_invoice_line(self):
        self._check_invoice_rounding(
            self.create_invoice(100.2, self.create_cash_rounding(0.5, "UP", "add_invoice_line")), [100.2, 0.3]
        )
        self._check_invoice_rounding(
            self.create_invoice(100.9, self.create_cash_rounding(1.0, "DOWN", "add_invoice_line")), [100.9, -0.9]
        )
        self._check_invoice_rounding(
            self.create_invoice(100.5, self.create_cash_rounding(1.0, "HALF-UP", "add_invoice_line")), [100.5, 0.5]
        )

    def test_rounding_biggest_tax(self):
        self._check_invoice_rounding(
            self.create_invoice(100.2, self.create_cash_rounding(0.5, "UP", "biggest_tax"), 1.0), [100.2], [1.3]
        )
        self._check_invoice_rounding(
            self.create_invoice(100.9, self.create_cash_rounding(1.0, "DOWN", "biggest_tax"), 2.0), [100.9], [1.1]
        )
        self._check_invoice_rounding(
            self.create_invoice(100.5, self.create_cash_rounding(1.0, "HALF-UP", "biggest_tax"), 1.0), [100.5], [1.5]
        )

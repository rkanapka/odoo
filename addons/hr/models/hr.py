...
class Job(models.Model):
    ...
    no_of_recruitment = fields.Integer(
        string="Expected New Employees", copy=False, help="Number of new employees you expect to recruit.", default=1
    )
...
class Employee(models.Model):
    ...
    gender = fields.Selection(
        [("male", "Male"), ("female", "Female"), ("other", "Other")], groups="hr.group_hr_user", default="male"
    )
    ...
    category_ids = fields.Many2many(
        "hr.employee.category", "employee_category_rel", "emp_id", "category_id", string="Tags"
    )
    # misc
...
class Department(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company", string="Company", index=True, default=lambda self: self.env.user.company_id
    )

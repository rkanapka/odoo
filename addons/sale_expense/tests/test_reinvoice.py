...
class TestReInvoice(CommonTest):
    ...
    def test_at_cost(self):
        ...
        self.assertTrue(
            ...
            "Confirming SO with an expense product should trigger the analytic account creation",
        )
        ...
        self.assertEqual(
            sale_order_line.qty_delivered, 0, "Exising SO line should not be impacted by reinvoicing product at cost"
        )

        sol_ordered = self.sale_order.order_line.filtered(
            lambda sol: sol.product_id == self.product_ordered_cost and sol != sale_order_line
        )
        self.assertTrue(
            sol_ordered, "A new line with ordered expense should have been created on expense report posting"
        )
        self.assertEqual(
            sol_ordered.price_unit,
            expense1.unit_amount,
            "The unit price of new SO line should be the one from the expense (at cost)",
        )
        self.assertEqual(sol_ordered.product_uom_qty, 0, "The ordered quantity of new SO line should be zero")
        self.assertEqual(
            sol_ordered.qty_delivered,
            expense1.quantity,
            "The delivered quantity of new SO line should be the one from the expense",
        )

        sol_deliver = self.sale_order.order_line.filtered(
            lambda sol: sol.product_id == self.product_deliver_cost and sol != sale_order_line
        )
        self.assertTrue(
            sol_deliver, "A new line with delivered expense should have been created on expense report posting"
        )
        self.assertEqual(
            sol_deliver.price_unit,
            expense2.unit_amount,
            "The unit price of new SO line should be the one from the expense (at cost)",
        )
        self.assertEqual(sol_deliver.product_uom_qty, 0, "The ordered quantity of new SO line should be zero")
        self.assertEqual(
            sol_deliver.qty_delivered,
            expense2.quantity,
            "The delivered quantity of new SO line should be the one from the expense",
        )
    ...
    def test_sales_price_ordered(self):
        ...
        self.assertFalse(
            self.sale_order.analytic_account_id,
            "Confirming SO with no expense product should not trigger the analytic account creation",
        )
        ...
        self.assertTrue(
            self.sale_order.analytic_account_id,
            "Posting expense with an expense product should trigger the analytic account creation on SO",
        )
        self.assertEqual(
            self.sale_order.analytic_account_id,
            expense1.analytic_account_id,
            "SO analytic account should be the same for the expense",
        )
        self.assertEqual(
            len(self.sale_order.order_line), 1, "A new So line should have been created on expense report posting"
        )
        ...
        self.assertEqual(
            sol_ordered.price_unit,
            10,
            "The unit price of new SO line should be the one from the expense (at sales price)",
        )
        self.assertEqual(sol_ordered.product_uom_qty, 0, "The ordered quantity of new SO line should be zero")
        self.assertEqual(
            sol_ordered.qty_delivered,
    ...
    def test_sales_price_delivered(self):
        ...
        self.assertEqual(
            ...
            1,
            "No SO line should have been created (or removed) on expense report posting",
        )
        ...
        self.assertEqual(
            ...
            expense1.quantity,
            "The delivered quantity of SO line should have been incremented",
        )
    ...
    def test_no_expense(self):
        ...
        self.assertFalse(
            self.sale_order.analytic_account_id,
            "Confirming SO with an no-expense product should not trigger the analytic account creation",
        )
        ...
        self.assertTrue(
            self.sale_order.analytic_account_id,
            "Posting expense with an expense product (even with no expense pilocy) should trigger the analytic account creation",
        )
        self.assertEqual(
            self.sale_order.analytic_account_id,
            expense1.analytic_account_id,
        ...
        self.assertEqual(
            ...
            "The unit price of SO line should be the same",
        )

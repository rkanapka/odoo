...
class TestSaleExpense(TestSale):
    def test_sale_expense(self):
        ...
        journal = self.env["account.journal"].create(
            {"name": "Purchase Journal - Test", "code": "HRTPJ", "type": "purchase", "company_id": company.id}
        )
        ...
        employee = self.env["hr.employee"].create(
            {"name": "Test employee", "user_id": self.user.id, "address_home_id": self.user.partner_id.id}
        )
        ...
        self.assertEqual(
            (sol.price_unit, sol.qty_delivered), (621.54, 1.0), "Sale Expense: error when invoicing an expense at cost"
        )
        ...
        self.assertEqual(
            inv.amount_untaxed, 621.54 + (prod_exp_2.list_price * 100.0), "Sale Expense: invoicing of expense is wrong"
        )

...
class TestHolidaysFlow(TestHrHolidaysBase):
    ...
    def test_00_leave_request_flow(self):
        ...
        HolidayStatusManagerGroup.create(
            {
                ...
                "limit": True,
                "categ_id": self.env["calendar.event.type"]
                .sudo(self.user_hrmanager_id)
                .create({"name": "NotLimitedMeetingType"})
                .id,
            }
        )
        ...
        self.assertEqual(
            hol1_user_group.state, "confirm", "hr_holidays: newly created leave request should be in confirm state"
        )
        ...
        self.assertEqual(
            hol1_user_group.state, "validate", "hr_holidays: validates leave request should be in validate state"
        )
        ...
        self.assertEqual(
            hol2.state, "refuse", "hr_holidays: hr_user should not be able to reset a refused leave request"
        )

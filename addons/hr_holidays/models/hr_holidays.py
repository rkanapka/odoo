...
class HolidaysType(models.Model):
    ...
    remaining_leaves = fields.Float(
        compute="_compute_leaves", string="Remaining Leaves", help="Maximum Leaves Allowed - Leaves Already Taken"
    )
...
class Holidays(models.Model):
    ...
    notes = fields.Text(
        "Reasons", readonly=True, states={"draft": [("readonly", False)], "confirm": [("readonly", False)]}
    )
    ...
    number_of_days = fields.Float(
        "Number of Days", compute="_compute_number_of_days", store=True, track_visibility="onchange"
    )
    ...
    _sql_constraints = [
        ...
        (
            "date_check2",
            "CHECK ( (type='add') OR (date_from <= date_to))",
    ...
    def _check_state_access_right(self, vals):
        if (
            vals.get("state")
            and vals["state"] not in ["draft", "confirm", "cancel"]
            and not self.env["res.users"].has_group("hr_holidays.group_hr_holidays_user")
        ):
            return False
        return True
    ...
    def _prepare_holidays_meeting_values(self):
        ...
        meeting_values = {
            ...
            "categ_ids": [(6, 0, [self.holiday_status_id.categ_id.id])] if self.holiday_status_id.categ_id else [],
    ...
    def _notification_recipients(self, message, groups):
        ...
        new_group = (
            ...
            and any(user.has_group("hr_holidays.group_hr_holidays_user") for user in partner.user_ids),
            {
                "actions": hr_actions,
            },
        )
    ...
    def _message_notification_recipients(self, message, recipients):
        result = super()._message_notification_recipients(message, recipients)

...
class Department(models.Model):
    ...
    leave_to_approve_count = fields.Integer(compute="_compute_leave_count", string="Leave to Approve")
    allocation_to_approve_count = fields.Integer(compute="_compute_leave_count", string="Allocation to Approve")
...
class Employee(models.Model):
    ...
    current_leave_id = fields.Many2one(
        "hr.holidays.status", compute="_compute_leave_status", string="Current Leave Type"
    )
    ...
    is_absent_totay = fields.Boolean(
        "Absent Today", compute="_compute_absent_employee", search="_search_absent_employee"
    )
    ...
    def _inverse_remaining_leaves(self):
        ...
        for employee in self.filtered(lambda employee: employee.remaining_leaves):
            ...
            if len(status_list) != 1:
                raise UserError(
                    _(
                        "The feature behind the field 'Remaining Legal Leaves' can only be used when there is only one "
    ...
    def _compute_leaves_count(self):
        leaves = self.env["hr.holidays"].read_group(
            [("employee_id", "in", self.ids), ("holiday_status_id.limit", "=", False), ("state", "=", "validate")],

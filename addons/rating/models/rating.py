...
class Rating(models.Model):
    ...
    res_name = fields.Char(
        string="Resource name", compute="_compute_res_name", store=True, help="The name of the rated resource."
    )
    res_model_id = fields.Many2one(
        "ir.model", "Related Document Model", index=True, ondelete="cascade", help="Model of the followed resource"
    )
    ...
    parent_res_model = fields.Char(
        "Parent Document Model", store=True, related="parent_res_model_id.model", index=True
    )
    ...
    access_token = fields.Char(
        "Security Token", default=new_access_token, help="Access token to set the rating of the value"
    )
...
class RatingMixin(models.AbstractModel):
    ...
    rating_last_value = fields.Float(
        "Rating Last Value", compute="_compute_rating_last_value", compute_sudo=True, store=True
    )
    ...
    def _compute_rating_last_value(self):
        for record in self:
            ratings = self.env["rating.rating"].search(
                [("res_model", "=", self._name), ("res_id", "=", record.id)], limit=1
            )
            if ratings:
        ...
        if rating:
            ...
            if hasattr(self, "message_post"):
                ...
                self.message_post(
                    ...
                    author_id=rating.partner_id
                    and rating.partner_id.id

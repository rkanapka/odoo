...
class WebsiteSurvey(http.Controller):
    ...
    def fill_survey(self, survey, token, prev=None, **post):
        ...
        if user_input.state == "new":  # First page
            ...
            return request.render("survey.sfinished", {"survey": survey, "token": token, "user_input": user_input})
    ...
    @http.route(
        ['/survey/scores/<model("survey.survey"):survey>/<string:token>'], type="http", auth="public", website=True
    )
    ...
    @http.route(
        ['/survey/submit/<model("survey.survey"):survey>'], type="http", methods=["POST"], auth="public", website=True
    )
    ...
    def survey_reporting(self, survey, token=None, **post):
        ...
        if not survey.user_input_ids or not [
            input_id.id for input_id in survey.user_input_ids if input_id.state != "new"
        ]:
    ...
    def get_graph_data(self, question, current_filters=None):
        ...
        if question.type == "multiple_choice":
            result.append(
                {"key": ustr(question.question), "values": Survey.prepare_result(question, current_filters)["answers"]}
            )

...
class SurveyMailComposeMessage(models.TransientModel):
    ...
    public = fields.Selection(
        [
            ("public_link", "Share the public web link to your audience."),
    ...
    multi_email = fields.Text(
        string="List of emails",
    ...
    def send_mail(self, auto_commit=False):
        ...
        for wizard in self:
            ...
            if wizard.body.find("__URL__") < 0:
                raise UserError(
                    _(
                        "The content of the text don't contain '__URL__'. \
            ...
            context = self.env.context
            if (
                not wizard.multi_email
                and not wizard.partner_ids
                and (context.get("default_partner_ids") or context.get("default_multi_email"))
            ):

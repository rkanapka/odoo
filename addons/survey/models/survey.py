...
class SurveyStage(models.Model):
    ...
    _sql_constraints = [("positive_sequence", "CHECK(sequence >= 0)", "Sequence number MUST be a natural")]
...
class Survey(models.Model):
    ...
    thank_you_message = fields.Html(
        "Thanks Message", translate=True, help="This message will be displayed when survey is completed"
    )
    ...
    def _compute_survey_statistic(self):
        ...
        start_survey = UserInput.search(
            ["&", ("survey_id", "in", self.ids), "|", ("state", "=", "skip"), ("state", "=", "done")]
        )
            ...
            for current_filter in filters:
                ...
                if row_id == 0:
                    ...
                    domain_filter.extend(
                        ["|", ("value_suggested_row.id", "=", row_id), ("value_suggested.id", "=", answer_id)]
                    )
            if choice:
    ...
    def get_filter_display_data(self, filters):
        ...
        if filters:
            ...
            for current_filter in filters:
                ...
                filter_display_data.append({"question_text": question.question, "labels": labels.mapped("value")})
    ...
    def prepare_result(self, question, current_filters=None):
        ...
        if question.type in ["simple_choice", "multiple_choice"]:
            ...
            for input_line in question.user_input_line_ids:
                if (
                    input_line.answer_type == "suggestion"
                    and answers.get(input_line.value_suggested.id)
                    and (not (current_filters) or input_line.user_input_id.id in current_filters)
                ):
    ...
    def get_input_summary(self, question, current_filters=None):
        ...
        if question.survey_id.user_input_ids:
            total_input_ids = current_filters or [
                input_id.id for input_id in question.survey_id.user_input_ids if input_id.state != "new"
            ]
...
class SurveyQuestion(models.Model):
    ...
    labels_ids = fields.One2many(
        "survey.label", "question_id", string="Types of answers", oldname="answer_choice_ids", copy=True
    )
    ...
    column_nb = fields.Selection(
        [("12", "1"), ("6", "2"), ("4", "3"), ("3", "4"), ("2", "6")], "Number of columns", default="12"
    )
    ...
    # Comments
    comments_allowed = fields.Boolean("Show Comments Field", oldname="allow_comment")
    comments_message = fields.Char(
        "Comment Message", translate=True, default=lambda self: _("If other, please specify:")
    )
    comment_count_as_answer = fields.Boolean("Comment Field is an Answer Choice", oldname="make_comment_field")
    ...
    user_input_line_ids = fields.One2many(
        "survey.user_input_line", "question_id", string="Answers", domain=[("skipped", "=", False)]
    )
...
class SurveyUserInput(models.Model):
    ...
    date_create = fields.Datetime(
        "Creation Date", default=fields.Datetime.now, required=True, readonly=True, copy=False
    )
    deadline = fields.Datetime(
        "Deadline", help="Date by which the person can open the survey and submit answers", oldname="date_deadline"
    )
    type = fields.Selection(
    ...
    state = fields.Selection(
        [("new", "Not started yet"), ("skip", "Partially completed"), ("done", "Completed")],
        string="Status",
        default="new",
        readonly=True,
    )
    ...
    token = fields.Char(
        "Identification token", default=lambda self: str(uuid.uuid4()), readonly=True, required=True, copy=False
    )
    ...
    def do_clean_emptys(self):
        ...
        self.search([("type", "=", "manually"), ("state", "=", "new"), ("date_create", "<", an_hour_ago)]).unlink()
    ...
    def action_survey_results(self):
        ...
        self.ensure_one()
        return {"type": "ir.actions.act_url", "name": "Survey Results", "target": "self", "url": self.result_url}
        ...
        if question.matrix_subtype == "simple":
            for row in question.labels_ids_2:
                ...
                if a_tag in ca_dict:
                    no_answers = False
                    vals.update(
                        {"answer_type": "suggestion", "value_suggested": ca_dict[a_tag], "value_suggested_row": row.id}
                    )
                    self.create(vals)
            ...
            for col in question.labels_ids:
                for row in question.labels_ids_2:
                    ...
                    if a_tag in ca_dict:
                        no_answers = False
                        vals.update(
                            {"answer_type": "suggestion", "value_suggested": col.id, "value_suggested_row": row.id}
                        )
                        self.create(vals)
        if no_answers:
            vals.update({"answer_type": None, "skipped": True})

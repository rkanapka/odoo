...
class TestSurvey(TransactionCase):
    ...
    def test_00_create_minimal_survey(self):
        question = (
            self.env["survey.question"].sudo(self.survey_manager).create({"page_id": self.page1.id, "question": "Q0"})
        )
        self.assertEqual(self.survey1.title, "S0", msg="Title of the survey is somehow modified.")
        self.assertEqual(
            len(self.survey1.page_ids), 1, msg="Additional Pages are linked with the survey after creation."
        )
        self.assertEqual(self.page1.title, "P0", msg="Title of the page is somehow modified.")
        self.assertEqual(
            len(self.page1.question_ids), 1, msg="Additional questions are linked with the page after creation."
        )
    ...
    def test_01_question_type_validation_save_line_function(self):
        for question_type, text in self.env["survey.question"]._fields["type"].selection:
            ...
            self.assertTrue(
                hasattr(self.env["survey.question"], "validate_" + question_type),
            ...
            self.assertTrue(
                hasattr(self.env["survey.user_input_line"], "save_line_" + question_type),
                msg="Inputline must have Save method in \
    ...
    def test_02_question_answer_required(self):
        for question_type, text in self.env["survey.question"]._fields["type"].selection:
            ...
            self.assertDictEqual(
                ...
                msg=(
                    "Validation function for type %s is unable to generate error if it is mandatory and answer is blank."
                    % question_type
                ),
            )
    ...
    def test_03_question_textbox(self):
        questions = [
            self.env["survey.question"]
            .sudo(self.survey_manager)
            .create({"page_id": self.page1.id, "question": "Q0", "type": "textbox", "validation_email": True}),
            self.env["survey.question"]
            .sudo(self.survey_manager)
            .create(
                {
        ...
        for i in range(len(questions)):
            ...
            self.assertEqual(
                questions[i].validate_question({answer_tag: results[i][0]}, answer_tag),
    ...
    def test_04_question_numerical_box(self):
        ...
        for i in range(len(results)):
            self.assertEqual(
                question.validate_question({answer_tag: results[i][0]}, answer_tag),
                {answer_tag: results[i][1]},
                msg="\
    ...
    def test_06_survey_sharing(self):
        ...
        correct_survey = (
            ...
            .create(
                {
                    "title": "S0",
                    "stage_id": self.env["survey.stage"].search([("sequence", "=", 1)]).id,
                    "page_ids": [
                        (0, 0, {"title": "P0", "question_ids": [(0, 0, {"question": "Q0", "type": "free_text"})]})
                    ],
                }
            )
        )
        ...
        surveys = [
            self.env["survey.survey"]
            .sudo(self.survey_manager)
            .create({"title": "Test survey"}),  # Survey without any page or question.
            self.env["survey.survey"]
            .sudo(self.survey_manager)
            .create(
                {  # Closed Survey.
                    "title": "S0",
                    "stage_id": self.env["survey.stage"]
                    .search([("closed", "=", True)])
                    .id,  # Getting Closed stage id.
                    "page_ids": [
                        (0, 0, {"title": "P0", "question_ids": [(0, 0, {"question": "Q0", "type": "free_text"})]})
                    ],
                }
    ...
    def test_07_survey_email_message(self):
        ...
        mail_messages = [
            ...
            .sudo(self.survey_manager)
            .create(
                {"survey_id": self.survey1.id, "public": "email_public_link"}  # Mail Message without __URL__ in body.
            ),
            self.env["survey.mail.compose.message"]
            .sudo(self.survey_manager)
            .create(
    ...
    def test_08_survey_urls(self):
        ...
        for urltype, urltxt in urltypes.items():
            ...
            if urltype == "public":
                ...
                self.assertEqual(
                    url_html % full_url, getattr(self.survey1, urltype + "_url_html"), msg="Public URL is incorrect"
                )
                self.assertEqual(
                    url_html % ("/" + url),
                    getattr(self.survey1.with_context({"relative_url": True}), urltype + "_url_html"),
        ...
        for answer in answers:
            for field, value in expected_values.items():
                self.assertEqual(
                    answer[field],
                    value,
                    msg="Unable to answer the survey. Expected behaviour of %s is not proper." % (field),
                )
    ...
    def test_10_survey_result_simple_multiple_choice(self):
            ...
            .create(
                {
                    ...
                    "type": "simple_choice",
                    "labels_ids": [
                        (0, 0, {"value": "Choice0", "quizz_mark": 0}),
        ...
        for i in range(3):
            self.env["survey.user_input"].sudo(self.user_public).create(
                {
                    "survey_id": self.survey1.id,
                    "user_input_line_ids": [
                        (
        ...
        answers = [
            {"text": label.value, "count": lines.count(label.id), "answer_id": label.id}
            for label in question.labels_ids
        ]
        prp_result = self.env["survey.survey"].prepare_result(question)["answers"]
        self.assertItemsEqual(
            prp_result, answers, msg="Statistics of simple, multiple choice questions are different from expectation"
        )
    ...
    def test_11_survey_result_matrix(self):
        ...
        for i in product(question.labels_ids_2.ids, question.labels_ids.ids):
            res[i] = lines.count(i)
        self.assertEqual(
            self.env["survey.survey"].prepare_result(question)["result"],
            res,
            msg="Statistics of matrix type questions are different from expectations",
        )
    ...
    def test_12_survey_result_numeric_box(self):
        question = (
            self.env["survey.question"]
            .sudo(self.survey_manager)
            .create({"page_id": self.page1.id, "question": "Q0", "type": "numerical_box"})
        )
            ...
            self.env["survey.user_input"].sudo(self.user_public).create(
                {
                    "survey_id": self.survey1.id,
                    "user_input_line_ids": [
                        (0, 0, {"question_id": question.id, "answer_type": "number", "value_number": num[i]})
                    ],
                }
            )
        exresult = {
        ...
        for key in exresult:
            self.assertEqual(
                result[key],
                exresult[key],
                msg="Statistics of numeric box type questions are different from expectations",
            )

    def test_13_survey_actions(self):
        self.env["survey.question"].sudo(self.survey_manager).create(
            {"page_id": self.page1.id, "question": "Q0", "type": "numerical_box"}
        )
        ...
        for action, val in actions.items():
            result = getattr(
                self.survey1.with_context({"survey_token": val["token"][1:]}), "action_" + action + "_survey"
            )()

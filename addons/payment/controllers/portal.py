...
class WebsitePayment(http.Controller):
    ...
    def pay(self, reference="", amount=False, currency_id=None, acquirer_id=None, **kw):
        ...
        if not acquirers:
            acquirers = env["payment.acquirer"].search(
                [("website_published", "=", True), ("company_id", "=", user.company_id.id)]
            )
    ...
    def _get_existing_transaction(self, reference, amount, partner_id, currency_id, acquirer_id, tx_id):
        ...
        if tx and (
            tx.state != "draft"
            or tx.partner_id.id != partner_id
            or tx.amount != amount
            or tx.currency_id.id != currency_id
        ):
            tx = None
    ...
    def transaction(self, acquirer_id, reference, amount, currency_id, **kwargs):
        ...
        tx = self._get_existing_transaction(
            reference,
            float(amount),
            partner_id,
            int(currency_id),
    ...
    @http.route(
        [
            "/website_payment/token/<string:reference>/<string:amount>/<string:currency_id>",
    ...
    @http.route(
        [
            "/website_payment/json_token/<string:reference>/<string:amount>/<string:currency_id>",
            "/website_payment/json_token/v2/<string:amount>/<string:currency_id>/<path:reference>",
        ],
        type="json",
        auth="public",
    )

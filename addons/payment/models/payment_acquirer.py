...
class PaymentAcquirer(models.Model):
    ...
    provider = fields.Selection(
        selection=[("manual", "Manual Configuration")], string="Provider", default="manual", required=True
    )
    company_id = fields.Many2one(
        "res.company", "Company", default=lambda self: self.env.user.company_id.id, required=True
    )
    view_template_id = fields.Many2one(
        "ir.ui.view", "Form Button Template", required=True, default=_get_default_view_template_id
    )
    registration_view_template_id = fields.Many2one(
        "ir.ui.view", "S2S Form Template", domain=[("type", "=", "qweb")], help="Template for method registration"
    )
    environment = fields.Selection(
        [("test", "Test"), ("prod", "Production")], string="Environment", default="test", oldname="env", required=True
    )
    ...
    capture_manually = fields.Boolean(
        string="Capture Amount Manually", help="Capture the amount from Odoo, when the delivery is completed."
    )
    ...
    pre_msg = fields.Html(
        "Help Message", translate=True, help="Message displayed to explain and help the payment process."
    )
    post_msg = fields.Html(
        "Thanks Message", translate=True, help="Message displayed after having done the payment process."
    )
    ...
    save_token = fields.Selection(
        [("none", "Never"), ("ask", "Let the customer decide"), ("always", "Always")],
        string="Save Cards",
        default="none",
    ...
    token_implemented = fields.Boolean(
        "Saving Card Data supported", compute="_compute_feature_support", search="_search_is_tokenized"
    )
    ...
    image = fields.Binary(
        "Image", attachment=True, help="This field holds the image used for this provider, limited to 1024x1024px"
    )
            ...
            for k, f in acquirer._fields.items():
                if getattr(f, "required_if_provider", None) == acquirer.provider and not acquirer[k]:
                    empty_field.append(
                        ...
                        .search([("name", "=", k), ("model", "=", acquirer._name)])
                        .field_description
                    )
        if empty_field:
    ...
    def _prepare_account_journal_vals(self):
        ...
        """
        self.ensure_one()
        account_vals = self.env["account.journal"]._prepare_liquidity_account(self.name, self.company_id, None, "bank")
    ...
    def _get_available_payment_input(self, partner=None, company=None):
        ...
        s2s_acquirers = active_acquirers.filtered(
            lambda acq: acq.payment_flow == "s2s" and acq.registration_view_template_id
        )
        return {
            "form_acquirers": form_acquirers,
            "s2s_acquirers": s2s_acquirers,
            "pms": self.env["payment.token"].search(
                [("partner_id", "=", partner.id), ("acquirer_id", "in", s2s_acquirers.ids)]
            ),
        }
    ...
    def render(self, reference, amount, currency_id, partner_id=False, values=None):
            ...
            values.update(
                {
                    ...
                    "billing_partner_address": _partner_format_address(
        ...
        if not values.get("partner_address"):
            values["address"] = _partner_format_address(
                values.get("partner_street", ""), values.get("partner_street2", "")
            )
        ...
        if not values.get("billing_partner_address"):
            values["billing_address"] = _partner_format_address(
                values.get("billing_partner_street", ""), values.get("billing_partner_street2", "")
            )
        ...
        if hasattr(self, fees_method_name):
            fees = getattr(self, fees_method_name)(
                values["amount"], values["currency_id"], values.get("partner_country_id")
            )
    ...
    def get_s2s_form_xml_id(self):
        if self.registration_view_template_id:
            model_data = self.env["ir.model.data"].search(
                [("model", "=", "ir.ui.view"), ("res_id", "=", self.registration_view_template_id.id)]
            )
...
class PaymentIcon(models.Model):
    ...
    acquirer_ids = fields.Many2many(
        "payment.acquirer", string="Acquirers", help="List of Acquirers supporting this payment icon."
    )
    image = fields.Binary(
        "Image", attachment=True, help="This field holds the image used for this payment icon, limited to 1024x1024px"
    )

    image_payment_form = fields.Binary("Image displayed on the payment form", attachment=True)
...
class PaymentTransaction(models.Model):
    ...
    acquirer_reference = fields.Char(
        "Acquirer Reference", help="Reference of the TX as stored in the acquirer database"
    )
        ...
        if partner_id:
            ...
            return {
                "value": {
                    ...
                    "partner_address": _partner_format_address(
                        partner and partner.street or "", partner and partner.street2 or ""
                    ),
    ...
    def _check_authorize_state(self):
        failed_tx = self.filtered(
            ...
            and tx.acquirer_id.provider not in self.env["payment.acquirer"]._get_feature_support()["authorize"]
        )
        if failed_tx:
            raise exceptions.ValidationError(
                _(
                    "The %s payment acquirers are not allowed to manual capture mode!"
    ...
    def _generate_callback_hash(self):
        ...
        token = "%s%s%s" % (self.callback_model_id.model, self.callback_res_id, self.sudo().callback_method)
        ...
        for transaction in self:
            ...
            if record:
                ...
                _logger.warning(
                    ...
                    % (transaction.callback_model_id.model, transaction.callback_res_id, transaction.id)
                )
        return res

...
class Website(models.Model):
    ...
    currency_id = fields.Many2one(
        "res.currency", related="pricelist_id.currency_id", related_sudo=False, string="Default Currency"
    )
    salesperson_id = fields.Many2one("res.users", string="Salesperson")
    salesteam_id = fields.Many2one("crm.team", string="Sales Channel")
    pricelist_ids = fields.One2many(
        "product.pricelist", compute="_compute_pricelist_ids", string="Price list available for this Ecommerce/Website"
    )
    ...
    @tools.ormcache(
        "self.env.uid", "country_code", "show_visible", "website_pl", "current_pl", "all_pl", "partner_pl", "order_pl"
    )
    def _get_pl_partner_order(
        self, country_code, show_visible, website_pl, current_pl, all_pl, partner_pl=False, order_pl=False
    ):
        ...
        if country_code:
            for cgroup in self.env["res.country.group"].search([("country_ids.code", "=", country_code)]):
                for group_pricelists in cgroup.pricelist_ids:
                    if (
                        not show_visible
                        or group_pricelists.selectable
    ...
    def get_pricelist_available(self, show_visible=False):
        ...
        order_pl = (
            ...
            and partner.last_website_so_id.state == "draft"
            and partner.last_website_so_id.pricelist_id
        )
    ...
    def sale_get_payment_term(self, partner):
        return (
            ...
            or self.env.ref("account.account_payment_term_immediate", False)
            or self.env["account.payment.term"].sudo().search([("company_id", "=", self.company_id.id)], limit=1)
        ).id
    ...
    def _prepare_sale_order_values(self, partner, pricelist):
        ...
        salesperson_id = (
            ...
            else request.website.salesperson_id.id
        )
    ...
    def sale_get_order(self, force_create=False, code=None, update_pricelist=False, force_pricelist=False):
        ...
        if not sale_order_id:
            ...
            sale_order_id = (
                last_order.state == "draft" and last_order.pricelist_id in available_pricelists and last_order.id
            )
        ...
        sale_order = (
            self.env["sale.order"]
            .with_context(force_company=request.website.company_id.id)
            .sudo()
        ...
        if not sale_order and (force_create or code):
            ...
            sale_order = (
                self.env["sale.order"].with_context(force_company=request.website.company_id.id).sudo().create(so_data)
            )
            ...
            if request.website.partner_id.id != partner.id:
                ...
                if country_code:
                    ...
                    fp_id = (
                        ...
                        .sudo()
                        .with_context(force_company=request.website.company_id.id)
                        ._get_fpos_by_region(country_id)
                    )

...
class ProductTemplate(models.Model):
    ...
    website_public_price = fields.Float(
        "Website public price", compute="_website_price", digits=dp.get_precision("Product Price")
    )
    ...
    def _website_price(self):
        ...
        for p, p2 in pycompat.izip(self, self2):
            ...
            p.website_price_difference = (
                True
                if float_compare(
                    price_without_pricelist, p.website_price, precision_rounding=pricelist.currency_id.rounding
                )
                > 0
                else False
            )

...
class ResConfigSettings(models.TransientModel):
    ...
    salesteam_id = fields.Many2one(
        "crm.team", related="website_id.salesteam_id", string="Sales Channel", domain=[("team_type", "!=", "pos")]
    )
    ...
    cart_abandoned_delay = fields.Float(
        "Abandoned Delay", default=1.0, help="number of hours after which the cart is considered abandoned"
    )
    ...
    def get_values(self):
        ...
        if self.env["ir.module.module"].search([("name", "=", "delivery")], limit=1).state in (
            ...
            if self.env["ir.module.module"].search([("name", "=", "website_sale_delivery")], limit=1).state in (
                "installed",
                "to install",
                "to upgrade",
            ):
        ...
        cart_recovery_mail_template = literal_eval(
            params.get_param("website_sale.cart_recovery_mail_template_id", default="False")
        )
    ...
    def set_values(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "website_sale.cart_recovery_mail_template_id", self.cart_recovery_mail_template.id
        )
        self.env["ir.config_parameter"].sudo().set_param(

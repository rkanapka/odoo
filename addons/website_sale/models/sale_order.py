...
class SaleOrder(models.Model):
    ...
    @api.depends(
        "state", "payment_tx_id", "payment_tx_id.state", "payment_acquirer_id", "payment_acquirer_id.provider"
    )
    def _compute_can_directly_mark_as_paid(self):
        for order in self:
            order.can_directly_mark_as_paid = (
                order.state in ["sent", "sale"]
                and order.payment_tx_id
                and order.payment_acquirer_id.provider in ["transfer", "manual"]
            )
    ...
    def _compute_abandoned_cart(self):
        abandoned_delay = float(
            self.env["ir.config_parameter"].sudo().get_param("website_sale.cart_abandoned_delay", "1.0")
        )
        abandoned_datetime = fields.Datetime.to_string(datetime.utcnow() - relativedelta(hours=abandoned_delay))
        for order in self:
            domain = (
                order.date_order
                and order.date_order <= abandoned_datetime
    ...
    def _cart_find_product_line(self, product_id=None, line_id=None, **kwargs):
        ...
        if (
            product
            and product.mapped("attribute_line_ids").filtered(lambda r: not r.attribute_id.create_variant)
            and not line_id
        ):
    ...
    def _website_product_id_change(self, order_id, product_id, qty=0):
        ...
        if order.pricelist_id.discount_policy == "without_discount":
            ...
            # 'sale.order.line'.
            price, rule_id = order.pricelist_id.with_context(product_context).get_product_price_rule(
                product, qty or 1.0, order.partner_id
            )
            pu, currency_id = (
                request.env["sale.order.line"]
                .with_context(product_context)
                ._get_real_price_currency(product, rule_id, qty, product.uom_id, order.pricelist_id.id)
            )
            if pu != 0:
                if order.pricelist_id.currency_id.id != currency_id:
                    ...
                    pu = (
                        ...
                        .browse(currency_id)
                        .with_context(product_context)
                        .compute(pu, order.pricelist_id.currency_id)
                    )
            ...
            if order.pricelist_id and order.partner_id:
                ...
                if order_line:
                    pu = self.env["account.tax"]._fix_tax_included_price_company(
                        pu, product.taxes_id, order_line[0].tax_id, self.company_id
                    )
    ...
    def _cart_accessories(self):
        ...
        for order in self:
            accessory_products = order.website_order_line.mapped("product_id.accessory_product_ids").filtered(
    ...
    def action_recovery_email_send(self):
        ...
        try:
            ...
            template_id = int(
                ...
                .sudo()
                .get_param("website_sale.cart_recovery_mail_template_id", default_template_id)
            )
        except:
            template_id = False
        return {

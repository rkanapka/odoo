...
class WebsiteSaleBackend(WebsiteBackend):
    ...
    def fetch_dashboard_data(self, date_from, date_to):
        results = super().fetch_dashboard_data(date_from, date_to)
        ...
        sales_values["summary"].update(
            ...
            order_carts_abandoned_count=request.env["sale.order"].search_count(
                sale_order_domain + [("is_abandoned_cart", "=", True), ("cart_recovery_email_sent", "=", False)]
            ),
        ...
        sales_values["summary"]["order_sold_ratio"] = (
            round(float(sales_values["summary"]["total_sold"]) / sales_values["summary"]["order_count"], 2)
            if sales_values["summary"]["order_count"]
            else 0
        )
        sales_values["summary"]["order_convertion_pctg"] = (
            100.0 * sales_values["summary"]["order_count"] / sales_values["summary"]["order_carts_count"]
        ...
        sales_values["graph"] += [
            ...
            },
            {
                "values": self._compute_sale_graph(
                    date_date_from - timedelta(days=date_diff_days), date_date_from, sales_domain, previous=True
                ),
        ...
        sales_graph = [
            {
                ...
                "1": daily_sales_dict.get(
                    babel.dates.format_date(
                        d, format="dd MMM yyyy", locale=request.env.context.get("lang") or "en_US"
                    ),
                    0,
                ),
            }

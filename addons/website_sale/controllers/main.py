...
class Website(Website):
    ...
    def get_switchable_related_views(self, key):
        views = super().get_switchable_related_views(key)
...
class WebsiteSale(http.Controller):
    ...
    def get_attribute_value_ids(self, product):
        ...
        visible_attrs_ids = (
            product.attribute_line_ids.filtered(lambda l: len(l.value_ids) > 1).mapped("attribute_id").ids
        )
        ...
        for variant in product.product_variant_ids:
            ...
            visible_attribute_ids = [
                v.id for v in variant.attribute_value_ids if v.attribute_id.id in visible_attrs_ids
            ]
    ...
    def shop(self, page=0, category=None, search="", ppg=False, **post):
        ...
        if products:
            attributes = ProductAttribute.search([("attribute_line_ids.product_tmpl_id", "in", search_product.ids)])
        else:
    ...
    def product(self, product, category="", search="", **kwargs):
        product_context = dict(request.env.context, active_id=product.id, partner=request.env.user.partner_id)
    ...
    def pricelist_change(self, pl_id, **post):
        if (
            pl_id.selectable or pl_id == request.env.user.partner_id.property_product_pricelist
    ...
    def cart(self, access_token=None, revive="", **post):
        ...
        if access_token:
            ...
            if abandoned_order.state != "draft":  # abandoned cart already finished
                ...
                revive == "merge" and not request.session["sale_order_id"]
                ...
                abandoned_order.id != request.session["sale_order_id"]
    ...
    def cart_update_json(self, product_id, line_id=None, add_qty=None, set_qty=None, display=True):
        ...
        value["website_sale.cart_lines"] = request.env["ir.ui.view"].render_template(
            ...
            if shippings:
                if kw.get("partner_id") or "use_billing" in kw:
                    ...
                    last_order = (
                        ...
                        .sudo()
                        .search([("partner_id", "=", order.partner_id.id)], order="id desc", limit=1)
                    )
                    order.partner_shipping_id.id = last_order and last_order.id

        values = {"order": order, "shippings": shippings, "only_services": order and order.only_services or False}
        return values
    ...
    def _get_shop_payment_values(self, order, **kwargs):
        ...
        values["s2s_acquirers"] = [
            acq for acq in acquirers if acq.payment_flow == "s2s" and acq.registration_view_template_id
        ]
        ...
        for acq in values["form_acquirers"]:
            acq.form = (
                acq.with_context(submit_class="btn btn-primary", submit_txt=_("Pay Now"))
                .sudo()
                .render(
                    "/",
                    order.amount_total,
    ...
    def add_product(self, name=None, category=0, **post):
        product = request.env["product.product"].create(
            {"name": name or _("New Product"), "public_categ_ids": category}
        )

...
class SaleOrder(models.Model):
    ...
    payment_acquirer_id = fields.Many2one(
        "payment.acquirer", string="Payment Acquirer", related="payment_tx_id.acquirer_id", store=True
    )
    payment_transaction_count = fields.Integer(
        string="Number of payment transactions", compute="_compute_payment_transaction_count"
    )

    def _compute_payment_transaction_count(self):
        transaction_data = self.env["payment.transaction"].read_group(
            [("sale_order_id", "in", self.ids)], ["sale_order_id"], ["sale_order_id"]
        )
        mapped_data = {m["sale_order_id"][0]: m["sale_order_id_count"] for m in transaction_data}
        for order in self:

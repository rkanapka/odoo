...
class PaymentTransaction(models.Model):
    ...
    def _generate_and_pay_invoice(self):
        ...
        if created_invoice:
            ...
            created_invoice.with_context(default_currency_id=self.currency_id.id).pay_and_reconcile(
    ...
    def confirm_sale_token(self):
        ...
        if self.payment_token_id and self.partner_id == self.sale_order_id.partner_id:
            try:
                ...
                _logger.warning(_("<%s> transaction (%s) failed: <%s>") % (self.acquirer_id.provider, self.id, str(e)))
    ...
    def _check_or_create_sale_tx(
        self, order, acquirer, payment_token=None, tx_type="form", add_tx_values=None, reset_draft=True
    ):
        tx = self
        if not tx:
        ...
        if (tx and tx.acquirer_id != acquirer) or (tx and tx.sale_order_id != order):  # filter unmatching
            tx = False
        if (
            tx and payment_token and tx.payment_token_id and payment_token != tx.payment_token_id

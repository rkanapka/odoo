...
class AccountAnalyticAccount(models.Model):
    ...
    def _compute_debit_credit_balance(self):
        ...
        credit_groups = analytic_line_obj.read_group(
            domain=domain + [("amount", ">=", 0.0)], fields=["account_id", "amount"], groupby=["account_id"]
        )
    ...
    tag_ids = fields.Many2many(
        "account.analytic.tag", "account_analytic_account_tag_rel", "account_id", "tag_id", string="Tags", copy=True
    )
...
class AccountAnalyticLine(models.Model):
    ...
    account_id = fields.Many2one(
        "account.analytic.account", "Analytic Account", required=True, ondelete="restrict", index=True
    )
    partner_id = fields.Many2one("res.partner", string="Partner")
    user_id = fields.Many2one("res.users", string="User", default=_default_user)

    tag_ids = fields.Many2many(
        "account.analytic.tag", "account_analytic_line_tag_rel", "line_id", "tag_id", string="Tags", copy=True
    )

...
class WebsiteBlog(http.Controller):
    ...
    def blog_post(self, blog, blog_post, tag_id=None, page=1, enable_editor=None, **post):
        ...
        blog_url = QueryURL(
            "", ["blog", "tag"], blog=blog_post.blog_id, tag=tag, date_begin=date_begin, date_end=date_end
        )
            ...
            values.append(
                {
                    ...
                    "author_image": message.author_id.image
                    and (b"data:image/png;base64,%s" % message.author_id.image)
                    or b"/website_blog/static/src/img/anonymous.png",
                    "date": message.date,
    ...
    def blog_post_copy(self, blog_post_id, **post):
        ...
        """
        new_blog_post = (
            request.env["blog.post"].with_context(mail_create_nosubscribe=True).browse(int(blog_post_id)).copy()
        )
        return werkzeug.utils.redirect(
            "/blog/%s/post/%s?enable_editor=1" % (slug(new_blog_post.blog_id), slug(new_blog_post))
        )

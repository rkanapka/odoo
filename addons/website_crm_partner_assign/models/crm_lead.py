...
class CrmLead(models.Model):
    ...
    partner_declined_ids = fields.Many2many(
        "res.partner", "crm_lead_declined_partner", "lead_id", "partner_id", string="Partner not interested"
    )
    ...
    def _merge_data(self, fields):
        ...
        return super()._merge_data(fields)
        ...
        for lead in self:
            ...
            if lead.country_id:
                ...
                result = self.env["res.partner"]._geo_localize(
                    apikey, lead.street, lead.zip, lead.city, lead.state_id.name, lead.country_id.name
                )
                if result:

...
class WebsiteAccount(CustomerPortal):
    ...
    def get_domain_my_lead(self, user):
        return [("partner_assigned_id", "child_of", user.commercial_partner_id.id), ("type", "=", "lead")]

    def get_domain_my_opp(self, user):
        return [("partner_assigned_id", "child_of", user.commercial_partner_id.id), ("type", "=", "opportunity")]
    ...
    def portal_my_opportunities(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, **kw):
        ...
        searchbar_filters = {
            ...
            "won": {
                "label": _("Won"),
                "domain": [("stage_id.probability", "=", 100), ("stage_id.on_change", "=", True)],
            },
    ...
    @http.route(
        ["""/my/lead/<model('crm.lead', "[('type','=', 'lead')]"):lead>"""], type="http", auth="user", website=True
    )
...
class WebsiteCrmPartnerAssign(WebsitePartnerPage):
    ...
    def sitemap_partners(env, rule, qs):
        ...
        countries = (
            env["res.partner"].sudo().read_group(partners_dom, fields=["id", "country_id"], groupby="country_id")
        )
    ...
    def partners(self, country=None, grade=None, page=0, **post):
        ...
        grades = partner_obj.sudo().read_group(
            grade_domain, ["id", "grade_id"], groupby="grade_id", orderby="grade_id DESC"
        )
        ...
        for country_dict in countries:
            country_dict["active"] = (
                country and country_dict["country_id"] and country_dict["country_id"][0] == country.id
            )
        countries.insert(
            0,
            {
        ...
        pager = request.website.pager(
            url=url, total=partner_count, page=page, step=self._references_per_page, scope=7, url_args=url_args
        )

        # search partners matching current search parameters
        partner_ids = partner_obj.sudo().search(
            base_partner_domain,
            order="grade_sequence DESC, implemented_count DESC, display_name ASC, id ASC",
            offset=pager["offset"],

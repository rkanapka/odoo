...
class TestPartnerAssign(TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch("odoo.addons.website_crm_partner_assign.models.crm_lead.geo_find", wraps=geo_find)
        patcher.start()
        self.addCleanup(patcher.stop)
    ...
    def test_partner_assign(self):
        ...
        self.assertTrue(
            50 < partner2.partner_latitude < 51, "Latitude is wrong: 50 < %s < 51" % partner2.partner_latitude
        )
        self.assertTrue(
            3 < partner2.partner_longitude < 5, "Longitude is wrong: 3 < %s < 5" % partner2.partner_longitude
        )
        ...
        self.assertEqual(
            lead.partner_assigned_id, self.env.ref("base.res_partner_18"), "Opportuniy is not assigned nearest partner"
        )
...
class TestPartnerLeadPortal(TestCrmCases):
    ...
    def test_partner_lead_accept(self):
        ...
        self.assertEqual(
            self.lead.type, "opportunity", "Bad Type: accepted lead by portal user should become an opportunity."
        )
    ...
    def test_partner_lead_decline(self):
        ...
        self.lead.sudo(self.portal_user.id).partner_desinterested(
            comment="No thanks, I have enough leads !", contacted=True, spam=False
        )

        self.assertFalse(
            self.lead.partner_assigned_id.id, "The partner_assigned_id of the declined lead should be False."
        )
        self.assertTrue(
            self.portal_user.partner_id in self.lead.sudo().partner_declined_ids,

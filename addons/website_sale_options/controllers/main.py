...
class WebsiteSaleOptions(WebsiteSale):
    ...
    def product(self, product, category="", search="", **kwargs):
        r = super().product(product, category, search, **kwargs)
        r.qcontext["optional_product_ids"] = [p.with_context(active_id=p.id) for p in product.optional_product_ids]
        return r

    @http.route(
        ["/shop/cart/update_option"], type="http", auth="public", methods=["POST"], website=True, multilang=False
    )

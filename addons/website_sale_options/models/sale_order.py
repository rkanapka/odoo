...
class SaleOrderLine(models.Model):
    ...
    linked_line_id = fields.Many2one(
        "sale.order.line", string="Linked Order Line", domain="[('order_id', '!=', order_id)]", ondelete="cascade"
    )
...
class SaleOrder(models.Model):
    ...
    def _cart_find_product_line(self, product_id=None, line_id=None, **kwargs):
        self.ensure_one()
        lines = super()._cart_find_product_line(product_id, line_id)
        if line_id:
            return lines
        ...
        if optional_product_ids:
            ...
            lines = lines.filtered(
                lambda line: optional_product_ids == set(line.mapped("option_line_ids.product_id.id"))
            )
        else:
    ...
    def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0, **kwargs):
        ...
        value = super()._cart_update(product_id, line_id, add_qty, set_qty, **kwargs)
        ...
        for option_line_id in option_lines:
            super()._cart_update(option_line_id.product_id.id, option_line_id.id, add_qty, set_qty, **kwargs)

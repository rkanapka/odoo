...
from werkzeug import url_encode, urls
...
class PaymentTxOgone(models.Model):
    ...
    def _ogone_form_get_tx_from_data(self, data):
        ...
        reference, pay_id, shasign, alias = (
            data.get("orderID"),
            data.get("PAYID"),
            data.get("SHASIGN"),
            data.get("ALIAS"),
        ...
        if not reference or not pay_id or not shasign:
            error_msg = _("Ogone: received data with missing reference (%s) or pay_id (%s) or shasign (%s)") % (
                reference,
                pay_id,
                shasign,
            )
    ...
    def ogone_s2s_do_transaction(self, **kwargs):
        ...
        reference = self.reference or "ODOO-%s-%s" % (
            datetime.datetime.now().strftime("%y%m%d_%H%M%S"),
            self.partner_id.id,
        )

        param_plus = {"return_url": kwargs.get("return_url", False)}

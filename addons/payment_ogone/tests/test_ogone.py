...
class OgonePayment(PaymentAcquirerCommon):
    ...
    def test_10_ogone_form_render(self):
        ...
        res = self.ogone.render(
            "test_ref0", 0.01, self.currency_euro.id, partner_id=None, partner_values=self.buyer_values
        )

        # check form result
        tree = objectify.fromstring(res)
        self.assertEqual(
            tree.get("action"), "https://secure.ogone.com/ncol/test/orderstandard.asp", "ogone: wrong form POST url"
        )

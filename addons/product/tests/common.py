...
class TestProductCommon(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.product_0 = Product.create(
            {"name": "Work", "type": "service", "uom_id": cls.uom_unit.id, "uom_po_id": cls.uom_unit.id}
        )
        ...
        cls.product_2 = Product.create({"name": "Wood", "uom_id": cls.uom_unit.id, "uom_po_id": cls.uom_unit.id})
        cls.product_3 = Product.create({"name": "Stone", "uom_id": cls.uom_dozen.id, "uom_po_id": cls.uom_dozen.id})

        cls.product_4 = Product.create({"name": "Stick", "uom_id": cls.uom_dozen.id, "uom_po_id": cls.uom_dozen.id})
        cls.product_5 = Product.create(
            {"name": "Stone Tools", "uom_id": cls.uom_unit.id, "uom_po_id": cls.uom_unit.id}
        )

        cls.product_6 = Product.create({"name": "Door", "uom_id": cls.uom_unit.id, "uom_po_id": cls.uom_unit.id})
        ...
        cls.product_8 = Product.create({"name": "House", "uom_id": cls.uom_unit.id, "uom_po_id": cls.uom_unit.id})

        cls.product_9 = Product.create({"name": "Paper", "uom_id": cls.uom_unit.id, "uom_po_id": cls.uom_unit.id})

        cls.product_10 = Product.create({"name": "Stone", "uom_id": cls.uom_unit.id, "uom_po_id": cls.uom_unit.id})

...
class TestProductPricelist(TransactionCase):
    ...
    def test_10_calculation_price_of_products_pricelist(self):
        ...
        msg = "Wrong sale price: iPad Retina Display. should be %s instead of %s" % (
            ...
            (ipad_retina_display.lst_price - ipad_retina_display.lst_price * (0.10)),
        )
        self.assertEqual(
            float_compare(
                ipad_retina_display.price,
        ...
        self.assertEqual(
            float_compare(
                laptop_S3450.price, laptop_S3450.lst_price - laptop_S3450.lst_price * (0.05), precision_digits=2
            ),
            0,
            msg,
        )
        ...
        self.assertEqual(
            float_compare(ipad_mini.price, ipad_mini.lst_price - ipad_mini.lst_price * (0.30), precision_digits=2),
            0,
            msg,
        )
        ...
        msg = (
            "Wrong cost price: LCD Monitor. should be 790 instead of %s"
            % ipad_mini._select_seller(partner_id=partner, quantity=1.0).price
        )
        self.assertEqual(
            float_compare(ipad_mini._select_seller(partner_id=partner, quantity=1.0).price, 790, precision_digits=2),
            0,
            msg,
        )
        ...
        msg = (
            ...
            % ipad_mini._select_seller(partner_id=partner, quantity=3.0).price
        )
        self.assertEqual(
            float_compare(ipad_mini._select_seller(partner_id=partner, quantity=3.0).price, 785, precision_digits=2),
            0,
            msg,
        )
        ...
        test_reports.try_report_action(
            self.cr, self.uid, "action_product_price_list", wiz_data=data_dict, context=ctx, our_module="product"
        )

...
class TestVariantsSearch(TransactionCase):
    ...
    def test_attribute_line_search(self):
        search_not_to_be_found = self.env["product.template"].search([("attribute_line_ids", "=", "M")])
        self.assertNotIn(self.product_shirt_template, search_not_to_be_found, "Shirt should not be found searching M")

        search_attribute = self.env["product.template"].search([("attribute_line_ids", "=", "Size")])
    ...
    def test_name_search(self):
        ...
        self.assertIn(
            self.product_slip_template.product_variant_ids.id, res_ids, "Slip should be found searching 'not ilike'"
        )
...
class TestVariants(common.TestProductCommon):
    ...
    def test_variants_is_product_variant(self):
        ...
        self.assertEqual({True}, {v.is_product_variant for v in variants}, "Product variants are variants")
    ...
    def test_variants_creation_mono_double(self):
        ...
        self.assertEqual(len(test_template.product_variant_ids), 1)
        self.assertEqual(
            test_template.product_variant_ids.attribute_value_ids, self.size_attr_value_s + self.prod_attr1_v2
        )
                ...
                "attribute_line_ids": [
                    ...
                    (
                        ...
                        0,
                        {
                            "attribute_id": self.size_attr.id,
                            "value_ids": [
                                (4, self.size_attr_value_s.id),
                                (4, self.size_attr_value_m.id),
                                (4, self.size_attr_value_l.id),
                            ],
                        },
                    ),
                ],
    ...
    def test_variants_creation_multi_update(self):
        ...
        size_attribute_line = test_template.attribute_line_ids.filtered(
            lambda line: line.attribute_id == self.size_attr
        )
        test_template.write(
            {
                "attribute_line_ids": [

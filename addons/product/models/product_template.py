...
class ProductTemplate(models.Model):
    ...
    lst_price = fields.Float("Public Price", related="list_price", digits=dp.get_precision("Product Price"))
    ...
    volume = fields.Float(
        "Volume", compute="_compute_volume", inverse="_set_volume", help="The volume in m3.", store=True
    )
    weight = fields.Float(
        "Weight",
    ...
    sale_ok = fields.Boolean(
        "Can be Sold", default=True, help="Specify if the product can be selected in a sales order line."
    )
    ...
    active = fields.Boolean(
        "Active", default=True, help="If unchecked, it will allow you to hide the product without removing it."
    )
    ...
    default_code = fields.Char(
        "Internal Reference", compute="_compute_default_code", inverse="_set_default_code", store=True
    )
        ...
        if self._context.get("uom"):
            for template in self:
                value = (
                    ...
                    ._compute_price(template.price, template.uom_id)
                )
    ...
    def name_search(self, name="", args=None, operator="ilike", limit=100):
        ...
        return super().name_search("", args=[("id", "in", list(set(templates.ids)))], operator="ilike", limit=limit)
    ...
    def create_variant_ids(self):
        ...
        for tmpl_id in self.with_context(active_test=False):
            ...
            variant_alone = tmpl_id.attribute_line_ids.filtered(
                lambda line: line.attribute_id.create_variant and len(line.value_ids) == 1
            ).mapped("value_ids")
            ...
            existing_variants = {
                frozenset(variant.attribute_value_ids.filtered(lambda r: r.attribute_id.create_variant).ids)
            ...
            to_create_variants = [
                value_ids for value_ids in variant_matrix if set(value_ids.ids) not in existing_variants
            ]
            ...
            for product_id in tmpl_id.product_variant_ids:
                if (
                    not product_id.active
                    and product_id.attribute_value_ids.filtered(lambda r: r.attribute_id.create_variant)
                    in variant_matrix
                ):
                    variants_to_activate |= product_id
                elif (
                    product_id.attribute_value_ids.filtered(lambda r: r.attribute_id.create_variant)

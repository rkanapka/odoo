...
class Pricelist(models.Model):
    ...
    active = fields.Boolean(
        "Active", default=True, help="If unchecked, it will allow you to hide the pricelist without removing it."
    )
    item_ids = fields.One2many(
        "product.pricelist.item", "pricelist_id", "Pricelist Items", copy=True, default=_get_default_item_ids
    )
    ...
    def _compute_price_rule(self, products_qty_partner, date=False, uom_id=False):
        ...
        for product, qty, partner in products_qty_partner:
            ...
            if qty_uom_id != product.uom_id.id:
                try:
                    qty_in_product_uom = (
                        self.env["product.uom"].browse([self._context["uom"]])._compute_quantity(qty, product.uom_id)
                    )
                except UserError:
    ...
    def get_products_price(self, products, quantities, partners, date=False, uom_id=False):
        ...
        return {
            ...
            for product_id, res_tuple in self._compute_price_rule(
                list(pycompat.izip(products, quantities, partners)), date=date, uom_id=uom_id
            ).items()
        }
    ...
    def _price_get_multi(self, pricelist, products_by_qty_by_partner):
        ...
        return pricelist.get_products_price(list(pycompat.izip(**products_by_qty_by_partner)))
    ...
    def _get_partner_pricelist_multi(self, partner_ids, company_id=None):
        ...
        if remaining_partner_ids:
            ...
            pl_fallback = (
                Pricelist.search([("country_group_ids", "=", False)], limit=1)
                or Property.get("property_product_pricelist", "res.partner")
                or Pricelist.search([], limit=1)
            )
...
class PricelistItem(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company", "Company", readonly=True, related="pricelist_id.company_id", store=True
    )
    currency_id = fields.Many2one(
        "res.currency", "Currency", readonly=True, related="pricelist_id.currency_id", store=True
    )
    ...
    compute_price = fields.Selection(
        [("fixed", "Fix Price"), ("percentage", "Percentage (discount)"), ("formula", "Formula")],
        index=True,
        default="fixed",
    )
    ...
    name = fields.Char(
        "Name", compute="_get_pricelist_item_name_price", help="Explicit rule name for this pricelist line."
    )
    price = fields.Char(
        "Price", compute="_get_pricelist_item_name_price", help="Explicit rule name for this pricelist line."
    )

    @api.constrains("base_pricelist_id", "pricelist_id", "base")
    def _check_recursion(self):
        if any(
            item.base == "pricelist" and item.pricelist_id and item.pricelist_id == item.base_pricelist_id
            for item in self
        ):

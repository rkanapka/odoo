...
class DecimalPrecision(models.Model):
    ...
    def _check_main_currency_rounding(self):
        if any(
            ...
            and tools.float_compare(
                self.env.user.company_id.currency_id.rounding, 10**-precision.digits, precision_digits=6
            )
            == -1
    ...
    def _onchange_digits(self):
        ...
        if dangerous_uom:
            uom_descriptions = [
                " - %s (id=%s, precision=%s)" % (uom.name, uom.id, uom.rounding) for uom in dangerous_uom
            ]
            return {
                "warning": {
                    "title": _("Warning!"),

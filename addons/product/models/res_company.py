...
class ResCompany(models.Model):
    ...
    def create(self, vals):
        ...
        pricelist = ProductPricelist.search(
            [("currency_id", "=", new_company.currency_id.id), ("company_id", "=", False)], limit=1
        )
        if not pricelist:
        ...
        if currency_id and main_pricelist:
            ...
            for company in self:
                existing_pricelist = ProductPricelist.search(
                    [("company_id", "in", (False, company.id)), ("currency_id", "=", currency_id)]
                )
                ...
                if currency_match and company_match:
                    ...
                    params = {"currency": self.env["res.currency"].browse(currency_id).name, "company": company.name}
                    pricelist = ProductPricelist.create(
                        {
                    ...
                    field = self.env["ir.model.fields"].search(
                        [("model", "=", "res.partner"), ("name", "=", "property_product_pricelist")]
                    )

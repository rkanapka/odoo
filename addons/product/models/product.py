...
class ProductProduct(models.Model):
    ...
    product_tmpl_id = fields.Many2one(
        "product.template", "Product Template", auto_join=True, index=True, ondelete="cascade", required=True
    )
    barcode = fields.Char(
        "Barcode", copy=False, oldname="ean13", help="International Article Number used for product identification."
    )
    ...
    def _set_product_price(self):
        for product in self:
            if self._context.get("uom"):
                value = (
                    self.env["product.uom"].browse(self._context["uom"])._compute_price(product.price, product.uom_id)
                )
            else:
                value = product.price
    ...
    def _get_pricelist_items(self):
        self.pricelist_item_ids = (
            ...
            .search(["|", ("product_id", "=", self.id), ("product_tmpl_id", "=", self.product_tmpl_id.id)])
            .ids
        )
    ...
    def name_get(self):
        ...
        self.sudo().read(
            ["name", "default_code", "product_tmpl_id", "attribute_value_ids", "attribute_line_ids"], load=False
        )
...
class SupplierInfo(models.Model):
    ...
    sequence = fields.Integer("Sequence", default=1, help="Assigns the priority to the list of product vendor.")
    ...
    product_tmpl_id = fields.Many2one(
        "product.template", "Product Template", index=True, ondelete="cascade", oldname="product_id"
    )

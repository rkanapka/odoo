        ...
        if len(res) == 2:
            ...
            if with_date:
                real_date = time.strftime(
                    DEFAULT_SERVER_DATETIME_FORMAT, time.strptime(res[1], VIRTUALID_DATETIME_FORMAT)
                )
...
class Attendee(models.Model):
    ...
    def _send_mail_to_attendees(self, template_xmlid, force_send=False):
        ...
        colors = {"needsAction": "grey", "accepted": "green", "tentative": "#FFFF00", "declined": "red"}
    ...
    def do_accept(self):
        ...
        for attendee in self:
            attendee.event_id.message_post(
                body=_("%s has accepted invitation") % (attendee.common_name), subtype="calendar.subtype_invitation"
            )
        return result
    ...
    def do_decline(self):
        ...
        for attendee in self:
            attendee.event_id.message_post(
                body=_("%s has declined invitation") % (attendee.common_name), subtype="calendar.subtype_invitation"
            )
        return res
...
class AlarmManager(models.AbstractModel):
    ...
    def do_check_alarm_for_one_date(
        self, one_date, event, event_maxdelta, in_the_next_X_seconds, alarm_type, after=False, missing=False
    ):
        ...
        if one_date - timedelta(minutes=(missing and 0 or event_maxdelta)) < datetime.now() + timedelta(
            seconds=in_the_next_X_seconds
        ):  # if an alarm is possible for this date
            for alarm in event.alarm_ids:
                if (
                    alarm.type == alarm_type
                    and one_date - timedelta(minutes=(missing and 0 or alarm.duration_minutes))
                    < datetime.now() + timedelta(seconds=in_the_next_X_seconds)
                    and (
                        not after
                        or one_date - timedelta(minutes=alarm.duration_minutes) > fields.Datetime.from_string(after)
                    )
                ):
                    alert = {
                        "alarm_id": alarm.id,
            ...
            if meeting.recurrency:
                ...
                for one_date in meeting._get_recurrent_date_by_event():
                    ...
                    last_found = self.do_check_alarm_for_one_date(
                        in_date_format, meeting, max_delta, 0, "email", after=last_notif_mail, missing=True
                    )
                    for alert in last_found:
                        self.do_mail_reminder(alert)
                        at_least_one = True  # if it's the first alarm for this recurrent event
                    if (
                        at_least_one and not last_found
                    ):  # if the precedent event had an alarm but not this one, we can stop the search for this event
                        break
            else:
...
class Alarm(models.Model):
    ...
    type = fields.Selection(
        [("notification", "Notification"), ("email", "Email")], "Type", required=True, default="email"
    )
    ...
    duration_minutes = fields.Integer(
        "Duration in minutes", compute="_compute_duration_minutes", store=True, help="Duration in minutes"
    )
...
class Meeting(models.Model):
    ...
    def default_get(self, fields):
        ...
        if self.env.context.get("default_res_model") and not self.env.context.get("default_res_model_id"):
            self = self.with_context(
                ...
                .sudo()
                .search([("model", "=", self.env.context["default_res_model"])], limit=1)
                .id
            )
        ...
        if (
            ...
            defaults["res_model_id"] = (
                self.env["ir.model"].sudo().search([("model", "=", self.env.context["active_model"])], limit=1).id
            )
        if (
    ...
    def _get_recurrent_date_by_event(self, date_field="start"):
        ...
        def todate(date):
            val = parser.parse("".join((re.compile(r"\d")).findall(date)))
    ...
    def _get_date_formats(self):
        ...
        if lang:
            ...
            lang_params = {"date_format": record_lang.date_format, "time_format": record_lang.time_format}
    ...
    location = fields.Char(
        "Location", states={"done": [("readonly", True)]}, track_visibility="onchange", help="Location of Event"
    )
    show_as = fields.Selection(
        [("free", "Free"), ("busy", "Busy")], "Show Time as", states={"done": [("readonly", True)]}, default="busy"
    )
    ...
    month_by = fields.Selection(
        [("date", "Date of month"), ("day", "Day of month")], string="Option", default="date", oldname="select1"
    )
    ...
    user_id = fields.Many2one(
        "res.users", "Responsible", states={"done": [("readonly", True)]}, default=lambda self: self.env.user
    )
    ...
    alarm_ids = fields.Many2many(
        "calendar.alarm", "calendar_alarm_calendar_event_rel", string="Reminders", ondelete="restrict", copy=False
    )
    ...
    def get_recurrent_ids(self, domain, order=None):
        ...
        def key(record):
            ...
            return [(tools.Reverse(v) if desc else v) for v, desc in vals_spec]
    ...
    def _rrule_serialize(self):
        ...
        def get_end_date():
            end_date_new = (
                "".join((re.compile(r"\d")).findall(self.final_date)) + "T235959Z" if self.final_date else False
            )
        ...
        if freq:
            ...
            result = (
                "FREQ="
                + freq.upper()
                + get_week_string(freq)
                + interval_srting
                + get_end_date()
                + get_month_string(freq)
            )
        return result
        ...
        for meeting in self:
            ...
            if not is_calendar_id(meeting.id):
                ...
                if not values.get("recurrency", True) or not blacklisted:
                    ...
                    if data.get("rrule"):
                        new_ids = (
                            meeting.with_context(dont_notify=True).detach_recurring_event(values).ids
            ...
            if any(field in values for field in ["recurrency", "end_type", "count", "rrule_type", "start", "stop"]):
                for real_meeting in real_meetings:
                    if real_meeting.recurrency and real_meeting.end_type == "count":
            ...
            if values.get("partner_ids", False):
                attendees_create = all_meetings.with_context(
                    dont_notify=True
                ).create_attendees()  # to prevent multiple notify_next_alarm
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        return super(Meeting, self.with_context(virtual_id=False)).read_group(
    ...
    def read(self, fields=None, load="_classic_read"):
        ...
        for calendar_id, real_id in select:
            ...
            ls = calendar_id2real_id(
                calendar_id, with_date=res and res.get("duration", 0) > 0 and res.get("duration") or 1
            )
    ...
    def search(self, args, offset=0, limit=0, order=None, count=False):
        ...
        if any(arg[0] == "start" for arg in args) and not any(arg[0] in ("stop", "final_date") for arg in args):
        ...
        events = super().search(new_args, offset=0, limit=0, order=None, count=False)

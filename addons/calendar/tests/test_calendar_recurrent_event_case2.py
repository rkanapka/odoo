...
class TestRecurrentEvent(common.TransactionCase):
    ...
    def test_recurrent_meeting3(self):
        ...
        for meeting in meetings:
            self.assertEqual(
                meeting.name, "Sprint Review for google modules", "Name not changed for id: %s" % meeting.id
            )
        ...
        self.assertEqual(
            detached_meeting.description, False, "Detached event description changed for id: %s" % meeting.id
        )
        ...
        self.assertEqual(
            self.calendar_event_sprint_review.description,
            "Some description",
            "Event %d has not been updated" % self.calendar_event_sprint_review.id,
        )
    ...
    def test_recurrent_meeting4(self):
        ...
        meetings_count = self.CalendarEvent.with_context({"virtual_id": True}).search_count(
            [
                ("start", "<=", "2017-01-24 11:55:00"),
                ("stop", ">=", "2017-01-24 11:55:00"),
                ("name", "=", "Review code with programmer"),
            ]
        )
    ...
    def test_recurrent_meeting5(self):
        ...
        self.assertEqual(
            virutal_dates[1], "2012-04-13 11:00:00", "The virtual event doesn't have the correct start date !"
        )

        # virtual_dates are used by the calendar view and I check if the stop date for the first virtual event is correct.
        self.assertEqual(
            virutal_dates[2], "2012-04-13 12:00:00", "The virtual event doesn't have the correct stop date !"
        )

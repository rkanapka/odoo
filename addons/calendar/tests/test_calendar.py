...
class TestCalendar(TransactionCase):
    ...
    def test_calender_simple_event(self):
        ...
        self.assertEqual(
            (m.start_datetime, m.stop_datetime), ("2017-07-12 14:30:00", "2017-07-12 15:00:00"), "Sanity check"
        )
    ...
    def test_calender_event(self):
        ...
        self.assertEqual(
            len(after), len(before) + 1, "Wrong number of events found, after to have moved a virtual event"
        )
        ...
        res_alarm_day_before_event_starts = self.env["calendar.alarm"].create(
            {"name": "1 Day before event starts", "duration": 1, "interval": "days", "type": "notification"}
        )
    ...
    def test_event_activity(self):
        ...
        self.assertEqual(
            test_record.activity_ids.date_deadline, fields.Date.to_string((now + timedelta(days=-1)).date())
        )
        ...
        self.assertEqual(
            test_record.activity_ids.date_deadline, fields.Date.to_string((now + timedelta(days=-2)).date())
        )
        ...
        test_event = (
            self.env["calendar.event"]
            .sudo(self.env.ref("base.user_demo"))
            .with_context(
    ...
    def test_event_activity_timezone(self):
        activty_type = self.env["mail.activity.type"].create({"name": "Meeting", "category": "meeting"})
    ...
    def test_event_creation_mail(self):
        ...
        virtid = str(m.id) + "-" + "".join(re.split(r"[\D]", fields.Datetime.to_string(now + timedelta(days=12))))

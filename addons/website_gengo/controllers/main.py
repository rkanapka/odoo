...
class WebsiteGengo(http.Controller):
    ...
    def get_translated_length(self, translated_ids, lang):
        result = {"done": 0}
        gengo_translation_ids = request.env["ir.translation"].search(
            [("id", "in", translated_ids), ("gengo_translation", "!=", False)]
        )
    ...
    def set_translations(self, data, lang):
        ...
        for term in data:
            ...
            if not translation_ids:
                translations = IrTranslation.search_read(
                    [("lang", "=", lang), ("src", "=", initial_content)], fields=["id"]
                )
                if translations:

...
class HrEmployee(models.Model):
    ...
    barcode = fields.Char(
        string="Badge ID", help="ID used for employee identification.", default=_default_random_barcode, copy=False
    )
    pin = fields.Char(
        string="PIN",
    ...
    def _compute_manual_attendance(self):
        for employee in self:
            employee.manual_attendance = (
                employee.user_id.has_group("hr_attendance.group_hr_attendance") if employee.user_id else False
            )
    ...
    def attendance_scan(self, barcode):
        ...
        return (
            employee
            and employee.attendance_action("hr_attendance.hr_attendance_action_kiosk_mode")
            or {"warning": _("No employee corresponding to barcode %(barcode)s") % {"barcode": barcode}}
        )
    ...
    def attendance_manual(self, next_action, entered_pin=None):
        ...
        if (
            ...
            or self.env["res.users"].browse(SUPERUSER_ID).has_group("hr_attendance.group_hr_attendance_use_pin")
    ...
    def _init_column(self, column_name):
        ...
        if column_name not in ["barcode", "pin"]:
            ...
            for employee_id in employee_ids:
                ...
                query = 'UPDATE "%s" SET "%s"=%%s WHERE id = %s' % (self._table, column_name, employee_id[0])

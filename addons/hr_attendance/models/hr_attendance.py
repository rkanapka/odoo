...
class HrAttendance(models.Model):
    ...
    employee_id = fields.Many2one(
        "hr.employee", string="Employee", default=_default_employee, required=True, ondelete="cascade", index=True
    )
                ...
                if no_check_out_attendances:
                    raise exceptions.ValidationError(
                        ...
                        % {
                            ...
                            "datetime": fields.Datetime.to_string(
                                fields.Datetime.context_timestamp(
                                    self, fields.Datetime.from_string(no_check_out_attendances.check_in)
                                )
                            ),
                        }
                    )
                ...
                if (
                    ...
                    raise exceptions.ValidationError(
                        ...
                        % {
                            ...
                            "datetime": fields.Datetime.to_string(
                                fields.Datetime.context_timestamp(
                                    self, fields.Datetime.from_string(last_attendance_before_check_out.check_in)
                                )
                            ),
                        }
                    )

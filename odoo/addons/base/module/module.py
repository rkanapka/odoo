...
class Module(models.Model):
    ...
    dependencies_id = fields.One2many("ir.module.module.dependency", "module_id", string="Dependencies", readonly=True)
    exclusion_ids = fields.One2many("ir.module.module.exclusion", "module_id", string="Exclusions", readonly=True)
    ...
    def unlink(self):
        ...
        self.env["base.module.uninstall"].search([("module_id", "in", self.ids)]).unlink()
        return super().unlink()
        ...
        for category in exclusives:
            ...
            if modules and not any(modules <= closure(module) for module in modules):
                ...
                raise UserError(
                    "\n".join(
                        [msg % category.name]
                        + ["- %s (%s)" % (module.shortdesc, labels[module.state]) for module in modules]
                    )
                )
    ...
    @api.multi
    @api.returns("self")
    def downstream_dependencies(self, known_deps=None, exclude_states=("uninstalled", "uninstallable", "to remove")):
    ...
    @api.multi
    @api.returns("self")
    def upstream_dependencies(self, known_deps=None, exclude_states=("installed", "uninstallable", "to remove")):
    ...
    def _button_immediate_function(self, function):
        try:
            ...
            raise UserError(
                _(
                    "The server is busy right now, module operations are not possible at"
        ...
        for mod_name in modules.get_modules():
            ...
            if mod:
                ...
                for key in values:
                    old = getattr(mod, key)
                    updated = (
                        tools.ustr(values[key]) if isinstance(values[key], pycompat.string_types) else values[key]
                    )
    ...
    def _update_dependencies(self, depends=None):
        ...
        for dep in needed - existing:
            self._cr.execute(
                "INSERT INTO ir_module_module_dependency (module_id, name) values (%s, %s)", (self.id, dep)
            )
        for dep in existing - needed:
            self._cr.execute(
                "DELETE FROM ir_module_module_dependency WHERE module_id = %s and name = %s", (self.id, dep)
            )
    ...
    def _update_exclusions(self, excludes=None):
        ...
        for name in needed - existing:
            self._cr.execute(
                "INSERT INTO ir_module_module_exclusion (module_id, name) VALUES (%s, %s)", (self.id, name)
            )
    ...
    def _installed(self):
        ...
        return {module.name: module.id for module in self.sudo().search([("state", "=", "installed")])}

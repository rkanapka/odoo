...
class IrTranslationImport:
    ...
    def finish(self):
        ...
        if self._debug:
            ...
            total = cr.fetchone()[0]
            cr.execute(
                "SELECT COUNT(*) FROM ONLY %s AS irt, %s AS ti WHERE %s" % (self._model_table, self._table, find_expr),
                (tuple(src_relevant_fields), tuple(src_relevant_fields)),
            )
            count = cr.fetchone()[0]
            _logger.debug(
...
class IrTranslation(models.Model):
    ...
    source = fields.Text(
        string="Source term", compute="_compute_source", inverse="_inverse_source", search="_search_source"
    )
    ...
    def _check_selection_field_value(self, field, value):
        if field == "lang":
            return
        return super()._check_selection_field_value(field, value)
        ...
        for module_name in modules:
            ...
            for lang in langs:
                ...
                if trans_file:
                    ...
                    tools.trans_load(
                        self._cr, trans_file, lang, verbose=False, module_name=module_name, context=context
                    )
                ...
                if trans_extra_file:
                    ...
                    tools.trans_load(
                        self._cr, trans_extra_file, lang, verbose=False, module_name=module_name, context=context
                    )
        return True

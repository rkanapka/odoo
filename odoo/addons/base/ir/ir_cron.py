...
class ir_cron(models.Model):
    ...
    def _callback(self, cron_name, server_action_id, job_id):
        ...
        try:
            ...
            _logger.exception(
                "Call from cron %s for server action #%s failed in Job #%s", cron_name, server_action_id, job_id
            )
        ...
        try:
            ...
            for job in jobs:
                ...
                try:
                    ...
                    if not locked_job:
                        _logger.debug(
                            "Job `%s` already executed by another process/thread. skipping it", job["cron_name"]
                        )
                        continue
                    ...
                    if e.pgcode == "55P03":
                        ...
                        _logger.debug(
                            "Another process/thread is already busy executing job `%s`, skipping it.", job["cron_name"]
                        )
                        continue
                    else:

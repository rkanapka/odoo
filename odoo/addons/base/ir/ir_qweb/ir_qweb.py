...
from .qweb import Contextifier, QWeb
...
class IrQWeb(models.AbstractModel, QWeb):
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def render(self, id_or_xml_id, values=None, **options):
        ...
        return super().render(id_or_xml_id, values=values, **context)

    def default_values(self):
        """attributes add to the values for each computed template"""
        default = super().default_values()
        default.update(
    ...
    def compile(self, id_or_xml_id, options):
        return super().compile(id_or_xml_id, options=options)
    ...
    def _compile_directive_call_assets(self, el, options):
        ...
        return [
            ast.Assign(
                ...
                value=ast.Call(
                    func=ast.Attribute(
                        value=ast.Name(id="self", ctx=ast.Load()), attr="_get_asset_nodes", ctx=ast.Load()
                    ),
                    args=[
                    ...
                    keywords=[
                        ...
                        ast.keyword(
                            ...
                            ast.Call(
                                func=ast.Attribute(
                                    value=ast.Name(id="values", ctx=ast.Load()), attr="get", ctx=ast.Load()
                                ),
                                args=[ast.Str("debug")],
                                keywords=[],
                                starargs=None,
            ...
            ast.For(
                ...
                body=[
                    ast.If(test=ast.Name(id="index", ctx=ast.Load()), body=[self._append(ast.Str(sep))], orelse=[]),
    ...
    def _compile_widget_options(self, el, directive_type):
        field_options = super()._compile_widget_options(el, directive_type)
    ...
    def _get_asset(self, xmlid, options, css=True, js=True, debug=False, async_load=False, values=None, **kw):
        ...
        return asset.to_html(
            css=css, js=js, debug=debug, async_load=async_load, url_for=(values or {}).get("url_for", lambda url: url)
        )
    ...
    def _get_asset_content(self, xmlid, options):
        ...
        def can_aggregate(url):
            return (
                not urls.url_parse(url).scheme
                and not urls.url_parse(url).netloc
    ...
    def _get_field(self, record, field_name, expression, tagName, field_options, options, values):
        ...
        inherit_branding = options.get(
            "inherit_branding", options.get("inherit_branding_auto") and record.check_access_rights("write", False)
        )
    ...
    def _compile_expr(self, expr):
        ...
        assert_valid_codeobj(
            _SAFE_OPCODES, compile(st, "<>", "eval"), expr  # could be expr, but eval *should* be fine
        )

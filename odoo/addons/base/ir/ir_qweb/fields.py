...
class IntegerConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        return pycompat.to_text(
            self.user_lang().format("%d", value, grouping=True).replace(r"-", "-\N{ZERO WIDTH NO-BREAK SPACE}")
        )
...
class SelectionConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        if not value:
            return ""
        return html_escape(pycompat.to_text(options["selection"][value]) or "", options)
...
class ImageConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        ...
        return '<img src="data:%s;base64,%s">' % (Image.MIME[image.format], value.decode("ascii"))
...
class MonetaryConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        ...
        fmt = "%.{}f".format(display_currency.decimal_places)
...
class DurationConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        ...
        for unit, secs_per_unit in TIMEDELTA_UNITS:
            ...
            if not v:
                continue
            section = babel.dates.format_timedelta(v * secs_per_unit, threshold=1, locale=locale)
            if section:
...
class BarcodeConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options=None):
        ...
        return '<img src="data:png;base64,%s">' % base64.b64encode(barcode).decode("ascii")

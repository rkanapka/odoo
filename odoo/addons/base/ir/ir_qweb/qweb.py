...
from odoo.tools import freehash, pycompat, wrap_values
...
if pycompat.PY2:
    arguments = lambda args, vararg, kwarg, defaults, **kwargs: ast.arguments(
...
class Contextifier(ast.NodeTransformer):
    ...
    def visit_Name(self, node):
        ...
        return ast.copy_location(
            # values.get(name)
            ast.Call(
                func=ast.Attribute(value=ast.Name(id="values", ctx=ast.Load()), attr="get", ctx=ast.Load()),
    ...
    def _visit_comp(self, node):
        ...
        names = tuple(
            node.id for gen in node.generators for node in ast.walk(gen.target) if isinstance(node, ast.Name)
        )
...
class QWeb:
    ...
    def compile(self, template, options):
        ...
        try:
            ...
            raise QWebException(
                "Error when compiling AST", e, path, node and etree.tostring(node[0], encoding="unicode"), name
            )
    ...
    def _profiling(self, astmod, options):
        ...
        astmod.body.insert(
            ...
            ast.Assign(
                ...
                value=ast.Call(
                    func=ast.Attribute(value=ast.Str(code_line), attr="split", ctx=ast.Load()),
                    args=[ast.Str("\n")],
                    keywords=[],
                    starargs=None,
                    kwargs=None,
        ...
        # profiling = {}
        astmod.body.insert(
            0, ast.Assign(targets=[ast.Name(id="profiling", ctx=ast.Store())], value=ast.Dict(keys=[], values=[]))
        )
        ...
        def prof(code, time):
            ...
            return ast.Expr(
                ast.Call(
                    func=ast.Attribute(
                        value=ast.Name(id="profiling", ctx=ast.Load()), attr="setdefault", ctx=ast.Load()
                    ),
                    args=[
                        ast.Num(line_id[0]),
                        ast.BinOp(
            ...
            for code in body:
                ...
                profile_body.append(
                    ast.Assign(
                        ...
                        value=ast.Call(
                            func=ast.Name(id="time", ctx=ast.Load()), args=[], keywords=[], starargs=None, kwargs=None
                        ),
                    )
                )
    ...
    def _append(self, item):
        ...
        # append(ast item)
        return ast.Expr(
            ast.Call(func=ast.Name(id="append", ctx=ast.Load()), args=[item], keywords=[], starargs=None, kwargs=None)
        )
    ...
    def _values_var(self, varname, ctx):
        # # values[$varname]
        return ast.Subscript(value=ast.Name(id="values", ctx=ast.Load()), slice=ast.Index(varname), ctx=ctx)
    ...
    def _append_attributes(self):
        ...
        return [
            ast.Assign(
                ...
                value=ast.Call(
                    func=ast.Attribute(
                        value=ast.Name(id="self", ctx=ast.Load()), attr="_post_processing_att", ctx=ast.Load()
                    ),
                    args=[
            ...
            ast.For(
                target=ast.Tuple(
                    elts=[ast.Name(id="name", ctx=ast.Store()), ast.Name(id="value", ctx=ast.Store())], ctx=ast.Store()
                ),
                iter=ast.Call(
                    func=ast.Attribute(value=ast.Name(id="t_attrs", ctx=ast.Load()), attr="items", ctx=ast.Load()),
                    args=[],
                    keywords=[],
                    starargs=None,
                    kwargs=None,
    ...
    def _compile_static_node(self, el, options):
        ...
        tag = "<%s%s" % (
            el_tag,
            "".join([' %s="%s"' % (name, escape(pycompat.to_text(value))) for name, value in attrib.items()]),
        )
        ...
        for name, value in el.attrib.items():
            if name.startswith("t-attf-"):
                ...
                nodes.append(
                    ast.Call(
                        func=ast.Attribute(
                            value=ast.Name(id="self", ctx=ast.Load()), attr="_get_dynamic_att", ctx=ast.Load()
                        ),
                        args=[
                            ast.Str(el.tag),
    ...
    def _compile_all_attributes(self, el, options, attr_already_created=False):
                ...
                if isinstance(item, tuple):
                    ...
                    body.append(
                        ast.Expr(
                            ast.Call(
                                func=ast.Attribute(
                                    value=ast.Name(id="t_attrs", ctx=ast.Load()), attr="update", ctx=ast.Load()
                                ),
                                args=[item],
                                keywords=[],
                                starargs=None,
        ...
        if attr_already_created:
            # tagName = $el.tag
            body.append(ast.Assign(targets=[ast.Name(id="tagName", ctx=ast.Store())], value=ast.Str(el.tag)))
    ...
    def _compile_tag(self, el, content, options, attr_already_created=False):
            ...
            self._append(
                ast.Str(
                    ...
                    % (
                        ...
                        "".join(
                            [
                                ' %s="%s"' % (name, escape(pycompat.to_text(value)))
        ...
        if unqualified_el_tag in self._void_elements:
            ...
            body.append(self._append(ast.Str(">")))
            body.extend(content)
            body.append(self._append(ast.Str("</%s>" % el_tag)))
        return body
    ...
    def _compile_directive_debug(self, el, options):
        ...
        if options["dev_mode"]:
            body = (
                ast.parse("__import__('%s').set_trace()" % re.sub(r"[^a-zA-Z]", "", debugger)).body + body
            )  # pdb, ipdb, pudb, ...
        else:
    ...
    def _compile_directive_set(self, el, options):
        ...
        if "t-value" in el.attrib:
            ...
            if body:
                ...
                return [
                    # content = []
                    ast.Assign(
                        targets=[ast.Name(id="content", ctx=ast.Store())], value=ast.List(elts=[], ctx=ast.Load())
                    ),
                    ...
                    ast.Expr(
                        self._call_def(
                            ...
                            append=ast.Attribute(
                                value=ast.Name(id="content", ctx=ast.Load()), attr="append", ctx=ast.Load()
                            ),
                        )
                    ),
        ...
        # $varset = $value
        return [ast.Assign(targets=[self._values_var(ast.Str(varname), ctx=ast.Store())], value=value)]
    ...
    def _compile_directive_groups(self, el, options):
        return [
            ...
            ast.If(
                test=ast.Call(
                    func=ast.Attribute(
                        value=ast.Name(id="self", ctx=ast.Load()), attr="user_has_groups", ctx=ast.Load()
                    ),
    ...
    def _compile_directive_foreach(self, el, options):
        ...
        def_name = self._create_def(
            options, self._compile_directives(el, options), prefix="foreach", lineno=el.sourceline
        )
    ...
    def _compile_widget(self, el, expression, field_options, escape=None):
        if field_options:
            return [
                # value = t-(esc|raw)
                ast.Assign(targets=[ast.Name(id="content", ctx=ast.Store())], value=self._compile_expr0(expression)),
                ...
                ast.Assign(
                    ...
                    value=ast.Call(
                        func=ast.Attribute(
                            value=ast.Name(id="self", ctx=ast.Load()), attr="_get_widget", ctx=ast.Load()
                        ),
                        args=[
    ...
    def _compile_directive_field(self, el, options):
        ...
        node_name = el.tag
        assert node_name not in ("table", "tbody", "thead", "tfoot", "tr", "td", "li", "ul", "ol", "dl", "dt", "dd"), (
            "RTE widgets do not work correctly on %r elements" % node_name
        )
        assert node_name != "t", "t-field can not be used on a t element, provide an actual HTML node"
        ...
        return [
            ...
            ast.Assign(
                ...
                value=ast.Call(
                    func=ast.Attribute(value=ast.Name(id="self", ctx=ast.Load()), attr="_get_field", ctx=ast.Load()),
                    args=[
    ...
    def _compile_widget_value(self, el, options):
        ...
        if body:
            orelse = [
                # default_content = []
                ast.Assign(
                    targets=[ast.Name(id=default_content, ctx=ast.Store())], value=ast.List(elts=[], ctx=ast.Load())
                ),
                ...
                ast.Expr(
                    self._call_def(
                        ...
                        append=ast.Attribute(
                            value=ast.Name(id=default_content, ctx=ast.Load()), attr="append", ctx=ast.Load()
                        ),
                    )
                ),
                ...
                ast.If(
                    ...
                    body=self._compile_tag(
                        el, [self._append(ast.Name(id=default_content, ctx=ast.Load()))], options, True
                    )
                    or [ast.Pass()],
                    orelse=orelse,
                ),
    ...
    def _compile_directive_call(self, el, options):
        ...
        content = [
            ...
            ast.Assign(
                ...
                value=ast.Call(
                    func=ast.Attribute(value=ast.Name(id="values", ctx=ast.Load()), attr="copy", ctx=ast.Load()),
                    args=[],
                    keywords=[],
                    starargs=None,
                    kwargs=None,
        ...
        if body:
            ...
            content.append(
                ast.Assign(
                    targets=[ast.Name(id="call_content", ctx=ast.Store())], value=ast.List(elts=[], ctx=ast.Load())
                )
            )
            ...
            content.append(
                ast.Expr(
                    self._call_def(
                        def_name,
                        append=ast.Attribute(
                            value=ast.Name(id="call_content", ctx=ast.Load()), attr="append", ctx=ast.Load()
                        ),
                        values=_values,
                    )
                )
            ...
            content.append(
                ast.Assign(
                    targets=[
                        ast.Subscript(
                            value=ast.Name(id=_values, ctx=ast.Load()), slice=ast.Index(ast.Num(0)), ctx=ast.Store()
                        )
                    ],
        ...
        if nsmap or call_options:
            ...
            content.append(
                ...
                ast.Assign(
                    ...
                    value=ast.Call(
                        func=ast.Attribute(value=ast.Name(id="options", ctx=ast.Load()), attr="copy", ctx=ast.Load()),
                        args=[],
                        keywords=[],
                        starargs=None,
                        kwargs=None,
                ...
                content.extend(
                    [
                        ...
                        ast.Expr(
                            ast.Call(
                                func=ast.Attribute(
                                    value=ast.Name(id=name_options, ctx=ast.Load()), attr="update", ctx=ast.Load()
                                ),
                        ...
                        ast.If(
                            test=ast.Compare(
                                left=ast.Call(
                                    func=ast.Attribute(
                                        value=ast.Name(id="options", ctx=ast.Load()), attr="get", ctx=ast.Load()
                                    ),
                                    args=[ast.Str("lang")],
                                    keywords=[],
                                    starargs=None,
                                ...
                                comparators=[
                                    ast.Call(
                                        func=ast.Attribute(
                                            value=ast.Name(id=name_options, ctx=ast.Load()), attr="get", ctx=ast.Load()
                                        ),
                                        args=[ast.Str("lang")],
                                        keywords=[],
                                        starargs=None,
            ...
            if nsmap:
                ...
                nsmap_ast_dict = ast.Dict(keys=[ast.Str(s="nsmap")], values=[ast.Dict(keys=keys, values=values)])
        ...
        content.append(
            ast.Expr(
                ast.Call(
                    func=ast.Call(
                        func=ast.Attribute(value=ast.Name(id="self", ctx=ast.Load()), attr="compile", ctx=ast.Load()),
                        args=[

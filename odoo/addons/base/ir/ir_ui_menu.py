...
class IrUiMenu(models.Model):
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        menus = super().search(args, offset=0, limit=None, order=order, count=False)
        if menus:
    ...
    def copy(self, default=None):
        record = super().copy(default=default)
    ...
    def _set_menuitems_xmlids(self, menu_root):
        menuitems = self.env["ir.model.data"].sudo().search([("res_id", "in", self.ids), ("model", "=", "ir.ui.menu")])

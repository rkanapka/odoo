...
from odoo.tools import SKIPPED_ELEMENT_TYPES, ConstantMapping, config, graph, pycompat
...
class ViewCustom(models.Model):
    ...
    def _auto_init(self):
        ...
        tools.create_index(self._cr, "ir_ui_view_custom_user_id_ref_id", self._table, ["user_id", "ref_id"])
        return res
...
class View(models.Model):
    ...
    model_data_id = fields.Many2one(
        "ir.model.data", string="Model Data", compute="_compute_model_data_id", search="_search_model_data_id"
    )
...
* if False, the view currently does not extend its parent but can be enabled
    ...
    def _auto_init(self):
        res = super()._auto_init()
        tools.create_index(self._cr, "ir_ui_view_model_type_inherit_id", self._table, ["model", "inherit_id"])
        return res
    ...
    def create(self, values):
        ...
        self.clear_caches()
        return super().create(self._compute_defaults(values))
                ...
                if pos == "replace":
                    ...
                    for child in spec.getiterator("attribute"):
                        ...
                        if child.get("add") or child.get("remove"):
                            ...
                            to_add = (s for s in (s.strip() for s in child.get("add", "").split(separator)) if s)
    ...
    def _field_is_editable(self, field, node):
        ...
        return (not field.readonly or READONLY.search(str(field.states or ""))) and (
    ...
    def postprocess_and_fields(self, model, node, view_id):
        ...
        if node.tag == "diagram":
            if node.getchildren()[0].tag == "node":
                ...
                if (
                    not node.get("create")
                    and not node_model.check_access_rights("create", raise_exception=False)
                    or not self._context.get("create", True)
                    and is_base_model
                ):
            ...
            if group_by_name in Model._fields:
                ...
                if group_by_field.type == "many2one":
                    ...
                    for action, operation in (
                        ...
                        if (
                            not node.get(action)
                            and not group_by_model.check_access_rights(operation, raise_exception=False)
                            or not self._context.get(action, True)
                            and is_base_model
                        ):
                            node.set(action, "false")
    ...
    def _pop_view_branding(self, element):
        distributed_branding = {
            attribute: element.attrib.pop(attribute) for attribute in MOVABLE_BRANDING if element.get(attribute)
        }
    ...
    def is_node_branded(self, node):
        ...
        :rtype: boolean
        """
        return any((attr in ("data-oe-model", "groups") or (attr.startswith("t-"))) for attr in node.attrib)
    ...
    def _validate_module_views(self, module):
        ...
        query = """SELECT max(v.id)
                ...
                LEFT JOIN ir_model_data md ON (md.model = 'ir.ui.view' AND md.res_id = v.id)
                    WHERE md.module = %s {}
                 GROUP BY coalesce(v.inherit_id, v.id)""".format(
            xmlid_filter
        )

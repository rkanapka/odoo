...
def query_insert(cr, table, values):
    query = INSERT_QUERY.format(
        table=table,
        cols=",".join(values),
        vals=",".join("%({})s".format(v) for v in values),
    )
...
class IrModel(models.Model):
    ...
    field_id = fields.One2many(
        "ir.model.fields", "model_id", string="Fields", required=True, copy=True, default=_default_field_id
    )
    ...
    state = fields.Selection(
        [("manual", "Custom Object"), ("base", "Base Object")], string="Type", default="manual", readonly=True
    )
    access_ids = fields.One2many("ir.model.access", "model_id", string="Access")
    transient = fields.Boolean(string="Transient Model")
    modules = fields.Char(
        compute="_in_modules", string="In Apps", help="List of modules in which the object is defined or inherited"
    )
    view_ids = fields.One2many("ir.ui.view", compute="_view_ids", string="Views")
    count = fields.Integer(
        compute="_compute_count", string="Count (incl. archived)", help="Total number of records in this model"
    )
    ...
    def _reflect_model(self, model):
        ...
        if model._module == self._context.get("module"):
            ...
            cr.execute("SELECT * FROM ir_model_data WHERE name=%s AND module=%s", (xmlid, self._context["module"]))
            if not cr.rowcount:
                cr.execute(
...
class IrModelFields(models.Model):
    ...
    model = fields.Char(
        string="Object Name", required=True, index=True, help="The technical name of the model this field belongs to"
    )
    relation = fields.Char(
        string="Object Relation", help="For relationship fields, the technical name of the target model"
    )
    ...
    modules = fields.Char(
        compute="_in_modules", string="In Apps", help="List of modules in which the field is defined"
    )
    ...
    def _check_selection(self, selection):
        try:
            ...
            if not (
                ...
                and all(isinstance(item, (tuple, list)) and len(item) == 2 for item in items)
            ):
    ...
    def _check_related(self):
        if self.state == "manual" and self.related:
            ...
            if field.relational and field.comodel_name != self.relation:
                raise ValidationError(
                    _("Related field '%s' does not have comodel '%s'") % (self.related, self.relation)
                )
        ...
        if self.relation_table:
            ...
            if others:
                ...
                return {
                    "warning": {
                        "title": _("Warning"),
                        "message": _("The table %r if used for other, possibly incompatible fields.")
                        % self.relation_table,
                    }
                }
    ...
    def _get_id(self, model_name, name):
        self.env.cr.execute("SELECT id FROM ir_model_fields WHERE model=%s AND name=%s", (model_name, name))
        result = self.env.cr.fetchone()
        return result and result[0]
        ...
        if vals and self:
            ...
            for item in self:
                ...
                if vals.get("ttype", item.ttype) != item.ttype:
                    raise UserError(
                        _("Changing the type of a field is not yet supported. " "Please drop it and create it again!")
                    )
        ...
        if field_data["ttype"] in ("char", "text", "html"):
            ...
            if not self.pool.loaded and not (
                ...
                and (
                    ...
                    or field_data["relation_field"] in self._get_manual_field_data(field_data["relation"])
                )
            ):
                return
            attrs["comodel_name"] = field_data["relation"]
...
class IrModelConstraint(models.Model):
    ...
    name = fields.Char(
        string="Constraint", required=True, index=True, help="PostgreSQL constraint or foreign key name."
    )
    ...
    _sql_constraints = [
        ("module_name_uniq", "unique(name, module)", "Constraints with the same name are unique per module."),
    ]
...
class IrModelAccess(models.Model):
    ...
    model_id = fields.Many2one(
        "ir.model", string="Object", required=True, domain=[("transient", "=", False)], index=True, ondelete="cascade"
    )
    ...
    def check(self, model, mode="read", raise_exception=True):
        ...
        if not r and raise_exception:
            ...
            if groups:
                msg_tail = (
                    _("Only users with the following access level are currently allowed to do that")
                    + ":\n%s\n\n("
                    + _("Document model")
                    + ": %s)"
                )
                ...
                msg_tail = (
                    _("Please contact your system administrator if you think this is an error.")
                    + "\n\n("
                    + _("Document model")
                    + ": %s)"
                )
...
class IrModelData(models.Model):
    ...
    def _auto_init(self):
        ...
        tools.create_unique_index(self._cr, "ir_model_data_module_name_uniq_index", self._table, ["module", "name"])
        tools.create_index(self._cr, "ir_model_data_model_res_id_index", self._table, ["model", "res_id"])
        return res
    ...
    def _update(self, model, module, values, xml_id=False, store=True, noupdate=False, mode="init", res_id=False):
        ...
        if action and record:
            ...
            if xml_id:
                ...
                self.sudo().create(
                    {"name": xml_id, "model": model, "module": module, "res_id": record.id, "noupdate": noupdate}
                )
                if current_module and module != current_module:
                    _logger.warning(
                        "Creating the ir.model.data %s in module %s instead of %s.", xml_id, module, current_module
                    )
    ...
    def _module_data_uninstall(self, modules_to_remove):
        ...
        undeletable += unlink_if_refcount(
            item for item in to_unlink if item[0] not in ("ir.model", "ir.model.fields", "ir.model.constraint")
        )

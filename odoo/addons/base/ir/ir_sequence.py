...
def _update_nogap(self, number_increment):
    ...
    self._cr.execute(
        "UPDATE %s SET number_next=number_next+%s WHERE id=%s " % (self._table, number_increment, self.id)
    )
...
class IrSequence(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company", string="Company", default=lambda s: s.env["res.company"]._company_default_get("ir.sequence")
    )
    ...
    def create(self, values):
        ...
        seq = super().create(values)
        if values.get("implementation", "standard") == "standard":
            _create_sequence(
                self._cr, "ir_sequence_%03d" % seq.id, values.get("number_increment", 1), values.get("number_next", 1)
            )
        return seq
    ...
    def next_by_code(self, sequence_code):
        ...
        seq_ids = self.search(
            [("code", "=", sequence_code), ("company_id", "in", [force_company, False])], order="company_id"
        )
        if not seq_ids:
            _logger.debug(

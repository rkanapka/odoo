...
class IrActions(models.Model):
    ...
    binding_type = fields.Selection(
        [("action", "Action"), ("action_form_only", "Form-only"), ("report", "Report")],
        required=True,
        default="action",
    )
...
class IrActionsActWindow(models.Model):
    ...
    domain = fields.Char(
        string="Domain Value", help="Optional domain filtering of the destination data, as a Python expression"
    )
    context = fields.Char(
    ...
    res_model = fields.Char(
        string="Destination Model", required=True, help="Model name of the object to open in the view window"
    )
    src_model = fields.Char(
        string="Source Model", help="Optional model name of the objects on which this action should be visible"
    )
...
class IrActionsActWindowView(models.Model):
    ...
    def _auto_init(self):
        ...
        tools.create_unique_index(
            self._cr, "act_window_view_unique_mode_per_action", self._table, ["act_window_id", "view_mode"]
        )
        return res
...
class IrActionsActUrl(models.Model):
    ...
    target = fields.Selection(
        [("new", "New Window"), ("self", "This Window")], string="Action Target", default="new", required=True
    )
...
# To return an action, assign: action = {...}\n\n\n\n"""
    ...
    model_id = fields.Many2one(
        "ir.model", string="Model", required=True, ondelete="cascade", help="Model on which the server action runs."
    )
    ...
    link_field_id = fields.Many2one(
        ...
        help="Provide the field used to link the newly created record " "on the record on used by the server action.",
    )

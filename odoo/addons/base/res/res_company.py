...
class Company(models.Model):
    ...
    def _get_logo(self):
        return base64.b64encode(
            open(os.path.join(tools.config["root_path"], "addons", "base", "res", "res_company_logo.png"), "rb").read()
        )
    ...
    report_footer = fields.Text(
        string="Report Footer", translate=True, help="Footer text displayed at the bottom of all reports."
    )
    ...
    currency_id = fields.Many2one(
        "res.currency", string="Currency", required=True, default=lambda self: self._get_user_currency()
    )
    ...
    state_id = fields.Many2one(
        "res.country.state", compute="_compute_address", inverse="_inverse_state", string="Fed. State"
    )
    bank_ids = fields.One2many(
        "res.partner.bank", "company_id", string="Bank Accounts", help="Bank accounts related to this company"
    )
    ...
    def name_search(self, name="", args=None, operator="ilike", limit=100):
        ...
        return super(Company, newself.with_context(context)).name_search(

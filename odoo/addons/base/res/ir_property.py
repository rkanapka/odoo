...
class Property(models.Model):
    ...
    def write(self, values):
        return super().write(self._update_values(values))

    @api.model
    def create(self, values):
        return super().create(self._update_values(values))
    ...
    def _get_domain(self, prop_name, model):
        ...
        if not field_id:
            return None
        company_id = (
            self._context.get("force_company") or self.env["res.company"]._company_default_get(model, field_id).id
        )
    ...
    def set_multi(self, name, model, values, default_value=None):
        ...
        field_id = self.env["ir.model.fields"]._get_id(model, name)
        company_id = (
            self.env.context.get("force_company") or self.env["res.company"]._company_default_get(model, field_id).id
        )

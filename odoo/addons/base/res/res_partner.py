...
class Partner(models.Model):
    ...
    category_id = fields.Many2many(
        "res.partner.category", column1="partner_id", column2="category_id", string="Tags", default=_default_category
    )
    credit_limit = fields.Float(string="Credit Limit")
    barcode = fields.Char(oldname="ean13")
    active = fields.Boolean(default=True)
    customer = fields.Boolean(
        string="Is a Customer", default=True, help="Check this box if this contact is a customer."
    )
    ...
    email_formatted = fields.Char(
        "Formatted Email", compute="_compute_email_formatted", help='Format email address "Name <email@domain>"'
    )
    phone = fields.Char()
    mobile = fields.Char()
    is_company = fields.Boolean(
        string="Is a Company", default=False, help="Check if the contact is a company, otherwise it is a person"
    )
    ...
    commercial_partner_id = fields.Many2one(
        "res.partner", compute="_compute_commercial_partner", string="Commercial Entity", store=True, index=True
    )
    commercial_partner_country_id = fields.Many2one(
        "res.country", related="commercial_partner_id.country_id", store=True, string="Commercial Entity's Country"
    )
    ...
    def _compute_email_formatted(self):
        for partner in self:
            partner.email_formatted = tools.formataddr((partner.name or "False", partner.email or "False"))
            ...
            self.write(
                {
                    ...
                    "child_ids": [
                        (1, partner_id, dict(parent_id=new_company.id)) for partner_id in self.child_ids.ids
                    ],
                }
            )
        return True
    ...
    def _display_address(self, without_company=False):
        ...
        address_format = self.country_id.address_format or self._get_default_address_format()
        args = {

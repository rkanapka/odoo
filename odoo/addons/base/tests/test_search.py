...
class test_search(TransactionCase):
    ...
    def test_00_search_order(self):
        ...
        self.assertEqual(
            [d, e, c, a, b, ab], list(active_asc_id_asc), "Search with 'ACTIVE ASC, ID ASC' order failed."
        )
        ...
        self.assertEqual(
            [c, a, b, ab, d, e], list(active_desc_id_asc), "Search with 'ACTIVE DESC, ID ASC' order failed."
        )
        ...
        self.assertEqual(
            [e, d, ab, b, a, c], list(active_asc_id_desc), "Search with 'ACTIVE ASC, ID DESC' order failed."
        )
        ...
        self.assertEqual(
            [ab, b, a, c, e, d], list(active_desc_id_desc), "Search with 'ACTIVE DESC, ID DESC' order failed."
        )
        ...
        self.assertEqual(
            [c, d, a, b, ab, e], list(id_asc_active_asc), "Search with 'ID ASC, ACTIVE ASC' order failed."
        )
        ...
        self.assertEqual(
            [c, d, a, b, ab, e], list(id_asc_active_desc), "Search with 'ID ASC, ACTIVE DESC' order failed."
        )
        ...
        self.assertEqual(
            [e, ab, b, a, d, c], list(id_desc_active_asc), "Search with 'ID DESC, ACTIVE ASC' order failed."
        )
        ...
        self.assertEqual(
            [e, ab, b, a, d, c], list(id_desc_active_desc), "Search with 'ID DESC, ACTIVE DESC' order failed."
        )
    ...
    def test_10_inherits_m2order(self):
        ...
        user_ids = Users.search(
            [("id", "in", expected_ids)], order="state_id asc, country_id desc, name asc, login desc"
        ).ids
        self.assertEqual(user_ids, expected_ids, "search on res_users did not provide expected ids or expected order")
        ...
        user_ids = Users.search(
            [("id", "in", expected_ids)], order="country_id desc, state_id desc, name asc, login desc"
        ).ids
        self.assertEqual(user_ids, expected_ids, "search on res_users did not provide expected ids or expected order")
    ...
    def test_11_indirect_inherits_m2o_order(self):
        ...
        for u in "BAC":
            ...
            cron_ids[u] = Cron.create(
                {"name": u, "model_id": self.env.ref("base.model_res_partner").id, "user_id": user_ids[u]}
            ).id
    ...
    def test_13_m2o_order_loop_multi(self):
        ...
        kw = dict(groups_id=[(6, 0, [self.ref("base.group_system"), self.ref("base.group_partner_manager")])])

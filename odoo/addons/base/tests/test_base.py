...
from odoo.tools.safe_eval import const_eval, safe_eval
...
class TestBase(TransactionCase):
    ...
    def test_40_res_partner_address_get(self):
        ...
        branch1 = res_partner.create({"name": "Branch 1", "parent_id": elmtree.id, "is_company": True})
        leaf10 = res_partner.create({"name": "Leaf 10", "parent_id": branch1.id, "type": "invoice"})
        branch11 = res_partner.create({"name": "Branch 11", "parent_id": branch1.id, "type": "other"})
        leaf111 = res_partner.create({"name": "Leaf 111", "parent_id": branch11.id, "type": "delivery"})
        branch11.write({"is_company": False})  # force is_company after creating 1rst child
        branch2 = res_partner.create({"name": "Branch 2", "parent_id": elmtree.id, "is_company": True})
        leaf21 = res_partner.create({"name": "Leaf 21", "parent_id": branch2.id, "type": "delivery"})
        ...
        self.assertEqual(
            ...
            {"delivery": leaf111.id, "invoice": leaf10.id, "contact": branch1.id, "other": branch11.id},
        ...
        self.assertEqual(
            ...
            {"delivery": elmtree.id, "invoice": elmtree.id, "contact": elmtree.id, "other": elmtree.id},
            "Invalid address resolution",
        )
        ...
        self.assertEqual(
            ...
            {"delivery": leaf21.id, "invoice": branch2.id, "contact": branch2.id, "other": branch2.id},
    ...
    def test_50_res_partner_commercial_sync(self):
        ...
        sunhelm = res_partner.create(
            {
                ...
                "vat": "BE0477472701",
                "child_ids": [(4, p0.id), (0, 0, {"name": "Alrik Greenthorn", "email": "agr@sunhelm.com"})],
            }
        )
        p1 = res_partner.create(
            {"name": "Otto Blackwood", "email": "otto.blackwood@sunhelm.com", "parent_id": sunhelm.id}
        )
        p11 = res_partner.create({"name": "Gini Graywool", "email": "ggr@sunhelm.com", "parent_id": p1.id})
        p2 = res_partner.search([("email", "=", "agr@sunhelm.com")], limit=1)
        sunhelm.write({"child_ids": [(0, 0, {"name": "Ulrik Greenthorn", "email": "ugr@sunhelm.com"})]})
        ...
        for p in (sunhelm, p0, p11, p2, p3):
            self.assertEqual(
                p.vat, sunhelmvat, "Sync to children should only work downstream and on commercial entities"
            )
        ...
        self.assertEqual(
            p1.commercial_partner_id, p1, "Incorrect commercial entity resolution after setting is_company"
        )
    ...
    def test_60_read_group(self):
        ...
        self.assertEqual(
            ["5$ Wrench", "Eavesdropper", "Friend"], [g["function"] for g in groups_data], "incorrect read_group order"
        )
        ...
        self.assertEqual(
            ["Nab", "Eve", "Bob", "Alice"], [g["name"] for g in groups_data], "Incorrect ordering of the list"
        )
        ...
        self.assertEqual(
            ["Eve", "Nab", "Bob", "Alice"], [g["name"] for g in groups_data], "Incorrect ordering of the list"
        )
        ...
        groups_data = res_users.read_group(
            domain, fields=["function", "color", "title"], groupby=["title"], orderby="color desc, title desc"
        )
        self.assertEqual(len(groups_data), 2, "Incorrect number of results when grouping on a field")
        # m2o is returned as a (id, label) pair
        self.assertEqual(
...
class TestPartnerRecursion(TransactionCase):
    ...
    def test_104_res_partner_recursion_indirect_cycle(self):
        ...
        with self.assertRaises(ValidationError):
            self.p2.write(
                {"child_ids": [(1, self.p3.id, {"parent_id": p3b.id}), (1, p3b.id, {"parent_id": self.p3.id})]}
            )
...
class TestGroups(TransactionCase):
    ...
    def test_res_groups_fullname_search(self):
        ...
        self.assertItemsEqual(
            groups.ids, [g.id for g in all_groups if "Sale" in g.full_name], "did not match search for 'Sale'"
        )
        ...
        self.assertItemsEqual(
            groups.ids, [g.id for g in all_groups if "Sales /" in g.full_name], "did not match search for 'Sales /'"
        )

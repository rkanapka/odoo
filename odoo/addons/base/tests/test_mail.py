...
<p>
    ...
    def test_sanitize_escape_emails(self):
        ...
        for email in emails:
            self.assertIn(
                misc.html_escape(email), html_sanitize(email), "html_sanitize stripped emails of original html"
            )
   ...
   font-size: 30px; max-width: 100%; after: "Not sure
    ...
    ; some-property: 2px; top: 3'>youplaboum</div>""",
        ...
        for test, in_lst, out_lst in test_data:
            new_html = html_sanitize(
                test, sanitize_attributes=False, sanitize_style=True, strip_style=False, strip_classes=False
            )
            for text in in_lst:
    ...
    def test_style_class(self):
        html = html_sanitize(
            test_mail_examples.REMOVE_CLASS, sanitize_attributes=True, sanitize_style=True, strip_classes=True
        )
    ...
    def test_style_class_only(self):
        html = html_sanitize(
            test_mail_examples.REMOVE_CLASS, sanitize_attributes=False, sanitize_style=True, strip_classes=True
        )
        for ext in test_mail_examples.REMOVE_CLASS_IN:
            self.assertIn(ext, html)
        for ext in test_mail_examples.REMOVE_CLASS_OUT:
    ...
    def test_edi_source(self):
        ...
        self.assertIn(
            'src="https://www.paypal.com/en_US/i/btn/btn_paynowCC_LG.gif"', html, "html_sanitize removed valid img"
        )

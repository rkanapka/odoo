...
class QueryTestCase(unittest.TestCase):
    ...
    def test_basic_query(self):
        ...
        query.add_join(
            ("product_template", "product_category", "categ_id", "id", "categ_id"), implicit=False, outer=False
        )  # add normal join
        query.add_join(
            ("product_product", "res_user", "user_id", "id", "user_id"), implicit=False, outer=True
        )  # outer join
        self.assertEqual(
    ...
    def test_query_chained_explicit_joins(self):
        ...
        query.add_join(
            ("product_template__categ_id", "res_user", "user_id", "id", "user_id"), implicit=False, outer=True
        )  # CHAINED outer join
        self.assertEqual(
            query.get_sql()[0].strip(),
    ...
    def test_mixed_query_chained_explicit_implicit_joins(self):
        ...
        self.assertEqual(
            ...
            """product_product.template_id = product_template.id AND product_category.expense_account_id = account_account.id""".strip(),
        )
    ...
    def test_raise_missing_lhs(self):
        ...
        self.assertRaises(
            AssertionError,
            query.add_join,
            ("product_template", "product_category", "categ_id", "id", "categ_id"),
            implicit=False,
            outer=False,
        )

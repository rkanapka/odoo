...
class test_ir_http_mimetype(common.TransactionCase):
    ...
    def test_ir_http_mimetype_attachment(self):
        ...
        status, headers, content = self.env["ir.http"].binary_content(
            id=attachment.id, mimetype=None, default_mimetype="application/octet-stream", env=self.env
        )
    ...
    def test_ir_http_mimetype_attachment_name(self):
        ...
        attachment = self.env["ir.attachment"].create(
            {"datas": GIF, "name": "Test mimetype gif with png name", "datas_fname": "file.png"}
        )
    ...
    def test_ir_http_mimetype_computed_field(self):
        ...
        resized = odoo.tools.image_get_resized_images(prop.value_binary, return_big=True, avoid_resize_medium=True)[
            "image_small"
        ]
    ...
    def test_ir_http_attachment_access(self):
        ...
        attachment = self.env["ir.attachment"].create(
            {"datas": GIF, "name": "Test valid access token with image", "datas_fname": "image.gif"}
        )
        ...
        def test_access(**kwargs):
            status, _, _ = self.env["ir.http"].binary_content(**dict(defaults, **kwargs))
            return status
        ...
        self.assertEqual(status, 403, "no access if access token for attachment without access token")
        ...
        status = test_access(access_token="Wrong")
        self.assertEqual(status, 403, "no access for wrong access token for attachment with access")

...
class TestQWebNS(TransactionCase):
    ...
    def test_render_static_xml_with_t_call(self):
        ...
        self.assertEqual(
            result.count(b'xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2"'), 1
        )
    ...
    def test_render_static_xml_with_extension(self):
        ...
        self.assertEqual(
            etree.fromstring(view1.with_context(check_view_ids=[view1.id, view2.id]).render()), expected_result
        )
    ...
    def test_render_t_call_propagates_t_lang(self):
        ...
        self.env["res.lang"].search([("code", "=", other_lang)], limit=1).write(
            {"active": True, "decimal_point": "*", "thousands_sep": "/"}
        )

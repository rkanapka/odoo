...
class TestFloatPrecision(TransactionCase):
    ...
    def test_rounding_03(self):
        ...
        def try_zero(amount, expected):
            self.assertEqual(
                float_is_zero(amount, precision_digits=3), expected, "Rounding error: %s should be zero!" % amount
            )
    ...
    def test_rounding_04(self):
        ...
        def try_roundtrip(value, expected, date):
            ...
            self.assertEqual(
                rate.rate, expected, "Roundtrip error: got %s back from db, expected %s" % (rate, expected)
            )
    ...
    def test_amount_to_text_10(self):
        ...
        self.assertNotEqual(amount_test, amount_target, "Amount in text should not depend on float representation")

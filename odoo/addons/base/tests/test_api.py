...
class TestAPI(common.TransactionCase):
    ...
    def test_60_prefetch(self):
        ...
        for partner in partners:
            partner.country_id
            break
        partner_ids_with_field = [partner.id for partner in partners if "country_id" in partner._cache]
        self.assertItemsEqual(partner_ids_with_field, partners.ids)

        # partners' countries are ready for prefetching
        country_ids = {cid for partner in partners for cid in partner._cache["country_id"]}
        ...
        for partner in partners:
            if partner.country_id:
                ...
                break
        country_ids_with_field = [country.id for country in partners.mapped("country_id") if "name" in country._cache]
    ...
    def test_80_filter(self):
        ...
        self.assertEqual(ps.filtered(lambda p: p.parent_id.customer), ps.filtered("parent_id.customer"))

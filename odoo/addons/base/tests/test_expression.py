...
class TestExpression(TransactionCase):
    ...
    def test_00_in_not_in_m2m(self):
        ...
        self.assertEqual(
            a + b + ab, with_a_or_with_b, "Search for category_id contains cat_a or contains cat_b failed."
        )
        ...
        self.assertFalse(
            without_a_or_b & (a + b + ab), "Search for category_id doesn't contain cat_a or cat_b failed (1)."
        )
        ...
        without_a_and_without_b = partners.search(
            [("category_id", "not in", [cat_a.id]), ("category_id", "not in", [cat_b.id])]
        )
        self.assertFalse(
        ...
        self.assertTrue(
            c in without_a_and_without_b, "Search for category_id doesn't contain cat_a and cat_b failed (2)."
        )
    ...
    def test_10_equivalent_id(self):
        ...
        Partner.search([("name", "in", [one.name])])
    ...
    def test_15_m2o(self):
        ...
        res_0 = Partner.search(
            [("parent_id", "not like", "probably_unexisting_name")]
    ...
    def test_15_equivalent_one2many_1(self):
        ...
        res_1 = Company.search(
            [("child_ids", "in", company3.child_ids.ids)]
        )  # any company having a child of company3 as child
        self.assertEqual(res_1, company3)
        res_2 = Company.search(
            [("child_ids", "in", company3.child_ids[0].ids)]
        ...
        self.assertEqual(
            [p1],
            Partner.search([("user_ids", "=", "Dédé Boitaclou")]).ids,
            "o2m NOT IN matches none on the right side",
        )
        self.assertEqual(
            [], Partner.search([("user_ids", "in", [10000])]).ids, "o2m NOT IN matches none on the right side"
        )
        self.assertEqual(
            [p1, p2], Partner.search([("user_ids", "in", [u1a, u2])]).ids, "o2m IN matches any on the right side"
        )
        all_ids = Partner.search([]).ids
        self.assertEqual(
            set(all_ids) - {p1},
            set(Partner.search([("user_ids", "not in", u1a)]).ids),
            "o2m NOT IN matches none on the right side",
        )
        self.assertEqual(
            set(all_ids) - {p1},
            set(Partner.search([("user_ids", "!=", "Dédé Boitaclou")]).ids),
            "o2m NOT IN matches none on the right side",
        )
        self.assertEqual(
            set(all_ids) - {p1, p2},
            set(Partner.search([("user_ids", "not in", [u1b, u2])]).ids),
            "o2m NOT IN matches none on the right side",
        )
    ...
    def test_15_equivalent_one2many_2(self):
        ...
        res_3 = Currency.search(
            [("rate_ids", "in", default_currency.rate_ids.ids)]
        )  # currencies having a rate of main currency
        self.assertEqual(res_3, default_currency)
        res_4 = Currency.search(
            [("rate_ids", "in", default_currency.rate_ids[0].ids)]
        )  # currencies having first rate of main currency
        self.assertEqual(res_4, default_currency)
        res_5 = Currency.search(
            [("rate_ids", "in", default_currency.rate_ids[0].id)]
        )  # currencies having first rate of main currency
    ...
    def test_30_normalize_domain(self):
        ...
        self.assertEqual(
            norm_domain, expression.normalize_domain(domain), "Normalized domains should be left untouched"
        )
        domain = [
            ("x", "in", ["y", "z"]),
            ("a.v", "=", "e"),
        ...
        norm_domain = ["&", "&", "&"] + domain
        self.assertEqual(
            norm_domain, expression.normalize_domain(domain), "Non-normalized domains should be properly normalized"
        )
    ...
    def test_40_negating_long_expression(self):
        ...
        self.assertEqual(
            expression.distribute_not(source), expect, "distribute_not on long expression applied wrongly"
        )
...
class TestAutoJoin(TransactionCase):
    ...
    def test_auto_join(self):
        ...
        # Test result
        self.assertEqual(
            partners, p_aa, "_auto_join off: ('bank_ids.sanitized_acc_number', 'like', '..'): incorrect result"
        )
        ...
        # Test result
        self.assertEqual(
            partners, p_a + p_b, "_auto_join off: ('child_ids.bank_ids.id', 'in', [..]): incorrect result"
        )
        # Test produced queries
        self.assertEqual(
            len(self.query_list),
        ...
        # Test result
        self.assertEqual(
            partners, p_aa, "_auto_join on: ('bank_ids.sanitized_acc_number', 'like', '..') incorrect result"
        )
        # Test produced queries
        self.assertEqual(
            len(self.query_list),
        ...
        self.assertLessEqual(
            set(bank_ids), set(sql_query[2]), "_auto_join on: ('bank_ids.id', 'in', [..]) query incorrect parameter"
        )
        ...
        # Test result
        self.assertEqual(
            partners, p_a + p_b, "_auto_join on: ('child_ids.bank_ids.id', 'not in', [..]): incorrect result"
        )
        # # Test produced queries
        self.assertEqual(
        ...
        expected = "%s like %s" % (
            unaccent('"res_partner__child_ids__bank_ids"."sanitized_acc_number"::text'),
            unaccent("%s"),
        )
        ...
        # Test result: at least our added data + demo data
        self.assertLessEqual(
            p_a + p_b, partners, "_auto_join on: ('child_ids.state_id.country_id.code', 'like', '..') incorrect result"
        )
        # Test produced queries
        self.assertEqual(
            len(self.query_list),

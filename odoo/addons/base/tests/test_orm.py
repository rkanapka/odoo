...
class TestORM(TransactionCase):
    ...
    def test_groupby_date(self):
        ...
        res = partners.read_group([("id", "in", partner_ids)], ["date"], groupby=["date:month"], orderby="date DESC")
        self.assertEqual([item["date:month"] for item in res], months)
        ...
        res = partners.read_group(
            [("id", "in", partner_ids)], ["date"], groupby=["date:month", "date:day"], orderby="date DESC", lazy=False
        )
    ...
    def test_unlink_with_property(self):
            ...
            .create(
                {
                    ...
                    .search([("model", "=", "res.partner"), ("name", "=", "ref")], limit=1)
                    .id,
        ...
        self.assertEqual(p1_prop.exists(), self.env["ir.property"], "p1_prop should have been deleted")
...
class TestInherits(TransactionCase):
    ...
    def test_copy_with_ancestor(self):
        ...
        user_foo = self.env["res.users"].create(
            {"name": "Foo", "login": "foo", "password": "foo", "login_date": "2016-01-01", "signature": "XXX"}
        )

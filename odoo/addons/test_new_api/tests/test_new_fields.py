...
from odoo.tools import float_repr, mute_logger, pycompat
...
class TestFields(common.TransactionCase):
    ...
    def test_27_company_dependent(self):
        ...
        user0 = self.env["res.users"].create(
            {"name": "Foo", "login": "foo", "company_id": company0.id, "company_ids": []}
        )
        user1 = self.env["res.users"].create(
            {"name": "Bar", "login": "bar", "company_id": company1.id, "company_ids": []}
        )
        user2 = self.env["res.users"].create(
            {"name": "Baz", "login": "baz", "company_id": company2.id, "company_ids": []}
        )
        ...
        self.env["ir.property"].create(
            {"name": "foo", "fields_id": field_tag_id.id, "value": tag0, "type": "many2one"}
        )
    ...
    def test_50_search_many2one(self):
        ...
        messages = self.env["test_new_api.message"].search([("author_partner.name", "=", "Demo User")])
...
%endif"""
        ...
        self.assertEqual(
            record.comment1, some_ugly_html, "Error in HTML field: content was sanitized but field has sanitize=False"
        )

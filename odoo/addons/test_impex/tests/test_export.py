...
class test_integer_field(CreatorCase):
    ...
    def test_huge(self):
        self.assertEqual(self.export(2**31 - 1), [[pycompat.text_type(2**31 - 1)]])
...
class test_float_field(CreatorCase):
    ...
    def test_epsilon(self):
        self.assertEqual(self.export(0.000000000027), [["2.7e-11"]])
...
class test_date(CreatorCase):
    ...
    def test_basic(self):
        self.assertEqual(self.export("2011-11-07"), [["2011-11-07"]])
...
class test_datetime(CreatorCase):
    ...
    def test_basic(self):
        self.assertEqual(self.export("2011-11-07 21:05:48"), [["2011-11-07 21:05:48"]])
    ...
    def test_tz(self):
        ...
        self.assertEqual(
            self.export("2011-11-07 21:05:48", context={"tz": "Pacific/Norfolk"}), [["2011-11-07 21:05:48"]]
        )
...
class test_selection(CreatorCase):
    ...
    def test_localized_export(self):
        ...
        for source, value in self.translations_fr:
            Translations.create(
                {"name": "export.selection,value", "lang": "fr_FR", "type": "selection", "src": source, "value": value}
            )
        self.assertEqual(self.export(2, context={"lang": "fr_FR"}), [["Bar"]])
...
class test_m2o(CreatorCase):
    ...
    def test_path(self):
        ...
        record = self.env["export.integer"].create({"value": 42})
        self.assertEqual(
            self.export(record.id, fields=["value/.id", "value/value"]), [[pycompat.text_type(record.id), "42"]]
        )
    ...
    def test_external_id(self):
        ...
        self.assertRegex(
            self.export(record.id, fields=["value/id"])[0][0], "__export__.export_integer_%d_[0-9a-f]{8}" % record.id
        )
...
class test_o2m(CreatorCase):
    ...
    def test_single_subfield(self):
        self.assertEqual(
            self.export([(0, False, {"value": 42})], fields=["value", "value/value"]),
            [["export.one2many.child:42", "42"]],
        )

    def test_integrate_one_in_parent(self):
        self.assertEqual(self.export([(0, False, {"value": 42})], fields=["const", "value/value"]), [["4", "42"]])
...
class test_o2m_multiple(CreatorCase):
    ...
    def test_empty(self):
        self.assertEqual(self.export(child1=False, child2=False), [[False, False]])

    def test_single_per_side(self):
        self.assertEqual(
            self.export(child1=False, child2=[(0, False, {"value": 42})]), [[False, "export.one2many.child.2:42"]]
        )

        self.assertEqual(
            self.export(child1=[(0, False, {"value": 43})], child2=False), [["export.one2many.child.1:43", False]]
        )

        self.assertEqual(
            self.export(child1=[(0, False, {"value": 43})], child2=[(0, False, {"value": 42})]),
    ...
    def test_single_integrate_subfield(self):
        ...
        self.assertEqual(
            self.export(child1=False, child2=[(0, False, {"value": 42})], fields=fields), [["36", False, "42"]]
        )

        self.assertEqual(
            self.export(child1=[(0, False, {"value": 43})], child2=False, fields=fields), [["36", "43", False]]
        )

        self.assertEqual(
            self.export(child1=[(0, False, {"value": 43})], child2=[(0, False, {"value": 42})], fields=fields),
            [["36", "43", "42"]],
        )
    ...
    def test_multiple(self):
        ...
        fields = ["const", "child1/value", "child2/value"]
        child1 = [
            (0, False, {"value": v, "str": "record%.02d" % index})

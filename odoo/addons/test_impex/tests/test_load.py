...
class test_boolean_field(ImporterCase):
    ...
    def test_trues(self):
        ...
        self.assertEqual(
            ...
            [
                message(
                    "Unknown value '%s' for boolean field 'Value', assuming 'yes'" % v[0],
...
class test_integer_field(ImporterCase):
    ...
    def test_out_of_range(self):
        ...
        self.assertEqual(
            result["messages"],
            [{"type": "error", "rows": {"from": 0, "to": 0}, "record": 0, "message": "integer out of range\n"}],
        )
...
class test_float_field(ImporterCase):
    ...
    def test_negatives(self):
        ...
        self.assertFalse(result["messages"])
        self.assertEqual([-1, -42, -(2**31 - 1), -(2**31), -12345678, -(2.0**33), -0.000001], values(self.read()))

    def test_nonsense(self):
        result = self.import_(["value"], [["foobar"]])
        self.assertIs(result["ids"], False)
        self.assertEqual(result["messages"], [message("'foobar' does not seem to be a number for field 'Value'")])
...
class test_unbound_string_field(ImporterCase):
    ...
    def test_imported(self):
        ...
        self.assertFalse(result["messages"])
        self.assertEqual(
            ["í dag viðrar vel til loftárása", "If they ask you about fun, you tell them – fun is a filthy parasite"],
            values(self.read()),
        )
...
class test_m2o(ImporterCase):
    ...
    def test_by_names(self):
        ...
        self.assertEqual(
            result["messages"], [message("Found multiple matches for field 'Value' (2 matches)", type="warning")]
        )
        self.assertEqual(len(result["ids"]), 1)
...
class test_m2m(ImporterCase):
    ...
    def test_ids(self):
        ...
        result = self.import_(
            ["value/.id"],
            [["%d,%d" % (id1, id2)], ["%d,%d,%d" % (id1, id3, id4)], ["%d,%d,%d" % (id1, id2, id3)], ["%d" % id5]],
        )
        self.assertFalse(result["messages"])
        self.assertEqual(len(result["ids"]), 4)
...
class test_o2m(ImporterCase):
    ...
    def test_name_get(self):
        ...
        self.assertEqual(
            ...
            [
                message(
                    "No matching record found for name '%s' in field 'Value'" % s,

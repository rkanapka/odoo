...
class TestJavascriptAssetsBundle(TransactionCase):
    ...
    def _any_ira_for_bundle(self, type):
        ...
        return self.env["ir.attachment"].search([("url", "=like", "/web/content/%-%/{}%.{}".format(bundle, type))])
    ...
    def test_11_paginated_css_content_invalidation(self):
        ...
        bundle1 = self._get_asset(
            self.cssbundle_xmlid, env=self.env(context={"check_view_ids": view.ids, "max_css_rules": 1})
        )
        bundle1.css()
        files1 = bundle1.files
        version1 = bundle1.version
    ...
    def test_15_exteral_lib_assets(self):
        ...
        attachments = self.env["ir.attachment"].search(
            [("url", "=like", "/web/content/%-%/test_assetsbundle.bundle4.%")]
        )
...
<html>
    ...
    <body>
                ...
                % {"js": attachments[0].url, "css": attachments[1].url}
            ).encode("utf8"),
        )
...
class TestAssetsBundleInBrowser(HttpCase):
    def test_01_js_interpretation(self):
        ...
        self.phantom_js(
            "/test_assetsbundle/js", "a + b + c === 6 ? console.log('ok') : console.log('error')", login="admin"
        )
